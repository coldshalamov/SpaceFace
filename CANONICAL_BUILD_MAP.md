...
2. live code, current executable checks, and player-route evidence;
3. [`ARCHITECTURE.md`](./ARCHITECTURE.md) for technical invariants and owner boundaries;
...
- one candidate digest receives at most one acceptance launch unless source or the owning fast-gate receipt changes;
- an expensive failure must be reproduced by a seconds-scale regression before another expensive launch;
- unchanged failure fingerprints block reruns;
- independent review has one discovery pass and one causal re-review after repairs;
