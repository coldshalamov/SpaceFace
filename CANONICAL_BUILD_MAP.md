...
**CONFIRMED THE SAME DAY BY AN INDEPENDENT CLEAN-MACHINE RUN.** The first reading below was taken while a delegated lane was still live, and the report carried the contention signature (frame interval vs measured callback interval disagreeing by a mean of -220.4 ms), so it was flagged as unconfirmed. A second lane then measured the same route twice on a quiet machine before changing anything, and **reproduced the brick**: `presentation` p95 **5.5 ms** / max **3237.5 ms**, 787 frames, **13 hitches**. Two independent runs, one contended and one not, both find a ~3.2-3.6 s stall at `stage entering-flight`. **The brick is real.** Prefer the clean figures (p95 5.5, max 3237.5, 13/787 hitches) over the contended ones below.
