...
| **`PQ-115` / `PERF-75-VFX-ONGLASS`** | Near IMPL | Trails/lights/flipbooks follow the table. Station-side, seam, and NPC job-signature draw use `tableVfxDrawWuFromState` (live glass), not a 1500/640/300 WU horizon. Station side-event planning anchors on `tableSimAuthorityWuFromState` plus that station type's farthest eligible mover path, not a 1400 WU horizon. Player trails stay. |
