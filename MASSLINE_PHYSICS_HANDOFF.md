...
2. **Enemy tether use: specialists only, by owner decision (2026-08-10).** This principle originally banned enemy tethers outright ("they'll just grapple the shit out of me"). The owner has since explicitly approved keeping enemy Massline use for *specialist raider doctrines* (the shipped `tether_control_raider` behavior — flank, attach, control, escape), because a specialist disrupting the player's plan is exactly `design/VISION.md`'s combat framing. The original fear still governs calibration: tether use stays rare and role-specific — never a default enemy behavior, never universal grappling, and AI line-cutting that negates the player's mechanic remains banned.
