...
1. **Rendered frames are the authority; prose is subordinate.** The direction is fixed by frames
   the owner approved, committed at `design/frontend/direction/approved/<surface>.png`, plus a
   one-page `DIRECTION_SHEET.md` that describes *those frames* — never the reverse.
2. **The owner chooses between real options.** Phase 0 renders the same three screens in three
   materially different directions, as real HTML at real size in the game's own lab, and the owner
   picks one. Nothing else starts first. This is enforced in the queue: every surface packet depends
   on `PQ-187.03`, so `program-dispatch --next` cannot hand out surface work before the lock.
3. **Every phase ends with a contact sheet the owner says yes or no to.** A phase is done when the
   owner has looked, not when a check is green.
...
built; **the owner's pick is the decision, and "none of these" is a valid answer** that sends
Phase 0 round again with new candidates.
...
the kit exists. **Every phase ends with a contact sheet and an owner yes.** Packet IDs are real and
...
|---|---|---|---|
| **0 · Direction** | `PQ-187.00` reference board · `.01` three directions rendered · `.02` direction lock | the board reactions; three real comps of three screens; the owner's pick committed as `direction/approved/` + `DIRECTION_SHEET.md` | **the pick.** Nothing below starts until this exists |
| **1 · The kit** | `PQ-187.03` | tokens, the type scale to hero size, the display/text/numeral faces, colour-as-mood rules, the transition system, the UI sound palette, and every component rendered on one kit page in the lab; old stylesheets are not cleaned — screens migrate onto the kit and their old CSS is deleted as they move | the kit page |
...
- `PQ-162`, `PQ-168`, `PQ-181`, `PQ-182`, `PQ-185` gain a dependency on `PQ-187.03` and a note
  that their taste gate is the owner's contact sheet; `PQ-183` and `PQ-184` are unchanged in scope
