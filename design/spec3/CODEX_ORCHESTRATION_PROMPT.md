...
EVERY BRIEF MUST SAY: "Read design/spec3/_context/06_PLANNING_CONSTITUTION.md fully, then implement <SPEC3-XX §N> from <thread file> exactly. Acceptance = the spec's named check script green. Touch only files your spec names. git add -N every new file immediately (this env deletes untracked files). Never edit test/*.expected.json. No new runtime deps. Print a 10-line summary."
