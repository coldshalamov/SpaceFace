...
**Where it stands (end of 2026-09-13).** On the owner's laptop, with nothing else running, Launch to flying takes 4.4 s on a returning launch (32.2 s this morning), with no freeze while loading longer than 146 ms, and 12-14 s on a fresh install (34.7 s).
Effects had not been running in the first 20 s of any run: engine exhaust, weapon tracers and particles stayed frozen until game time 20 s, and switching them on there froze the game for 600 ms the first time (50-67 ms once cached). They now run from the first second and that freeze is gone. Opening flight holds 60 fps with a worst frame of 17-18 ms even on a fresh install, where a 433-550 ms freeze when the first rocks came into view is also gone; a Crucible run keeps one 50 ms frame as the first picture appears. A jump to Ceres runs with nothing over 77 ms, and the busy scene there has a worst frame of 67 ms with effects running. On a fresh install the longest freeze on the loading screen is 0.9 s, down from 1.9 s.
The price is loading work: building the effects, rocks and field markers while loading keeps a returning launch within its earlier range (4.4 s against 4.2 s and 2.9 s; the rock step itself takes 0.3 s) and adds 1-3 s to a fresh install. A rendering bug found on the way is fixed: while the renderer held shadow refreshes in the first 20 s of flight, the graphics driver rejected every lit draw that needed the not-yet-created shadow map, so the player's own ship was not drawn; drawing it costs real frame time in early flight (main thread 47 % -> 83 % busy in the first 10 s, still with no freeze). Running the game on Vulkan instead of Direct3D 11 was measured and is worse.
...
|---|---|---|---|
| Launch click to flying (second launch) | `node scripts/probe-main-thread-profile.mjs --from-launch --keep-profile=p0913a` | 32.2 s | 4.4 s with engine effects, rocks and field markers built while loading (live cook 1.39 s, rock step 0.29 s); 4.2 s and 2.9 s before those changes, one run at 5.6 s in between |
| Launch click to flying (first launch) | same, fresh `--keep-profile` name | 34.7 s | 12.1-14.4 s with engine effects and rocks built while loading (launch profile 12.1 and 14.2 s, fresh-install jump probe 12.4 and 14.4 s); 10.8-10.9 s before those changes |
| Long freezes while loading | same report, long tasks from Launch to flight | 24 tasks, 7.4 s in gpu-resources | 4 tasks, 341 ms in total, longest 146 ms (second launch; earlier 6 tasks, 667 ms, 198 ms). First launch: longest 0.9 s and 2.7 s in total, from 1.9 s and 2.9 s, now that the New Game preview no longer loses its graphics context mid-link |
