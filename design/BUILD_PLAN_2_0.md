...
| # | Item | Owner | Status | Accept when |
|---|---|---|---|---|
| D1 | **Tether gameplay integration** (flagship): fire/latch/reel/cut on input contract, slingshot physics, tension telegraphy events, mass-ratio yank rules, break thresholds | **CODEX-1** | **DONE** (sim + in-world segmented cable/HUD readout covered by later tether feel pass) | `check:sg02:tether*` green; latch changes no ordinary flight control; tangent release preserves real momentum and any flourish is capped at `15% × actual speed × line load`, with zero slack/parked bonus; events `tether:latched/strain/broke/released` emitted |
