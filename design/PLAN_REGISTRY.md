...
2. Repo-root [`../build_map.md`](../build_map.md) is the **agent program front door**
...
| [`program/FLEET_VISUAL_INTEGRITY.md`](./program/FLEET_VISUAL_INTEGRITY.md) | Cross-cutting proof that every live ship publishes atomically as a complete body and that sky/debris presentation remains continuous through floating-origin rebases | **ACTIVE EXECUTION PLAN — runtime continuity fix plus fleet-wide census/remediation contract** | Runtime owners close coordinate/publication defects; the ordered 3D board is `PQ-193` (`build_map.md` §13D); flyable remaster remains `PQ-050`; unused-pack fielding remains `PQ-136` |
| `design/arcade-core/` (62 docs, unmerged — lives on `origin/arcade-core-plans`, PR #95) | Physics-first arcade loop plan program: kill economy / loot burst / vacuum pickup, style-kill taxonomy, market coherence, factory loop (claim→extractor→depot→refinery→fab) | **FUTURE — NOT RATIFIED, NOT ON MASTER** | Its `00_INVARIANTS.md` I-1 ("no far away", automatic FAIL for LOD swapping) contradicts §8.2/§8.3 shipped perf work and the PQ-130 camera ruling, so wholesale admission needs an owner decision. The four clusters named here are genuinely uncovered by PQ-133/PQ-134 (measured 2026-08-24) and are the porting candidates; VFX docs 31–39 partially overlap PQ-134 and need per-doc triage |
| [`perf/TABLE_AUTHORITY_PLAN.md`](./perf/TABLE_AUTHORITY_PLAN.md) | 2026-09-09 independent diagnosis and implementation: glass+runway 60 Hz, ledger catch-up, Ceres measurements, four collision-safe lanes. Operator prompt [`program/TABLE_AUTHORITY_GOAL.txt`](./program/TABLE_AUTHORITY_GOAL.txt) | **ACTIVE IMPLEMENTATION PLAN — parent spawns lane subagents** | Live code and a Ceres census decide done; do not restart from “sleep 317 Rapier bodies.” Scaffolding (clocks, asteroidField, dressing, farActors) is already in-tree |
| [`perf/PERFORMANCE_CONVERGENCE_2026-08-24.md`](./perf/PERFORMANCE_CONVERGENCE_2026-08-24.md) | Cloud-agent performance-convergence research: opening geometry-residency diagnosis (the PR #100 finding), cost classes, ten-family quality-preserving backlog, experiment queue | **DURABLE RESEARCH — no dispatch authority** | Code half already ported to master as `PQ-129.19`; backlog items admit only through `program-queue.json` onto the reserved `PQ-051`–`PQ-128` identities; `PERF_BUDGET.md` and map §8 stay authoritative |
...
| [`revamp/FRONTEND_DIRECTION_AND_MENU_ARCHITECTURE.md`](./revamp/FRONTEND_DIRECTION_AND_MENU_ARCHITECTURE.md) | 2D surface direction for **screens, menus, in-flight access, and progression**, with a measured diagnosis and a ranked work order. Companion prototype at repo-root `_uilab.html` | **ACTIVE DETAIL — direction, not admitted work** | Its §0 cedes the flight HUD to [`HUD_FLIGHT_ATTENTION.md`](./HUD_FLIGHT_ATTENTION.md); an admitted packet owns any implementation |
| [`frontend/`](./frontend/README.md) | **Design authority for every 2D surface.** [`INSTRUMENT_GRAMMAR.md`](./frontend/INSTRUMENT_GRAMMAR.md) is binding (type, colour roles, motion contract, layout skeleton, disclosure tiers, naming rules, definition of done); per-screen specs, the Power Bar, and the AI icon pipeline sit beside it. Routed from `build_map.md` §11 | **ACTIVE AUTHORITY — binding on frontend design; not admitted work** | The grammar outranks per-screen docs and any agent's taste; an admitted packet still owns implementation and acceptance |
| [`frontend/direction/FIELD_HARDWARE_PROGRAM.md`](./frontend/direction/FIELD_HARDWARE_PROGRAM.md) | **The UI production program (2026-09-10):** style frames first, produced asset kits (PNG/SVG/3D), code assembling them to match the frames, picture-comparison review; 27 packets under `frontend/direction/packets/` | **ACTIVE AUTHORITY — outranks every other frontend document on aesthetics; frames under `frontend/direction/approved/` outrank it** | Queue `PQ-194`; map `build_map.md` §20.15; supersedes `PQ-187` / §20.14 and the `frontend/` grammar on aesthetics |
...
| [`vision/CRUCIBLE_SURVIVAL_MASTER_PLAN.md`](./vision/CRUCIBLE_SURVIVAL_MASTER_PLAN.md) | Durable product and implementation quarry for Survival, Combat Lab, attack composition, arena laws, and Adventure combat convergence (§30 phases, §31 packets, Appendix A schemas) | **DURABLE DESIGN SOURCE — admitted 2026-08-21 as `PQ-133`; its phases are the dispatch leaves** | Product/design owner curates direction; `program/` owns live status; the build map §12 routes it |
