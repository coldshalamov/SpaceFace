...
| [`frontend/`](./frontend/README.md) | **Design authority for every 2D surface.** [`INSTRUMENT_GRAMMAR.md`](./frontend/INSTRUMENT_GRAMMAR.md) is binding (type, colour roles, motion contract, layout skeleton, disclosure tiers, naming rules, definition of done); per-screen specs, the Power Bar, and the AI icon pipeline sit beside it. Routed from `CANONICAL_BUILD_MAP.md` §11 | **ACTIVE AUTHORITY — binding on frontend design; not admitted work** | The grammar outranks per-screen docs and any agent's taste; an admitted packet still owns implementation and acceptance |
