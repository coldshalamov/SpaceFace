...
2. read current design/program/NOW.md and use program-dispatch to locate the admitted packet;
3. read design/inference-workflows/README.md, 00_SPACEFACE_TEAM_MINDSET.md, 01_SCALE_AND_DISPATCH.md, 02_CREATIVE_CONVERGENCE_LOOP.md, 03_REFERENCE_GAME_PATTERN_LIBRARY.md, 04_REPO_SEAM_AND_AUTHORITY_MAP.md, the selected workflow, 05_ADVERSARIAL_REVIEW_PROTOCOL.md and 06_PORTFOLIO_INTEGRATION_AND_LEARNING.md;
4. inspect only current owners, tests, assets and evidence relevant to the workflow;
5. record current ordinary-player evidence before proposing solutions.

Nx counts accepted production units. It does not count files, rows, candidates, attempts, recolors, commits, source-only packs, tests or documents.
...
- extract mechanisms from references without copying surfaces;
- generate the workflow's required diverse candidate pool;
- score and select a coherent slate plus replacements;
- implement through current owners;
- prove the player-facing route;
- give evidence to a fresh adversarial reviewer without implementation persuasion;
- revise, rebuild, replace or cut according to the verdict;
- at 3x/5x, review the units together in one route/portfolio;
- record the successful production grammar for later multiplication.
