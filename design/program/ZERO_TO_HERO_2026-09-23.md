...
| 5 | The fifteen-minute path plays end to end, and the belt is on the glass after the Crucible: presentation trackers kept stale-epoch envelopes across Crucible → "Take it to the belt" (simTime restarts), inflating rocks to −2.4e8 and every hull to −1.3e7 × scale, which also lifted the chase camera to y≈1.4e8 — the adventure had shown only sky | `probe:demo-path` headed, 4242: **first clean pass** (title → real death → results → belt → job → paid → upgrade → end card, 7.2 min, 0 console / 0 shader errors); belt arrival camera y **1.4e8 → 110**, bodies with \|scale\| > 2000 **1 → 0**; Crucible launch → flight 62–65 s → 36–48 s (host 49–80 %); adventure roster settle 8 s timeout → 0 | `9ddb4a439`, `4420db897`, `fc3d2cd33`, `c223255c7`, `5810784a6` |
...
| 5 | Demo builds record the funnel locally: boot → crucible → round 3 → results → "Take it to the belt" → end card, latched on first reach from real bus events, surfaced ordered in the session report; non-demo sessions record nothing | `test/demo-funnel-telemetry.test.mjs` (5 pins: order, first-reach latch, out-of-order reach, abort predicate, demo-off silence); telemetry/session-report suites 23/23, `check:baseline` 16/16 | `eeab67842` |
