"""Warden chase-camera outbox form rebuild (player gunship).

Imports live Warden only for root, sockets, and collision. Replaces render
meshes with one continuous formed dark-gunmetal shell that reads at the live
chase camera. Aggressive bow, command tower, broadside casemates, twin bow
guns, three-drive transom. Cockpit / casemate / tower wells are holes with
rims. No tube/paddle lobes, no open cage. Hitch/Kestrel compare-only.
Hornet/Drifter/Ranger/Ironback/Bastion/Atlas frozen — do not rebuild.
...
# Dark gunmetal gunship — orange accent, mid-heavy like Hitch.
HONEST = {
    # Dark gunmetal gunship — not navy Bastion, not sea-enamel Atlas, not Hitch.
    "Material_Hull": {"color": (0.14, 0.145, 0.155), "metallic": 0.22, "roughness": 0.44},
    "Material_Armor": {"color": (0.045, 0.048, 0.055), "metallic": 0.34, "roughness": 0.50},
    "Material_Course": {"color": (0.10, 0.105, 0.115), "metallic": 0.20, "roughness": 0.46},
    "Material_Mark": {"color": (0.42, 0.12, 0.06), "metallic": 0.08, "roughness": 0.34},
    "Material_Canopy": {"color": (0.03, 0.06, 0.08), "metallic": 0.0, "roughness": 0.05},
    "Material_Ceramic": {"color": (0.38, 0.32, 0.24), "metallic": 0.04, "roughness": 0.66},
    "Material_Mechanical": {"color": (0.20, 0.205, 0.21), "metallic": 0.58, "roughness": 0.34},
    "Material_Radiator": {"color": (0.09, 0.07, 0.06), "metallic": 0.32, "roughness": 0.56},
    "Material_Thruster": {"color": (0.02, 0.05, 0.07), "metallic": 0.22, "roughness": 0.26},
    "Material_Accent": {"color": (0.55, 0.18, 0.05), "metallic": 0.06, "roughness": 0.28, "emit": 2.2},
    "Material_Warning": {"color": (0.70, 0.32, 0.05), "metallic": 0.04, "roughness": 0.38, "emit": 1.6},
    "Material_Dirt": {"color": (0.040, 0.036, 0.034), "metallic": 0.02, "roughness": 0.84},
}

GIRTH_XS = (6.80, 3.80, 0.60, -2.20, -5.20)
MARK_XS = (5.40, -1.60)
STRINGER_YS = (0.65, 1.25)
STRINGER_SPANS = ((7.60, 4.60), (3.40, 0.40), (-0.80, -3.60), (-4.20, -7.20))

# Corvette stations: (x, beam, hh, zc, keel, flat, box, chine)
# Knife bow → canopy shoulder → casemate waist → tower → twin-drive transom.
HULL_STATIONS = [
    # Gunship: aggressive bow → canopy shoulder → wide casemate mid → tower → three-drive transom.
    # Envelope ~20 m × halfWidth ~2.5 family.
    (9.80, 0.32, 0.24, 0.10, 0.08, 0.12, 0.14, 0.20),
    (8.80, 0.62, 0.40, 0.14, 0.10, 0.24, 0.20, 0.26),
    (7.40, 1.05, 0.62, 0.18, 0.12, 0.42, 0.32, 0.32),
    (5.80, 1.45, 0.82, 0.22, 0.13, 0.70, 0.48, 0.38),
    (4.00, 1.85, 0.98, 0.20, 0.13, 0.82, 0.62, 0.42),
    (2.20, 2.20, 0.88, 0.14, 0.12, 0.48, 0.80, 0.46),
    (0.20, 2.50, 0.95, 0.12, 0.11, 0.40, 0.92, 0.48),
    (-1.60, 2.35, 1.10, 0.16, 0.11, 0.38, 0.94, 0.44),
    (-3.20, 2.10, 1.15, 0.18, 0.10, 0.36, 0.95, 0.40),
    (-5.00, 1.70, 0.98, 0.14, 0.09, 0.34, 0.95, 0.32),
    (-6.80, 1.35, 0.78, 0.12, 0.08, 0.32, 0.94, 0.26),
    (-8.20, 1.05, 0.62, 0.10, 0.07, 0.30, 0.92, 0.20),
    (-9.40, 0.62, 0.42, 0.10, 0.06, 0.28, 0.90, 0.16),
]


def parse_args(argv):
    parser = argparse.ArgumentParser()
    parser.add_argument("--source-lod0", type=Path, default=LIVE_PARTS / "warden_production_v1.glb")
    parser.add_argument("--source-lod1", type=Path, default=LIVE_PARTS / "warden_production_v1_lod1.glb")
    parser.add_argument("--source-lod2", type=Path, default=LIVE_PARTS / "warden_production_v1_lod2.glb")
...
def find_root():
    for name in ("WARDEN_LOD0_ROOT", "WARDEN_LOD1_ROOT", "WARDEN_LOD2_ROOT"):
        named = bpy.data.objects.get(name)
        if named:
            return named
    matches = [
        obj for obj in bpy.data.objects
        if obj.type == "EMPTY" and "WARDEN" in obj.name.upper() and "ROOT" in obj.name.upper()
    ]
    if len(matches) == 1:
        return matches[0]
    ...
    if len(matches) == 1:
        return matches[0]
    raise RuntimeError("missing Warden root")
...
def dorsal_blocked(x, y):
    # cockpit well
    if abs(x - 3.85) < 0.90 and abs(y) < 0.60:
        return True
    # tower well / carapace
    if abs(x + 1.85) < 0.90 and abs(y) < 0.60:
        return True
    return False
...
def paint_shell(hull, mats):
    ...
    for poly in mesh.polygons:
        ...
        bow_cap = centroid.x > 8.20
        armor_belt = abs(centroid.z - zc) < hh * 0.22 and abs(centroid.y) > beam * 0.42
        waist_course = 1.40 < centroid.x < 4.80 and centroid.z > zc + hh * 0.08
...
def build_hull(mats):
    ...
    for sign, side in ((-1.0, "P"), (1.0, "S")):
        def dorsal_seam(name=f"DorsalSeam_{side}", loc=(0.60, 0.48 * sign, 1.02)):
            return add_box(name, (13.6, 0.055, 0.080), loc, hull_mat, 0.0)

        cut(hull, dorsal_seam)

        def bilge_seam(name=f"BilgeSeam_{side}", loc=(-0.40, 1.25 * sign, -0.50)):
            return add_box(name, (12.0, 0.07, 0.12), loc, hull_mat, 0.0)
    ...
    def bow_step():
        return add_box("BowCourseStep", (0.14, 2.80, 0.22), (7.80, 0.0, 0.72), hull_mat, 0.0)

    cut(hull, bow_step)

    def shoulder_step():
        return add_box("ShoulderCourseStep", (0.16, 3.80, 0.20), (4.60, 0.0, 1.02), hull_mat, 0.0)

    cut(hull, shoulder_step)

    def waist_break():
        return add_box("WaistBreak", (0.80, 3.20, 0.18), (1.40, 0.0, 1.00), hull_mat, 0.0)
...
def build_cockpit_well(hull, mats, lod):
    ...
    def cockpit_cut():
        return add_box("CockpitCut", (1.50, 0.82, 0.78), (3.85, 0.0, 1.18), mech, 0.0)

    cut(hull, cockpit_cut)
    bits = add_open_well("LOD0_CockpitTub", (1.38, 0.74, 0.72), (3.85, 0.0, 0.60), mech, floor=True, wall=0.048)
    rings = []
    for x, hw, hh, zc in (
        (4.50, 0.40, 0.22, 1.14),
        (3.85, 0.52, 0.30, 1.26),
    ...
    bits.append(add_box("LOD0_CabinShoulder", (0.90, 0.66, 0.055), (3.15, 0.0, 1.04), armor, 0.006))
    return bits, {"kind": "tub+canopy", "location": [3.85, 0.0, 0.60]}
...
def build_tower_well(hull, mats, lod):
    ...
    def tower_cut():
        return add_box("TowerCut", (1.20, 0.82, 0.55), (-1.85, 0.0, 1.22), mech, 0.0)

    cut(hull, tower_cut)
    bits = add_open_well("LOD0_TowerWell", (1.10, 0.72, 0.48), (-1.85, 0.0, 0.92), mech, floor=True, wall=0.048)
    bits.append(add_box("LOD0_TowerRim", (1.25, 0.86, 0.06), (-1.85, 0.0, 1.25), armor, 0.006))
    # Command tower massing — readable at D=144
    bits.append(add_box("LOD0_TowerBase", (0.98, 0.70, 0.28), (-1.85, 0.0, 1.32), armor, 0.010))
    bits.append(add_box("LOD0_TowerTrunk", (0.58, 0.42, 0.50), (-1.90, 0.0, 1.82), mats["Material_Hull"], 0.010))
    bits.append(add_box("LOD0_TowerShoulder", (0.76, 0.54, 0.11), (-1.80, 0.0, 2.18), armor, 0.006))
    bits.append(add_box("LOD0_TowerBridge", (0.88, 0.64, 0.22), (-1.65, 0.0, 2.42), armor, 0.008))
    bits.append(add_box("LOD0_TowerGlass", (0.05, 0.50, 0.12), (-1.16, 0.0, 2.44), canopy, 0.003))
    bits.append(add_box("LOD0_TowerMast", (0.055, 0.055, 0.36), (-2.00, 0.0, 2.82), mech, 0.003))
    bits.append(add_box("LOD0_TowerYard", (0.04, 0.24, 0.035), (-2.00, 0.0, 2.98), mech, 0.002))
    if lod == 0:
        bits.append(add_cylinder(
            "LOD0_TowerRadar", 0.13, 0.05, (-1.50, 0.22, 2.58), armor, 0.002,
            rotation=(0, math.pi / 2.6, 0), vertices=12,
        ))
    return bits, {"kind": "hole+stack", "location": [-1.85, 0.0, 0.92]}
...
def build_casemate_wells(hull, mats, lod):
    ...
    for sign, side in ((-1.0, "Port"), (1.0, "Starboard")):
        ...
        def bay(n=f"CaseBay_{side}", loc=(0.15, y, 0.20)):
            return add_box(n, (2.15, 0.56, 0.60), loc, mech, 0.0)

        cut(hull, bay)
        bits.extend(add_open_well(
            f"LOD0_CaseWell_{side}", (2.00, 0.48, 0.52),
            (0.15, 2.10 * sign, 0.12), mech, floor=True, wall=0.042,
        ))
        bits.append(add_box(f"LOD0_CaseLip_{side}", (2.08, 0.06, 0.34), (0.15, 2.48 * sign, 0.18), mech, 0.004))
        bits.append(add_box(f"LOD0_CaseDeck_{side}", (1.40, 0.22, 0.045), (0.15, 1.75 * sign, 0.48), armor, 0.004))
        # Broadside guns — mass at chase distance
        bits.append(add_cylinder(f"LOD0_Broad_Fore_{side}", 0.080, 0.72, (0.70, 2.62 * sign, 0.18), mech, 0.003, vertices=10))
        bits.append(add_cylinder(f"LOD0_Broad_Aft_{side}", 0.080, 0.72, (-0.45, 2.62 * sign, 0.18), mech, 0.003, vertices=10))
        bits.append(add_cylinder(f"LOD0_Broad_Mid_{side}", 0.058, 0.52, (0.15, 2.62 * sign, 0.32), mech, 0.002, vertices=8))
...
def build_drive_houses(mats, lod):
    ...
    bits = []
    report = {"houses": []}
    # Three-drive gunship transom: Port / Center / Starboard at y = ±1.80 and 0.0
    for y, side in ((-1.80, "Port"), (0.0, "Center"), (1.80, "Starboard")):
        house_rings = []
        for x, hw, hh, zc in (
            (-6.40, 0.34, 0.26, 0.12),
            (-7.40, 0.44, 0.34, 0.10),
        ...
        bits.append(add_cylinder(f"LOD0_DriveCollar_{side}", 0.36, 0.14, (-8.55, y, 0.10), ceramic, 0.006, vertices=14))
        bits.append(add_cylinder(f"LOD0_Bell_{side}", 0.38, 0.48, (-9.25, y, 0.10), thruster, 0.004, vertices=16))
        bits.append(add_cylinder(f"LOD0_BellThroat_{side}", 0.22, 0.32, (-9.18, y, 0.10), dirt, 0.0, vertices=12))
        if lod == 0:
            bits.append(add_box(f"LOD0_DriveBand_{side}", (0.08, 0.50, 0.34), (-7.40, y, 0.10), armor, 0.003))
        report["houses"].append({"side": side, "y": y})
    bits.append(add_box("LOD0_TransomHeader", (0.85, 3.60, 0.05), (-8.20, 0.0, 0.58), armor, 0.005))
    return bits, report
...
def build_guns(mats, lod):
    ...
    bits = []
    for sign, side in ((-1.0, "P"), (1.0, "S")):
        bits.append(add_cylinder(f"LOD0_GunHouse_{side}", 0.110, 1.15, (5.80, 0.55 * sign, 0.04), mech, 0.004, vertices=10))
        bits.append(add_cylinder(f"LOD0_GunBarrel_{side}", 0.040, 0.88, (6.80, 0.55 * sign, 0.04), armor, 0.002, vertices=8))
        bits.append(add_box(f"LOD0_GunMantlet_{side}", (0.14, 0.12, 0.12), (5.20, 0.55 * sign, 0.04), armor, 0.004))
        bits.append(add_cylinder(f"LOD0_RearGun_{side}", 0.048, 0.60, (-6.40, 0.72 * sign, 0.50), mech, 0.003, vertices=8))
    bits.append(add_cylinder("LOD0_TurretRing", 0.36, 0.10, (0.55, 0.0, 1.10), mech, 0.005, rotation=(0, 0, 0), vertices=14))
    bits.append(add_box("LOD0_TurretHead", (0.36, 0.22, 0.12), (0.70, 0.0, 1.26), armor, 0.005))
    return bits
...
def build_hardware(hull, mats, lod):
    ...
    bits = []
    bits.append(add_box("LOD0_TransomPlate", (0.07, 2.10, 0.48), (-9.15, 0.0, 0.14), armor, 0.004))
    bits.append(add_box("LOD0_KeelSpine", (5.2, 0.28, 0.05), (0.20, 0.0, -0.98), mech, 0.008))
    bits.append(add_box("LOD0_AftWalk", (1.8, 0.16, 0.016), (-3.40, 0.0, 0.58), mech, 0.002))
    bits.append(add_box("LOD0_RepairPatch", (0.34, 0.16, 0.014), (2.00, -0.70, 0.92), warning, 0.002))
    bits.append(add_box("LOD0_WarnChevron", (0.24, 0.10, 0.012), (-3.55, -0.90, 0.82), warning, 0.002))
    bits.append(add_box("LOD0_HatchLid", (0.34, 0.20, 0.018), (-0.85, 0.40, 0.86), armor, 0.003))
    bits.append(add_cylinder("LOD0_CommMast", 0.038, 0.70, (1.40, 0.0, 1.22), mech, 0.002, rotation=(0, 0, 0), vertices=8))
    bits.append(add_box("LOD0_CommHead", (0.12, 0.16, 0.06), (1.48, 0.0, 1.60), armor, 0.003))
    bits.append(add_box("LOD0_AccentBand", (0.10, 3.60, 0.08), (5.40, 0.0, 0.40), accent, 0.003))
    for sign, side in ((-1.0, "P"), (1.0, "S")):
        bits.append(add_box(f"LOD0_RCS_{side}", (0.24, 0.16, 0.16), (-1.40, 2.40 * sign, 0.20), mech, 0.003))
    if lod == 0:
        for sign, side in ((-1.0, "P"), (1.0, "S")):
            for i in range(5):
                bits.append(add_box(
                    ...
                    (0.014, 0.18, 0.20),
                    (-0.25 + i * 0.20, 2.00 * sign, 0.24),
                    mech, 0.001,
                ))
        bits.append(add_box("LOD0_DeckCrate", (0.18, 0.12, 0.10), (-1.60, 0.48, 0.78), warning, 0.003))
    return bits
...
def build_one(source: Path, output: Path, lod: int):
    ...
    # Gunship envelope: ~20 m long, wide casemate beam, tower height
    if size.x < 18.5 or size.x > 22.5 or size.y < 4.8 or size.y > 7.2 or size.z < 2.4 or size.z > 4.8:
