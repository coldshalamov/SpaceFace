...
if (!gas) throw new Error('no gas');

// Prime: one empty update so writtenCount is settled.
gas.setAccessibility(resolveVfxAccessibilityProfile(state.settings));
gas.update(state.simTime, cam);

function runBefore() {
  // Pre-#124 residual: always resolve + set + empty update.
  system._gasQuietEmpty = false;
  const gasSimTime = state.simTime;
  gas.setAccessibility(resolveVfxAccessibilityProfile(state.settings));
  gas.update(gasSimTime, cam);
}

function runAfter() {
  if (system._gasQuietEmpty && !(gas.liveCount > 0)) return;
  system._gasQuietEmpty = false;
  const gasSimTime = state.simTime;
  gas.setAccessibility(resolveVfxAccessibilityProfile(state.settings));
  gas.update(gasSimTime, cam);
  if (!(gas.liveCount > 0)) system._gasQuietEmpty = true;
}
