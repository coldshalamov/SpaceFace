/**
 * #126 weapon-presenter-composite-quiet-latch — portable quiet CPU.
 * Before = full update residual with latch forced off each tick.
 * After  = composite latch early-out (pool-counter dirty-wake).
...
const ITERS = 80000;
const PAIRS = 13;
const worker = 'artifacts/weapon-presenter-composite-quiet-latch-microbench-worker.mjs';

function runMode(mode) {
  const r = spawnSync(process.execPath, [worker, mode, String(ITERS)], {
    encoding: 'utf8', cwd: process.cwd(), timeout: 180000,
  });
...
pairs.sort((x, y) => x - y);
const out = {
  name: 'weapon-presenter-composite-quiet-latch',
  iters: ITERS,
  pairs: PAIRS,
...
writeFileSync('artifacts/weapon-presenter-composite-quiet-latch-microbench.json', JSON.stringify(out, null, 2));
