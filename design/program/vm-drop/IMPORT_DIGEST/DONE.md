...
Scratch `vm-work/hillclimb-20260924h` through #58; +#59 @ `8119853f7`.
...
Cite used to rank; #59 cuts redundant preStep movables re-check.

**Shipped this pass:**
- **#59 `prestep-movables-trust`** — portable quiet `isMovableEntity` re-check
  on `index.movables` **~2.19–2.28×** median (80k × 9; admit parity); focused
  lifecycle + coreSystem + interp + volatile-classes **29/29**.
...
isMovableEntity type-first (~1.10× — different from #59 lane trust),
propulsion, soft-GPU draw-batch, spaceBg steady-state,
midflight-wave-hull-decode, syncCombatantBounds,
