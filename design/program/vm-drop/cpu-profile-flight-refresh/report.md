...
| 8 | 57.3 | 0.09% | `stepSimulation` @ `src/core/simulationRunner.js:275` | (sim owner; flight-dormant-skip helps) |
| 9 | 53.2 | 0.09% | `preStep` @ `src/core/coreSystem.js:202` | — |
| 10 | 52.9 | 0.09% | `frame` @ `src/core/presentationRunner.js:780` | soft-GPU present owner (ignore fps) |
| 11 | 50.5 | 0.08% | `runRenderUpdatePhase` @ `src/core/renderUpdatePhase.js:14` | — |
| 12 | 47.0 | 0.08% | `drawRangePlate` @ `src/ui/radar.js:397` | radar-range-plate-cache |
| 13 | 45.0 | 0.07% | `_stepCraft` @ `src/systems/flightV3.js:219` | **flight-propulsion-scratch (new)** |
| 14 | 43.0 | 0.07% | `syncCombatantBounds` @ `src/combat/runtime.js:84` | measured miss (early-out slower) |
| 15 | 42.4 | 0.07% | `setStyle` @ `src/ui/hud.js:950` | already on master (_sfStyle) |
| 16 | 41.6 | 0.07% | `_stepFixed` @ `src/core/sg02DynamicBodyOwner.js:670` | — |
| 17 | 40.3 | 0.07% | `makeResult` @ `src/core/flight/propulsionKernel.js:1098` | **flight-propulsion-scratch (new)** |
| 18 | 39.6 | 0.07% | `isHostileForAI` @ `src/ai/engagementAuthority.js:313` | left for later |
| 19 | 38.0 | 0.06% | `queryAsteroidField` @ `src/world/asteroidField.js:146` | asteroid-query-callers |
| 20 | 32.3 | 0.05% | `append` @ `src/core/presentationJournal.js:274` | alloc-journal-churn |
...
9. **NEW** `flight-propulsion-scratch` `aa6ec09ec` — coolRuntime ~174× + trust body ~10.7×
10. **NEW** `massline-settext-cache` `c0718f881` — massline setText cache
...
- **flight-propulsion-scratch** @ `aa6ec09ec9c15729986033dfd099165c4d44acea` — coolRuntime scratch (~174×) + trust `_sfNormalized` bodySnapshot (~10.7×); tests 52/52
- **massline-settext-cache** @ `c0718f8816f03e96104b605878a074af90b6b687` — massline `_sfText` cache (0 DOM text reads); tests 33+69 pass

## Honest note

Most top portable poles were already covered by unimported vm-drop packages. Import list above first. New poles that still existed under the exclusion list: propulsion coolRuntime/normalizeBody, massline textContent reads — both shipped this session.
