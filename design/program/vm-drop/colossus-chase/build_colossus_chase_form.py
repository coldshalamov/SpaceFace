"""Colossus chase-camera outbox form rebuild (player capital).

Imports live Colossus only for root, sockets, and collision. Replaces render
meshes with one continuous formed dark-slate capital shell that reads at the
live chase camera — not as a diagram. Command island, broadside batteries,
four-drive transom, dorsal fins, triple bow guns. Cockpit / battery / island /
radiator wells are holes with rims. No tube/paddle lobes, no open cage.
Hitch/Kestrel compare-only. Prior ships frozen — do not rebuild.
...
HONEST = {
    # Dark slate capital — not gunmetal Warden, not navy Bastion, not Hitch.
    "Material_Hull": {"color": (0.13, 0.14, 0.155), "metallic": 0.18, "roughness": 0.48},
    "Material_Armor": {"color": (0.040, 0.044, 0.052), "metallic": 0.32, "roughness": 0.54},
    "Material_Course": {"color": (0.095, 0.100, 0.112), "metallic": 0.18, "roughness": 0.50},
    "Material_Mark": {"color": (0.48, 0.16, 0.05), "metallic": 0.08, "roughness": 0.36},
    "Material_Canopy": {"color": (0.025, 0.055, 0.070), "metallic": 0.0, "roughness": 0.05},
    "Material_Ceramic": {"color": (0.40, 0.34, 0.26), "metallic": 0.04, "roughness": 0.66},
    "Material_Mechanical": {"color": (0.18, 0.185, 0.195), "metallic": 0.58, "roughness": 0.34},
    "Material_Radiator": {"color": (0.085, 0.065, 0.055), "metallic": 0.30, "roughness": 0.58},
    "Material_Thruster": {"color": (0.018, 0.045, 0.065), "metallic": 0.22, "roughness": 0.26},
    "Material_Accent": {"color": (0.58, 0.20, 0.05), "metallic": 0.06, "roughness": 0.28, "emit": 2.0},
    "Material_Warning": {"color": (0.72, 0.34, 0.05), "metallic": 0.04, "roughness": 0.38, "emit": 1.6},
    "Material_Dirt": {"color": (0.038, 0.034, 0.032), "metallic": 0.02, "roughness": 0.84},
}

GIRTH_XS = (8.40, 5.20, 2.00, -1.20, -4.40, -7.60)
MARK_XS = (6.80, -2.40)
STRINGER_YS = (0.85, 1.55, 2.15)
STRINGER_SPANS = ((9.40, 6.20), (4.80, 1.60), (0.40, -2.80), (-3.60, -6.80), (-7.40, -10.20))

# Capital stations: (x, beam, hh, zc, keel, flat, box, chine)
# Blunt capital bow → bridge shoulder → battery waist → command island → four-drive transom.
# Envelope ~24 m × halfWidth ~2.9 family (MATERIAL_TRUTH_PREFLIGHT).
HULL_STATIONS = [
    (11.80, 0.38, 0.28, 0.10, 0.08, 0.14, 0.16, 0.22),
    (10.60, 0.78, 0.48, 0.14, 0.10, 0.28, 0.22, 0.28),
    (9.00, 1.35, 0.72, 0.18, 0.12, 0.48, 0.34, 0.34),
    (7.20, 1.85, 0.92, 0.22, 0.13, 0.74, 0.50, 0.40),
    (5.00, 2.35, 1.08, 0.20, 0.13, 0.86, 0.64, 0.44),
    (2.80, 2.70, 0.98, 0.14, 0.12, 0.50, 0.82, 0.48),
    (0.40, 2.90, 1.05, 0.12, 0.11, 0.42, 0.92, 0.50),
    (-1.80, 2.80, 1.20, 0.16, 0.11, 0.40, 0.94, 0.46),
    (-3.80, 2.55, 1.28, 0.20, 0.10, 0.38, 0.95, 0.42),
    (-5.80, 2.20, 1.10, 0.16, 0.09, 0.36, 0.95, 0.36),
    (-7.80, 1.75, 0.90, 0.14, 0.08, 0.34, 0.94, 0.30),
    (-9.60, 1.30, 0.70, 0.12, 0.07, 0.32, 0.92, 0.24),
    (-11.10, 0.78, 0.48, 0.10, 0.06, 0.30, 0.90, 0.18),
]


def parse_args(argv):
    parser = argparse.ArgumentParser()
    parser.add_argument("--source-lod0", type=Path, default=LIVE_PARTS / "colossus_production_v1.glb")
    parser.add_argument("--source-lod1", type=Path, default=LIVE_PARTS / "colossus_production_v1_lod1.glb")
    parser.add_argument("--source-lod2", type=Path, default=LIVE_PARTS / "colossus_production_v1_lod2.glb")
...
def find_root():
    for name in ("COLOSSUS_LOD0_ROOT", "COLOSSUS_LOD1_ROOT", "COLOSSUS_LOD2_ROOT"):
        named = bpy.data.objects.get(name)
        if named:
            return named
    matches = [
        obj for obj in bpy.data.objects
        if obj.type == "EMPTY" and "COLOSSUS" in obj.name.upper() and "ROOT" in obj.name.upper()
    ]
    if len(matches) == 1:
        return matches[0]
    ...
    if len(matches) == 1:
        return matches[0]
    raise RuntimeError("missing Colossus root")
...
def dorsal_blocked(x, y):
    # bridge / cockpit well
    if abs(x - 4.80) < 1.10 and abs(y) < 0.75:
        return True
    # command island well / carapace
    if abs(x + 2.40) < 1.20 and abs(y) < 0.85:
        return True
    # radiator cassette wells
    if abs(x + 6.20) < 0.90 and 1.20 < abs(y) < 2.40:
        return True
    return False
...
def paint_shell(hull, mats):
    ...
    for poly in mesh.polygons:
        ...
        bow_cap = centroid.x > 9.80
        armor_belt = abs(centroid.z - zc) < hh * 0.22 and abs(centroid.y) > beam * 0.42
        waist_course = 1.80 < centroid.x < 6.20 and centroid.z > zc + hh * 0.08
...
def build_cockpit_well(hull, mats, lod):
    ...
    def cockpit_cut():
        return add_box("CockpitCut", (1.80, 1.00, 0.90), (4.80, 0.0, 1.28), mech, 0.0)

    cut(hull, cockpit_cut)
    bits = add_open_well("LOD0_CockpitTub", (1.65, 0.90, 0.82), (4.80, 0.0, 0.68), mech, floor=True, wall=0.050)
    rings = []
    for x, hw, hh, zc in (
        (5.55, 0.48, 0.26, 1.22),
        (4.80, 0.64, 0.36, 1.38),
    ...
    bits.append(add_box("LOD0_CabinShoulder", (1.10, 0.82, 0.060), (4.00, 0.0, 1.12), armor, 0.006))
    return bits, {"kind": "tub+canopy", "location": [4.80, 0.0, 0.68]}
...
def build_tower_well(hull, mats, lod):
    ...
    def tower_cut():
        return add_box("TowerCut", (1.55, 1.05, 0.70), (-2.40, 0.0, 1.35), mech, 0.0)

    cut(hull, tower_cut)
    bits = add_open_well("LOD0_TowerWell", (1.40, 0.92, 0.58), (-2.40, 0.0, 1.00), mech, floor=True, wall=0.050)
    bits.append(add_box("LOD0_TowerRim", (1.55, 1.05, 0.07), (-2.40, 0.0, 1.38), armor, 0.006))
    # Command island — capital massing readable at D=144
    bits.append(add_box("LOD0_TowerBase", (1.25, 0.92, 0.36), (-2.40, 0.0, 1.48), armor, 0.010))
    bits.append(add_box("LOD0_TowerTrunk", (0.78, 0.58, 0.70), (-2.50, 0.0, 2.10), mats["Material_Hull"], 0.010))
    bits.append(add_box("LOD0_TowerShoulder", (1.00, 0.74, 0.14), (-2.30, 0.0, 2.58), armor, 0.006))
    bits.append(add_box("LOD0_TowerBridge", (1.15, 0.86, 0.30), (-2.10, 0.0, 2.90), armor, 0.008))
    bits.append(add_box("LOD0_TowerGlass", (0.06, 0.68, 0.16), (-1.48, 0.0, 2.94), canopy, 0.003))
    bits.append(add_box("LOD0_TowerMast", (0.065, 0.065, 0.48), (-2.60, 0.0, 3.40), mech, 0.003))
    bits.append(add_box("LOD0_TowerYard", (0.05, 0.32, 0.040), (-2.60, 0.0, 3.62), mech, 0.002))
    if lod == 0:
        bits.append(add_cylinder(
            "LOD0_TowerRadar", 0.16, 0.06, (-1.90, 0.28, 3.10), armor, 0.002,
            rotation=(0, math.pi / 2.6, 0), vertices=12,
        ))
    return bits, {"kind": "hole+stack", "location": [-2.40, 0.0, 1.00]}
...
def build_casemate_wells(hull, mats, lod):
    ...
    for sign, side in ((-1.0, "Port"), (1.0, "Starboard")):
        ...
        def bay(n=f"CaseBay_{side}", loc=(0.40, y, 0.22)):
            return add_box(n, (3.20, 0.70, 0.72), loc, mech, 0.0)

        cut(hull, bay)
        bits.extend(add_open_well(
            f"LOD0_CaseWell_{side}", (3.00, 0.58, 0.62),
            (0.40, 2.45 * sign, 0.14), mech, floor=True, wall=0.045,
        ))
        bits.append(add_box(f"LOD0_CaseLip_{side}", (3.10, 0.07, 0.40), (0.40, 2.92 * sign, 0.20), mech, 0.004))
        bits.append(add_box(f"LOD0_CaseDeck_{side}", (2.00, 0.28, 0.050), (0.40, 2.05 * sign, 0.55), armor, 0.004))
        # Broadside battery cluster — mass at chase distance
        bits.append(add_cylinder(f"LOD0_Broad_Fore_{side}", 0.095, 0.90, (1.20, 3.05 * sign, 0.20), mech, 0.003, vertices=10))
        bits.append(add_cylinder(f"LOD0_Broad_Mid_{side}", 0.095, 0.90, (0.40, 3.05 * sign, 0.20), mech, 0.003, vertices=10))
        bits.append(add_cylinder(f"LOD0_Broad_Aft_{side}", 0.095, 0.90, (-0.40, 3.05 * sign, 0.20), mech, 0.003, vertices=10))
        bits.append(add_cylinder(f"LOD0_Broad_Upper_{side}", 0.065, 0.62, (0.40, 3.05 * sign, 0.40), mech, 0.002, vertices=8))
        # Casemate blister as continuous flank volume
        case_rings = []
        for x, hw, hh, zc in (
            (4.20, 0.36, 0.36, 0.10),
            (0.40, 0.52, 0.44, 0.08),
...
def build_radiator_wells(hull, mats, lod):
    ...
    for sign, side in ((-1.0, "Port"), (1.0, "Starboard")):
        ...
        def rad_cut(n=f"RadCut_{side}", loc=(-6.20, y, 0.55)):
            return add_box(n, (1.60, 0.72, 0.55), loc, mech, 0.0)

        cut(hull, rad_cut)
        bits.extend(add_open_well(
            f"LOD0_RadWell_{side}", (1.45, 0.60, 0.48),
            (-6.20, y, 0.35), mech, floor=True, wall=0.040,
        ))
        if lod == 0:
            for i in range(6):
                bits.append(add_box(
...
def build_drive_houses(mats, lod):
    ...
    bits = []
    report = {"houses": []}
    # Four-drive capital transom: y = ±2.0 and ±0.75 (MATERIAL_TRUTH)
    for y, side in ((-2.00, "PortOut"), (-0.75, "PortIn"), (0.75, "StbdIn"), (2.00, "StbdOut")):
        house_rings = []
        for x, hw, hh, zc in (
            (-8.20, 0.32, 0.24, 0.12),
            (-9.20, 0.42, 0.32, 0.10),
            (-10.10, 0.38, 0.30, 0.10),
            (-10.85, 0.26, 0.20, 0.10),
        ):
            house_rings.append([
                (x, y + hw, zc),
                (x, y + hw * 0.35, zc + hh),
        ...
        bits.append(add_cylinder(f"LOD0_DriveCollar_{side}", 0.34, 0.14, (-10.40, y, 0.10), ceramic, 0.006, vertices=14))
        bits.append(add_cylinder(f"LOD0_Bell_{side}", 0.36, 0.48, (-11.15, y, 0.10), thruster, 0.004, vertices=16))
        bits.append(add_cylinder(f"LOD0_BellThroat_{side}", 0.20, 0.32, (-11.05, y, 0.10), dirt, 0.0, vertices=12))
        if lod == 0:
            bits.append(add_box(f"LOD0_DriveBand_{side}", (0.08, 0.46, 0.32), (-9.20, y, 0.10), armor, 0.003))
        report["houses"].append({"side": side, "y": y})
    bits.append(add_box("LOD0_TransomHeader", (0.95, 4.60, 0.06), (-10.10, 0.0, 0.62), armor, 0.005))
    return bits, report
...
def build_fins(mats, lod):
    ...
    for sign, side in ((-1.0, "P"), (1.0, "S")):
        ...
        for x, root_y, tip_y, z0, zh in (
            (-0.80, 1.10, 1.55, 1.05, 0.55),
            (-2.40, 1.20, 1.85, 1.15, 0.85),
            (-4.00, 1.05, 1.55, 1.05, 0.55),
        ):
            y_root = root_y * sign
            y_tip = tip_y * sign
            fin_rings.append([
        ...
        bits.append(add_box(f"LOD0_FinRoot_{side}", (2.80, 0.18, 0.12), (-2.40, 1.05 * sign, 1.05), hull_mat, 0.006))
    return bits
...
def build_guns(mats, lod):
    ...
    armor = mats["Material_Armor"]
    bits = []
    # Triple bow guns — capital forward battery
    for y, tag in ((-0.55, "P"), (0.0, "C"), (0.55, "S")):
        bits.append(add_cylinder(f"LOD0_GunHouse_{tag}", 0.125, 1.35, (7.40, y, 0.06), mech, 0.004, vertices=10))
        bits.append(add_cylinder(f"LOD0_GunBarrel_{tag}", 0.048, 1.05, (8.55, y, 0.06), armor, 0.002, vertices=8))
        bits.append(add_box(f"LOD0_GunMantlet_{tag}", (0.16, 0.14, 0.14), (6.65, y, 0.06), armor, 0.004))
    for sign, side in ((-1.0, "P"), (1.0, "S")):
        bits.append(add_cylinder(f"LOD0_RearGun_{side}", 0.055, 0.70, (-7.60, 0.90 * sign, 0.55), mech, 0.003, vertices=8))
    bits.append(add_cylinder("LOD0_TurretRing", 0.42, 0.12, (0.80, 0.0, 1.20), mech, 0.005, rotation=(0, 0, 0), vertices=14))
    bits.append(add_box("LOD0_TurretHead", (0.44, 0.28, 0.14), (1.00, 0.0, 1.40), armor, 0.005))
    return bits
...
def build_hardware(hull, mats, lod):
    ...
    bits = []
    bits.append(add_box("LOD0_TransomPlate", (0.08, 3.40, 0.55), (-11.00, 0.0, 0.16), armor, 0.004))
    bits.append(add_box("LOD0_KeelSpine", (7.5, 0.34, 0.06), (0.20, 0.0, -1.10), mech, 0.008))
    bits.append(add_box("LOD0_AftWalk", (2.4, 0.18, 0.018), (-4.20, 0.0, 0.68), mech, 0.002))
    bits.append(add_box("LOD0_RepairPatch", (0.40, 0.18, 0.016), (2.40, -0.90, 1.05), warning, 0.002))
    bits.append(add_box("LOD0_WarnChevron", (0.28, 0.12, 0.014), (-4.40, -1.15, 0.95), warning, 0.002))
    bits.append(add_box("LOD0_HatchLid", (0.40, 0.24, 0.020), (-0.60, 0.50, 0.98), armor, 0.003))
    bits.append(add_cylinder("LOD0_CommMast", 0.042, 0.85, (1.80, 0.0, 1.35), mech, 0.002, rotation=(0, 0, 0), vertices=8))
    bits.append(add_box("LOD0_CommHead", (0.14, 0.18, 0.07), (1.90, 0.0, 1.82), armor, 0.003))
    bits.append(add_box("LOD0_AccentBand", (0.12, 4.60, 0.09), (6.80, 0.0, 0.45), accent, 0.003))
    for sign, side in ((-1.0, "P"), (1.0, "S")):
        bits.append(add_box(f"LOD0_RCS_{side}", (0.28, 0.18, 0.18), (-1.60, 2.85 * sign, 0.22), mech, 0.003))
    if lod == 0:
        bits.append(add_box("LOD0_DeckCrate", (0.22, 0.14, 0.12), (-1.80, 0.55, 0.90), warning, 0.003))
    return bits
...
def build_one(source: Path, output: Path, lod: int):
    ...
    # Capital envelope: ~24 m long, ~2.9 half-width family, island height
    if size.x < 22.0 or size.x > 27.5 or size.y < 5.4 or size.y > 8.5 or size.z < 2.8 or size.z > 5.8:
