"""Ironback chase-camera outbox form rebuild (player barge).

Imports live Ironback only for root, sockets, and collision. Replaces render
meshes with one continuous formed barge shell that reads at the live chase
camera. Hopper is a hole with a rim. No tube/paddle lobes, no open cage.
Hitch/Kestrel compare-only. Hornet/Drifter closed — do not rebuild.
...
# Industrial barge — orange-brown hull, darker armor, mid-heavy like Hitch.
HONEST = {
    "Material_Hull": {"color": (0.30, 0.16, 0.08), "metallic": 0.12, "roughness": 0.52},
    "Material_Armor": {"color": (0.055, 0.042, 0.034), "metallic": 0.24, "roughness": 0.60},
    "Material_Course": {"color": (0.18, 0.11, 0.06), "metallic": 0.16, "roughness": 0.54},
    "Material_Mark": {"color": (0.72, 0.42, 0.12), "metallic": 0.06, "roughness": 0.38},
    "Material_Canopy": {"color": (0.012, 0.016, 0.020), "metallic": 0.0, "roughness": 0.06},
    "Material_Ceramic": {"color": (0.10, 0.08, 0.06), "metallic": 0.08, "roughness": 0.66},
    "Material_Mechanical": {"color": (0.048, 0.050, 0.052), "metallic": 0.40, "roughness": 0.48},
    "Material_Radiator": {"color": (0.050, 0.038, 0.032), "metallic": 0.22, "roughness": 0.56},
    "Material_Thruster": {"color": (0.028, 0.028, 0.032), "metallic": 0.28, "roughness": 0.50},
    "Material_Accent": {"color": (0.85, 0.48, 0.10), "metallic": 0.04, "roughness": 0.32, "emit": 2.8},
    "Material_Warning": {"color": (0.78, 0.36, 0.10), "metallic": 0.02, "roughness": 0.34, "emit": 2.2},
    "Material_Dirt": {"color": (0.070, 0.048, 0.032), "metallic": 0.02, "roughness": 0.80},
}

GIRTH_XS = (5.40, 2.10, -1.20, -4.60)
MARK_XS = (3.40, -2.80)
STRINGER_YS = (0.70, 1.35)
STRINGER_SPANS = ((6.40, 3.60), (2.80, -0.40), (-1.60, -5.40))

# Blunt barge stations: (x, beam, hh, zc, keel, flat, box, chine)
HULL_STATIONS = [
    (8.40, 1.55, 0.72, 0.10, 0.10, 0.70, 0.55, 0.28),
    (7.70, 2.05, 0.92, 0.16, 0.12, 0.80, 0.58, 0.32),
    (6.80, 2.45, 1.10, 0.22, 0.14, 0.90, 0.62, 0.36),
    (5.60, 2.70, 1.22, 0.24, 0.14, 0.94, 0.68, 0.38),
    (4.20, 2.85, 1.28, 0.22, 0.13, 0.96, 0.72, 0.40),
    (2.60, 2.95, 1.20, 0.18, 0.12, 0.88, 0.80, 0.42),
    (0.80, 3.05, 1.08, 0.14, 0.12, 0.55, 0.90, 0.40),
    (-0.60, 3.02, 1.00, 0.12, 0.11, 0.40, 0.92, 0.38),
    (-2.20, 2.85, 1.05, 0.14, 0.11, 0.45, 0.88, 0.34),
    (-4.00, 2.55, 1.08, 0.16, 0.10, 0.50, 0.86, 0.30),
    (-5.60, 2.20, 1.00, 0.14, 0.09, 0.48, 0.90, 0.26),
    (-7.00, 1.85, 0.88, 0.12, 0.08, 0.42, 0.92, 0.22),
    (-8.20, 1.35, 0.70, 0.10, 0.07, 0.35, 0.90, 0.18),
]


def parse_args(argv):
    parser = argparse.ArgumentParser()
    parser.add_argument("--source-lod0", type=Path, default=LIVE_PARTS / "ironback_production_v1.glb")
    parser.add_argument("--source-lod1", type=Path, default=LIVE_PARTS / "ironback_production_v1_lod1.glb")
    parser.add_argument("--source-lod2", type=Path, default=LIVE_PARTS / "ironback_production_v1_lod2.glb")
...
def find_root():
    for name in ("IRONBACK_LOD0_ROOT", "IRONBACK_LOD1_ROOT", "IRONBACK_LOD2_ROOT"):
        named = bpy.data.objects.get(name)
        if named:
            return named
    matches = [
        obj for obj in bpy.data.objects
        if obj.type == "EMPTY" and "IRONBACK" in obj.name.upper() and "ROOT" in obj.name.upper()
    ]
    if len(matches) == 1:
        return matches[0]
    raise RuntimeError("missing Ironback root")
...
def densify_stations(stations, mids=18):
    out = [stations[0]]
...
def dorsal_blocked(x, y):
    if abs(x - 0.10) < 1.35 and abs(y) < 0.95:
        return True
    if abs(x - 4.10) < 1.10 and abs(y) < 0.70:
        return True
    return False
...
def paint_shell(hull, mats):
    ...
    for poly in mesh.polygons:
        ...
        on_girth = any(abs(centroid.x - gx) < 0.24 for gx in GIRTH_XS)
        on_mark = any(abs(centroid.x - mx) < 0.32 for mx in MARK_XS)
        island = (int(abs(centroid.x) * 2.2) + int(centroid.z * 3.8)) % 8 == 0
        bow_cap = centroid.x > 7.20
        armor_belt = abs(centroid.z - zc) < hh * 0.22 and abs(centroid.y) > beam * 0.40
        mid_course = 2.40 < centroid.x < 5.20 and centroid.z > zc + hh * 0.10
...
def build_hull(mats):
    ...
    for index, x in enumerate(GIRTH_XS):
        ...
        def crown_seam(name=f"SeamCrown_{index}", loc=(x, 0.0, crown_z), width=min(beam * 1.40, 3.4)):
            return add_box(name, (0.030, width, 0.060), loc, hull_mat, 0.0)

        cut(hull, crown_seam)
        for sign, side in ((-1.0, "P"), (1.0, "S")):
            def flank_seam(name=f"SeamFlank_{index}_{side}", loc=(x, beam * 0.88 * sign, zc)):
                return add_box(name, (0.030, 0.08, hh * 0.85), loc, hull_mat, 0.0)

            cut(hull, flank_seam)

    for sign, side in ((-1.0, "P"), (1.0, "S")):
        def dorsal_seam(name=f"DorsalSeam_{side}", loc=(0.20, 0.42 * sign, 0.95)):
            return add_box(name, (11.2, 0.060, 0.085), loc, hull_mat, 0.0)

        cut(hull, dorsal_seam)

        def bilge_seam(name=f"BilgeSeam_{side}", loc=(-0.40, 1.05 * sign, -0.35)):
            return add_box(name, (10.0, 0.08, 0.14), loc, hull_mat, 0.0)
    ...
    def bow_step():
        return add_box("BowCourseStep", (0.16, 4.60, 0.26), (7.10, 0.0, 0.85), hull_mat, 0.0)

    cut(hull, bow_step)

    def shoulder_step():
        return add_box("ShoulderCourseStep", (0.18, 5.40, 0.24), (4.40, 0.0, 0.98), hull_mat, 0.0)

    cut(hull, shoulder_step)

    def hopper_break():
        return add_box("HopperLoadBreak", (0.80, 2.20, 0.20), (1.40, 0.0, 0.92), hull_mat, 0.0)

    cut(hull, hopper_break)
...
def build_hopper_well(hull, mats, lod):
    mech = mats["Material_Mechanical"]
    armor = mats["Material_Armor"]
    warning = mats["Material_Warning"]

    def hopper_cut():
        return add_box("HopperCut", (2.55, 1.85, 1.10), (0.10, 0.0, 1.05), mech, 0.0)

    cut(hull, hopper_cut)
    bits = add_open_well("LOD0_HopperWell", (2.40, 1.70, 0.95), (0.10, 0.0, 0.55), mech, floor=True, wall=0.055)
    bits.append(add_box("LOD0_HopperRim", (2.65, 1.95, 0.07), (0.10, 0.0, 1.18), armor, 0.006))
    bits.append(add_box("LOD0_HopperLipP", (2.30, 0.07, 0.10), (0.10, -0.88, 1.10), mech, 0.004))
    bits.append(add_box("LOD0_HopperLipS", (2.30, 0.07, 0.10), (0.10, 0.88, 1.10), mech, 0.004))
    if lod == 0:
        bits.append(add_box("LOD0_OreCrateA", (0.28, 0.22, 0.16), (-0.35, 0.35, 0.35), warning, 0.004))
        bits.append(add_box("LOD0_OreCrateB", (0.22, 0.18, 0.14), (0.45, -0.40, 0.32), mech, 0.004))
    return bits, {"kind": "hole", "size": [2.40, 1.70, 0.95], "location": [0.10, 0.0, 0.55]}
...
def build_cabin(hull, mats, lod):
    ...
    def cabin_cut():
        return add_box("CabinCut", (1.55, 0.95, 0.85), (4.10, 0.0, 1.15), mech, 0.0)

    cut(hull, cabin_cut)
    bits = add_open_well("LOD0_CabinTub", (1.45, 0.88, 0.80), (4.10, 0.0, 0.55), mech, floor=True, wall=0.050)
    rings = []
    for x, hw, hh, zc in (
        (4.75, 0.42, 0.22, 1.15),
        (4.10, 0.55, 0.30, 1.28),
    ...
    bits.append(add_box("LOD0_CabinShoulder", (0.95, 0.70, 0.06), (3.55, 0.0, 1.05), armor, 0.006))
    return bits, {"kind": "tub+canopy", "location": [4.10, 0.0, 0.55]}
...
def build_drive_houses(mats, lod):
    ...
    for sign, side in ((-1.0, "Port"), (1.0, "Starboard")):
        ...
        bits.append(add_cylinder(f"LOD0_DriveCollar_{side}", 0.40, 0.14, (-7.35, y, 0.10), ceramic, 0.006, vertices=14))
        bits.append(add_cylinder(f"LOD0_Bell_{side}", 0.36, 0.42, (-8.00, y, 0.10), thruster, 0.004, vertices=16))
        bits.append(add_cylinder(f"LOD0_BellThroat_{side}", 0.22, 0.28, (-7.95, y, 0.10), dirt, 0.0, vertices=12))
        if lod == 0:
            bits.append(add_box(f"LOD0_DriveBand_{side}", (0.08, 0.52, 0.34), (-6.35, y, 0.10), armor, 0.003))
        report["houses"].append({"side": side, "y": y})
    return bits, report


def build_cutter_arms(mats, lod):
    mech = mats["Material_Mechanical"]
    armor = mats["Material_Armor"]
    ceramic = mats["Material_Ceramic"]
    bits = []
    for sign, side in ((-1.0, "P"), (1.0, "S")):
        for tag, x0, x1, xh in (
            ...
            boom_rings = [
                [
                    (x0, y0 + 0.14, -0.02), (x0, y0, 0.12), (x0, y0 - 0.14, -0.02),
                    (x0, y0, -0.16),
                ],
                [
                    (x1, y1 + 0.12, -0.14), (x1, y1, 0.02), (x1, y1 - 0.12, -0.14),
                    (x1, y1, -0.26),
                ],
            ]
            bits.append(loft_rings(f"LOD0_ArmBoom_{tag}_{side}", boom_rings, mech, 0.006))
            bits.append(add_box(
                f"LOD0_ArmSaddle_{tag}_{side}", (0.32, 0.26, 0.20),
                (x0 - 0.15 * (1 if tag == "Fore" else -1), y0, -0.02), armor, 0.008,
            ))
            bits.append(add_box(
                f"LOD0_ArmHead_{tag}_{side}", (0.30, 0.24, 0.24),
                (xh - 0.35 * (1 if tag == "Fore" else -1), y1, -0.22), armor, 0.006,
            ))
            bits.append(add_cylinder(
                f"LOD0_Cutter_{tag}_{side}", 0.10, 0.58,
                (xh, y1, -0.22), ceramic, 0.004, vertices=10,
            ))
    return bits
...
def build_guns(mats, lod):
    ...
    bits = []
    for sign, side in ((-1.0, "P"), (1.0, "S")):
        bits.append(add_cylinder(f"LOD0_GunHouse_{side}", 0.09, 0.95, (6.10, 0.55 * sign, -0.02), mech, 0.004, vertices=10))
        bits.append(add_cylinder(f"LOD0_GunBarrel_{side}", 0.034, 0.72, (6.95, 0.55 * sign, -0.02), armor, 0.002, vertices=8))
        bits.append(add_cylinder(f"LOD0_RearGun_{side}", 0.042, 0.52, (-5.80, 0.60 * sign, 0.45), mech, 0.003, vertices=8))
    bits.append(add_cylinder("LOD0_TurretRing", 0.36, 0.10, (1.80, 0.0, 1.28), mech, 0.005, rotation=(0, 0, 0), vertices=14))
    bits.append(add_box("LOD0_TurretHead", (0.36, 0.22, 0.12), (1.95, 0.0, 1.42), armor, 0.005))
    return bits
...
def build_hardware(hull, mats, lod):
    ...
    warning = mats["Material_Warning"]
    bits = []
    bits.append(add_box("LOD0_TransomPlate", (0.06, 0.70, 0.38), (-7.95, 0.0, 0.12), armor, 0.004))
    bits.append(add_box("LOD0_KeelSpine", (3.2, 0.28, 0.05), (0.20, 0.0, -0.95), mech, 0.008))
    bits.append(add_box("LOD0_AftWalk", (1.8, 0.18, 0.016), (-3.20, 0.0, 0.55), mech, 0.002))
    bits.append(add_box("LOD0_RepairPatch", (0.36, 0.18, 0.014), (2.40, -0.70, 0.88), warning, 0.002))
    bits.append(add_box("LOD0_HatchLid", (0.36, 0.22, 0.018), (-1.40, 0.40, 0.82), armor, 0.003))
    bits.append(add_cylinder("LOD0_CommMast", 0.038, 0.70, (-0.50, 0.0, 1.35), mech, 0.002, rotation=(0, 0, 0), vertices=8))
    bits.append(add_box("LOD0_CommHead", (0.12, 0.16, 0.06), (-0.42, 0.0, 1.72), armor, 0.003))
    for sign, side in ((-1.0, "P"), (1.0, "S")):
        def bay(n=f"SideBay_{side}", loc=(0.20, 2.55 * sign, 0.15)):
            return add_box(n, (1.70, 0.55, 0.55), loc, mech, 0.0)

        cut(hull, bay)
        bits.extend(add_open_well(
            f"LOD0_SideWell_{side}", (1.55, 0.48, 0.48),
            (0.20, 2.30 * sign, 0.10), mech, floor=True, wall=0.040,
        ))
        bits.append(add_box(f"LOD0_RCS_{side}", (0.22, 0.14, 0.14), (-1.40, 2.55 * sign, 0.20), mech, 0.003))
    if lod == 0:
        for sign, side in ((-1.0, "P"), (1.0, "S")):
            for i in range(4):
                bits.append(add_box(
...
def build_one(source: Path, output: Path, lod: int):
    ...
    if size.x < 15.5 or size.x > 19.5 or size.y < 5.2 or size.y > 7.8 or size.z > 5.0:
