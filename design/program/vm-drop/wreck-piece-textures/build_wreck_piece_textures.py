#!/usr/bin/env python3
"""VM-drop job wreck-piece-textures: rebuild seven untextured aftermath pieces.

Reads incubator source + maps (read-only). Writes only under this job folder.
Forked from tools/blender/author_ceres_wreck_dressing.py; place composition is
skipped (--pieces-only is the default for the outbox lane).
...
def cli() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="VM-drop wreck-piece-textures: author seven aftermath pieces."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional; ignored when --pieces-only (default).",
    )
    parser.add_argument(
        "--blend-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Export the seven per-asset GLBs only (no Ceres place compose).",
    )
    parser.add_argument(
        "--with-places",
        action="store_true",
        help="Also compose place_ceres_* (requires --places-root and --blend-root).",
    )
    args = parser.parse_args(values)
    if args.with_places:
        args.pieces_only = False
        if args.places_root is None or args.blend_root is None:
            parser.error("--with-places requires --places-root and --blend-root")
    return args
...
def main() -> None:
    ...
    places = {}
    if not args.pieces_only:
        # Rebuild all pieces into one scene for composition by re-importing authored.
        for obj in list(bpy.context.scene.objects):
            bpy.data.objects.remove(obj, do_unlink=True)
        for mesh in list(bpy.data.meshes):
            if mesh.users == 0:
        ...
        for asset_id in SELECTED:
            ...
            bpy.ops.import_scene.gltf(
                filepath=str((args.authored_root / f"{asset_id}.glb").resolve())
            )
            ...
            if root is None:
                ...
                root = roots[0] if roots else (new_objs[0] if new_objs else None)
            imported_roots[asset_id] = root

        def build_place_from_authored(place_id: str, piece_ids: list[str]):
            # Local import of original place builder body is not replicated here;
            # --with-places is unsupported in the outbox fork. Keep stub for clarity.
            raise RuntimeError(
                "Outbox fork is pieces-only; use tools/blender/author_ceres_wreck_dressing.py for places"
            )
    ...
    final_report = {
        "schema": "spaceface.wreckPieceTextures.v1",
        "packet": "vm-drop/wreck-piece-textures",
        "generator": "design/program/vm-drop/wreck-piece-textures/build_wreck_piece_textures.py",
        "blender": bpy.app.version_string,
        "selectedAssets": list(SELECTED),
        "mapsRoot": str(args.maps_root.as_posix()),
        "sourceRoot": str(args.source_root.as_posix()),
        "textureMegabytesShared": texture_megabytes(args.maps_root),
        "cubeUvMetres": CUBE_UV_METRES,
        "piecesOnly": bool(args.pieces_only),
        "rawPack": {
        ...
        "places": places,
        "roleMaterials": ROLE_BY_MATERIAL,
    }
    args.report.write_text(json.dumps(final_report, indent=2) + "\n", encoding="utf-8")
    print(f"[report] {args.report}")
    print("[done] wreck-piece-textures author-down complete")
