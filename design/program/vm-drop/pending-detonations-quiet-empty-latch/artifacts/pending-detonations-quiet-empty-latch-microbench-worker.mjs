...
if (!system._pendingDetonations || system._pendingDetonations.length !== 12) {
  throw new Error('pending detonations pool missing');
}

// Prime empty observe so after-mode can latch.
system._pendingDetonationsQuietEmpty = false;
system._updatePendingDetonations();
if (!system._pendingDetonationsQuietEmpty) throw new Error('failed to latch on empty prime');

function runBefore() {
  // Pre-#125 residual: always walk 12 slots.
  system._pendingDetonationsQuietEmpty = false;
  system._updatePendingDetonations();
}

function runAfter() {
  // Production latched path (already quiet after prime; schedule never fires).
  system._updatePendingDetonations();
}
