...
| `DRIFT-RECONCILE-20260727` | `scratch/drift-reconcile-20260727/PROPOSAL.md`, stranded refs `agent/gfx-production-remaster-lark`, `codex/recovery-worldbuilding-20260723`, `claude/pq018-rebase-20260725`, `codex/vp220-propulsion-graphics` | REFERENCE | **Drift-reconcile residual, do not re-merge.** A 2026-07-27 pass (codex `gpt-5.6-sol` adversarial review) classified the July 21–25 stranded branches. Integrated to master as pure additions: 45 Lark Blender scripts + evidence (commit `705ee011`, GLB/blend swap **deferred** because it re-opens the `54548e09` Helios accept) and 23 worldbuilding drafts as **non-authoritative** (commit `7fa373c8`, see [`docs/worldbuilding/RECONCILIATION_STATUS_2026-07-27.md`](../../docs/worldbuilding/RECONCILIATION_STATUS_2026-07-27.md); committed canon wins on conflict). **Rejected, do not resurrect:** the 7 stranded PQ-018 route-contract scripts (import planner symbols absent from current master — coupled to the reverted approach planner) and the 2 stranded tether-break scripts (encode the pre-resilience behavior master deliberately replaced). Two deferred admission tasks live here: (a) port a PQ-018 Phase-4 route-contract harness **against current master** from the rejected scripts' intent — re-derive, do not copy; (b) the Helios Lark GLB/blend re-acceptance, only if iter19's hard-surface rebuild is judged a clear improvement over the accepted `54548e09` vertical. | The two deferred tasks land as fresh, current-master-validated commits under PQ-018 / a Helios re-acceptance respectively, and the stranded branches become safe to delete once Claude's perf worktrees are no longer protected. |
