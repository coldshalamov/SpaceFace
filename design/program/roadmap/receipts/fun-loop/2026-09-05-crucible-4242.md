...
| B13 The player is never knocked around | contact knocks per minute on the player, crucible (worst of 9 run(s)): 10 events/min<br>largest single knock, fraction of cruise, crucible (worst of 9 run(s)): 0.1 fraction<br>knock events that changed the player's heading, crucible (worst of 9 run(s)): 0 events | ≤ 2 contact knocks per minute; largest ≤ 10 % of cruise; zero heading changes; no visible jitter | no | crucible/helios_core/energy_baseline/s4242<br>crucible/helios_core/physics_toolkit/s4242<br>crucible/helios_core/massline_rig/s4242<br>crucible/lagrange_crucible/energy_baseline/s4242<br>crucible/lagrange_crucible/physics_toolkit/s4242<br>crucible/lagrange_crucible/massline_rig/s4242<br>crucible/cinder_sluice/energy_baseline/s4242<br>crucible/cinder_sluice/physics_toolkit/s4242<br>crucible/cinder_sluice/massline_rig/s4242 |
...
### crucible helios_core/energy_baseline seed 4242 (run 89629068)

Bars this run feeds (FEEL_CONTRACT §B — pooled verdict for the whole receipt):

| bar | value(s) | target | met |
|---|---|---|---|
| B13 The player is never knocked around | contact knocks per minute on the player, crucible (worst of 9 run(s)): 10 events/min<br>largest single knock, fraction of cruise, crucible (worst of 9 run(s)): 0.1 fraction<br>knock events that changed the player's heading, crucible (worst of 9 run(s)): 0 events | ≤ 2 contact knocks per minute; largest ≤ 10 % of cruise; zero heading changes; no visible jitter | no |
...
### crucible helios_core/physics_toolkit seed 4242 (run 15c5a697)
...
### crucible helios_core/massline_rig seed 4242 (run cc1556d2)
...
| nothing-happened seconds | 0 s | none | clean |
| knock budget on the player | 10/min, max 10% of cruise, 0 heading changes | <= 2/min and <= 10% of cruise, never a heading change | over budget |
| deaths by cause | weapon 14, brake 1, collision 12 | informational | recorded |

Gaps: knockBudget: visible jitter is unmeasured (headless); full B13 cannot pass

### crucible lagrange_crucible/energy_baseline seed 4242 (run ae990d40)
...
| nothing-happened seconds | 0 s | none | clean |
| knock budget on the player | 3.333333/min, max 10% of cruise, 0 heading changes | <= 2/min and <= 10% of cruise, never a heading change | over budget |
| deaths by cause | weapon 12, collision 3 | informational | recorded |

Gaps: knockBudget: visible jitter is unmeasured (headless); full B13 cannot pass

### crucible lagrange_crucible/physics_toolkit seed 4242 (run b87d1242)
...
### crucible lagrange_crucible/massline_rig seed 4242 (run be47b08a)
...
### crucible cinder_sluice/energy_baseline seed 4242 (run 0aba3dd2)
...
| nothing-happened seconds | 0 s | none | clean |
| knock budget on the player | 0.666667/min, max 2.301539% of cruise, 0 heading changes | <= 2/min and <= 10% of cruise, never a heading change | not measured |
| deaths by cause | collision 5, weapon 11 | informational | recorded |

Gaps: knockBudget: visible jitter is unmeasured (headless); full B13 cannot pass

### crucible cinder_sluice/physics_toolkit seed 4242 (run a7cdedf4)
...
| nothing-happened seconds | 0 s | none | clean |
| knock budget on the player | 0.666667/min, max 3.427436% of cruise, 0 heading changes | <= 2/min and <= 10% of cruise, never a heading change | not measured |
| deaths by cause | weapon 10, collision 4 | informational | recorded |

Gaps: knockBudget: visible jitter is unmeasured (headless); full B13 cannot pass

### crucible cinder_sluice/massline_rig seed 4242 (run 32015bee)
...
| nothing-happened seconds | 0 s | none | clean |
| knock budget on the player | 8/min, max 10% of cruise, 0 heading changes | <= 2/min and <= 10% of cruise, never a heading change | over budget |
