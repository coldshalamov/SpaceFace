DONE — Jumping Helios → Ceres no longer arrives in a sector that never publishes anything.

DONE WHEN (verbatim, from the lane brief): `station:ready` (and every ship/pod within 340 WU ready) after a real gate jump to Ceres, proven headed on the real GPU, plus the same via `world.enterSector` and via Continue; a regression test for the latch logic and the headed probe wired into a headed check group.
...
3. **The destination station was not in the hulls-only cook cohort.** `isLoadingHullUpgradeJob` admitted `ship` and `place` but not `station`, so the jump cook's hulls-only pass skipped the biggest authored body in the sector and left it behind the leftover-FX hold. (Predecessor's fix; inert until the lane above was alive again, which is why it never showed on its own.)
