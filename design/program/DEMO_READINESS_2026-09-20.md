...
| D37 | Works-screen fault strip can outlive its recovered conduit network (`src/ui/asteroid/asteroidRenderer3d.js:254`) | After a D34 watchdog `retry` or loud `failed`, a late template settle can still mount a fully working conduit network (the authored rebuild path at `asteroidRenderer3d.js:351-373`; watchdog test 5 asserts exactly this late mount) — but `hideConduitMountFault` is wired only to screen-initiated rebuild promises (`:5520-5523`), and the watchdog's fire-and-forget retry rebuilds (`void rebuild(lastDesired).catch(() => {})`, `:254`) never clear the strip. The player sees "network layer unavailable — works conduits failed to load" (or the stale `retry` variant) sitting over a live, dressed network until the next rebuildOverlays-triggering event — a false alarm on glass. Hand-traced by the D34 cold review (2026-09-24), not yet reproduced on glass | Mining screen, works tab, right after a contended-host recovery | open — repro path: drive the watchdog into `retry`/`failed` (D36 contention, or a forced loader failure), let a late settle mount succeed, observe the stale strip over the live network. Fix shape: clear the fault strip whenever any rebuild resolves, not only screen-initiated ones |
| D38 | Small on-screen bodies can wait 1–2.5 s with no mesh at all | `probe:frame-solid` after the 2026-09-24 solid-world fixes (`.devshots/frame-solid/after3.txt`, contended host, ~85 ms frames): classification, collider and residency are clean (no station gaps, 0 root swaps, 1 blink), but a drone (id 331, 26 frames) and belt rocks (ids 9/10/20, ~10 frames each) sat `S0_EXACT / R0_GLASS / inGlassSet` with `entity.mesh` unset — the mesh build/owe queue had not bound a root yet. Whether this survives an uncontended 60 fps machine is unmeasured | Flight, first approach to a rock field or drone | open — repro: `node scripts/probe-frame-solid.mjs`, read offender `reasons` for `noMesh`; find which queue owns first mesh bind for glass entities (not authored admission) and why it lags an on-glass entity |
