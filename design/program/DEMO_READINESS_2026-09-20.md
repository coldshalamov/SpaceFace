...
| D23 | station market register (`src/ui/station/screens/market.js:634`) | `tbodyEl.innerHTML` is rebuilt on every price/stock/demand tick, so a row click landing mid-rebuild hits a detached node and silently selects nothing — a player sees "click did nothing" on a live register. The soak driver's public-click path loses the hit test the same way and must dispatch on the node | Trading at any station | open — repro: dock at Helios, open Market, click register rows during a price tick; root = full innerHTML rebuild per tick instead of patching cells |
| D24 | renderer resource residency across save/load/dock cycles | Confirmed at scale by the 200-cycle soak (`.devshots/browser-soak-200`, 52.8 min): JS heap +411 MB post-GC (`retainedAfterGc`, ~2 MB/cycle, monotonic), geometries 1090→1519 (+2.1/cycle), textures 811→929, programs +24. The earlier 8-cycle "heap flat" reading was warmup-window noise — the real slope is a true retained leak tied to the save/load/dock/trade cycle, heading for tab OOM at ~450+ cycles | Long play session | open — repro: `node scripts/check-release-soak-browser.mjs --cycles=40` with `SF_SOAK_HEAP_SNAPSHOTS=1` (start/end snapshot pair lands in the evidence dir), then `node scripts/lib/heapSnapshotDiff.mjs <a> <b>` names the retained classes; keep-GPU reattach + residency holds are the first suspects |
...
| D26 | transient powered-flight freeze (rare) | Two soak windows (browser + Electron) measured live `moveZ`, `blocked:false`, `docked:false`, `defeated:false`, `physicsSleeping:false`, empty `screenStack`, advancing sim ticks — and speed pinned at exactly 0 under held W. Input/UI/dock/sleep ruled out; residue is a physics-command-level pin (degenerate maxSpeed clamp, non-dynamic body, or zero derived thrust). Probe now dumps `physicsBody.dynamic`, thruster healths, `entity.thrust`, and `timeScale` at failure so the next occurrence self-localizes | First flight after New Game → Launch (rare) | open — watch soak evidence `flight-input` steps |
...
| D28 | save file grows ~6 KB per dock/save/load/trade cycle; persistent-entity ratchet | 200-cycle soak: saveBytes climbed monotonically 537 KB → 1.77 MB with no plateau. Headless attribution (15 dock/trade/save/load cycles, `C:\tmp\save-section-probe.mjs`): `entities` section is the top grower (+1.2 KB/cycle ≈ +1 hull/cycle). Mechanism: traffic stamps `flags.persistent` on role hulls (mid-job cutters/tugs/claim haulers) but no path clears the stamp when the job releases — `npcJobsRuntime.release()` drops `data.jobId` only — so jobless hulls serialize forever (14/15 observed persistent hulls held no job). Restored persistent hulls also stay invisible to spawn-cap accounting (`_countYardTugs`/`_countGeneralSalvors` count `traffic.freighters` records with `jobId`), so each restore mints replacements for hulls that are still alive. Not a dead-end (all 200 restores passed) but unbounded bloat | Long play session | open — repro: `node C:\tmp\save-section-probe.mjs 15` (worktree sources); fix seam: clear `flags.persistent` in `npcJobsRuntime.release()` when no other persistence anchor exists (`data.freightCustodyPersistence`, `surrenderRecovery`, `itinerary`, `claimDepotId`, `durable`), and/or count live restored role hulls in the spawn caps |
