...
| D55 | `test/mass-seed.test.mjs` "manual override: cursor paint beats the seed" + `test/pq-147-02-counterplay.test.mjs` "opposite-polarity field cancels a live pin" — red on the dirty tree | Massline cursor paint no longer wins the acquisition receipt over the seed anchor (a player painting a specific target may rope the wrong body), and the deployed Well's pin is not landing on a hull inside it (area denial may silently miss). Both fail identically with INFERENCE-16's fields.js hunks reverted — repro with a clean checkout + this tree's foreign in-flight diffs (`masslineThrow.js` tangent-release reorder, `combat/runtime.js` profile memo, `ai/doctrine.js`/`maneuver.js`) to attribute | Tether reliability + Well counterplay | open — likely mid-edit state of the foreign massline/combat lane; re-run after it lands |
...
| D56 | `npm run check:pause-brief` red at HEAD: `scripts/check-pause-brief.mjs:28` asserts the pre-ORRERY render pattern `/mk\('Review ' \+ mapAction\.label/` against `src/ui/screens/pause.js`, which no longer contains the string `Review ` at all (grep exit 1); the check's earlier assertions (`INTER-SYSTEM ROUTE`, `no jump route is required`) pass and it dies at :28, so the later exit-confirm assertions are unreached. Verified red 2026-09-26 (`npm run check:pause-brief` → exit 1; same failure in the 2026-09-26 `check:all` sweep, `scratch/check-ci-report/2026-09-26T04-00-50-901Z/`) | No player harm found — the waypoint map review action exists in its new ORRERY form (`pauseMapAction` at `src/ui/screens/pause.js:221`, `openPauseMapReview` at :273) and is wired; this is demo build health (a stale structural assertion that will red every `check:all` until re-pinned, hiding any real regression in the same assertion list) | open — re-pin belongs to the ORRERY/UI lane that owns the new pause-screen contract: replace the :28 pattern with a deliberate anchor of the new form (e.g. the `openPauseMapReview` route wiring), not a mechanical green. Logged by the PQ-033.02 review follow-up 2026-09-26; new since D46's explicit list |
| D57 | Save/load during a reinforcement's 90-tick window silently drops the pending squad — and the caller never re-calls | `state.aiEncounter` is deliberately transient: `saveSystem.js` rebuilds it as `{schemaVersion, nextSeq:1, commands:[]}` on load (asserted by `check-gameplay-core.mjs`), so `owner.pendingReinforcements` evaporates; meanwhile the caller's `data.ai._calledReinforcements` latch rides the serialized entity and survives, so it can never issue the call again. Player route: fight a reaver down to its call threshold, save before the wasps arrive, load — the announced reinforcements simply never come | Any save mid-fight near a reinforcement-capable hull (reaver packs, dreadnought) | open — fix owner: aiEncounter/save lane; options are serializing `pendingReinforcements` or clearing the persisted caller latch on load. Surfaced by the INFERENCE-17 reviewer; ring-cap and throw-path leaks it also found are fixed in the U17 tail |
| D58 | `test/charon-bounty-board.test.mjs` "keeps refinery semantics while declaring its hunter exchange" red at clean HEAD: `salvage must be favored over an ordinary refinery` | `b65ad1f40` retuned `OFFER_MIX.refinery` `[3,2,2,4,2,1,1,0,1,1] → [4,4,1,2,4,1,1,0,1,1]`, raising refinery salvage weight 2→4; `bounty_board[4]` stayed 4, so the Charon board's authored hunter-exchange promise (salvage favored over a plain refinery) now sits at parity and the pin fails. Deliberate decision needed: bump `bounty_board` salvage to 5 (restore the exchange identity) or re-pin the test to accept parity. Verified red on the clean file | Charon hunter exchange flavor — the writ wall quietly lost its salvage edge in the physical-mix rebalance | open — surfaced by INFERENCE-19's regression pass 2026-09-26 |
| D59 | `npm run check:m3:hunter-route` red: `defect:combat_damage_authority_failed:no_hull_damage` ×12 → no repair spend → `above_healthy_band 1103.97 > 400` | The route's `applyCombatDamage` (`src/balance/hunterPublicRoute.js:333`) never enters its damage loop on any of the 12 fights — `lastResult` is null so `onHit` was never rejected; the player entity's `e.hull` already reads ≤ target (empty/dead at fight time), independent of what the mark/target is doing. Verified identical at pre-inference clean checkout `b310e81e6` (same 1103.97 cr/min) — committed foreign work predates the INFERENCE-19/20 units and all named-mark logic; `playerDockProtected` only guards `state.playerId` so named-place spawns are not the gate | Public-route hunter run reports inflated earnings — a check-health failure masking real combat-damage regressions behind a null path, not a player-visible combat bug in live sim (focused mark settlement tests pass) | open — bisect owner: whoever lands the route/combat lane; likely an entity-shape change (hull field rename/drop in makeShipEntitySpec or recomputeEntity) that the route's raw `e.hull` read no longer sees. Surfaced by INFERENCE-19's verification pass 2026-09-26 |
