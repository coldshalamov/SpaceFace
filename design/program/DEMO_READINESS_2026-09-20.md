...
| D33 | Shipworks hull preview never reports settled (headless) | `check-station-ui-temporal-stability` waits 30 s on the Shipworks tab for `.sx-sw__canvas` to read `previewAssetState:'authored'` + `previewReveal:'settled'`; headless it never does (the stage holds the produced hull poster since 30f7663c2), so the Shipworks hover and every sample after it go unmeasured | Station Shipworks / station release checks | open — repro: `SPACEFACE_PLAYER_STORE_DIR=<temp dir> node scripts/check-station-ui-temporal-stability.mjs`; decide whether a poster-held stage is a settled state the probe should accept, or why the live hull never admits there. Also: with `SF_STABILITY_TAB=market` (Shipworks skipped) the check still ran 10 min with no output and was killed (2026-09-23) — a stall in its hover sampling (`sampleFrames`, 120 animation frames in-page) on the idle station; cause not isolated |
