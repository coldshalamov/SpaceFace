...
| D34 | `newGame` screen, ORRERY `createStopScale` | The stop-scale stations are laid out at a fixed 470 px width, so the difficulty words and hull names (Hitch / Pelican / Wasp) clip horizontally in longer languages — reproduced under the qps-ploc pseudo-locale | New Game | open — repro: `npm run check:m6:localization` (the browser probe now reaches `newGame` after the stale `.sf-col` selector was fixed to `[data-action="newGame"]`, and `assertVisiblePseudoSurface` fails on the clipped stop-scale) |
