...
| D63 | Regression sweep inside the PQ-153.02 landmark lane blast radius — three red tests at HEAD | (1) depth-program-w1-planet-states: sector_vesta_forge semantic hash drifted 3339003051→1659813738 (POI/topology fingerprint moved with the lane's landmark edits); (2) lung-of-charon-rescue:95 — insertDressingRow→allocateDressingId NPE on table.byId when _spawnPOIs is driven by a bare fixture state (the dirty world.js hunk now routes memorial hulls through insertDressingRow; ensureDressingTable yields no byId-bearing table outside full sim); (3) lung-of-charon-rescue:281 — Continue respawns 3 entities where the test pins 2 | Charon boarding/rescue flow on the demo path — if the null-table path can be reached outside the fixture, boarding the Lung POI throws | open — lane-owned: PQ-153.02 still-review row is LIVE (files: world.js/dressing machinery). Any sitting dispatched into those files handles that row as part of its unit. Raised by the INFERENCE-26 regression set |
| D62 | Helm-loss is not one law across sources (`test/hitstun-curve.test.mjs` "B11 met clauses hold") | Measured 2026-09-26 on the real path at matched ΔV (u in [0.25, 0.4], light hull): a gun hit takes the helm for ~3.9–4.0 s while rope_throw 1.12 s, well_fling 1.32 s and collision 1.07 s — relative spread 0.733 against a bound of 0.10; and gun helm-loss is non-monotone in k (4.00 s @0.30 → 3.12 s @0.60 → 3.82 s @1.30). Every other B11 clause holds (all light sources ≥ 1 s, heavy gun-scale = 0, mass ordering, no hidden gyro). The Well-fling cell had been unmeasurable since INF-042's windup crushed the instrument's short pulse, which masked this | Every fight: a light hull shot once loses control ~3× longer than one slammed or thrown equally hard — guns out-stun the physics verbs the game is built on | open — THE HAND lane: find why the gun path's helm-loss ignores the shared (ΔV ÷ cruise, mass ratio) function (likely a gun-specific stagger/tumble stacked on top); do not loosen the 0.10 bound. Repro: `node --test test/hitstun-curve.test.mjs` |
