...
| D46 | 2026-09-25 `check:all` sweep — foreign red classes new since the 2026-09-23 receipt set | Clean-HEAD (`3d6491e1e`) re-run of every named id 2026-09-25: **green now** — check-bundle (the `loadingTerminalArt`/`localization` esbuild errors were dirty-WIP artifacts; bundle builds clean at HEAD), check-camera-shake-scope, check-map-information-depth; **fixed in the D46 sitting** — check-src-reachability (baselined the two staged ORRERY orphans `flightInstruments.js`/`flightPreview.js` per the check's own ratchet contract) and check-authored-place-runtime (assertion re-pinned to the landed PIC-11 `da2abd5f1` empty-substrate contract). **Still red at clean HEAD:** check-type-floor — 285 declarations render below the binding INSTRUMENT_GRAMMAR 12 px floor, all inside the live frontend refactor's new modules (`src/ui/orrery/*`, `deckplate/screens.js`, `masslineCadenceReadout.js`, `nemesisComms.js`, `styles/intro.css`); the shipped type scale contradicts the grammar rule — owner-level scale-or-grammar call, not a mechanical fix. And the asset-art family, one root: production GLBs re-exported under a newer Blender toolchain while manifests/receipts/gates still pin the old outputs — check-parts-manifest (59 rows: byte-profile/`Khronos glTF Blender I/O v5.1.20` generator/extras-dimension/texture-profile drift), check-graphics-asset-receipts (Helios GLB 82,073,572 B vs receipt 79,717,896 B), check-kestrel-wholeship (source lod0 40,468 tris vs the 36,000–38,000 gate), check-sg04-release-assets (source↔release `release.primitiveTopology` count mismatch), check-station-archetype-glb-load (3 concept-linked GLB generator mismatches: refinery/military/jump ring), check-shipworks-dock-composition (`place_dock_interior.glb` missing KHR_materials_clearcoat/transmission/emissive_strength/anisotropy/ior — needs re-export). probe-flight-visual re-run twice 2026-09-25 in the clean worktree — the GPU browser capture cannot finish on this host (`page.goto` page crash, then 30 s navigation timeout at `127.0.0.1:8124/?debug=flight`); no verdict either way. Do not re-record receipts/goldens to silence; the asset-pipeline lane owns the drift. | Demo build health — each red hides regressions in its own area | open — repro: run each `node scripts/check-*.mjs` id individually in a clean-HEAD worktree; asset-art family waits on the Blender/asset-pipeline lane; check-type-floor waits on the type-scale decision |
| D48 | `test/bounded-autosave.test.mjs` + `test/save-player-bounded-capture.test.mjs` — real-clock 12 ms slice assertions flap on this contended host | Intermittent reds at HEAD with no code defect found: single blocking samples spike to 17–75 ms inside one ~6.2 ms-floor dispatch task while every sibling sample sits at the floor — host preemption/GC inside `structuredClone`, not batching logic (`_postAutosaveEncodeParts` yields at the 8 ms target as designed; 6.2+6.2 double-posts never appear). Seen 2026-09-25 on 'batched worker dispatch yields before repeated clone posts can cross the 12ms hard slice' (3 of 4 runs red, spikes 56–75 ms) and 'large player collections are captured and posted in bounded autosave slices' (one 17.5 ms `capture_player` among 2–9 ms siblings). Raw-wall reporting is the deliberate contract, so the fix is a test-design call (quarantine/outlier policy or a quieter host), never silent weakening | None player-facing; an intermittently red save-budget suite at HEAD masks real autosave regressions and trains agents to ignore reds | open — repro: `node --test test/bounded-autosave.test.mjs` in a loop on the loaded host (roughly half of runs red); a green run with all samples ≤12 ms proves the code path — re-run rather than relax |
