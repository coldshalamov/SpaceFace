...
| D39 | `check-pq020-ceres-topology` fails on the dirty tree while the PQ-033.02 far-row/job-anchor WIP is uncommitted | Ceres world sites materialize short: wreck cathedral 1/15 planned entities, `world_site_ceres_cinder_sluice` 1/5 (missing its 4 wreck bodies + 2 colliders). Clean HEAD `a0824bc3d` passes the same check in a fresh worktree, so the red is the in-flight dirty diff (suspects: `src/systems/world.js` `clearWorldRecordJobAnchor`, `src/world/farActorTable.js` `farRowIsDurable` now verifying anchors against live bags). If that lane lands as-is, players see missing wrecks/sites in the belt | Ceres flight — wreck fields and sites | open — repro: `node scripts/check-pq020-ceres-topology.mjs` on the dirty tree vs a clean-HEAD worktree; the lane owning the world/farActor diff should re-run before committing |
