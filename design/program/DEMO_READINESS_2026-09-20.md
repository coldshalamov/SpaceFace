...
|---|---|---|---|---|
| D15 | every non-station screen | `fh.css` is injected only at first dock, so menus render differently before and after docking once | Whole demo | open: retired as ORRERY moves each screen off the kit sheets |
| D18 | opening mission 47-A (fixed-seed scripted run) | The scripted opening run lands **zero** shots; bisect points at 9d453e1f6 ("shots born in composed chase frame and live ≥ 0.7 s", 2026-09-21). Either the recorded aim is stale or opening shots really miss | Adventure: first fight | open: combat lane to decide; repro = the 47-A fixed-seed sim test |
| D20 | first-sector moving flight, authored NPC admission | A patrol Wasp was visible only as the resolving marker at 100 WU while its GLB was loading; other runway traffic had no mesh yet. This remains possible when a ship reaches the glass before its boundary exists, even after the queue starts nearby ships and one bounded asset lookahead. | Flight / first contact | open — public New Game → Launch probe `node scripts/probe-first-sector-ships.mjs --fly` at 15 s; solve in render discovery/opening cook so the actual authored body is prepared before close contact |
| D17 | `check:baseline` red links on dirty foreign lanes | Four baseline links red against in-flight edits: vfx-techniques (B10 spool-to-full 0.12s vs 0.5–0.75s, `vfx.js`/`weaponDischargePool.js` dirty), save-schema (generated doc stale vs dirty `gameState`/data files), sim (reload-at-600 hash divergence, many `src/systems` dirty), pq020-ceres-topology (structuralCostDigest drift, `world.js`/`terrainAnchors.js` dirty) | Whole demo | open — repro: `npm run check:baseline`; all four sit on files another live lane is mutating |
| D21 | crucible wave warm-up | `[crucible] crucible warm package instance failed SF_K0_KESTREL_BORROWED_TIME_V4 — Render package sf.render.kestrel must be retained before creating an instance`, followed by `warm probe palette tint override failed` on the same package; the Kestrel hostiles enter the wave with their render package never retained, so the first one pays full package acquire on first spawn instead of in warm | Crucible: wave spawn hitch/possibly unskinned hostile | open — repro: `npm run probe:body-scale -- --route=crucible` (seed 4242), console warnings during the ~70 s crucible cook; root = `packageInstanceRetain`/`crucible` warm path not retaining `sf.render.kestrel` before `crucible warm package instance` |
| D22 | freight custody suite | 8 of 38 `freight-cargo-custody` checks red at clean HEAD (reproduced on a frozen worktree): raider escape/doctrine, raider-collect-by-contact, diverted-cargo settlement, carrier transit timeouts, save/Continue custody restore, ceresDistressJurisdiction | Freight custody / piracy beats | open — repro: `node --test test/freight-cargo-custody.test.mjs`; likely a mid-flight custody refactor in another lane |
...
| D25 | opening GPU admission validation | `[render] opening submission post-submit validation failed {reason:uncaptured-first-draw-resource}` — the admission ledger snapshots before first draw, so programs/geometries/textures created at first draw are never captured; fires as warnings on every launch | Every launch (console warnings) | open — repro: any browser boot; root = capture snapshot runs pre-first-draw in `openingGpuAdmission` — lazy-capture the first-draw set or treat the warning as expected |
| D26 | transient powered-flight freeze (rare) | Third capture (Electron soak v3, 2026-09-23, `1b01633aa`): full live forensics during the pin — `mode:flight`, `timeScale:1`, ticks advancing, `moveZ:1` held ~0.95 s, `blocked:false`, `docked:false`, `defeated:false`, `physicsSleeping:false`, `physicsDynamic:true`, `thrustHealth:[1,1,1,1]`, `thrust:309` — and `speedStart:0 → speedEnd:0`, `displacement:0`. Demand-side telemetry is healthy, so the applied force is being lost between `writePhysicsControl` and the Rapier body, or velocity is re-zeroed each tick. Ruled out: cryo-lock (0.2 floor), sleeping body (`addForce(force,true)` wakes), `_clampSpeed` (only clamps speed > cap), berth-creep clamp (autopilot-scoped). Suspects left: a zeroed `command.control.maxSpeed`, a per-tick `setLinvel(0)` writer, or the command consumed against a stale/duplicate entity object | First flight after New Game → Launch (rare) | open — probe now re-measures once on the exact-zero signature (`flight-input-retry-pin` mark in evidence); a persistent pin still fails with doubled forensics |
| D27 | ORRERY Hand on the title screen | At arrival with no pointer walk, the Hand points at Archive instead of the primary verb — but only because the bench leaves New Game/Continue/Load disabled (their screens aren't registered there), so the Hand lands on the first *enabled* verb. In the demo shot the Hand correctly wakes on Crucible (primary + first in list). If a live boot shows the same miss with the primary enabled, the Hand's arrival target logic is the suspect | Title screen (non-demo bench) | open — repro: `node .devshots/aplan/title-hand-probe.mjs` (arrives, waits 2 s, no walk, dumps `aria-current` + Hand rect); Hand tick lands beside Archive (y≈836) while New Game is the intended primary |
| D30 | Crucible refit footer keys | On the last wave neither footer action has a key, and in swarm "Extract" has none; Escape now only resumes (it never ends a run), so keyboard and pad players must walk focus to the words | Crucible refit | open — needs bindings in the shared keyboard/pad layer (continue on Space / Y; end the run on a 0.6 s hold of F / X with a filling ring), each cap shown on its word; not the screen's to invent |
| D29 | authored composition on identity-less entities | 306-cycle browser soak (2026-09-23) logged 10 × `[partsLibrary] authored composition failed; no substitute visual published {entity: 491/504/508/458, defId: undefined, trafficRole: undefined, assetRef: undefined, lootTableId: undefined}` — the composed-ship path is asked for a whole-ship visual on entities carrying no selector fields at all, so no kit can match. The ship presumably keeps its procedural fallback; the repeated warn is the real cost. | Far traffic / LOD-authored hulls (console warnings) | open — repro: any 2h soak; the diagnostic now names the selector fields (e8f96e22f), so the next occurrence says which entity class needs a defId or needs to be excluded from whole-ship composition |
| D32 | authored admission gate vs weapons/draw — inert waves + sky-only opening | In the Crucible (seed 4242, pilot holding still) all 10 hostiles stay `presentationAdmission:'pending'` for the whole first 20+ s of control: **zero** hostile `combat:fire` events while ships close to 16–60 WU. `weapons.js:359` requires `presentationAllowsPlayerFacingAction` (admission `ready`), and the serial authored-admission lane is flooded: a priority-1 `critical-hub:2` station job runs 13–30 s in-flight and ~50 queued jobs (≈12 station props, asteroids, fx) interleave ahead of the hostile ship jobs. The same pending gate keeps meshes hidden (`authoredPending` in `shouldSubmitEntityMesh`) — the same fingerprint as the sky-only adventure frames on the belt-bridge leg (world present, 5–8 drawables unpainted, 14/16 sector hostiles pending at ~18 s). A/B verdict: present at HEAD `2f9c0d819`, at HEAD with `2685e58b8`/`e680c9116`/`d84fb8f45` reverted, and at `ce206fb31` (pre-`42429f04d`, plus `dc1ac615a` export repair) — NOT introduced by the 2026-09-23 crucible-warm commits nor by the foreign admission-priority pair; introducing commit not isolated (predates `ce206fb31`). Load-dependent race: the same route produced kills earlier today on a quieter host. | Crucible: inert waves / unreachable honest death; adventure: sky-only opening | open — repro: `node .devshots/aplan/admission-probe.mjs --label=<name> [--root=<tree>] --belt` (writes `.devshots/aplan/admission-probe-<name>.json`; every 5 s logs hostile `combat:fire` count, `presentationAdmission` breakdown, `describeAuthoredUpgradeQueue` head, unpainted-mesh count) |
