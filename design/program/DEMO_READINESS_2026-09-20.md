...
| D16 | whole-ship routing (`partsLibrary.js` / `assetResidency.js`, retro-mounts lane in flight) | Whole-ship routing tests red at the worktree: an NPC resolves the player's bespoke `kestrel.glb` body, and wasp/pelican decode sets now pin `weapons/weapon_pulse_cannon.glb` beside the hull — wrong body for NPCs + extra pinned decode | Flight / any swarm with mixed hulls | open — repro: `node --test test/kestrel-wholeship-routing.test.mjs test/wasp-production-routing.test.mjs test/pelican-production-routing.test.mjs`; tests clean at HEAD, likely mid-edit foreign regressions in the retro-mounts feature |
