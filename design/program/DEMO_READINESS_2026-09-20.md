...
| D38 | Wave-1 Crucible swarm cannot reliably finish a helpless player — shield regen outpaces sporadic engagement DPS | `probe:demo-path` rounds phase, seed 4242 wave 1 (~12 wasps vs the 260-hull/240-shield draft ship): across four death-mode strategies (brake to rest, gunless full-throttle ram, 240px mouse orbit, 90px pirouette with ~110 wu/s speed cap) the swarm engaged only in bursts — `hostiles400` read 0 for most samples while shield dipped to ~96–187 then regenerated to full, for 95+ sim-s without a kill. One earlier braking run did die (~real death observed once in five runs), so the swarm can finish when pressure stays continuous — but wave-1 sustained DPS is marginal against regen and engagement is intermittent (wasps trail/orbit rather than pressing in). A pilot who stops fighting should die quickly; instead the round can stall indefinitely — reads as "limp combat" on the demo's first screen | Crucible wave 1 — a passive or gunless player may survive indefinitely | open — repro: `SPACEFACE_DEMO_WEAVE_SIM_S=0 npm run probe:demo-path` and read the `[rounds]` shield/hostiles400 trace (or `.devshots/demo-path/rounds-trace.json` on timeout). Suspects: wasp engagement spacing/desire vs player shield regen rate; whether wasps are tuned to threaten a fleeing target but not close a kill. A swarm-pressure or regen-during-combat retune is the likely fix — verify a gunless stationary player dies inside ~30–60 sim-s |
