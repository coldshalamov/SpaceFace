...
| D59 | `npm run check:m3:hunter-route` red: `above_healthy_band` — hunter earnings 1064 / 5044 / 5786 cr/min (band 400) | The `no_hull_damage` ×12 half is FIXED (2026-09-26): `724342234` raised the Wasp gun to 48 dps, the route's modeled return damage (211) exceeded the 140 hull pool, the per-hit floor overshot, the kernel legally killed the player and every later fight skipped; the harness now clamps each hit packet to the remaining allowance. That death cascade was masking a second regression: since Economy Pulse (`economyDerived.js` + `quoteMissionEconomics`) `bounty_hunt` rewards price off `netCrPerHour` phases and `economicRiskTier` escalates the phase with faction standing — measured board rewards ~3.1–8.7k each vs ~900 at the last green. Cells 2–3 (~300k cr/hr) sit far above the pulse's own top phase (~72k/hr), so this reads as hunter reward escalation, not just a stale band | Hunter career: a player who leans into bounties out-earns every other path by ~4× within the first hours | open — economy-owner call: cap/tune the standing-driven phase escalation for `bounty_hunt` (preferred reading) or recalibrate `HUNTER_HEALTHY_UPPER_CR_PER_MIN` in `src/balance/hunterPublicRoute.js` with an authored reason; never loosen it silently. Repro: `npm run check:m3:hunter-route` |
...
| D58 | flight HUD at 960x600 ΓÇö STATUS column, BAND feed and target card gone; radar + OBJ readout clip at the right edge | `#hud` children are px-anchored absolute panels; `styles/hud.css` carries no width breakpoint, so narrow windows clip instruments rather than reflow. Census: `.devshots/playtest-screens/20-e02-resize-small.png` vs the 1600x900 HUD | demo shown on a laptop projector / small window loses the objective feed and comms stack entirely | open ΓÇö PQ-180 owns responsive scalars (screen grammar incl. scalars); a sitting in `styles/hud.css` + `hud.js` adds a `--ui-scale`-driven transform clamp or a width media rule that keeps OBJ/STATUS reachable at 960 |
