...
| D40 | An authored-admission job's inner await can park forever; lane starvation is now rescued but the dead await is unrooted | PQ-040 diagnostics on 2026-09-24/25: the trade-hub's `critical-hub:2` job (`place_station_trade_hub.glb`, 82 MB) sat `status:'running'` for 371–708 s across five runs while every queued ship admission waited behind it (serial flight lane = concurrency 1). The job chain (`admitNextUpgradeJob` → `upgradePlaceBoundary` → `loadPart`/`prepareAuthoredVisualPipelines`/publication gate) has no per-stage stall detector — whichever await never resolves (decode worker, KTX2 transcoder, GPU residency) wedges the boundary at `loading` forever. 2026-09-25 rescue: `stalledNonShipHogCanPassShip` + `settleStalledNonShipDiagnostics` let one queued ship pass a non-ship hog past 120 s and close its diagnostic so `activeJobs`-gated quiet windows drain — the *starvation* is fixed; the parked await itself still never resolves, so the hub can sit on its fallback for a whole session. | Station approach / any authored admission on a loaded host | open — the parked await still needs a named site: the starve dump now carries `runningJobs[].boundaryState/preparePhase/graphFrozen`; next repro (`node scripts/check-performance-dirty-ranges.mjs --runtime=browser --diagnostic` under contention) says which stage parks, then bound or cancel that await |
