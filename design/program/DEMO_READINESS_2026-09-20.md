...
| D31 | Continue/load finalizer dead-ends at menu on a transient staging stall | Electron soak v2 (2026-09-23, `78ae2e8a0`): a mid-session save→load on a contended host stalled inside `finalizeLoadedGame`'s authored-visual staging long enough to throw → `save:error visual_gate_failed` → `failGameStart` set `runtime:start-failed` scale-0 and dumped the player to a frozen menu with no retry affordance — the loaded save is intact but the run is dead. Same D25 admission/staging family, harder manifestation: a one-shot 180 s wait with zero recovery path | Continue / quick-load on a stalled pipeline (rare, host-contention-triggered) | open — the finalizer should retry the staging wait once (or offer a retry verb) before declaring startup failure; a transient GPU stall must not dead-end a valid save |
