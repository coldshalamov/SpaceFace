...
| D26 | transient powered-flight freeze (unreproduced) | One browser soak window measured live `moveZ` + advancing sim ticks + speed pinned at exactly 0 under held W; not reproducible on demand (same build accelerates normally in a standalone headed-Chrome repro). Probe now dumps `controls.blocked/docked/physicsSleeping/screenStack` at failure so the next occurrence self-localizes | First flight after New Game → Launch (rare) | open — watch soak evidence `flight-input` steps; suspects narrowed to a per-tick velocity pin or a skipped craft step, not input loss |
