...
| D24 | renderer resource residency across save/load/dock cycles | Soak run (8 cycles, `.devshots/dockfix-verify2`): JS heap is flat (−28 MB docked-market to docked-market) but renderer counts climb ~+26 geometries and ~+11 textures per cycle through ordinary flight — entity/sector spawn churn retains GPU objects. Over a long demo session this grows GPU memory and draw-state churn | Long play session | open — repro: `node scripts/check-release-soak-browser.mjs --cycles=8`, read `memory-telemetry.json`/`resource-trail.json`; residency cleanup lanes are already active on this surface |
