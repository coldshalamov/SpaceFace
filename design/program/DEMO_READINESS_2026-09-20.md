...
|| D37 | bloomScene first-compile of a fresh authored ship's bloom-target program variant | A 0.25–0.65 s single-frame stall the first time a newly admitted authored ship is drawn into the bloom pass — the PQ-040 acceptance run (2026-09-24 18:44Z) recorded three `[GPU brick] bloomScene` warnings (393/257/649 ms) owned by `ship_kestrel_DirectAuthoredAdmission` substrates during scenario prepare/warmup. Mechanism, from `src/render/bloom.js` `hideUnreadySceneDrawables`/`hideIfProgramUnready`: a shared material (e.g. `SF_Shared_mechanical_dark`) keeps a warm *canvas*-variant `currentProgram`, so the unready scan treats it as ready and draws — but bloom renders the scene into its offscreen target, which resolves a different program key (linear output path) that links lazily inside the presented pass. `touchExactTargetSubject` → `bloom.touchScenePipelines` exists to warm precisely that variant during `admitSubjectPipelines`; the bricks prove authored parts can reach a presented bloom frame outside that latch — suspect the authored-attach boundary publishing parts before/without the pending-subject hold covering the exact-target touch | First contact with any new ship class in a bloom-visible fight (a demo player feels it as a hitch exactly when a fight gets interesting) | open — repro: `node scripts/check-performance-dirty-ranges.mjs --runtime=browser --diagnostic` (fires the combat_vfx_burst injection that produced all three bricks); bricks land in `performance-attribution.json` `errors.warnings` with the owner object/material/program key |
