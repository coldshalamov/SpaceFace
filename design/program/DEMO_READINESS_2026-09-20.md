...
| D34 | Asteroid Works conduit mount can sit in `loading` indefinitely | `check:asteroid-theater` §7 wired a real power+lane spine and the mount read `{"phase":"loading","desiredCount":8,"authoredCount":0,"templateCount":0,"failure":null}` for the full 60 s settle budget — no failure, no watchdog, no retry: if `acquireWorksConduitTemplates`' lease promise starves behind the shared admission queue (or a worker dies mid-decode — the assetLoader comments already name a forever-pending texture-promise class), the built network simply never draws its runs, with no on-screen signal. Same defect family as D33. Intermittent: a same-host run minutes earlier settled with all 8 runs drawn | Mining screen network layer | open — repro: `npm run check:asteroid-theater` on a contended host; fix is either a mount watchdog that retries/fails loudly, or giving works conduit templates admission priority over background preview loads |
