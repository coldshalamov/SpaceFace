...
| D65 | Wrecks shed ~29 % of their momentum at the kill (`test/world-reaction-bars.test.mjs` "B10 the world reacts", clause PQ-138.03: 70.9 % at 2 s vs ≥ 80 %) | Attributed 2026-09-26 by bisect on clean HEAD: first bad `cc93d8461` "SG-02: guard coincident spawns against the degenerate narrow phase" (parent green, commit red at exactly 70.9 %). Before it, the wreck and its manifest payload spawned concentric at `victim.pos`, Rapier's EPA degenerated and the wreck kept its speed only because it was teleporting (~760k WU drift, the A4 flake). The fix nudges the second body +2.5 WU, which creates a valid wreck↔payload contact, and that contact bleeds 120 → 85 WU/s in the first moments; drift is now 170 WU. Both halves of the fix are right; the bleed is the co-spawn pair colliding with itself | Every kill: the wreck brakes instead of carrying the dead ship's line — weakens "wrecks as terrain and ammunition" | open — physics owner (SG-02 / aftermathWrecks / lootShards): give the co-spawned wreck and its payload the victim's velocity and a short mutual-collision grace (or spawn the payload outside the wreck's hull), keeping cc93d8461's nudge; do not revert it. Repro: `node --test --test-name-pattern="B10 the world reacts" test/world-reaction-bars.test.mjs` |
| D64 | Courier public route still misses its authored capital bars (`node --test test/m3-courier-route.test.mjs` 3/5) | Harness defects fixed 2026-09-26: wear packets had been landing only on the regenerating shield since the QoL shield law (`5507700e0`) so no repair was ever bought; the qty-1 arbitrage probe inverted every trade under Economy Pulse bulk pricing (`ff6a0cae9`, the first-bad for dead_income); board picks ignored rep gates. Prospector route is fully green (inventory off-by-one was start-stock bookkeeping). What remains: 30 min capital 14,180 < 15,000; 90 min 107.2 < 112.5 cr/min and 14,648 < 35,000 for the role hull. Pre-Pulse baseline at `ff6a0cae9^` was already red on all three (14,052 / 63.76 / 10,738), and two different competent-trader models converge on ~14.2k at 30 min, so the authored bars exceed honest post-Pulse freight yield | Courier career: the starter freight loop earns, but reaches the role hull far later than the authored pacing promises | open — economy-owner call: raise freight yield (contract pay or spreads) toward the bars, or recalibrate the 15k/35k/112.5 bars with an authored reason; never loosen silently |
