...
| D28 | save file grows ~6 KB per dock/save/load/trade cycle | 200-cycle soak: saveBytes climbed monotonically 537 KB → 1.77 MB with no plateau — durable `world.records` capture re-banks traffic/convoy actors each save while nothing expires them (convoy cap is per-sector alive-count only). Not a dead-end (all 200 restores passed) but unbounded bloat for long sessions | Long play session | open — repro: run any soak and diff `evidence.cycles.results[].saveBytes`; suspects: `_captureSectorDurableRecords` re-stamping traffic that already has `worldRecordId`, plus aftermath/marker records that never expire |
