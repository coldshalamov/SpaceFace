...
| D45 | Station Missions detail: the ORRERY `decrypt` reveal can be left mid-scramble on a settled screen | Live demo walk 2026-09-25 (`.devshots/demo-path/09-adv-upgrade.png`): the detail row label reads `READINE01` instead of READINESS; an earlier walk showed a title half-dimmed mid-word ("…near Te**thys**"). Both on a heavily loaded host, so the timer-driven reveal is probably starved or cancelled before its last frame | Adventure dock, Missions tab | open — repro: dock at Helios on the demo path under host load, open Missions, select a job; check that every `decrypt()` target ends on its final string even if frames are dropped (end state should not depend on the animation completing) |
