...
| D50 | Dormant-rock materialization can still spawn a scaled collider over a live hull on three paths sibling to the fixed A6 fling (`_tickAsteroidFieldInteractions` now promotes at collider skin; `_growFieldSeam` gained hull clearance + the authored collider scale) | Same defect class as the seed-4242 player fling (player ejected to a ~200k-unit orbit at ~3 km/s, `wave-a6` `fail_latch`): a dormant field rock becomes a live body while already inside a hull, and positional depenetration is unclamped — velocity clamps (`MAX_CONTACT_DV`, player give) do not cover positional writeback in `sg02DynamicBodyOwner`. Remaining overlap-capable admits: `src/systems/traffic.js` (~5019) promotes a barge's dormant rock at `dist < 40` to rock CENTER — radius-blind, so a barge parking on a big gas-rock (collider ~33) can materialize the ball over itself; `src/render/renderer.js` cook/prewarm promotes (~7160/7207/8840) batch-promote nearest dormant rocks by record radius — an NPC hull already overlapping the record eats a spawn-overlap; `src/core/physics.js` projectile sweep (~850) tests records at `rec.radius` while the promoted body carries the scaled collider, so bystanders under the record disc can be overlapped. Defense-in-depth gap behind all of them: no admission-time overlap check or per-tick positional-correction bound in the Rapier owner — any new path that admits a collider already-overlapped reproduces the yeet | Flight anywhere near dormant rocks — a mining barge or the player yeeted off the sector mid-demo | open — repro for the traffic row: park an NPC barge within 40 of a dormant `ast_gas_cloud` (R 14–30) record's center and let `_step*OreCarrier` promote; the spawned ball starts intersecting the barge. Cheapest coverage: extend `test/asteroid-field.test.mjs` collider-skin pattern to the traffic promote, or add an admission-time overlap guard in `promoteAsteroidFieldRock` |
| D49 | `test/pq146-escape-routes.test.mjs`, `test/pq146-evidence.test.mjs`, `test/pq146-projectiles.test.mjs`, `test/pq146-save-roundtrip.test.mjs`, `test/pq146-save-episodes.test.mjs`, `test/pq146-clothesline-route.test.mjs`, `test/pq146-narrative.test.mjs`, `test/pq146-field-routes.test.mjs`, `test/pq146-flight-evidence.test.mjs` — 9 stunt-detection tests red at clean HEAD | The stunt-proof fixtures (Kickstart, Needle Thread, Clothesline, Rapier shove, trajectory observer, save roundtrip) assert launch/retention speeds and intercepts tuned to the OLD propulsion ceilings; after `e8d10fed7e` the scripted launches no longer cross the stunt gates (the needle gate is `1.25*life.cruise`), so the proofs read receipts 0 where 1 is expected. Pinned by bisect in a throwaway clean worktree (`git bisect run node --test test/pq146-escape-routes.test.mjs`, 2026-09-25): first bad commit `e8d10fed7e` (2026-09-16 "Restore fast cruise ceilings" — deliberate MOTION_CHANGED, motion goldens re-recorded but these nine fixtures were not re-derived); identical failure-name set at HEAD `45d844376` and on the dirty tree, and the later `703917d86` threat prefilter is exonerated (red at its parent too). Found by the PQ-210.01 lane while gating its own perf unit | Stunt receipts (the score language) silently stop firing in the regimes the proofs encode | open — repro: `node --test test/pq146-escape-routes.test.mjs` at clean HEAD (3 of 4 red, same names every run). Fix is fixture re-derivation against the restored ceilings (recompute each scripted launch's expected speed/intercept from the eight profiles' restored combatSpeed/precisionSpeed), NOT re-recording motion goldens and NOT weakening the needle gate; a sitting in `src/combat/stunt*` or the propulsion fixtures handles this as part of its unit CAUSE BISECTED 2026-09-25: first bad `e8d10fed7` (2026-09-16, "Restore fast cruise ceilings…"); reverting only `src/core/flight/propulsionCatalog.js` there restores 6/6 — the doubled combat/precision speeds feed stunt evidence (`src/combat/stuntEvidence.js`) and the SG-02 contact-give budget, while the fixtures hard-code kestrel bodies and payload `vz=110` tuned to the old ceiling. Stale fixture, not a sim bug: derive the speeds from the catalog as that commit did for the pq-030 sweep test. |
| D51 | `scripts/check-first-hour.mjs` red at clean HEAD: `distress_call must not admit an elite or multi-squad encounter into the starter neighborhood` | Committed `1f28def48` ("allow distress in tier 1 sectors…") emptied `src/data/encounters/100-distress-call.js` `gates` — the encounter now admits in tier-1 starter sectors while its bait squad still anchors an elite `corsair_raider` plus 4–6 `wasp_swarmer`s, which is exactly the class the first-hour gate exists to keep out. Content and check disagree; the failing assert is working as designed, so the fix is a deliberate decision (re-gate the encounter's threat table or squad for tier 1, or amend the check with an authored reason), not a silent check edit | First-hour flight in starter sectors — a new player can spring a 4–6 ship elite-anchored ambush | open — repro: `node scripts/check-first-hour.mjs` (fails identically at clean HEAD and on the dirty tree; verified 2026-09-26) |
| D52 | Ceres pocket (`sector_ceres_belt`) offers only 2 distinct combat shapes — below the E6 repetition bar | E6's budget ("the same encounter shape cannot be the only combat offer for three hours; at least three shape ids in hours 0–3 and 5–9") measured across a 60-day `planEncounters` walk on `sector_ceres_belt` (tier 1, security 0.72, zones `mining_belt/refinery_approach/ambush_lane/derelict_field`): only `patrol_scan` + `bounty_hunter` are plannable — every other `deck:'combat'` shape is filtered by `minSectorTier ≥ 2`, `maxSecurity < 0.72`, zone-type mismatch, or `externalOnly`/`anchorPoiId` gating, and `bounty_hunter`'s `bountyOnly` is a fire-time gate so the runtime diet can be thinner still. The deliberate starter-pocket design (`sectors.js` "Tier-2 sectors restore the full combat band") leaves Ceres's combat diet one authored loss from a mono-offer. The demo route `sector_helios_prime` passes at exactly 3 (`opening_hauler_raid`, `patrol_scan`, `bounty_hunter`); frontier sectors pass at 9–12. Pinned by the new `test/e6-encounter-shape-budget.test.mjs` | Hours 1–10 in Ceres — same two combat beats repeating for a player who stays in the starter pocket | open — fix is authored: one or two more Ceres-plannable combat shapes (`deck:'combat'`, `zoneTypes ⊆ {mining_belt, refinery_approach, ambush_lane, derelict_field}`, no `minSectorTier > 1`, no `maxSecurity < 0.72`, not `externalOnly`, not `bountyOnly`-gated) would lift Ceres over the bar; the fixture covers the letter on helios_prime/nyx_march/io_reach today |
| D53 | `src/systems/world.js` `_stripSectorFullExtras` → `dropDressingSector` (sector first materialized REDUCED as a neighbour, then entered FULL by gate) | The hero landmark is simply not there: the Skerris Throne (Sker) and the Resonant Cathedral (Vesta) are neither live entities nor dressing rows when you arrive, and their scanner/discovery POIs are gone too. Every POI of any sector reached from a neighbour is erased the instant the neighbour materializes REDUCED, and FULL entry never re-spawns them | Adventure travel; any landmark or scanner beat outside the start sector | open — ready patch `.devshots/landmark-diag/proposed-poi-residency.patch` (a `keep` predicate on `dropDressingSector` retaining POI rows, plus `_promotePoiRowsToLive` on FULL entry); verified with a loader redirect (Throne returns live, no new test failures). Not applied 2026-09-25 only because `world.js` carried another lane's live hunks; it spawns entities, so run sim-compare when it lands. Repro: `node .devshots/landmark-diag/probe-landmarks.mjs` (capture order Helios→Ceres→Tethys→Vesta→Pallas→Sker, seed 4242) |
