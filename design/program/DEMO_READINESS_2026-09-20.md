...
| D40 | `test/works-conduit-wire.test.mjs` (PQ-131.06) red at HEAD: conduit render-package hash drift | `node test/works-conduit-wire.test.mjs` fails on `place_works_conduit_power_straight package render hash`: expected golden `6BC1640…`, committed `assets/ships/release/render-packages/works-conduit-power-straight/render.glb` hashes `1565C63A…`. Every input the test reads is clean at HEAD and the render.glb was last touched by AQ-LOD commit `38cbe05ef`, so the packaged conduit art and the test's frozen `EXPECTED_ARTIFACTS` disagree in the committed tree — either the regeneration was intentional and the golden needs a reviewed re-record, or the package content drifted. Player risk if unresolved: the works conduit network dresses itself from these packages; a silent content change ships unreviewed | Mining screen conduits (asset truth, not a visible symptom yet) | open — repro: `node test/works-conduit-wire.test.mjs` on a clean-HEAD worktree; reconcile render.glb vs the golden with the AQ-LOD lane, then re-record only with a deliberate reason per test/AGENTS.md |
| D41 | `test/time-effects.test.mjs` red at HEAD: `createTimeEffects` API-shape assertion predates a shipped method | Committed `src/core/timeEffects.js` exposes `describeRequests()` (line ~123, relied on by `scripts/lib/releaseSoakProbe.mjs` and the D35 work) while the committed test still asserts `Object.keys(effects)` is exactly `['clear','getEffectiveScale','reset','set']`. `node --test test/time-effects.test.mjs` fails at line 39 on a clean-HEAD tree — the API addition was intentional (soak diagnostics), so the assertion list needs the deliberate update, not the method removed | None player-facing; a red test at HEAD hides real regressions in the save/time freeze contract it covers | open — repro: `node --test test/time-effects.test.mjs` on clean HEAD; add `'describeRequests'` to the expected key list after confirming no other caller promises the four-method surface |
| D42 | `test/pq-158-00-sample-library.test.mjs` orphan-wav guard red at clean HEAD: delivered bark corpus and `enumerateDeliveredBarkWavs()` disagree | The committed `assets/audio/voice/` corpus (281 `line_NNN_*.wav`) was generated from an older pipeline ordering than the committed `enumerateBarkPipeline()` (277 rows): 33 wavs are orphans and 39 enumerated rows have no file. The register blocks shifted wholesale — disk runs mts→dmc→reach→quiet→choir→free→vael while the enumeration expects mts extended +33 rows further and each later register offset by one block (e.g. disk `line_068_faction_dmc` vs enumerated `line_068_faction_mts`). All inputs are clean at HEAD — `9cc039b11` delivered the wavs and `f919edca2`/`ba13918f9` later changed the corpus. Until reconciled the guard stays red and ~39 authored bark lines have no spoken file | In-game barks (register blocks may play wrong or missing voices) | open — repro: `node --test test/pq-158-00-sample-library.test.mjs` on clean HEAD; the bark-corpus owner either regenerates the voice wavs via `assets/audio/generate-bark-voice.mjs --all` to match the current enumeration, or reconciles the enumeration to the delivered corpus |
| D43 | `test/render-package-pilots.test.mjs` red at clean HEAD: ~8 pilot cases settle `procedural-settled` instead of `authored` | Found 2026-09-25 while verifying the loading-plan work; reproduced identically in a clean `git worktree` at `b0578ef16` without any local diff. The failing cases route through `mayComposeAuthoredShipLive`'s flight-compose gate in partsLibrary.js — the fixture's flight state likely no longer satisfies a gate added since the golden cases were written | Unknown; if the live gate matches the fixture, render-package pilot hulls could fall back to a procedural body in flight | open — repro: `node --test test/render-package-pilots.test.mjs` on clean HEAD; read which `mayComposeAuthoredShipLive` condition refuses, then fix the fixture or the gate, whichever is wrong |
...
| D46 | 2026-09-25 `check:all` sweep — foreign red classes new since the 2026-09-23 receipt set | 46 failing ids in `scratch/check-ci-report/2026-09-25T11-05-22-022Z/report.json`. The documented 9/23 foreign set covers the sim-goldens/47a/sg/station-screen families, but these are red NEW since then: check-bundle (esbuild errors in `loadingTerminalArt.js`/`localization`/`pipeline.js`/`onboarding.js`/`settings.js`), check-type-floor, check-src-reachability (names `src/ui/orrery/flightInstruments.js` + `flightPreview.js` — live ORRERY refactor WIP), check-camera-shake-scope, probe-flight-visual, check-map-information-depth, and the asset-art family (graphics receipts, kestrel wholeship, parts manifest, sg04 release, station glb, authored-place, shipworks clearcoat). Verified 2026-09-25 that none name the PQ-033.02 save-growth lane's files; several plausibly read the ~100 foreign dirty worktree paths rather than HEAD | Demo build health — each red hides regressions in its own area | open — repro: read the report.json above or re-run `npm run check:all`; before chasing any single one, re-run it in a clean-HEAD worktree to split dirty-WIP artifacts from real defects |
| D47 | `test/bounded-autosave.test.mjs:807` — 'new-game run epoch supersedes an in-flight autosave' | Red at HEAD, pre-existing and foreign to the PQ-033.02 save-growth lane (verified by that lane's independent reviewer, 2026-09-25): `saveSystem.js`'s `_epochOpenedByNew` guard (commit `6280dca20`) suppresses the second epoch bump the test expects; the test file is untouched since its fresh-root commit and neither file is in the save-growth lane's paths | None player-facing directly; a red save-contract test at HEAD masks real autosave regressions | open — repro: `node --test test/bounded-autosave.test.mjs`; decide whether the guard or the assertion carries the intent and land the deliberate side |
| D48 | An authored-admission job's inner await can park forever; lane starvation is now rescued but the dead await is unrooted | PQ-040 diagnostics on 2026-09-24/25: the trade-hub's `critical-hub:2` job (`place_station_trade_hub.glb`, 82 MB) sat `status:'running'` for 371–708 s across five runs while every queued ship admission waited behind it (serial flight lane = concurrency 1). The job chain (`admitNextUpgradeJob` → `upgradePlaceBoundary` → `loadPart`/`prepareAuthoredVisualPipelines`/publication gate) has no per-stage stall detector — whichever await never resolves (decode worker, KTX2 transcoder, GPU residency) wedges the boundary at `loading` forever. 2026-09-25 rescue: `stalledHogsCanPassShip` + `settleStalledUpgradeDiagnostics` + a 5 s wake poll let one queued ship pass once *every* unreleased in-flight job is 120 s+ stale (ship is hoisted ahead of same-priority dressing), and close the stale job's diagnostic so `activeJobs`-gated quiet windows drain. The closed record keeps verdict `stalled-slot-released` and stays in the starve dump. Limit: the serial slot itself (`inFlight`) is NOT released — the bypass widens the lane by one while the hog stays parked; if the bypassed ship also stalls past 120 s, another may pass, but each stale hog keeps its slot until its own promise unwinds. | Station approach / any authored admission on a loaded host | open — the parked await still needs a named site: the starve dump now carries `runningJobs[].status/boundaryState/preparePhase/graphFrozen` including watchdog-closed jobs; next repro (`node scripts/check-performance-dirty-ranges.mjs --runtime=browser --diagnostic` under contention) says which stage parks, then bound or cancel that await |
