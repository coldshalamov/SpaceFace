...
| D38 | First arrival of a body can sit on screen undrawn while it loads / links shaders | `probe:frame-solid` after the 2026-09-24 solid-world rounds (`.devshots/frame-solid/after4.txt`, heavily contended host, ~52 ms frames): 0 blinks, 0 root swaps, 0 unloads on screen, station collider always present — but first-time arrivals still wait: ship_pelican 349 (94 frames `pipelinesPending + asset:loading`), ship_atlas 307 / ship_mule 306 (`noMesh`, 35–47), station_helios (30 frames `compiling-pipelines`), all correctly `S0_EXACT / R0_GLASS`. The runway was widened the same day (prefetch 3.5 s, evict 5 s, authored decode 6 s) so loads start earlier; the remaining wait is the build + shader-link queue itself, and whether it survives an uncontended 60 fps machine is unmeasured | Flight, first sight of a ship/station/rock | open — repro: `node scripts/probe-frame-solid.mjs --census`, read offender `reasons`; measure on an idle machine first; then find which queue (mesh owe/bind vs `compileObjectPipelines`) is the long pole for an on-glass root and give on-glass roots priority. Build-map §13D Wave F3 (its own low-cost body as the instant stand-in) is the player-facing answer if the wait cannot reach ~0 |
