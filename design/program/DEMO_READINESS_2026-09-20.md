...
|---|---|---|---|---|
| D14 | `flight`, `crucibleHud` | The flight HUD is still the old instrument set (a card, bar rows, a key tray) rather than one cluster; an off-screen marker ("CAN" + diamond) draws over the integrity glyph at the left edge (seen live at 1280x720, seed 47) | Whole demo | open: ORRERY Phase 0a/1 rebuilds it |
...
| D19 | save | `canSave()` treats player id 0 as "no player", so Save would be silently disabled if the player entity ever gets id 0 | Save / continue | open: check how the live game assigns the player id |
