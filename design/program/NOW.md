...
| Perf pipeline: plan + leaf implementation | devin-perf-pipeline | IN PROGRESS | `src/render/partsLibrary.js` (primeNextAuthoredAssetPlan + helpers), `src/render/assetResidency.js` (canonicalDiagnostics epoch memo), `src/render/hlod.js` (detail-level memo), `src/render/asteroidInstancePool.js` (per-root matrix refresh), `design/perf/PERF_MASTER_PLAN_2026-09-26.md` (new), `design/program/NOW.md` | frame-solid probe: upgrade queue 21-deep/1 in-flight, station+fx miss jobs 6.7-8.4s, appearOnTime 0.60; prefetch only covers ships — extend to every queued job's authored assets |
