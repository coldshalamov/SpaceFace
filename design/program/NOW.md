...
| Finish-game orchestration | Cursor Grok orchestrator (`.codex/agent-checkpoints/ORCH-FINISH.json`) | MUTATING | `design/program/NOW.md` | Landed 026.01 and 147.01; 174.02 in flight. PR #111 remains VFX foundation. |
| Frontend Task C — station as a place, THE SHIP, THE FOOTPRINT, THE RANGE, the chart (PQ-162.00–.02, PQ-188.01–.02, PQ-168.00–.01) | Cursor Claude (`.codex/agent-checkpoints/FRONTEND-C.json`) | MUTATING | `src/ui/station/**`, `styles/station.css`, `styles/station-workbench.css`, `styles/station-berth.css`, `src/ui/ship/**`, `src/ui/screens/footprint.js`, `src/ui/screens/range.js`, `src/ui/galaxyMap.js` (CSS/palette/class swaps only), `src/ui/dockArrival.js`, `src/ui/shipLedgerPanel.js`, `src/ui/uiRoot.js` (dock cue line only), `styles/kit.css` (Task C additions only), `styles/ui.css` (fp-* rules only) | shell + berth first, then market, then the rest per screen |
| PQ-022.refinery-reauthor-h1 | Antigravity (`.codex/agent-checkpoints/PQ-022.refinery-reauthor-h1.json`) | MUTATING | `assets/ships/parts/parts_manifest.json`, `design/program/roadmap/evidence/h1/row7-pq022-asset-leaves/refinery-reauthor`, `design/program/roadmap/active/PQ-022.md`, `design/program/roadmap/program-queue.json`, `design/program/02_REMAINING_WORK.md` | capture browser and electron presentations, retain evidence, close leaf |
