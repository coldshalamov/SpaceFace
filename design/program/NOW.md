...
|---|---|---|---|---|
| PQ-033.02 min-spec soak + PQ-170.00 fronts | devin-cli | MUTATING | `scripts/lib/releaseSoakProbe.mjs` (harness hardening only), `design/program/roadmap/receipts/PQ-033.02-REPORT.md`, `design/program/roadmap/active/PQ-170.md`. Committed: GPU admission seal, save dup/quota + convoy cap + backup-quota degrade, autopilot dock limit-cycle, corridor lane-lock gated on lane membership + in-lane traffic pacing (7842a4b2a), soak write-trap instrumentation, occlusion-stall fix (624466961). PQ-170.00 fronts-tilt + 8-faction reactions committed and leaf marked. | Soak halted by owner at ~30 clean cycles (browser kept relaunching Chrome). Best clean evidence dir `.devshots/spec2/PQ-033.02-2026-09-16T00-22-31-981Z-10472-1442383e` (31 cycles, died on occlusion stall now fixed). Remaining: full 2h browser soak on 624466961, electron soak, check:pq033:min-spec, check:all, probe:runtime-witness, finalize report numbers, finish checkpoint `.codex/agent-checkpoints/PQ-033.02-min-spec-soak-and-PQ-170-fronts.json` |
