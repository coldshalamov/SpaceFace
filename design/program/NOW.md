...
| PQ-210.00 Crucible roster prewarm + D44 in-round instance-pool promotion | zcode-pq210-00-prewarm | DONE 343d3f25d | `src/render/renderer.js` (warm-time pool promotion from the decoded record, wave-1 keep set), `test/crucible-launch-warm-twin-promotion.test.mjs`, `design/program/DEMO_READINESS_2026-09-20.md` (D44 row removed in the fixing commit) | recorded, released — chunk promotion at flight 0 → 36 chunks/16 owners; in-round hull chunk uploads +71 → +3; shaderLinks in-round → 0; quiet-host absolute sign-off of `.00`'s bar is the honest leftover |
| PQ-146 Phase 3 presentation: named trick callouts + combo + results rows | zcode-pq146-presentation | DONE 71a46475f + review repair cb12a303e | `src/ui/stuntCallout.js` (new), `src/ui/screens/crucible.js`, `src/systems/survivalResults.js`, `test/pq-146-03-stunt-callout.test.mjs` | recorded, released — callouts name tricks with combo meter + bank receipts, Crucible-only; results rows; independent review found the frame step died after its first dark frame (scheduler drops false-returning listeners) — repaired with a re-arm regression test; no HUD files touched |
| PQ-137.06 terrain-is-lethal + .07 rope-is-a-rope (sim halves) | zcode-pq137-feel | DONE cc82832e4 | `test/pq-137-terrain-lethal.test.mjs` (new law-level suite), `test/rope-swing-release.test.mjs` (stale 195 cruise-guard fix), receipts (dated addenda), `design/program/NOW.md` | recorded, released — sim laws verified live at seed 4242: B6 real path all five clauses green; B7 stretch 8.481 %, line held, release keeps 100.03 % at 5 s; solver bound asserted a rate limit, break asserted by load rating |
| D46 red-check triage: classify 46 ids dirty-WIP vs real, fix small real ones | zcode-d46-triage | DONE b9967eb19 + ec7ce0306 | `design/program/DEMO_READINESS_2026-09-20.md` (D46 row rewritten), `scripts/check-market-navigation.mjs`, `scripts/check-sg08-render-vfx.mjs` | recorded, released — all 46 classified (10 green, 3 flake, 13 dirty-WIP/WIP-resolved, 1 fixed + 1 partial, 14 owned by HUD/asset/47-A lanes, 4 environment); market-nav green; sg08 token re-pinned, residual named to the VFX owner |
| PQ-210.01 Crucible CPU budget: profile, name top-3 owners, cheapest fix | zcode-pq210-01-cpu | DONE cfded2e15 | `src/combat/stuntEvidence.js`, `src/combat/stuntFlightEvidence.js`, `src/combat/stuntProjectileEvidence.js`, `src/systems/stuntGrammar.js`, `test/pq-210-01-stunt-evidence-cadence.test.mjs`, `scripts/probe-crucible-cpu-profile.mjs` | recorded, released — top-3 owners named from a real profile (draw-submit 18.4 %, classifyWorld 5.75 %, updateMatrixWorld 6.8 %); 22-light suspect REFUTED (baked count 12); stunt-evidence family 3.27 % → 1.58 % of callback; HUD DOM + AI allocation handed to their owners |
| Ten-polish campaign waves B/C: opening tail settle, kill-cam, wave grading, wreck terrain, first-hour choice, D38 evidence | zcode-campaign-waves-bc | DONE dbcece7e0, 30d1c41d6, ac9bcac91, 581d284de, aeb26231c + 7e18dbd77, b310e81e6 | `src/main.js`, `src/render/precompile.js`, `scripts/probe-smooth-flight.mjs`, `test/opening-composition-tail-settle.test.mjs`, `src/sim/killcamTape.js`, `src/render/killcamStage.js`, `src/render/uiStage.js`, `src/core/registry.js`, `src/ui/screens/crucible.js`, `styles/kit.css`, `test/killcam-tape.test.mjs`, `src/data/survivalWaves.js`, `src/systems/survivalAnnounce.js`, `src/systems/survivalWavePlanner.js`, `test/pq-140-wave-physical-problems.test.mjs`, `test/pq-154-wreck-terrain.test.mjs`, `src/onboarding/rescueOpening.js`, `src/systems/onboarding.js`, `test/pq-163-opening-choice-pacing.test.mjs`, `src/render/partsLibrary.js`, `scripts/probe-frame-solid.mjs`, receipts | recorded, released — opening: novel in-window links 1 → 0, 8.5 MB mid-flight burst gone (launch cost shift flagged); kill-cam pose-tape replay behind the results plate with Skip + reduced-motion still; all 70 wave recipes machine-verified closable as physics and named by the announcer; wreck-terrain contract proven (18/18 across the PQ-154 set); first refusable choice measured at t=26 s with both branches consequential; D38 per-job cost profiled — the gate is driver link time, the reducible half (instancing-twin programs) named in build_map §21.4 with evidence |
| INFERENCE 20 — look-then-rotate grunt pass | devin-inference-20 | DONE 2026-09-26 | all 20 units landed + review tails; U16 seed lock-ring (13ae49cb9 + tail 99b90761a); U17 reinforcement tell (139462c0a + tail 182faf57b); U18 ashcache fence (73287b288 + tail c0f18bbe6); U19 bounty marks name the mark (3d7ee135b + tails a02a277cf, f9f536ff4); U20 Metronome beam (d77197454 + tail cc55ac79c) | 20 complete production units, rotating kinds, each committed with pathspec + inference-record |
| PQ-153.02 Landmarks fielded | wf-buildmap-five | IN PROGRESS 2026-09-25 | `src/data/sectors.js`, `src/data/frontierRegions/`, `src/data/authoredPlaces.js`, `src/systems/regionalEcology.js`, `design/world-identity/` | implement, run the unit's checks, commit by pathspec, release row |
| PQ-133.04 Surface receipt + Ricochet Foundry vertical slice (Phase 4) | wf-buildmap-five | IN PROGRESS 2026-09-25 | `src/core/physicsAuthority.js`, `src/systems/arenas/ricochetFoundry.js`, `src/data/survivalArenas.js`, `src/data/survivalWaves.js`, `src/combat/attackPropagation.js`, `src/data/enemies.js` | implement, run the unit's checks, commit by pathspec, release row |
| PQ-033.02 Min-spec floors and soak | wf-buildmap-five | IN PROGRESS 2026-09-25 | `electron/`, `package.json`, `build/`, `src/ui/screens/mainMenu.js`, `src/ui/screens/pause.js`, `LICENSE`, `NOTICE` | implement, run the unit's checks, commit by pathspec, release row |
| PQ-025.calibration-qualification Freeze, calibrate, and run the held-out 30/90-minute Gold Corridor qualification | wf-buildmap-five | IN PROGRESS 2026-09-25 | `design/program/roadmap/active/PQ-025.md`, `scripts/validation-manifests/` | implement, run the unit's checks, commit by pathspec, release row |
| PQ-040.native-acceptance PERF-06 dirty-range Browser/Electron acceptance | wf-buildmap-five | IN PROGRESS 2026-09-25 | `.devshots/perf/dirty-ranges/`, `design/program/roadmap/active/PQ-040.md`, `design/program/roadmap/receipts/PQ-040-dirty-range-uploads-REPORT.md` | implement, run the unit's checks, commit by pathspec, release row |
