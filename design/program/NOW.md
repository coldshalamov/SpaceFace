...
|---|---|---|---|---|
| PQ-033.02 min-spec soak + PQ-170.00 fronts | devin-cli | MUTATING | `scripts/lib/releaseSoakProbe.mjs` (harness hardening only), `design/program/roadmap/receipts/PQ-033.02-REPORT.md`, `design/program/roadmap/active/PQ-170.md`. Committed: GPU admission seal, save dup/quota + convoy cap + backup-quota degrade, autopilot dock limit-cycle, corridor lane-lock gated on lane membership + in-lane traffic pacing (7842a4b2a), soak write-trap instrumentation, occlusion-stall fix (624466961). PQ-170.00 fronts-tilt + 8-faction reactions committed and leaf marked. | browser soak RUNNING PID 19304 on 624466961 (2h/200cyc; prior run died at cycle 31 on a document.hidden occlusion stall — harness now immune). Then: electron soak, check:pq033:min-spec, check:all, probe:runtime-witness, finalize report numbers, finish checkpoint `.codex/agent-checkpoints/PQ-033.02-min-spec-soak-and-PQ-170-fronts.json` |
