...
| PQ-032.02 Deep Reach climax is a physical toy | grok-pq032-climax → devin adopted | LANDED 39c7d92eb | `src/story/campaign47a/embodiedMissions.js`, `src/systems/missions.js`, `src/data/missions.js`, `test/contract-47a-b7-deep-reach-operation.test.mjs`, `test/story-campaign47a-live.test.mjs` | stale row adopted; committed; independent review in flight before queue state |
...
| PQ-033.02 save-growth: bound ledger ramps + orphan hardening | zcode-wf-pq033-02-savegrowth | DONE | `test/save-growth-dock-trade-flat.test.mjs`, `src/world/farActorTable.js`, `src/systems/npcJobsRuntime.js`, `src/systems/economy.js`, `src/systems/economyCycles.js`, `test/pq-033-02-far-row-budget.test.mjs`, `design/program/NOW.md` | recorded, released — 52-cycle soak tail slope -0.59 KB/cycle (was +4.75 pre-fix, red run), far-row orphans 0, assign-pin companion green, lab-save-load G1 parity green; ORPHAN sweep + assign-time record pin + stale-pin reconcile + closed-blend-tail drop + packed-ts quantize |
