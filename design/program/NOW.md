...
|---|---|---|---|---|
| Build map five: B4 specialists, C6 skies, E1 pad verbs, B9 kill replay, A7 muzzle flow | grok-buildmap-five | IN PROGRESS | `src/ai/specialistCounterplay.js`, `src/combat/damage.js`, `src/systems/fields.js`, `src/data/sectorVisualProfiles.js`, `src/systems/gamepad.js`, `src/systems/input.js`, `src/ui/input.js`, `src/ui/bindings.js`, `src/ui/screens/help.js`, `src/ui/screens/settings.js`, `src/ui/controlPrompts.js`, `src/ui/screens/crucible.js`, `src/systems/survivalResults.js`, `src/systems/killReplay.js`, `src/render/forceLanguage/weaponDischargePool.js`, `src/render/weapons/recipes.js`, `test/wave-b4-specialist-plans.test.mjs`, `test/wave-c6-sky-luminance.test.mjs`, `test/wave-e1-pad-verbs.test.mjs`, `test/wave-b9-kill-replay.test.mjs`, `test/wave-a7-muzzle-flow.test.mjs`, `design/program/NOW.md` | focused tests, review, pathspec publish |
| Build map five: Wave A5, F3, E2, B8, B11 | antigravity-buildmap-five | DONE | `src/data/weapons.js`, `src/data/encounters/015-opening-hauler-raid.js`, `src/systems/encounterDirector.js`, `src/ui/screens/sandbox.js`, `src/ui/screens/crucibleLabControls.js`, `src/ui/screens/mainMenu.js`, `src/ui/uiRoot.js`, `test/wave-a5-shot-life.test.mjs`, `test/wave-f3-opening-trio-well.test.mjs`, `test/wave-e2-reduced-motion-information.test.mjs`, `test/wave-b8-progression-verbs.test.mjs`, `test/wave-b11-physics-lab.test.mjs`, `design/program/NOW.md` | recorded, released — A5 (shot life >= 0.70s), F3 (opening trio well fitting), E2 (reduced motion gameplay facts), B8 (10-verb progression curve), B11 (physics lab on front door); subagent review APPROVED, all 9 wave tests green |
| PQ-033.02 sitting: warning classes, D24 heap attribution, storm stacks | zcode-pq033-02-sitting | IN PROGRESS | `scripts/lib/releaseSoakProbe.mjs`, `test/pq-033-02-gl-delete-trap.test.mjs`, `src/render/renderer.js` (additive only), `src/render/contextResourceLifecycle.js`, `design/program/roadmap/receipts/PQ-033.02-REPORT.md`, `design/program/NOW.md` | receipt addendum + lane release |
...
| PQ-033.02 autopilot dock wedges + restore-wait bound (soak evidence lane) | devin-pq033 | DONE 84f8c2016, afaeaac6d, a7fd6f31c, 649b1cfbf | `src/systems/npcJobsRuntime.js`, `test/npc-jobs-runtime-wiring.test.mjs`, `scripts/lib/heapSnapshotDiff.mjs` | recorded, released — browser soak **200/200 cycles clean** (52.8 min, context-loss recovery green); **D28 fixed**: job-owned `flags.persistent` now releases/sweeps when the job ends (anchor check preserves mission/custody/itinerary/site/tow owners) — headless probe entities −37KB, persistent set 15→4 anchored, save +0.19KB/cycle flat; 2h Electron soak in flight; floors: heap +411MB/52min (D24, may shrink post-D28), frame/boot floors contention-dominated on this host |
| PQ-033.02 soak instrument: Electron GL traps armed live-page + delete-trap storm-class recording | glm-pq033-02 | DONE (uncommitted; paths exact) | `scripts/lib/releaseSoakProbe.mjs`, `test/pq-033-02-gl-delete-trap.test.mjs`, `design/program/roadmap/receipts/PQ-033.02-REPORT.md`, `design/program/NOW.md` | recorded, released — v6 pair re-verified via `check-min-spec-floors` both runtimes (exit 1, honest RED, numbers match receipt); Electron acceptance soak ran both GL traps disarmed (256-warning delete storm, 0 trapped) because arming rode the opt-in tier1 reload — now armed on the live page post-canonical-load; delete trap records every delete with `alive` flag (isX blind spot = the documented isProgram lesson), 128-entry cap FIFO-evicts common class (alive:false never evicted) with `{entries, evicted, dropped}` saturation visible in evidence; 9/9 vm-sandbox trap tests (incl. 200-chain cap storm-admission), 22/22 with contract suite, baseline 16/16; leaf open on DONE-WHEN (quiet reference host) |
...
| Demo sew-up: salvage contract names the spill, hot-dock pods stay in the bay, demo end and game over on the Hand, combat verb cues | cursor-demo-five | IN PROGRESS | `src/systems/missions.js`, `src/systems/cargo.js`, `src/ui/screens/demoEnd.js`, `src/ui/screens/gameOver.js`, `src/audio/combatVerbCues.js`, `test/cargo-kill-salvage-sale-opportunity.test.mjs`, `test/hot-dock-cargo-spill.test.mjs`, `test/demo-path.test.mjs`, `test/combat-verb-cues.test.mjs`, `design/program/NOW.md` | review + pathspec publish |
