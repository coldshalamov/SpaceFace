...
| **D-2** | P1 | presentation | **RESOLVED 2026-08-06.** The Travel Burn cue now reads the executor's real `engaged` field through one exported HUD truth helper. Route interruption, failed replan and arrival retire only the route-owned waypoint and target, so an abandoned leg cannot become manual BRAKE guidance; local manual waypoints remain intact. | `node scripts/repro-abandoned-route-hud.mjs` now passes the original abort/replan/interdiction outcomes plus a never-engaged control. `test/navigation-stale-route.test.mjs` carries the durable route-owner and HUD truth regressions. | W1-9 correctness |
...
| **D-8** | P2 | navigation | **RESOLVED before 2026-08-06 reconciliation.** Reaching a sector-changing leg's gate requests the shipped `world:requestJump` handoff, and only a matching real `sector:enter` advances/completes that leg. Autopilot arrival remains sufficient only for an intra-sector leg. | `test/route-follower.test.mjs` pins gate arrival without completion, one handoff request, matching sector entry, and final completion; `routeFollower.js` has no fake sector mutation. | Correctness of R.3 in the release gate |
| **D-9** | P2 | propulsion | **RESOLVED 2026-08-06.** Engine tiers now publish a refit-derived Travel Burn ceiling while retaining the exact propulsion profile ordinary flight already used. On one real L engine bay, Ion/Fusion/Warp produce **472.5 → 543.375 → 614.25 WU/s**; thrust, reverse, strafe, combat speed, family and controls are unchanged. | `check:propulsion:authority` walks the real fitting → ship derivation → spawn → profile route, asserts the 1.00×/1.15×/1.30× progression, and proves repeated flight reads reuse one complete profile instead of allocating per tick. `check:propulsion:extreme`, Travel Burn, role-ordering and outfitting checks remain green. | D5's stated upgrade path; ruling **R-5** is satisfied |
...
| **D-13** | P3 | navigation | **RESOLVED 2026-08-06.** `save:loaded` revalidates sector identity, leg continuity, executor/route agreement, current origin and named target against the current Atlas before route state can steer. Invalid restored guidance is disarmed and cleared with one visible replot message; a valid itinerary resumes silently. | `test/route-follower.test.mjs` drives a structurally accepted stale save through the real sanitizer, proves rejection at the owner boundary, and separately proves a valid engaged itinerary survives. | Corrupt/content-outdated saves no longer steer toward cached ghost coordinates |
| **D-14** | P3 | propulsion | `ship_bastion` spends **85% of its own hull radius per tick** at its catalogue ceiling (1120 WU/s → 18.67 WU/tick vs radius 22.0), against D5's justification that the ceiling stays "well under hull-radius scale". Collision is not a swept test, so the margin before tunnelling is 15%. | `check:propulsion:extreme` evidence table. Emitted as a WARN, not a failure: the hard bound is physical fact but the acceptable margin is a design call belonging to D5's owner. | Advisory. Engine-tier overrides can now reach the 1200 WU/s absolute bound; that bound remains the authority and is not raised by this upgrade path. |
| **D-15** | P3 | propulsion | **MITIGATED 2026-08-06; strict closure retained.** The old helper-only estimate was stale: the first real steady-tick scenario measured **2,537 B/op** with six resident haulers and fourteen beacons. Retained status/projection storage, scalar traffic updates and allocation-free change detection reduced that to **696 B/op** (72.6%, about 152 KB/s removed at 60 Hz) and median tick time from **0.00399 to 0.00132 ms**. The one allowed after run attributed the remaining churn to `resolveTravelCeiling` defensively normalizing one immutable profile each tick; the production path now caches that ceiling by propulsion-profile identity and invalidates on refit/load. | `probe-atlas-perf-baseline.mjs` now measures `travelLanes.update@steady`, not convenience wrappers. Focused lane/Throughline tests prove retained read-model identity, event snapshot safety, refit invalidation and byte-identical replay. The predeclared **256 B/op** ceiling stays strict; PQ-033 owns its next clean-platform evidence rather than an unchanged retry in this slice. | Ordinary lane flight no longer allocates status objects, signature strings, traffic arrays/positions, map-iterator entries, or repeated normalized propulsion profiles every fixed step. |
