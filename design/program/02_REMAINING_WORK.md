...
| `PQ-034.native-closure` | The tenth Browser claim completed the full matrix with the prior fleet/bloom program drift fixed, then exposed one docked-window false positive: the harness projected stored ship velocity through wall time while `timeScale=0`, tick/program/residency stayed fixed, and no sim or admission work occurred. Simulation-time horizon repair `d521232a` is pushed and focused-green. Run one fresh Browser claim and, only after it passes, Electron on the same repaired source, then close overhead and causal review. Report uncertainty/variance honestly. |
