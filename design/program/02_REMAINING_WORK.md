...
| M6-WASP | PARTIAL — LIVE BODY WIRED; QUALITY OPEN | A dedicated Wasp wholeship is on the live player map. `PQ-050.03` is the remaining careful upgrade: do not drop hull below 800 triangles, and do not replace the live file if stills get worse. Blocked accessory `wasp.glb` stays unwired. | `PQ-050.03` stills vs current live Wasp; hull-triangle floor; player-route/perf review. |
