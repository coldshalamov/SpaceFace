...
| `PQ-034.native-closure` | The ninth Browser claim isolated the deeper rare-VFX cache mismatch: doubled temporary-light cardinality, non-live ribbon attributes, and disposed salvo owners. Exact repair `12b6b905` is pushed. A no-launch retry guard then exposed that the paired manifest omitted the new repair regressions from its digest; broker binding `e343fe57` is pushed and focused-green, with the claim count still at nine. Run one fresh Browser claim and, only after it passes, Electron on the same repaired source, then close overhead and causal review. Report uncertainty/variance honestly. |
