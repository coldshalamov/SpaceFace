...
| `DONE` | 31 — `PQ-022.billboard-buoy-reauthor-h1` | Prove the revised billboard and buoy on the real route. | Browser and Electron presentation proven; receipt published. | Existing billboard/buoy implementation |
...
| `DONE` | 230 — `PQ-018.cathedral-reauthor` | Re-author Cathedral hull/rupture construction language (implementation leaf). | Shipped source/release + receipt; whole-asset KEEP deferred to h1/review. | World Site runtime and current art verdict |
...
|---|---|---|
| `CLOSED` | **Mining-prop stragglers (verify-open 2026-08-18; closed 2026-09-19).** The finding measured the pre-V3 bevel pass. Both live release GLBs are already the V3 material-truth re-author (manifest note "Opening-route industrial prop V3 2026-08-18"): the drone release is 17,924 triangles / 7 materials / 22 KTX2 with LOD0/1/2 8716/6704/2504, and the barge release is 37,332 triangles / 9 materials / 29 KTX2 with LOD 19500/12608/5224. Source, release, release-manifest, and render-package provenance agree by sha256. | No action. `PQ-210` was admitted on this stale finding and is withdrawn; see [`RESIDUALS_LEDGER.md`](./RESIDUALS_LEDGER.md) §A3 and receipt [`roadmap/receipts/MINING-PROP-V3-VERIFY-2026-09-19.md`](./roadmap/receipts/MINING-PROP-V3-VERIFY-2026-09-19.md). |
...
| M1-VISUAL-FAMILY | PARTIAL — PROMOTED CHECKPOINT | Kestrel, pooled thrusters/RCS, Helios surfaces, the `54548e09` Helios Lark/Cradle/Span family, seamed/graffiti landmarks, loading presentation, the de-hazed background substrate, common-rock maps, authored fail-closed admission, typed projectile/mine/impulse/wreck identities, geology truth `e8838e2c`, and the RCS evidence repair `3d2dc765` are wired on current `master`. `PQ-049` now admits the separate `GFX-MASSLINE-EXPRESS-LINER` route; it does not reopen or replace the accepted courier Lark. `PQ-050` admits the remaining non-Hitch flyable fleet remaster to Hitch-plus; Hitch stays frozen and current factory bodies are not accepted. Finish Ashline V2 runtime admission, compact/reduced-motion/dense/Spector propulsion proof, combat/destruction visual acceptance, natural-route rock parity, localized authored space structure, gate/hero assets, the new express liner, Hitch-plus fleet remaster, and the next high-frequency PBR families. | Normal public routes with authored assets, classification/provenance records, asset/live/visual/perf checks, and reviewed browser/Electron media; `PQ-049.05` specifically requires natural express Browser/Electron, tether/save, dense-pocket performance, and independent whole-asset G7 evidence; each `PQ-050` ship leaf requires matched Hitch-vs-remaster stills before that ship is wired. |
...
| M4-FAMILIES | PARTIAL | Finish fighter/interceptor, miner/hauler, weapon/impact, station, POI, and ecology visual/behavior families. Flyable non-Hitch ship graphics continue through `PQ-050`. | Sparse/normal/crowded public routes and accepted visual classifications; fleet remaster stills must beat live Hitch. |
...
| M6-WASP | PARTIAL — LIVE BODY WIRED; QUALITY OPEN | A dedicated Wasp wholeship is on the live player map. `PQ-050.12` is the remaining careful upgrade: do not drop hull below 800 triangles, and do not replace the live file if stills get worse. Blocked accessory `wasp.glb` stays unwired. | `PQ-050.12` stills vs current live Wasp; hull-triangle floor; player-route/perf review. |
...
| M6-WORLD-CLEANUP | TODO — OWNER PLAYTEST 2026-09-08 | Kessler / Helios opening: NPC hulls that do not belong in the same game (including a detached engine and glow blobs) and reverse thrust that reads as two bow needles. Door: [`VISUAL_WORLD_CLEANUP.md`](./VISUAL_WORLD_CLEANUP.md). Taste model chooses among options; Hitch stays frozen; volumetric retro already exists and still fails on camera. | Chase-camera stills: one game, complete hulls, readable reverse jets; no new top frame-time bucket. |
