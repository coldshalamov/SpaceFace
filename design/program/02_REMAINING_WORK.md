...
| M1-VISUAL-FAMILY | PARTIAL — PROMOTED CHECKPOINT | Kestrel, pooled thrusters/RCS, Helios surfaces, seamed/graffiti landmarks, loading presentation, the de-hazed background substrate, common-rock maps, authored fail-closed admission, and typed projectile/mine/impulse/wreck identities are wired on current `master` through the `b235f062` synthesis and propulsion repair `59f91d19`. Finish exact-head compact/reduced-motion/dense propulsion proof, combat/destruction visual acceptance, natural-route Helios/rock parity, localized authored space structure, gate/hero assets, and the next high-frequency PBR families. | Normal public routes with authored assets, classification/provenance records, asset/live/visual/perf checks, and reviewed browser/Electron media. |
...
| M6-PERFORMANCE | PARTIAL — SYNTHESIS INTEGRATED | Performance tip `99cad5b5` is integrated through `b235f062` with graphics semantics preserved; strict evidence hardening is integrated through `280cafb0`. The tree passes all 167 modified performance tests and 35 final-evidence contract tests; bounded autosave retains its 8 ms scheduling target and unchanged 12 ms hard slice. The isolated `04805924..9d626fd8` scene-pool/BatchedMesh range was measured and rejected: every architecture lost to current ship-local static batching, and the final target-Intel p95 was 250.1/616.8/433.3 ms for 10/25/50 ships. Prior diagnostic matrices do not count as <=16.7 ms acceptance runs. | Fresh combined-tree three-profile/three-full-matrix promotion gate on one exact clean commit, plus target/floor hardware, decoder/loader lifecycle, and no graphics-admission regression. Do not replay the rejected pooling range. |
| M6-VISUAL-DEFECTS | PARTIAL | Hitch plume glare and Helios opaque/unshaded bulk materials were repaired; the closeout adds common-rock PBR maps, semantic native-color preservation, no-blue-box authored admission, typed combat/world identities, and `59f91d19` propulsion wiring/lifecycle repair. Remaining visible-risk work is exact-head compact/reduced/dense propulsion evidence; natural Helios/rock motion and mip continuity; rock-shaped `fx` whose visuals disagree with drill/tether semantics; natural-route admission/rebase/interpolation/LOD/HLOD/pool flicker continuity; Electron/GPU combat/destruction acceptance; localized background structure; and fleet-wide asset-specific PBR. | Same-framing browser/Electron captures, independent visual acceptance, asset/live/visual/perf checks on the final combined tree. |
...
5. Keep residual concurrent WIP separate, re-run status counts, and update
   ...
   integration batch.
6. Use [`09_DONOR_VALUE_LEDGER.md`](./09_DONOR_VALUE_LEDGER.md) before removing any remaining
   worktree. Never whole-merge the Depth or Kimi satellites; port only the recorded product-bearing
   slices after comparing them with current `master`.
