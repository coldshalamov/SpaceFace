...
| M4-ECOLOGY | FOCUSED-GREEN — 9/9 (2026-08-05) | No ecology product failure was reproduced. The sole red was an obsolete source-text adjacency pin left behind when runtime and fresh-run order moved to authoritative exported manifests; the check now reads those owners directly. Regional identities, traffic/yield/encounter/law seams, causal aftermath, and deterministic save/reload all pass. | `npm run check:m4:regional-ecology` 9/9. Sparse/normal/crowded public-route diversity and art classification remain broader M4 acceptance, not an ecology initialization repair. |
