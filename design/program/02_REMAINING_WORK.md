...
| M6-PERFORMANCE | PARTIAL — ISOLATED FOLLOW-UP PENDING | The earlier integrated route retains staged loading, presentation admission, shared GPU residency, repaired attribution probes, and the recorded five clean flight runs. A newer `codex/performance-closure-20260719` lane is still isolated and touches AI/save/economy/world plus renderer overlaps. Integrate it only after review, then reprofile headed frame pacing, startup, memory/soak, dense combat, autosave completion, KTX2 lifecycle, target hardware, and floor hardware. | Fresh combined-tree headed frame/startup/memory profiles on target and floor hardware, including decoder/loader lifecycle evidence and no regression from graphics admission/preload. |
| M6-VISUAL-DEFECTS | PARTIAL | Hitch plume glare and Helios opaque/unshaded bulk materials were repaired; the closeout adds common-rock PBR maps, semantic native-color preservation, no-blue-box authored admission, and typed combat/world identities. Remaining visible-risk work is natural Helios/rock motion and mip continuity, Electron/GPU combat/destruction acceptance, localized background structure, and fleet-wide asset-specific PBR. | Same-framing browser/Electron captures, independent visual acceptance, asset/live/visual/perf checks on the final combined tree. |
