...
| M1-VISUAL-FAMILY | PARTIAL — PROMOTED CHECKPOINT | Kestrel, pooled thrusters/RCS, Helios surfaces, seamed/graffiti landmarks, loading presentation, and the de-hazed background substrate are wired on `master`. Finish combat/destruction visual acceptance, natural-route Helios/rock parity, localized authored space structure, gate/hero assets, and the next high-frequency PBR families. | Normal public routes with authored assets, classification/provenance records, asset/live/visual/perf checks, and reviewed browser/Electron media. |
...
| M6-PERFORMANCE | PARTIAL — REBASELINE REQUIRED | The integrated route now has staged loading feedback, presentation admission, shared GPU residency, repaired attribution probes, 1.02 ms/tick at 240 ships, and five clean desktop/mobile flight runs. Reprofile headed frame pacing, startup, memory/soak, dense combat, autosave completion, KTX2 lifecycle, target hardware, and floor hardware before retiring the older 49.4 ms p95 baseline. | Fresh headed frame/startup/memory profiles on target and floor hardware, including decoder/loader lifecycle evidence. |
| M6-VISUAL-DEFECTS | PARTIAL | Hitch plume glare and Helios opaque/unshaded bulk materials were repaired in the promoted checkpoint. Remaining visible-risk work is natural Helios/rock motion and mip continuity, combat/destruction family acceptance, localized background structure, and fleet-wide asset-specific PBR. | Same-framing browser/Electron captures, independent visual acceptance, asset/live/visual/perf checks. |
