...
| `PQ-034.native-closure` | The eleventh Browser claim proved the docked/fleet/bloom repairs and completed the full matrix, then exposed one admission-validator defect: the Ceres transition completed a new authored station admission and stable residency but released one net old-sector program, which the monotonic-count rule rejected. Semantic admission repair `20f78f49` requires a newer terminal authored-admission sequence and rejects no-ops. Run one fresh Browser claim and, only after it passes, Electron on the same repaired source, then close overhead and causal review. Report uncertainty/variance honestly. |
