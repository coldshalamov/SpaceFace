...
| **B2 Nimble regime** | From rest to cruise ≤ 1.5 s. Full 180° velocity reversal ≤ 3.0 s. Turn radius at cruise ≤ 1 screen depth. | 0.667 s / 1.583 s / 0.946 screens (Hitch, seed 4242; Wasp 0.467 / 1.133 / 0.445) | `feel.reversal_course` (Motion Lab M4/M6) | **MET** on the route 2026-09-05; `PQ-137.03` awaits its shipping-camera capture |
| **B3 The fight stays on screen** | At cruise the hull needs ≥ 1.2 s to cross the visible depth. Above the cap the camera opens with speed: visible depth grows monotonically, reaches ≥ 1.5× the at-cruise depth at 2× cruise and ≥ 2.5× at 3× cruise, and the starter hull never falls below 4 % of frame width. *(Rewritten 2026-09-03 by the taste director: the original "≥ 2 s of travel at a 2× cruise exit" implied a ~3.3× opening at 2× cruise and contradicted §C's camera band; the band is the law, this clause is its measurable form.)* | 0.6–1.0 s; camera opens only to 1.55× at 3× hull max | `feel.screen_crossing` | OPEN `PQ-137.03` |
| **B4 Shove magnitude** | The dedicated shove weapon changes a light hostile's velocity by ≥ 30 % of its cruise per hit. The **starter** gun changes it by ≥ 5 % per hit. A light hostile already at cruise gets **faster** when shoved along its motion. | starter 5.0 % / concussion 54.5 % / mine 45 % of light cruise per hit; 1.605× faster along motion; no clamp | `feel.shove_magnitude` | **MET** 2026-09-05 (magnitudes and clamp); `PQ-137.05` owes the Crucible kit-balance clause |
| **B5 Shove displacement** | 2 s after a shove-weapon hit, a light hostile is ≥ 1 screen depth off the line it was flying and has not fired. | ~1 hull length; fires after 0.5 s | `feel.shove_magnitude` (its B5 clause: displacement 2 s after the hit, screen depths) | OPEN `PQ-137.04` |
| **B6 Terrain is lethal** | A light hostile meeting rock at ≥ 50 % of cruise loses ≥ 60 % of hull and its helm; at ≥ 75 % of cruise it dies. A heavy at the same speed loses ≤ 15 % and keeps its helm. | 86.96 % hull lost at 50 % cruise, dies at 76 %; heavy 0 % and keeps helm | `feel.terrain_slam` | **MET** 2026-09-05 (helm and damage); `PQ-137.06` awaits its shipping-camera capture |
...
| **B13 The player is never knocked around** | In ten minutes of ordinary flight (no rope, no fields, no deliberate ram), contact changes the player's velocity ≤ 2 times per minute and never by more than 10 % of cruise in one event, never changes the player's heading, and never produces visible jitter; a deliberate big event (a slam the player chose, a well the player flew into) may, and it must be legible. | **Flight bench MET 2026-09-05** — ten minutes at seed 4242: 1.0 knocks/min, largest 6.6 % of cruise, 0 heading changes; 120 s seeds 4242/8008: 2.0 and 1.5 per min, 5.6 % and 4.5 %, 0 heading changes; jitter sim-sampled clean at frame cadence and at 60 Hz (pictures witness = the Crucible strip). **Crucible bench NOT met** — worst of nine cells 10.0 knocks/min (rope kit at Helios), largest 10.0 % of cruise (at the ceiling), 0 heading changes on all nine; the one photographed cell shows 1 screen reversal in 6 windows after a contact of unknown class, so the Crucible jitter clause is unresolved rather than failed | `feel.knock_budget`, `feel.knock_budget_10min`, crucible knock metrics + a normal-speed knock strip | OPEN `PQ-137.11` — the heading and magnitude clauses are met on both benches; what remains is the ambient RATE in the Crucible rope/starter cells (and 22 % of player-knock receipts naming no causal actor, which makes that rate an upper bound) |
