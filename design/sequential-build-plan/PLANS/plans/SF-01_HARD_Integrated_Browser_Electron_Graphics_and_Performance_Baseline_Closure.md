...
recommended_agent_profile: "Strong integration agent; vision-capable reviewer recommended for visual evidence"
depends_on: ["SF-00"]
roadmap_alignment: "M0 evidence/baseline, M1 visual family, M2 revalidation, M6 performance/visual/release; R12\u2013R14"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-01 — [HARD][FULLSTACK][FRONTEND-MEDIUM][VISION-RECOMMENDED] Integrated Browser/Electron, Graphics, and Performance Baseline Closure

<agent_assignment>
  <role>Strong integration agent; vision-capable reviewer recommended for visual evidence</role>
...
<mission>
The latest integrated tree has a trustworthy browser/Electron baseline with current route identity, graphics/performance coexistence, clean teardown, and explicitly classified remaining failures.
</mission>
...
<sequence_position>
  <prompt_id>SF-01</prompt_id>
  <declared_dependencies>SF-00</declared_dependencies>
  <roadmap_alignment>M0 evidence/baseline, M1 visual family, M2 revalidation, M6 performance/visual/release; R12–R14</roadmap_alignment>
...
<problem>
The latest branch may have merged graphics and performance work after the program ledgers were written. Without one current combined-tree baseline, later physics and world changes cannot distinguish their regressions from inherited harness drift, stale evidence, renderer overlap, or Electron-specific failure.
</problem>
...
A reliable baseline turns future debugging from séance into engineering. It also prevents agents from “fixing performance” by undoing graphics admission or “fixing visuals” by reintroducing startup hitches and leaks.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Reconcile any stale status language about an isolated performance lane against current `master` history.</deliverable>
  <deliverable>Run current browser and Electron launch/corridor smoke routes on an owned profile/port with clean teardown.</deliverable>
  <deliverable>Measure startup handoff, steady flight, representative station/rock visibility, dense combat or the current representative crowded scenario, autosave completion, and basic memory/resource cleanup.</deliverable>
  <deliverable>Confirm the accepted authored-admission and no-blue-box contracts remain intact after the performance merge.</deliverable>
  <deliverable>Repair only defects that are clearly caused by combined integration and can be closed as one coherent baseline slice; classify unrelated reds.</deliverable>
  <deliverable>Produce hash-bound baseline receipts and representative same-framing browser/Electron captures suitable for later comparisons.</deliverable>
  <deliverable>Update only the appropriate current evidence/checkpoint artifacts through the lead-authorized path; do not claim milestone exit.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>No new gameplay mechanics, asset family overhaul, or broad renderer rewrite.</non_goal>
  <non_goal>No lowering default quality, removing authored visuals, or shrinking representative scenes to pass.</non_goal>
  <non_goal>No mass golden re-record.</non_goal>
  <non_goal>No full release closeout; this establishes a floor for subsequent prompts.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>`design/program/08_GRAPHICS_OVERHAUL_CHECKPOINT.md`, long-term graphics overhaul plan, current performance scripts.</anchor>
  <anchor>Current public corridor pilot, launch policy, asset/live, visual-stability, flight-clean, startup/perf, Electron harnesses.</anchor>
  <anchor>Renderer, parts/material admission, background, save/autosave, AI cadence, economy/world cadence only as measured owners.</anchor>
...
<implementation_direction>
  <direction>Start from the public New Game/Continue route, not injected stable-camera state as the sole proof.</direction>
  <direction>Use current runtime admission/readiness signals, not guessed DOM selectors or fixed sleeps.</direction>
  <direction>Attribute frame/startup costs by measured subsystem, preserving graphics semantics in renderer/parts/background overlap seams.</direction>
  <direction>Capture scenario identity, commit, settings, renderer/GPU route, entity counts, frame windows, console/page errors, resource teardown, and artifact hashes.</direction>
  <direction>Where browser and Electron differ, isolate launcher/profile/driver issues from gameplay code before editing.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>One current browser route and one current Electron route reach flight, show the intended authored starter and representative world composition, and tear down without orphan processes.</criterion>
  <criterion>Combined graphics/performance focused checks remain green or every red has exact attribution and a retained log.</criterion>
  <criterion>Representative performance numbers are measured on the final combined tree, not copied from donor branches.</criterion>
  <criterion>No visible placeholder identity appears before authored entities are interactive.</criterion>
  <criterion>Baseline artifacts identify the exact revision and can be used for same-framing comparisons by later prompts.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>Calling a source-pattern check visual acceptance.</failure>
  <failure>Using a hidden capture route that never exercises public launch.</failure>
  <failure>Claiming parity after testing only browser.</failure>
  <failure>Passing by turning off effects, lowering quality, or reducing scene load.</failure>
  <failure>Overwriting accepted graphics semantics with an older performance branch version.</failure>
...
Task-specific required verification:
  <verification>Run launch-policy, current asset/live/admission/stability checks, deterministic sim compare, focused perf scenarios, and public browser/Electron smoke routes.</verification>
  <verification>Inspect console/page errors, process cleanup, renderer/resource disposal, and artifact manifests.</verification>
  <verification>Use a vision-capable reviewer for captured normal-game frames; a no-vision agent may implement fixes but may not self-sign visual acceptance.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-01.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-01.yaml`.
...
- [ ] Receipt written to `receipts/SF-01.yaml`.
