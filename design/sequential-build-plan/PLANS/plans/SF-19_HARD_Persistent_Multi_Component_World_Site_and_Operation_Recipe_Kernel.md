...
recommended_agent_profile: "Strong architecture/full-stack game agent"
depends_on: ["SF-08", "SF-17", "SF-18"]
roadmap_alignment: "World Site foundation; W07\u2013W17; Depth landmark substrate"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-19 — [HARD][FULLSTACK][FRONTEND-MEDIUM][VISION-RECOMMENDED] Persistent Multi-Component World Site and Operation-Recipe Kernel

<agent_assignment>
  <role>Strong architecture/full-stack game agent</role>
...
<mission>
The engine can instantiate one large site composed of stable components, recipes, dynamic payloads, receivers, persistent states, consequences, and visible before/after transformations without creating a bespoke monolith.
</mission>
...
<sequence_position>
  <prompt_id>SF-19</prompt_id>
  <declared_dependencies>SF-08, SF-17, SF-18</declared_dependencies>
  <roadmap_alignment>World Site foundation; W07–W17; Depth landmark substrate</roadmap_alignment>
...
<problem>
SpaceFace has many names for sites but few embodied operations. Building the Wreck Cathedral or industrial moon directly would encourage one-off scripts, generic entity blobs, and duplicated persistence. The game needs one small, reusable site runtime first.
</problem>
...
A good World Site layer is a grammar, not a content dump. It lets wrecks, refineries, orbital machinery, alien rings, checkpoints, and construction sites share the same proven foundations while retaining authored identity.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Define a versioned site specification: stable site ID, placement/global coordinates, visual root, compound proxies, component descriptors, operation recipes, payload/receiver definitions, state machine/stages, consequences, persistence, discovery, producer/carrier hooks, and debug metadata.</deliverable>
  <deliverable>Implement runtime site instantiation through existing world/entity/place/asset authorities with resolve→prepare→admit semantics and one stable root identity.</deliverable>
  <deliverable>Implement event-sourced or equivalent bounded persistent component/site state with idempotent transitions and safe old-save defaults.</deliverable>
  <deliverable>Allow recipes from SF-18 to target components and transition site stages, spawn/detach payloads, enable/disable proxies/receivers, and emit consequences through current economy/faction/mission/world intents.</deliverable>
  <deliverable>Implement visible state binding: lights, animation, attachments, damaged/stripped/active variants, traffic hook, and map/ledger annotation based on authoritative site state.</deliverable>
  <deliverable>Create a modest reference site with 5–7 components and at least two verbs, one payload, one receiver, one failure/recovery path, and a visible transformed state.</deliverable>
  <deliverable>Add validators for IDs, dependency cycles, unreachable recipes, incompatible proxies, missing assets/sockets, persistence schema, and bounded entities.</deliverable>
  <deliverable>Add producer/carrier/natural-route registration so a site is not merely data.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>No monumental Wreck Cathedral asset/content yet.</non_goal>
  <non_goal>No universal scripting language or editor.</non_goal>
  <non_goal>No full procedural site generator.</non_goal>
  <non_goal>No duplicated mission/economy/save authority.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>World/place/entity registration, site/claim/wreck foundations, SF-08/17/18 primitives.</anchor>
  <anchor>Save schema/migrations, event/intent owners, asset admission/stable transform, map/ledger producers.</anchor>
...
<implementation_direction>
  <direction>Prefer declarative definitions plus small pure transition kernels and runtime adapters.</direction>
  <direction>Keep site state compact and stable; derive render state rather than saving transient Three.js objects.</direction>
  <direction>All state transitions produce deterministic receipts and are idempotent under repeated events.</direction>
  <direction>Assets may stream off-scene but the site cannot become targetable/interactive before matching authored identity/proxies are admitted.</direction>
  <direction>Use explicit site/component ownership and lifecycle cleanup on sector unload/reload.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>Reference site naturally materializes, is visible/solid/targetable according to one spec, and supports at least two distinct operations.</criterion>
  <criterion>Actions produce persistent component and site-stage changes visible after leave/return and save/Continue.</criterion>
  <criterion>One physical payload and receiver path settles once and changes the site.</criterion>
  <criterion>Failure/recovery is possible without softlock.</criterion>
  <criterion>Validator catches malformed IDs, unreachable operation, missing binding, and duplicate receipt cases.</criterion>
  <criterion>Runtime remains bounded and deterministic in repeated load/unload cycles.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>A giant switch statement for one site.</failure>
  <failure>Saving render objects or ephemeral indices.</failure>
  <failure>Site as one entity radius and one global interaction.</failure>
  <failure>Data entry with no natural producer/carrier.</failure>
  <failure>Transition effects only in UI text.</failure>
  <failure>Automatic completion when a timer expires offscreen without matching state/receipt.</failure>
...
Task-specific required verification:
  <verification>Spec/transition/validator tests, idempotence, save/migration, unload/reload, producer reachability.</verification>
  <verification>Browser/Electron reference-site route and visual state comparison.</verification>
  <verification>Perf/memory/entity-lifecycle and asset admission checks.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-19.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-19.yaml`.
...
- [ ] Receipt written to `receipts/SF-19.yaml`.
