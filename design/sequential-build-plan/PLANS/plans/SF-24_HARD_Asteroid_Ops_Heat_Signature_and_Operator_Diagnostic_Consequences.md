...
recommended_agent_profile: "Strong simulation/economy agent plus strong UI/frontend agent"
depends_on: ["SF-23"]
roadmap_alignment: "A07/A09/A11/A12; G08; M4 ecology"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-24 — [HARD][FULLSTACK][FRONTEND-HARD][VISION-YES] Asteroid Ops Heat, Signature, and Operator-Diagnostic Consequences

<agent_assignment>
  <role>Strong simulation/economy agent plus strong UI/frontend agent</role>
...
<mission>
Heat, emissions/signature, contact, power, input, export, and logistics visibly constrain a live asteroid operation; the operator console diagnoses the real bottleneck and exterior space reacts with derating, faults, attention, traffic, and concealment choices.
</mission>
...
<sequence_position>
  <prompt_id>SF-24</prompt_id>
  <declared_dependencies>SF-23</declared_dependencies>
  <roadmap_alignment>A07/A09/A11/A12; G08; M4 ecology</roadmap_alignment>
...
<problem>
Pure thermal, signature, contact, and topology kernels can pass while the player sees only rates and revenue. Hidden losses and generic warnings make automation feel administrative. The game needs causes that are visible, actionable, and connected to the exterior world.
</problem>
...
This gives mining strategic texture and a reason to build systems beyond higher yield. Cooling surfaces, masking, export routes, and machine placement change production, risk, traffic, and hostile attention—turning the asteroid into an operating machine the player understands.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Audit existing A05/A06/A08/A10 kernels, current site production/machines/recipes, Asteroid Ops UI, exterior effects, encounter/signature consumers, save/offline, and economy receipts.</deliverable>
  <deliverable>Wire live thermal consequences: visible derating, fault, shutdown, recoverable damage risk, cooling behavior, and explicit no-hidden-loss receipts.</deliverable>
  <deliverable>Wire signature consequences through existing sensors/encounter/economy/faction systems: discovery/attention/contract or concealment choices with telegraph and lawful recovery.</deliverable>
  <deliverable>Build a pure operator diagnostic presenter that ranks actual bottlenecks among contact, power, input, export, heat, signature, capacity, and topology, including stale/unknown data.</deliverable>
  <deliverable>Implement UI with progressive disclosure: primary constraint and next action first, detailed graph/flows/receipts on inspection; keyboard/gamepad/trackpad accessible.</deliverable>
  <deliverable>Exteriorize consequences: heat plume/radiators, machine state/lights, traffic/export, scanner signature, incoming interest or masking, without requiring the console open.</deliverable>
  <deliverable>Calibrate the six-machine/recipe starter loop or current equivalent for at least one viable nontrivial formation.</deliverable>
  <deliverable>Preserve deterministic/offline caps and exact granted-versus-gross receipts.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>No full cluster/station construction yet.</non_goal>
  <non_goal>No per-item logistics simulation.</non_goal>
  <non_goal>No upkeep meter spam or constant decay while away.</non_goal>
  <non_goal>No generic red warning panel disconnected from source.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>A05/A06/A08/A10 kernels and tests, site production/machine definitions, encounter/sensor/economy ports.</anchor>
  <anchor>Asteroid Ops UI/presenter patterns, exterior site bindings, save/offline receipts.</anchor>
...
<implementation_direction>
  <direction>Keep pure kernels authoritative and adapt live snapshots into them; do not duplicate equations in UI.</direction>
  <direction>State transitions use hysteresis and explicit recovery thresholds.</direction>
  <direction>Signature emits intents/events; encounter/faction owners decide consequences.</direction>
  <direction>Diagnostics are cause-ordered and explain evidence, not prescribe a fixed build.</direction>
  <direction>Exterior VFX reflects actual heat/signature values with bounded LOD/pooling.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>Player can intentionally cause and recover from heat derating/fault/shutdown and see matching exterior behavior.</criterion>
  <criterion>Increasing/masking signature produces deterministic, telegraphed world consequences through current owners.</criterion>
  <criterion>Diagnostic identifies the true active bottleneck across representative fixtures and never blames a downstream symptom first without explanation.</criterion>
  <criterion>Console is usable without clutter at normal scale and critical state is visible outside it.</criterion>
  <criterion>Save/Continue/offline progression preserves topology, caps, states, and receipts.</criterion>
  <criterion>Browser/Electron operation route, accessibility, visual acceptance, and performance pass.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>Heat as a number that only lowers output.</failure>
  <failure>Signature as random enemy spawn.</failure>
  <failure>Diagnostic hard-coded to one fixture.</failure>
  <failure>UI recomputing different laws from simulation.</failure>
  <failure>Passive income while hidden constraints silently delete output.</failure>
  <failure>Exterior effects unrelated to actual state.</failure>
...
Task-specific required verification:
  <verification>Thermal/signature/logistics integration, transitions/hysteresis, diagnostic ordering, encounter intent, save/offline tests.</verification>
  <verification>Public operation/fault/recovery/attention route and current captures.</verification>
  <verification>UI/a11y/perf and VFX lifecycle/visual review.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-24.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-24.yaml`.
...
- [ ] Receipt written to `receipts/SF-24.yaml`.
