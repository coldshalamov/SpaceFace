...
recommended_agent_profile: "Top-tier full-stack/asset/frontend game agent"
depends_on: ["SF-19", "SF-24"]
roadmap_alignment: "A13\u2013A16/W17/M5 ownership; transforming industrial claim"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-25 — [HARD][FULLSTACK][FRONTEND-HARD][VISION-YES] Transforming Industrial Claim and Visible Outpost Assembly

<agent_assignment>
  <role>Top-tier full-stack/asset/frontend game agent</role>
...
<mission>
One abandoned industrial claim begins cold and broken, is repaired and supplied through physical/site operations, then visibly grows into an operating depot/refinery with modules, storage, drones/traffic, output, failure/recovery, and strategic use.
</mission>
...
<sequence_position>
  <prompt_id>SF-25</prompt_id>
  <declared_dependencies>SF-19, SF-24</declared_dependencies>
  <roadmap_alignment>A13–A16/W17/M5 ownership; transforming industrial claim</roadmap_alignment>
...
<problem>
Existing claim designs may describe depots, refineries, relays, and defense but present as small rocks or barges and collapse progression into menus/upkeep numbers. Building more station screens would split UI without creating new play.
</problem>
...
This proves the long-term game: the world remembers the player’s industry. Dark structures light up, conveyors move, modules attach, storage fills, drones launch, haulers arrive, and a previously dead pocket becomes an economy.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Select one existing industrial claim/body and migrate it onto the World Site/components/recipes substrate while preserving IDs/progression where possible.</deliverable>
  <deliverable>Author a modular industrial silhouette sized and framed to read as infrastructure, with compound proxies, approach/docking/receiver geometry, LOD/HLOD, PBR roles, sockets, and stage variants.</deliverable>
  <deliverable>Implement stages such as discovered/abandoned, stabilized, depot online, refinery attached, drone/export online, defended/connected; exact count may adapt to current systems.</deliverable>
  <deliverable>Require repair operations, starter materials, physical delivery/receiver, topology/power/heat/signature constraints, and current claim/ownership/economy intents.</deliverable>
  <deliverable>Show stored input/output and stage growth physically through containers/modules/lights/conveyors/traffic, using aggregate state rather than per-item simulation.</deliverable>
  <deliverable>Spawn/assign miner/hauler/drone jobs from actual site policies and inventory.</deliverable>
  <deliverable>Implement fault/raid/depletion/severed route/abandonment recovery without galactic whack-a-mole or constant decay.</deliverable>
  <deliverable>Persist construction, modules, inventory aggregates, policies, traffic linkage, and visuals across leave/return/Continue/offline caps.</deliverable>
  <deliverable>Provide one strategic benefit beyond credits—fabrication, route support, drone range, sensor/market knowledge, repair, or physics infrastructure prerequisite.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>No generic base-building editor.</non_goal>
  <non_goal>No all-claims conversion.</non_goal>
  <non_goal>No per-minute upkeep drain as the main challenge.</non_goal>
  <non_goal>No station UI clone for every module.</non_goal>
  <non_goal>No instant completed model after one timer.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>Current claim/body/outpost systems, A13–A16/W17 plans, World Site and Asteroid Ops consequences.</anchor>
  <anchor>NPC jobs, cargo/economy/ownership/save, modular asset/PBR/LOD pipeline.</anchor>
...
<implementation_direction>
  <direction>Use authored stage graph and stable module/socket IDs; construction consumes exact receipts and produces visible intermediate states.</direction>
  <direction>Site state remains authoritative; render derives stage/modules/storage cues.</direction>
  <direction>Jobs use SF-15 controller and absent resolution; inventory/economy remain current owners.</direction>
  <direction>Failure events are exceptional, telegraphed, and recoverable; lack of input idles production rather than decaying everything.</direction>
  <direction>Use physical receivers for major construction deliveries where practical.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>Site visibly transforms through at least four meaningful stages and each stage changes available capability/activity.</criterion>
  <criterion>Player actions/materials/operations correspond to exact receipts and no duplicate funding.</criterion>
  <criterion>Modules/proxies/visuals remain aligned and persist through Continue.</criterion>
  <criterion>Input/output/traffic are visibly connected to real production state.</criterion>
  <criterion>At least one fault/failure can be repaired without softlock or repeated punishment.</criterion>
  <criterion>Site grants a strategic non-credit capability used by a later prompt.</criterion>
  <criterion>Browser/Electron, asset, performance, save/offline, and visual acceptance pass.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>Rock placeholder labeled refinery.</failure>
  <failure>One menu purchase swapping directly to final asset.</failure>
  <failure>Visual growth unrelated to site state.</failure>
  <failure>Passive credit number as sole payoff.</failure>
  <failure>Constant upkeep/decay meter.</failure>
  <failure>Fake drone particles with no job/inventory relationship.</failure>
...
Task-specific required verification:
  <verification>Stage/prerequisite/receipt/topology/production/failure/save/offline tests.</verification>
  <verification>Public discovery→repair→delivery→production→fault/recovery route, possibly using a fixture save for later stages as supporting—not sole—evidence.</verification>
  <verification>Same-framing stage captures, traffic motion, asset/proxy validation, performance profile.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-25.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-25.yaml`.
...
- [ ] Receipt written to `receipts/SF-25.yaml`.
