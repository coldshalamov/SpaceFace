...
recommended_agent_profile: "Top-tier full-stack game agent with strong asset/Three.js/vision capability"
depends_on: ["SF-19"]
roadmap_alignment: "Depth H1a, W15 unique wreck route, R1/R2 wreck foundations"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-20 — [HARD][FULLSTACK][FRONTEND-HARD][VISION-YES] Wreck Cathedral Monumental Site Vertical Slice

<agent_assignment>
  <role>Top-tier full-stack game agent with strong asset/Three.js/vision capability</role>
...
<mission>
A 300–600 world-unit wreck complex is a navigable, solid, multi-component place with scanning, cutting, stabilization/repair, Massline extraction, receiver delivery, short environmental story, unique reward, and a persistent stripped aftermath.
</mission>
...
<sequence_position>
  <prompt_id>SF-20</prompt_id>
  <declared_dependencies>SF-19</declared_dependencies>
  <roadmap_alignment>Depth H1a, W15 unique wreck route, R1/R2 wreck foundations</roadmap_alignment>
...
<problem>
The repository’s authored wreck data can contain rumor, provenance, hazards, unique hardware, and consequences, yet ordinary runtime wrecks can collapse into a radius-nine salvage blob. Building another pretty mesh without physical truth would repeat the disease at a larger scale.
</problem>
...
This is the best proof that SpaceFace can turn excellent paper architecture into an experience. The player does not “salvage a wreck”; they learn its silhouette, enter its channels, identify powered compartments, cut a module free, haul it, and leave behind a changed skeleton the world remembers.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Select or author one unique wreck identity and integrate existing rumor→bearing→scan→decision foundations where compatible.</deliverable>
  <deliverable>Create a monumental top-down silhouette with several recognizable sections, deliberate navigable channels, multiple collision islands, distant lighting, LOD/HLOD, stable transforms, and professional PBR/material storytelling.</deliverable>
  <deliverable>Define at least five independently targetable components corresponding to visible geometry: e.g. relay, brace/clamp, emergency-power node, reactor/weapon assembly, black box/cargo module.</deliverable>
  <deliverable>Implement a staged operation sequence using SF-18/19: scan/reveal; repair/energize or stabilize; cut/sever; detach/tether/tow; receiver or cargo recovery.</deliverable>
  <deliverable>Include at least two optional approaches or ordering choices without permanently deleting most content.</deliverable>
  <deliverable>Add hazards/counterplay grounded in components/environment, not generic damage circles.</deliverable>
  <deliverable>Deliver five or more concise ambient/inspection fragments and persistent Ship’s Ledger entries; no dialogue gate.</deliverable>
  <deliverable>Implement unique blueprint/module/provenance and follow-up consequences through existing owners.</deliverable>
  <deliverable>Persist stripped/repaired/destroyed components and altered silhouette/state after revisit and Continue.</deliverable>
  <deliverable>Add normal rumor/discovery route, success/failure/recovery evidence, asset classification/provenance, and performance receipts.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>No first-person interior, walking, cinematic cutscene, or full modular destruction.</non_goal>
  <non_goal>No twelve-wreck content replication.</non_goal>
  <non_goal>No orange sphere with boxes, asteroid stand-in, or central collider.</non_goal>
  <non_goal>No global RMB salvage timer.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>World Site kernel, existing unique-wreck data/runtime, rumor/ledger/salvage/provenance systems.</anchor>
  <anchor>Graphics foundry, compound collision, component targeting, contextual operations, payloads/receivers.</anchor>
...
<implementation_direction>
  <direction>Use modular authored hull sections and props bound to stable site components/proxies; detachable pieces may be separate bodies from the start.</direction>
  <direction>Author collision/interaction envelopes with the asset, validate at export/admission, and keep channels readable at top-down zoom.</direction>
  <direction>Use inspection fragments triggered by physical investigation, not a click-through sequence.</direction>
  <direction>Persist state as component/stage records and derive the stripped visual composition.</direction>
  <direction>Use current unique-wreck legality/provenance/story systems rather than shadow loot state.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>Wreck is recognizable before interaction range and cannot be mistaken for a generic asteroid/entity.</criterion>
  <criterion>Solid hull and open channels match visible geometry; player can intentionally traverse channels and cannot ghost through solid sections.</criterion>
  <criterion>At least five components can be selected and have distinct states/actions.</criterion>
  <criterion>One module becomes a real detachable payload and must be moved/recovered physically.</criterion>
  <criterion>Revisit/save/Continue shows a stripped/transformed wreck, not reset content.</criterion>
  <criterion>Rumor→bearing→scan→operation→recovery is reachable without injected state as primary proof.</criterion>
  <criterion>Browser/Electron motion, gameplay-scale visual acceptance, accessibility, asset identity, and performance pass.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>One giant mesh plus one interaction point.</failure>
  <failure>Unrelated central collider.</failure>
  <failure>All components invisible hotspots.</failure>
  <failure>Reward granted after holding RMB anywhere on the wreck.</failure>
  <failure>Story only in a long modal panel.</failure>
  <failure>State resets when sector unloads.</failure>
  <failure>Asset turntable used as acceptance instead of normal camera.</failure>
...
Task-specific required verification:
  <verification>Site spec/component/persistence/unique-loot/provenance tests and route reachability.</verification>
  <verification>Collision/channel/detach/receiver/mission/save failure matrices.</verification>
  <verification>Normal browser/Electron approach, operation, aftermath, and Continue captures/videos reviewed by vision agent.</verification>
  <verification>Asset/LOD/material/proxy validation, performance/memory/cleanup, reduced settings.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-20.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-20.yaml`.
...
- [ ] Receipt written to `receipts/SF-20.yaml`.
