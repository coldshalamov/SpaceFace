...
recommended_agent_profile: "Strong input/systems agent; frontend review recommended"
depends_on: ["SF-03"]
roadmap_alignment: "T04/T06/T07/T16; R07; prerequisite for route slings and specialized heads"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-04 — [HARD][FULLSTACK][FRONTEND-MEDIUM][VISION-RECOMMENDED] Massline Input Grammar, Buffered Intent, Reel, Pay-Out, and Cut

<agent_assignment>
  <role>Strong input/systems agent; frontend review recommended</role>
...
<mission>
One rebindable thumb-friendly Massline action supports predictable latch, cut, line-control mode, reel-in, pay-out, orbit direction, and release without frame-perfect chords or key conflicts.
</mission>
...
<sequence_position>
  <prompt_id>SF-04</prompt_id>
  <declared_dependencies>SF-03</declared_dependencies>
  <roadmap_alignment>T04/T06/T07/T16; R07; prerequisite for route slings and specialized heads</roadmap_alignment>
...
<problem>
The current F-based grammar overloads latch, hold-to-reel, and tap-to-cut, while steering and boost occupy nearby fingers. There is no complete pay-out verb. The player’s intended combined gesture can be split across ticks and misread, and adding new heads would create key hell.
</problem>
...
A physics game lives or dies in the hands. A clean grammar turns complex maneuvers into learnable phrases: point/turn, latch, hold, reel or pay out, pump, release. The system can remain deep while the keys remain few.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Audit current action edges/levels, rebind UI, Space brake/fire history, gamepad mappings, modal focus fences, tap/hold logic, and any active conflicts.</deliverable>
  <deliverable>Define and implement one central Massline action with configurable default chosen from current ergonomics; retain F as an alias or migration-safe secondary when feasible.</deliverable>
  <deliverable>Buffer 150–250 ms of relevant steering/turn intent around Massline activation so `turn then press` and `press then turn` express one gesture.</deliverable>
  <deliverable>While attached, define a line-control mode with unambiguous axes: reel-in, pay-out, desired orbit direction, optional boost pump, and cut/release. State the reference frame for each.</deliverable>
  <deliverable>Implement pay-out through the existing attachment authority with min/max lengths, rate limits, load policy, deterministic behavior, and clear denial reasons.</deliverable>
  <deliverable>Preserve immediate emergency cut and manual flight authority; avoid a modal state that traps controls.</deliverable>
  <deliverable>Update settings/help/control presentation and migrations without breaking existing bindings.</deliverable>
  <deliverable>Add input telemetry and contract tests for keyboard, arrows, mouse/trackpad, gamepad, rebinding, pause/modal transitions, and save/restore.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>Do not implement the final orbit controller or specialized spools.</non_goal>
  <non_goal>Do not repurpose critical UI keys without conflict analysis and migration.</non_goal>
  <non_goal>Do not require simultaneous D+F precision.</non_goal>
  <non_goal>Do not create separate keys for every future Massline variant.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>`src/systems/input.js`, gamepad/touch input, settings/control rebind owners, `tetherGameplay`, attachment service.</anchor>
  <anchor>Current input contract and nested `src/systems`, `src/ui`, `styles`, and test instructions.</anchor>
...
<implementation_direction>
  <direction>Separate edge-triggered actions (`latch`, `cut/tap`, `mode toggle`) from level axes (`reel`, `payOut`, `orbitDirection`, `pump`).</direction>
  <direction>Resolve tap/hold with explicit state machine and timestamps on simulation/input sampling semantics, not ad hoc timeout races.</direction>
  <direction>Publish a normalized Massline command packet consumed by tether gameplay; raw device bindings remain input-owner concerns.</direction>
  <direction>Use named denial/status reasons and one-shot user feedback rather than silent no-ops.</direction>
  <direction>Ensure Space or any proposed thumb key retains its existing action through context or deliberate remap; document conflict resolution.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>A player can latch, pay out, reel in, establish clockwise/counterclockwise intent, pump with boost, and cut using one coherent grammar.</criterion>
  <criterion>Turn intent immediately before/after latch is recognized without exact chord timing.</criterion>
  <criterion>Pay-out changes authoritative rest length and survives deterministic replay; it cannot exceed max length or bypass load policy.</criterion>
  <criterion>Emergency cut works within one tick of the normalized action.</criterion>
  <criterion>Rebindings persist and all critical actions remain reachable on keyboard/mouse/trackpad and gamepad.</criterion>
  <criterion>Normal UI text/help accurately reflects the live mapping.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>Hard-coding a new key only in one consumer.</failure>
  <failure>Using wall-clock timers that diverge from replay.</failure>
  <failure>Implementing pay-out by moving the target or player.</failure>
  <failure>A sticky mode that continues steering/reeling after focus loss or menu open.</failure>
  <failure>Changing Space/F behavior without migration, help, and accessibility proof.</failure>
...
Task-specific required verification:
  <verification>Focused input/rebind/tap-hold state-machine tests and deterministic event sequences.</verification>
  <verification>Attachment authority tests for positive and negative reel deltas, limits, load, save/reload.</verification>
  <verification>Public control route in the lab and one ordinary flight route.</verification>
  <verification>UI/a11y checks and current control-screen capture.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-04.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-04.yaml`.
...
- [ ] Receipt written to `receipts/SF-04.yaml`.
