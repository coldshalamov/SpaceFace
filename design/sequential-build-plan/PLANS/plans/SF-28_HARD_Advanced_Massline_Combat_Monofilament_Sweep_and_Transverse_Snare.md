...
recommended_agent_profile: "Strong physics/combat agent plus strong VFX/vision agent"
depends_on: ["SF-09", "SF-17", "SF-27"]
roadmap_alignment: "T08/T13/T14 advanced combat; specialized line progression"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-28 — [HARD][FULLSTACK][FRONTEND-HARD][VISION-YES] Advanced Massline Combat: Monofilament Sweep and Transverse Snare

<agent_assignment>
  <role>Strong physics/combat agent plus strong VFX/vision agent</role>
...
<mission>
The line itself becomes an attack and area-control surface: high-tension sweep can cut light craft/components, while a deployed transverse snare catches crossing targets under explicit eligibility and counterplay.
</mission>
...
<sequence_position>
  <prompt_id>SF-28</prompt_id>
  <declared_dependencies>SF-09, SF-17, SF-27</declared_dependencies>
  <roadmap_alignment>T08/T13/T14 advanced combat; specialized line progression</roadmap_alignment>
...
<problem>
Tethering a target and then shooting it is only one extra step. To make the Massline a true combat language, positioning the line through space must matter. But naive line damage can become invisible, overpowered, collision-expensive, and confusing.
</problem>
...
Monofilament turns flight path into offense; snare turns route prediction into area denial. Both reward geometry rather than aim-lock DPS and create memorable emergent moments without requiring a second aiming device.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Define shared swept-line collision/query primitive with segment history, thickness, relative crossing speed, tension/load, target eligibility, cooldown, obstruction, and bounded candidate query.</deliverable>
  <deliverable>Implement Monofilament head/profile: line becomes damaging only above declared tension and relative sweep speed; supports light craft, drones, loose objects, and authored weak components; armored hulls resist.</deliverable>
  <deliverable>Route line hits through component/health/stagger/physics owners with deterministic receipts and no repeated per-tick damage.</deliverable>
  <deliverable>Implement Transverse Snare deployable: launch two endpoints or one authored anchor pair, arm a visible line across space, catch eligible crossing craft/payloads, apply bounded attachment/slow/tumble policy, expire/break/destroy, and enforce caps.</deliverable>
  <deliverable>Integrate selection/deployment grammar without creating ambiguous multi-target controls; use cursor/preview and explicit confirmation state.</deliverable>
  <deliverable>Add AI telegraph/avoid/cut response and friendly/station law policy.</deliverable>
  <deliverable>Create one combat encounter with line-sweep opportunities and one route/interdiction scenario for snare.</deliverable>
  <deliverable>Build bright, thin, antialiased/pooling-aware cable/ribbon VFX with tension/readiness and reduced-flash alternatives; ensure black-space and clutter readability.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>No Twin Bridle or drag net.</non_goal>
  <non_goal>No cutting arbitrary station meshes.</non_goal>
  <non_goal>No lethal line at low tension or while slack.</non_goal>
  <non_goal>No invisible infinite plane trap.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>SF-09 impulse/collision, SF-17 components, attachment line/cable render and spatial query.</anchor>
  <anchor>Mine/deployable lifecycle, tactical AI and protected/friendly policies.</anchor>
...
<implementation_direction>
  <direction>Sweep query runs on current and previous authoritative line segments and spatially filters candidates.</direction>
  <direction>Damage scales from bounded tension/relative normal crossing speed and component policy, not just overlap.</direction>
  <direction>Use per-target cooldown/contact episode to prevent sawmill tick damage.</direction>
  <direction>Snare endpoints and armed segment are physical/world-owned entities with stable IDs and cleanup.</direction>
  <direction>Line break/obstruction/expiry follow attachment authority and produce readable states.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>Player can intentionally sweep a light enemy/component by orbit/positioning; a slack or slowly moving line does no damage.</criterion>
  <criterion>Same target cannot receive unbounded repeated hits during one crossing.</criterion>
  <criterion>Heavy/armored targets resist except authored weak points.</criterion>
  <criterion>Snare deployment is previewed, bounded, avoidable/counterable, and catches only crossing eligible targets.</criterion>
  <criterion>AI responds imperfectly but legibly; law/friendly policies work.</criterion>
  <criterion>Browser/Electron, dense query performance, reduced settings, save policy, and visual acceptance pass.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>Damage whenever target is near the line, regardless of motion/tension.</failure>
  <failure>Raycast against every entity every frame.</failure>
  <failure>Snare as invisible radius slow.</failure>
  <failure>Cutting visual components with no state/damage owner.</failure>
  <failure>Neon line/bloom so thick it hides positioning.</failure>
...
Task-specific required verification:
  <verification>Swept-segment geometry/property tests, crossing directions/speeds, cooldown, obstruction, component policy.</verification>
  <verification>Attachment/combat trace/AI/save/lifecycle and crowded performance.</verification>
  <verification>Public sweep/snare success/failure routes and vision-reviewed motion captures.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-28.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-28.yaml`.
...
- [ ] Receipt written to `receipts/SF-28.yaml`.
