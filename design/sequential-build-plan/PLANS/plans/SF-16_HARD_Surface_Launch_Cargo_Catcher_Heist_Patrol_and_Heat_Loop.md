...
recommended_agent_profile: "Strong world/full-stack agent plus vision-capable frontend/VFX agent"
depends_on: ["SF-14", "SF-15"]
roadmap_alignment: "GTA-in-space crime loop, planetary operations, W11/W16, sector postcard"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-16 — [HARD][WORLD][FRONTEND-HARD][VISION-YES] Surface-Launch Cargo, Catcher, Heist, Patrol, and Heat Loop

<agent_assignment>
  <role>Strong world/full-stack agent plus vision-capable frontend/VFX agent</role>
...
<mission>
A scheduled planetary mass driver launches a physical cargo capsule toward an orbital catcher; the player may escort it, intercept and steal it, or witness lawful traffic, triggering patrol pursuit, heat, escape, laundering/delivery, and persistent consequences.
</mission>
...
<sequence_position>
  <prompt_id>SF-16</prompt_id>
  <declared_dependencies>SF-14, SF-15</declared_dependencies>
  <roadmap_alignment>GTA-in-space crime loop, planetary operations, W11/W16, sector postcard</roadmap_alignment>
...
<problem>
Trade, crime, and missions can collapse into station menus and inventory deltas. The user wants a world where valuable things physically move and can be robbed, defended, redirected, or lost. A heist loop must reuse real cargo/heat/faction/job systems rather than script a one-off cinematic.
</problem>
...
This is the “GTA in space” proof. The player sees an opportunity, chooses crime in the world, wrestles with momentum and pursuit, and deals with consequences. It also gives NPC jobs and the planet something meaningful to do when the player is not the center of the universe.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Author one planet-side mass-driver emitter, launch schedule, trajectory, orbital catcher/receiver, cargo capsule type, manifest, lawful owner, destination, and Atlas/mission/ledger hooks.</deliverable>
  <deliverable>Implement deterministic launch/catcher state machine and physical capsule motion through authority/prediction; no cutscene teleport.</deliverable>
  <deliverable>Integrate hauler/patrol jobs around the route and one legal escort or interception contract.</deliverable>
  <deliverable>Implement player theft: target/tether/frame-couple/redirect or collect according to current primitives, transfer cargo once, mark provenance/contraband, emit witnesses and heat through existing owners.</deliverable>
  <deliverable>Implement pursuit/escape boundary, patrol response, loss/destruction outcomes, lawful recovery, and laundering/sale/delivery path through existing services.</deliverable>
  <deliverable>Support absent resolution when the player misses the launch while keeping witnessed physical truth consistent.</deliverable>
  <deliverable>Present schedule, ownership, trajectory, catcher state, theft warning, pursuit, and consequences without giant modal UI.</deliverable>
  <deliverable>Add polished launch plume, capsule trail, catcher capture, theft feedback, and pursuit cues with bounded VFX.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>No city/surface gameplay, elaborate dialogue heist, or generic crime system rewrite.</non_goal>
  <non_goal>No arbitrary cargo spawning directly into inventory.</non_goal>
  <non_goal>No permanent lockout of lawful play.</non_goal>
  <non_goal>No one-time scripted mission only; the loop should be reusable after the vertical slice.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>Planet slice, NPC jobs, cargo/contraband/heat/faction/mission authorities, receivers and Massline payload handling.</anchor>
  <anchor>Atlas schedule/place registration and current activity/ledger presentation.</anchor>
...
<implementation_direction>
  <direction>Use a producer→physical carrier→receiver→settlement contract with stable IDs and idempotent receipts.</direction>
  <direction>Trajectory may be authored/stylized but uses the live body/field/physics path and can be intercepted.</direction>
  <direction>Heat/witness/faction/mission systems receive intents/events; they remain single writers.</direction>
  <direction>Capsule destruction, missed catch, theft, lawful arrival, and offscreen resolution are terminal states in one machine.</direction>
  <direction>Use job controller for patrol/escort and route schedule rather than special-case AI.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>Player can watch a lawful launch arrive and settle once.</criterion>
  <criterion>Player can intercept/steal through normal controls, receive physical cargo/provenance, and trigger readable heat/patrol response.</criterion>
  <criterion>Escape, capture/failure, sale/launder/delivery, and lawful recovery routes are all explicit and reproducible.</criterion>
  <criterion>Missing the event resolves consistently without duplicating capsule or cargo when returning.</criterion>
  <criterion>Patrol job resumes/aborts coherently and does not chase forever across the galaxy.</criterion>
  <criterion>Browser/Electron, save/Continue, Atlas/map, performance, and visual acceptance pass.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>Press E at a launcher to receive cargo.</failure>
  <failure>Capsule is a UI icon or particle with no authoritative body.</failure>
  <failure>Heat set directly by the heist script.</failure>
  <failure>Patrols spawned only after theft with no route/job identity.</failure>
  <failure>Offscreen timer grants both lawful delivery and stolen cargo.</failure>
...
Task-specific required verification:
  <verification>State-machine/idempotence, trajectory/catcher, cargo/provenance, witness/heat intent, job interrupt, save/offline tests.</verification>
  <verification>Legal and criminal public routes with current captures and receipts.</verification>
  <verification>Vision review of launch/intercept/capture/pursuit; entity/VFX/perf bounds.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-16.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-16.yaml`.
...
- [ ] Receipt written to `receipts/SF-16.yaml`.
