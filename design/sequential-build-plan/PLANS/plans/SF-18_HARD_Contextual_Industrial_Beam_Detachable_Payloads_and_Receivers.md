...
recommended_agent_profile: "Strong gameplay/full-stack agent plus vision-capable VFX/UI agent"
depends_on: ["SF-09", "SF-17"]
roadmap_alignment: "World-site interaction grammar, T12, W15, Asteroid Ops exteriorization"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-18 — [HARD][FULLSTACK][FRONTEND-HARD][VISION-YES] Contextual Industrial Beam, Detachable Payloads, and Receivers

<agent_assignment>
  <role>Strong gameplay/full-stack agent plus vision-capable VFX/UI agent</role>
...
<mission>
The existing RMB industrial/mining beam becomes a contextual tool that can mine, cut, stabilize, repair, or transfer at authored components, while at least one operation releases a real detachable payload that must be tethered or delivered to a receiver.
</mission>
...
<sequence_position>
  <prompt_id>SF-18</prompt_id>
  <declared_dependencies>SF-09, SF-17</declared_dependencies>
  <roadmap_alignment>World-site interaction grammar, T12, W15, Asteroid Ops exteriorization</roadmap_alignment>
...
<problem>
RMB currently risks meaning “auto-target nearest thing and increment resource.” Merely changing the toast for a wreck brace would add labels, not gameplay. The beam needs component context, operation state, physical consequences, and clear differentiation—without turning into a modal minigame or a dozen new keys.
</problem>
...
One beam can support many operations if the world supplies meaningful components: expose ore, sever braces, weld relays, purge heat, restore power, or charge receivers. Detachable payloads then connect the beam to Massline, cargo, missions, industry, and crime.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Audit mining beam targeting, energy/heat, asteroid yield, drill/mining minigame, salvage, repair, damage, effects, cargo settlement, and component selection.</deliverable>
  <deliverable>Define data-driven contextual operation recipes with required tool mode/component state, acquisition/hold/timing pattern, energy/heat/resource costs, progress semantics, interrupt/reset policy, state transition, physical spawn/detach, events/receipts, and failure reasons.</deliverable>
  <deliverable>Implement a small operation vocabulary using the existing beam input: `mine/extract`, `cut/sever`, `repair/energize`, and one receiver transfer/charge operation. Only expose operations supported by the selected component.</deliverable>
  <deliverable>Require component selection/preview and render distinct beam contact behavior based on operation; do not silently auto-target a different component.</deliverable>
  <deliverable>Implement a dynamic detachable payload with authoritative mass/response, visual identity, ownership/provenance, tetherability, collision policy, lifecycle, save behavior, and one-time cargo/economy settlement.</deliverable>
  <deliverable>Implement a physical receiver/delivery volume with accepted payload types, alignment/speed or capture policy, rejection reasons, state change, and idempotent receipt.</deliverable>
  <deliverable>Create one compact fixture: cut a clamp/brace, release a module/cargo pod, tether it, deliver it to a receiver, and observe a persistent component/site state change.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>No full industrial site or Wreck Cathedral.</non_goal>
  <non_goal>No universal tool wheel or new modal UI.</non_goal>
  <non_goal>No per-pixel cutting or destructible mesh simulation.</non_goal>
  <non_goal>No reward settlement before the physical payload/receiver step when the recipe claims one.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>Current mining beam/mining/drill systems, component selection SF-17, Massline payload/tow handling.</anchor>
  <anchor>Cargo/economy/mission intents, save owner, current beam/VFX profiles and receiver patterns.</anchor>
...
<implementation_direction>
  <direction>Separate beam acquisition/contact from operation recipe execution; recipes are plain data/state machines.</direction>
  <direction>Progress may be continuous but must react to contact continuity, angle/range, heat/energy, component state, and interruptions—not one global timer.</direction>
  <direction>Cutting changes component state and detaches/spawns through a stable authored transform/socket.</direction>
  <direction>Payload pickup/delivery uses cargo/economy intents and stable receipts; prevent double settlement after save/reload.</direction>
  <direction>Beam VFX uses pooled ribbons/contact mesh/sparks/material response with operation-specific shape and restrained bloom.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>Selected component clearly previews the correct beam operation and denial reason.</criterion>
  <criterion>Mining and cutting/repairing look and behave differently at gameplay zoom.</criterion>
  <criterion>Cutting the fixture changes the component/site, creates one physical payload, and preserves that outcome across save/Continue.</criterion>
  <criterion>Payload can be tethered, collides/behaves according to policy, and receiver capture settles exactly once.</criterion>
  <criterion>Breaking contact, overheating, losing selection, or destroying payload follows explicit recovery/failure behavior.</criterion>
  <criterion>Browser/Electron, accessibility, VFX lifecycle, performance, and normal-route evidence pass.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>Same beam plus different text toast.</failure>
  <failure>One global progress bar disconnected from contact/component state.</failure>
  <failure>Payload represented only as inventory after completion.</failure>
  <failure>Receiver as press-E radius with no physical object.</failure>
  <failure>Per-operation bespoke system bypassing shared recipes.</failure>
  <failure>Beam effect as two translucent cylinders that pulse larger.</failure>
...
Task-specific required verification:
  <verification>Recipe/state tests, contact interruption, costs, detach/socket identity, payload/receiver idempotence, save/reload.</verification>
  <verification>Physics/cargo/economy authority checks and sim compare.</verification>
  <verification>Browser/Electron fixture with vision review of contact/readability and reduced effects.</verification>
  <verification>Pool/allocation/profile checks.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-18.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-18.yaml`.
...
- [ ] Receipt written to `receipts/SF-18.yaml`.
