...
recommended_agent_profile: "Strong backend/world-systems agent; vision review recommended for legibility"
depends_on: ["SF-01", "SF-09"]
roadmap_alignment: "M4 ecology/families, W06\u2013W10, living-world foundations"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-15 — [HARD][WORLD][FRONTEND-MEDIUM][VISION-RECOMMENDED] Generic NPC Job Controller with Miner, Hauler, and Patrol Loops

<agent_assignment>
  <role>Strong backend/world-systems agent; vision review recommended for legibility</role>
...
<mission>
NPC purpose is visible in motion: miners work resource sites, haulers carry output along routes, and patrols traverse/check infrastructure, with combat interrupts, recovery, persistence, and absent-resolution consistency.
</mission>
...
<sequence_position>
  <prompt_id>SF-15</prompt_id>
  <declared_dependencies>SF-01, SF-09</declared_dependencies>
  <roadmap_alignment>M4 ecology/families, W06–W10, living-world foundations</roadmap_alignment>
...
<problem>
NPCs can have different labels or combat archetypes while still wandering identically. Existing regional ecology may adjust weights without spawning activity, and fleet mining may map to a generic defensive trader because no mining job exists. The world therefore feels staged around the player rather than alive independently.
</problem>
...
Simple job state machines generate far more life than cleverer dogfighting in empty space. A miner leaving a hub, working a field, transferring cargo, and feeding a hauler tells a story without dialogue and creates opportunities for escort, piracy, salvage, and economic observation.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Audit current tactical AI intent ports, traffic/ecology/encounter producers, economy/cargo abstractions, waypoint/autopilot, save/offline, and entity materialization rules.</deliverable>
  <deliverable>Implement a generic hierarchical job controller with stable job/step IDs, route plan, deadlines/timeouts, progress receipts, stuck detection/recovery, interrupts, resume/abort policy, and witnessed versus absent resolution.</deliverable>
  <deliverable>Implement Miner loop: acquire assignment, travel, approach field/site, visibly work/collect, transfer output, return/idle.</deliverable>
  <deliverable>Implement Hauler loop: acquire cargo contract, load/manifest, travel route, unload/settle, reroute/recover; physical cargo representation may be aggregate but must have authoritative receipts.</deliverable>
  <deliverable>Implement Patrol loop: follow lane/checkpoints, inspect/observe, respond to threat, pursue within bounds, return/resume.</deliverable>
  <deliverable>Ensure tactical combat AI is an interrupt, not a replacement job label; define flee/destroyed/stuck/lost-route behavior.</deliverable>
  <deliverable>Connect job outputs to current economy/traffic/encounter signals without creating a second credit/cargo writer.</deliverable>
  <deliverable>Add visible state cues through behavior, routes, docking/working VFX, and restrained scanner/ledger information.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>No universal GOAP/behavior tree rewrite.</non_goal>
  <non_goal>No per-item commodity simulation across the galaxy.</non_goal>
  <non_goal>No full faction AI or fleet command system.</non_goal>
  <non_goal>No perfect offscreen physical simulation.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>Tactical AI ports, regional ecology/traffic, encounter director, autopilot/waypoints, station docking/berths.</anchor>
  <anchor>Economy/cargo/save/offline and current NPC visual-family systems.</anchor>
...
<implementation_direction>
  <direction>Separate job planning state from moment-to-moment tactical intent; jobs publish destinations/actions, tactical AI owns local combat.</direction>
  <direction>Use deterministic schedules/seeds and stable route/place IDs.</direction>
  <direction>Materialized actors execute physical waypoints; absent actors advance statistically through the same state transitions and receipts.</direction>
  <direction>Reconcile on materialization so cargo, damage, location, and schedule cannot duplicate.</direction>
  <direction>Stuck recovery should escalate through local replan, safe reset at a lawful node, then abort with receipt—never teleport silently during witnessed play.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>Without labels, a reviewer can distinguish miner, hauler, and patrol from their route and activity.</criterion>
  <criterion>Each completes a normal loop, survives save/Continue, and has deterministic interrupt/resume/abort behavior.</criterion>
  <criterion>Hauler cargo/economic settlement occurs once and matches physical/aggregate state.</criterion>
  <criterion>Combat interrupt does not permanently erase the job unless declared.</criterion>
  <criterion>Witnessed and absent resolution reconcile without duplication or impossible location.</criterion>
  <criterion>Several concurrent jobs remain within measured sim/render budgets.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>NPC wandering with `job: miner` metadata.</failure>
  <failure>Timers that award cargo/credits disconnected from route state.</failure>
  <failure>Combat AI permanently replacing job state.</failure>
  <failure>Silent teleport as ordinary stuck handling.</failure>
  <failure>Spawning activity only when the player looks at it with no persistent schedule.</failure>
...
Task-specific required verification:
  <verification>Pure job transition tests, stuck/interrupt matrix, deterministic cohort, save/reload/offline reconciliation.</verification>
  <verification>Public observation route in sparse and crowded scenes, including one attack/recovery.</verification>
  <verification>Economy/cargo/faction single-writer checks and performance profile.</verification>
  <verification>Vision review for behavior legibility at gameplay scale.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-15.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-15.yaml`.
...
- [ ] Receipt written to `receipts/SF-15.yaml`.
