...
recommended_agent_profile: "Strong systems agent paired with a strong frontend/vision agent (Fable-class or equivalent)"
depends_on: ["SF-05"]
roadmap_alignment: "T07/T15, G corridor travel feedback, camera/VFX foundations"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-06 — [HARD][FULLSTACK][FRONTEND-HARD][VISION-YES] Shared Release Predictor, Validated Sling Course, and Speed-Language Presentation

<agent_assignment>
  <role>Strong systems agent paired with a strong frontend/vision agent (Fable-class or equivalent)</role>
...
<mission>
The player can read a predicted sling exit, time a release through a robust green window, chain to a second anchor, and feel physics-earned speed through camera and VFX without losing control or readability.
</mission>
...
<sequence_position>
  <prompt_id>SF-06</prompt_id>
  <declared_dependencies>SF-05</declared_dependencies>
  <roadmap_alignment>T07/T15, G corridor travel feedback, camera/VFX foundations</roadmap_alignment>
...
<problem>
A release that only works by intuition or on one lucky trajectory is not a reusable game verb. Current arc/release machinery may exist, but the player needs a trustworthy predictor, robust authored course geometry, and presentation that distinguishes “release now” from “do not release.”
</problem>
...
This creates the first unmistakable SpaceFace postcard: the ship winds around a massive anchor, the world pulls back, the exit ribbon turns green, and a well-timed release throws the player toward the next body. It is physics as movement, puzzle, and spectacle—not a speed buff.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Audit existing arc-data/render, release rating, earned-momentum governor, camera velocity language, trajectory ribbons, and any Atlas route nodes.</deliverable>
  <deliverable>Create one pure release predictor using the same live motion/constraint assumptions as the runtime; quantify prediction horizon/error and fail closed when state is insufficient.</deliverable>
  <deliverable>Create a data-driven sling-course definition with entry geometry, allowed direction, recommended line-length band, minimum entry speed, destination gate, valid release arc/window, miss recovery, and presentation metadata.</deliverable>
  <deliverable>Build a route compiler/validator that samples approach conditions and rejects courses with fragile or one-frame release windows.</deliverable>
  <deliverable>Author one two-anchor course in an appropriate developer or normal-world location, with no floating hoop aesthetic.</deliverable>
  <deliverable>Render a world-space trajectory ribbon, peri-release marker/window, destination gate, and redundant screen-edge cue using color plus shape/timing.</deliverable>
  <deliverable>Add speed-responsive orthographic camera scaling, trail persistence, audio/VFX intensity, and reduced-motion alternatives that preserve spatial information.</deliverable>
  <deliverable>Ensure physics-earned speed survives the ordinary governor according to existing earned-momentum semantics and decays predictably.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>No galaxy-wide course network, realistic orbital mechanics, or multiple planets.</non_goal>
  <non_goal>No full Massline HUD overhaul.</non_goal>
  <non_goal>No camera behavior that makes aiming or nearby collision unreadable.</non_goal>
  <non_goal>No fake release impulse unrelated to current state unless the product explicitly chooses and receipts a bounded stylized transfer.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>Existing Massline arc/release systems, camera velocity language, VFX trail instancing, earned-momentum governor.</anchor>
  <anchor>Atlas route/place data conventions where useful, but keep V1 bounded to one course.</anchor>
...
<implementation_direction>
  <direction>Predict by stepping a pure state copy with the same force/constraint laws or a validated approximation; never use a separate decorative curve.</direction>
  <direction>Define valid release in terms of destination gate crossing, exit vector, load, and safety—not merely color based on angle.</direction>
  <direction>Course validation should sample perturbations in entry speed, position, line length, and release timing to measure robustness.</direction>
  <direction>Use critically damped camera changes, hysteresis, near/far VFX LOD, pooled ribbon geometry, and bounded dynamic lights/particles.</direction>
  <direction>Make the exact target and next anchor acquisition integrate with SF-03 selection receipts.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>Predictor and actual released trajectory remain within declared tolerance over the useful window.</criterion>
  <criterion>The authored course has a release window wide enough for a human/trackpad player across tested approach variation.</criterion>
  <criterion>A correct release reaches the destination gate while early/late releases visibly miss and preserve recovery.</criterion>
  <criterion>Camera/VFX clearly communicate speed and timing without obscuring target, ship, collision, or HUD.</criterion>
  <criterion>Reduced-motion mode preserves the release state and route through non-motion cues.</criterion>
  <criterion>The course is reachable through a declared route and can reset/retry cleanly.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>A green ring that grants a speed multiplier.</failure>
  <failure>A decorative trajectory computed from different physics.</failure>
  <failure>One hand-placed course that passes only a scripted perfect input.</failure>
  <failure>Camera zoom snapping, unbounded zoom-out, or effects that scale without cap.</failure>
  <failure>Full-screen red/green overlay as the sole timing cue.</failure>
  <failure>Position animation through the course.</failure>
...
Task-specific required verification:
  <verification>Predictor differential tests versus live step traces, robustness sampling, route compiler rejection tests, deterministic reset/replay.</verification>
  <verification>Browser and Electron public/developer route captures with exact revision and settings.</verification>
  <verification>Vision review at normal gameplay distance, reduced motion, and high speed.</verification>
  <verification>Frame/allocation/profile checks for ribbons, trails, camera, and VFX.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-06.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-06.yaml`.
...
- [ ] Receipt written to `receipts/SF-06.yaml`.
