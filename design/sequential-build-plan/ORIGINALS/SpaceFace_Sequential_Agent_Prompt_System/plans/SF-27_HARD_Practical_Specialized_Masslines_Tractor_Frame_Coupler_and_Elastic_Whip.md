...
recommended_agent_profile: "Strong physics/control agent; vision review recommended"
depends_on: ["SF-04", "SF-05", "SF-09", "SF-16"]
roadmap_alignment: "T11/T12 and specialized spool progression"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-27 — [HARD][FULLSTACK][FRONTEND-MEDIUM][VISION-RECOMMENDED] Practical Specialized Masslines: Tractor, Frame Coupler, and Elastic Whip

<agent_assignment>
  <role>Strong physics/control agent; vision review recommended</role>
...
<mission>
Three researchable Massline heads share one input grammar but produce clearly different physical relationships: stable force-based towing, velocity matching/hitchhiking, and spring-energy storage/release.
</mission>
...
<sequence_position>
  <prompt_id>SF-27</prompt_id>
  <declared_dependencies>SF-04, SF-05, SF-09, SF-16</declared_dependencies>
  <roadmap_alignment>T11/T12 and specialized spool progression</roadmap_alignment>
...
<problem>
The standard tether is useful for orbiting and throwing but inhibitory for towing and capture. Adding independent tractor-beam keys or bespoke mission assists would fragment controls. The game needs variants that change the line’s law while preserving one learned interaction grammar.
</problem>
...
These three heads expand the Massline into utility, traversal, and playful kinetics: tow salvage without spinning, match a meteor/freighter to hitch a ride, or store tension and snap into a bungee launch. Players discover styles through the same core verb.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Audit attachment definitions/policies, reel/load/break, attachment physics authority, contextual attach, outfitting/research, save, HUD/cable VFX, NPC/cargo routes, and current hitchhiking tests.</deliverable>
  <deliverable>Define a spool/head interface separating target eligibility, force law, rest/desired-distance behavior, response masks, control axes, load/break, prediction, VFX profile, and progression metadata.</deliverable>
  <deliverable>Implement Tractor Spool as a capped spring-damper/force controller maintaining chosen distance for stable towing/capture without rigidly rotating the player.</deliverable>
  <deliverable>Implement Frame Coupler as bounded velocity-equalization relative to the target/frame, supporting freighters/meteors/cargo/satellites and safe detach.</deliverable>
  <deliverable>Implement Elastic Whip as spring-damper line with visible stored energy, overload, snap release, bungee turn, and light-target flick behavior.</deliverable>
  <deliverable>Integrate selection contexts, shared input/reel/pay-out/cut grammar, HUD state, trajectory/release preview where applicable, fitting/research/save, AI/ownership policy, and combat trace.</deliverable>
  <deliverable>Create one public/developer route per spool and at least one cross-system use: cargo heist/catcher, meteor hitch, salvage tow, or combat reposition.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>No monofilament, Twin Bridle, or drag net yet.</non_goal>
  <non_goal>No separate key per head.</non_goal>
  <non_goal>No hard-coded mission towing assistance.</non_goal>
  <non_goal>No silent auto-docking/auto-route while coupled.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>Attachment catalog/service, current hitchhiking/reel/pump/load/release, outfitting/research/save.</anchor>
  <anchor>SF-16 physical cargo/jobs and current tether cable/HUD/VFX.</anchor>
...
<implementation_direction>
  <direction>Spool definitions select a force-law adapter; live physics remains single authority.</direction>
  <direction>Tractor desired distance changes through the same reel/pay-out axis and has force/energy caps.</direction>
  <direction>Frame Coupler equalizes relative velocity over time, not position teleport; line/field visual communicates velocity error.</direction>
  <direction>Elastic stored energy derives from extension/relative velocity and declared stiffness; release uses actual state.</direction>
  <direction>Switching heads requires safe inactive state or explicit cut; save persists selected/fitted head and active state according to policy.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>Tractor can tow a representative payload/disabled craft stably through turns without rigid-orbit behavior.</criterion>
  <criterion>Frame Coupler can match and ride a moving hauler/meteor, then detach with inherited relative motion according to declared law.</criterion>
  <criterion>Elastic Whip stores/reports energy, supports a timed launch, and can fail/overload readably.</criterion>
  <criterion>All heads use one rebindable grammar and remain distinguishable by behavior/cable/VFX/HUD.</criterion>
  <criterion>No direct velocity/position writes, duplicate attachments, or save ghosts.</criterion>
  <criterion>Browser/Electron, deterministic replay, fitting/save, accessibility, and performance pass.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>Three names mapped to the same rigid tether.</failure>
  <failure>Tractor implemented by parenting target to player.</failure>
  <failure>Frame Coupler as teleport/snap-to-velocity.</failure>
  <failure>Elastic launch as fixed bonus unrelated to extension.</failure>
  <failure>Separate bespoke controls and HUD for every head.</failure>
...
Task-specific required verification:
  <verification>Pure force-law/energy/velocity-frame tests, mass ratios, caps, break, reversal, deterministic traces.</verification>
  <verification>Attachment/physics/save/fitting/targeting integration and public routes.</verification>
  <verification>Vision review of cable state and interaction legibility; performance with several NPC/player lines.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-27.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-27.yaml`.
...
- [ ] Receipt written to `receipts/SF-27.yaml`.
