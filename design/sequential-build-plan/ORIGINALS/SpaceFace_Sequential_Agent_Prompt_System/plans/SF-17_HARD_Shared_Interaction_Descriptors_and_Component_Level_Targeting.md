...
recommended_agent_profile: "Strong architecture/gameplay agent plus strong frontend/vision reviewer"
depends_on: ["SF-03", "SF-08"]
roadmap_alignment: "World-site/component foundation, T13, W15, graphics interaction truth"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-17 — [HARD][FULLSTACK][FRONTEND-HARD][VISION-YES] Shared Interaction Descriptors and Component-Level Targeting

<agent_assignment>
  <role>Strong architecture/gameplay agent plus strong frontend/vision reviewer</role>
...
<mission>
Visible structures expose component-level semantic targets whose selection, collision, scanner labels, allowed verbs, state, and presentation all resolve from one shared descriptor.
</mission>
...
<sequence_position>
  <prompt_id>SF-17</prompt_id>
  <declared_dependencies>SF-03, SF-08</declared_dependencies>
  <roadmap_alignment>World-site/component foundation, T13, W15, graphics interaction truth</roadmap_alignment>
...
<problem>
The repo can contain rich authored data while runtime collapses a station, wreck, asteroid, or industrial site into one entity radius and one interaction. Without component targeting, every new place becomes “hold RMB on the blob” or “press E near the center.” Separate renderer, scanner, collision, and interaction classifications also drift.
</problem>
...
Component targeting multiplies the existing verbs. A single wreck can have relays to scan, clamps to cut, a reactor to stabilize, a payload to tow, and a black box to recover—without building five unrelated minigames.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Audit current entity target lock, cursor/world picking, scanner/HUD labels, interaction descriptors, visual component metadata/sockets, compound proxies, mining/salvage, and save state.</deliverable>
  <deliverable>Define a shared gameplay-owned interaction descriptor for entity and component identity: stable ID, kind, bounds/pick proxy, parent, verbs, permissions, target priority, collision/proxy link, state schema, scan label, damage/repair/cut/tether/receiver semantics, and visual binding.</deliverable>
  <deliverable>Implement broad entity selection followed by component narrow-phase picking in the XZ/top-down view, supporting authored circles/capsules/polygons or bounded proxy types.</deliverable>
  <deliverable>Publish hover/paint/selected component receipts independent from weapon aim and Massline target choice, with explicit context arbitration.</deliverable>
  <deliverable>Wire scanner/target panel/reticle to show component name, state, allowed immediate action, failure reason, and parent site without clutter.</deliverable>
  <deliverable>Create one station or test structure with at least four distinct components and state transitions, using current visual geometry and collision proxies.</deliverable>
  <deliverable>Persist component states by stable IDs and handle asset/LOD/admission without changing selection identity.</deliverable>
  <deliverable>Add authoring validators that reject duplicate IDs, missing visual/proxy bindings, unsupported verbs, and invisible/off-shape pick volumes.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>No Wreck Cathedral yet.</non_goal>
  <non_goal>No universal mesh raycasting or per-triangle damage.</non_goal>
  <non_goal>No new interaction verbs beyond the minimal fixture.</non_goal>
  <non_goal>No renderer-owned gameplay state.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>Shared interaction descriptor direction in graphics plan, target/scanner/HUD owners, asset sockets/semantic roles.</anchor>
  <anchor>SF-08 compound proxies, current mining/salvage/docking/interaction systems and save owner.</anchor>
...
<implementation_direction>
  <direction>Keep descriptor authority in plain gameplay data; render consumes bindings and may provide pick transforms but does not decide verbs.</direction>
  <direction>Use projected/screen or world-space tolerance appropriate to top-down view, with zoom-aware but bounded grace.</direction>
  <direction>Resolve overlapping candidates by explicit component priority, cursor precision, occlusion/visibility, and context.</direction>
  <direction>Use one stable selection receipt so highlight and consumed action agree.</direction>
  <direction>Old single-entity interactions remain explicit compatibility descriptors.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>Player can independently select at least four visible components on one structure at normal zoom.</criterion>
  <criterion>Highlight/pick volume aligns with visible geometry and compound collision across LOD/transform changes.</criterion>
  <criterion>Scanner/HUD shows component-specific state and allowed action; consumed action matches preview.</criterion>
  <criterion>Component state survives save/Continue and does not reroll on asset load.</criterion>
  <criterion>Weapon aim, Massline selection, docking, and interaction selection do not secretly overwrite one another.</criterion>
  <criterion>Authoring validator rejects material descriptor errors.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>Four invisible hotspots around a decorative mesh.</failure>
  <failure>Component identity only in UI with one global runtime action.</failure>
  <failure>Full mesh raycast as the gameplay state model.</failure>
  <failure>Different IDs in render, collision, scanner, and save.</failure>
  <failure>Selection highlight that can disagree with action target.</failure>
...
Task-specific required verification:
  <verification>Pure proxy/picking/priority tests, LOD/transform stability, descriptor validation, save/migration.</verification>
  <verification>Browser/Electron component selection/action fixture and same-framing overlay captures.</verification>
  <verification>Input/a11y/perf and interaction-identity checks.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-17.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-17.yaml`.
...
- [ ] Receipt written to `receipts/SF-17.yaml`.
