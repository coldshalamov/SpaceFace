...
recommended_agent_profile: "Strong physics agent paired with strong VFX/frontend agent"
depends_on: ["SF-09", "SF-11"]
roadmap_alignment: "Gravity/mass expansion foundation; environmental machinery; physics combat"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-12 — [HARD][FULLSTACK][FRONTEND-HARD][VISION-YES] Continuous Field Kernel, Attractive Well, Repulsor, and Clearing Cone

<agent_assignment>
  <role>Strong physics agent paired with strong VFX/frontend agent</role>
...
<mission>
One deterministic finite-radius continuous-field primitive powers an attractive Mass Seed well, a repulsor seed/bomb, and a forward clearing cone, with shared prediction, selective coupling, readable force direction, and bounded performance.
</mission>
...
<sequence_position>
  <prompt_id>SF-12</prompt_id>
  <declared_dependencies>SF-09, SF-11</declared_dependencies>
  <roadmap_alignment>Gravity/mass expansion foundation; environmental machinery; physics combat</roadmap_alignment>
...
<problem>
Radial impulses already exist, but they are not gravity: they change velocity once. Gravity-themed gameplay needs continuous trajectory bending over time. Implementing each well, black hole, atmosphere, and weapon independently would create inconsistent laws, predictors, and performance hazards.
</problem>
...
A compact field vocabulary produces many toys and world systems: gather enemies, break formations, clear debris, curve flight, set up torpedoes, create orbit arenas, and later power planetary/anomaly machinery. One good kernel multiplies content.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Design a pure continuous-field acceleration/force kernel with source position/frame velocity, radius, falloff/type, strength, softening, max acceleration, affected masks, coupling/status inputs, and smooth edge/hysteresis semantics.</deliverable>
  <deliverable>Support at minimum finite attractive radial field, finite repulsive radial field, and directional cone field using explicit authored laws.</deliverable>
  <deliverable>Integrate fixed-step force application through physics authority for player, NPC craft, projectiles where declared, payloads, debris, and pickups under masks/caps.</deliverable>
  <deliverable>Create prediction helpers using the exact same kernel for trajectory previews and VFX/debug.</deliverable>
  <deliverable>Upgrade Anchor Seed with an attractive well variant; implement repulsor seed/bomb and forward clearing cone with separate lifecycle/resource balance.</deliverable>
  <deliverable>Add strongest-N or bounded source selection, spatial query/cadence, pooling, and telemetry to prevent O(entities×fields) runaway.</deliverable>
  <deliverable>Create force-readable VFX: streamlines/particles/ribbons, field boundary glyphs, object trails, distortion localized to the field, and reduced alternatives.</deliverable>
  <deliverable>Add AI hazard awareness/escape intent without perfect omniscience.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>No mutual N-body gravity, dynamic celestial simulation, or black-hole region yet.</non_goal>
  <non_goal>No instant immobilization or teleport.</non_goal>
  <non_goal>No projectile lensing unless it falls naturally from declared coupling and remains bounded; save dedicated lens tuning for later.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>Physics authority and spatial query, impulse charge/seed lifecycle, trajectory prediction, entity categories/coupling.</anchor>
  <anchor>Current VFX pooling, ribbons, shockwaves, settings/reduced effects.</anchor>
...
<implementation_direction>
  <direction>Prefer softened inverse-square for natural-like sources and compact polynomial falloff for artificial sources; name laws in data.</direction>
  <direction>Apply acceleration/force via the body authority; never set velocity inside field code.</direction>
  <direction>Separate gravity/field coupling from inertial/impulse response.</direction>
  <direction>Use one or two strongest relevant fields or another measured bounded policy; record ignored-source diagnostics.</direction>
  <direction>Use exact same evaluation in runtime and predictor.</direction>
  <direction>Render field behavior from real sampled law/force direction, not random particles.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>Attractive field curves trajectories continuously and gathers light targets without freezing them.</criterion>
  <criterion>Repulsor applies sustained outward force distinct from an impulse-charge blast.</criterion>
  <criterion>Clearing cone pushes objects in a directional volume and cannot affect behind/side entities outside declared falloff.</criterion>
  <criterion>Player, NPC, projectile/payload responses follow explicit masks/coupling; no player-only fake.</criterion>
  <criterion>Predictor and live path agree within tolerance.</criterion>
  <criterion>Multiple sources and crowded entities remain bounded and deterministic.</criterion>
  <criterion>Effects are legible against black space and world clutter in browser/Electron with reduced settings.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>A circular zone that grants a speed buff or periodic impulse.</failure>
  <failure>Direct velocity writes.</failure>
  <failure>One force law in gameplay and another in predictor/VFX.</failure>
  <failure>Unbounded all-pairs field evaluation.</failure>
  <failure>Field VFX as a glowing transparent sphere with unrelated particles.</failure>
  <failure>Artificial well so strong it replaces all other tactics.</failure>
...
Task-specific required verification:
  <verification>Pure law tests: finite center, smooth edge, monotonicity, symmetry, cone inclusion, caps, coupling, masks.</verification>
  <verification>Differential predictor/runtime traces, sim compare, save/lifecycle, source-cap and crowded profiles.</verification>
  <verification>Combat/traversal browser routes and vision-reviewed force readability.</verification>
  <verification>GPU/effect pool/lifecycle checks and no full-screen distortion.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-12.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-12.yaml`.
...
- [ ] Receipt written to `receipts/SF-12.yaml`.
