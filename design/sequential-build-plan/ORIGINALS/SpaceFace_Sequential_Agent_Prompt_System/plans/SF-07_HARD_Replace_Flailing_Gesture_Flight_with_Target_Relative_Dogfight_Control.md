...
recommended_agent_profile: "Strong control-systems agent with browser/vision capability"
depends_on: ["SF-06"]
roadmap_alignment: "Experimental control mode; T16/R07; M1 focus/camera; may be rejected by evidence"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-07 — [HARD][FULLSTACK][FRONTEND-MEDIUM][VISION-YES] Replace Flailing Gesture Flight with Target-Relative Dogfight Control

<agent_assignment>
  <role>Strong control-systems agent with browser/vision capability</role>
...
<mission>
The G/trackpad combat mode either becomes a stable target-relative control mode that separates weapon lock from flight intent, or is safely disabled/retired with evidence rather than remaining a flailing false promise.
</mission>
...
<sequence_position>
  <prompt_id>SF-07</prompt_id>
  <declared_dependencies>SF-06</declared_dependencies>
  <roadmap_alignment>Experimental control mode; T16/R07; M1 focus/camera; may be rejected by evidence</roadmap_alignment>
...
<problem>
The current experimental mode samples relative trackpad motion into world-space path points and converts path following into simultaneous thrust, strafe, and yaw. This mixes frames and can make the ship spin, chase arrows, or fly unpredictably. The underlying product need is valid—fight with the mouse while using trackpad gestures to shape relative motion—but the current primitive may be wrong.
</problem>
...
A successful target-relative mode could make dogfighting accessible and expressive without turning the game into cursor-chasing. An evidence-backed rejection is also valuable: removing a broken mode is better than carrying a UX landmine into every future physics feature.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Characterize the current G mode through deterministic fixtures and normal browser input, recording raw deltas, sampled path, commands, target-relative state, and actual motion.</deliverable>
  <deliverable>Define two candidate control contracts in pure form: target-relative polar-slot control (horizontal selects bearing around target; vertical selects range) and, only if still justified, a true command-curve follower with arc-length resampling/pure pursuit/curvature speed planning.</deliverable>
  <deliverable>Implement the lower-risk target-relative polar mode first behind the existing configurable experimental action, preserving weapon auto-aim independence.</deliverable>
  <deliverable>Use trackpad relative input to adjust desired bearing/range incrementally with deadzones, gain curves, recenter/hold semantics, and clear termination.</deliverable>
  <deliverable>Generate bounded Flight V3 commands through existing propulsion authority; manual controls break the mode instantly.</deliverable>
  <deliverable>Provide concise on-screen target-relative intent visualization—not an incomprehensible world arrow—and reduced-motion/non-color cues.</deliverable>
  <deliverable>Run comparative play/evidence and make an explicit product decision: promote target-relative mode, keep as experimental with named blockers, or retire G mode cleanly. Do not automatically implement the command-curve variant if polar control satisfies the goal.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>No generic ship-follows-cursor mode.</non_goal>
  <non_goal>No new weapon auto-aim behavior beyond existing lock.</non_goal>
  <non_goal>No hidden autopilot that wins combat.</non_goal>
  <non_goal>No requirement to preserve the current path implementation if it is structurally unsound.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>Current auto-target mode/path follower, input gesture sampler, Flight V3 autopursuit/manual ownership, target/lead owners.</anchor>
  <anchor>Control lab and camera/VFX work from SF-02/SF-06.</anchor>
...
<implementation_direction>
  <direction>Use target-relative polar coordinates: bearing error, range error, relative radial/tangential velocity, and target velocity feed a bounded controller.</direction>
  <direction>Mouse aim/weapon lead remains separate from flight control; the same target may inform both but through explicit independent contracts.</direction>
  <direction>Input deltas change desired slot, not a persistent yaw command.</direction>
  <direction>Define loss-of-target, target death, UI focus, finger lift, manual override, and range-limit behavior.</direction>
  <direction>Compare against a no-mode baseline and current implementation using the same fixtures.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>The ship responds predictably to small/large horizontal and vertical gestures without unexplained spin.</criterion>
  <criterion>Horizontal gesture changes orbit bearing around the target; vertical gesture changes range; finger lift holds or settles according to the documented rule.</criterion>
  <criterion>Weapon reticle continues tracking independently and flight control does not rewrite aim state.</criterion>
  <criterion>Manual input exits within one simulation tick and leaves neutral commands.</criterion>
  <criterion>Loss/death/modal transitions exit safely.</criterion>
  <criterion>Comparative browser evidence shows lower overshoot, spin events, and command disagreement than the old mode; otherwise the mode is retired and the receipt says so.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>Ship chases cursor or arrow.</failure>
  <failure>Relative trackpad displacement becoming indefinite turn rate.</failure>
  <failure>Mixed world/ship/target frames in one control calculation.</failure>
  <failure>Direct velocity/position writes.</failure>
  <failure>Promoting the mode because unit tests pass without normal trackpad/browser evidence.</failure>
...
Task-specific required verification:
  <verification>Pure polar controller tests, rotational/translation invariance, target motion cases, gesture lifecycle, and bounds.</verification>
  <verification>Browser automation plus human-scale trackpad route and visual capture.</verification>
  <verification>Telemetry metrics: slot error, spin count, overshoot, manual-disengage latency, target loss recovery.</verification>
  <verification>Input/rebind/a11y/perf checks and sim compare.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-07.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-07.yaml`.
...
- [ ] Receipt written to `receipts/SF-07.yaml`.
