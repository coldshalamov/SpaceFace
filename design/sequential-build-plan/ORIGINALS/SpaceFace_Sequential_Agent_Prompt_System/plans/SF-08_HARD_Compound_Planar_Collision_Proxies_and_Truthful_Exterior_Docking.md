...
recommended_agent_profile: "Strong physics/geometry agent plus strong frontend/vision reviewer"
depends_on: ["SF-01", "SF-02"]
roadmap_alignment: "World-site foundation, T13, G04/G13, graphics interaction descriptors, M1/M4 visual truth"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-08 — [HARD][FULLSTACK][FRONTEND-HARD][VISION-YES] Compound Planar Collision Proxies and Truthful Exterior Docking

<agent_assignment>
  <role>Strong physics/geometry agent plus strong frontend/vision reviewer</role>
...
<mission>
One authored station has collision and docking geometry that matches its visible top-down silhouette: solid regions are solid, deliberate channels are navigable, and the berth is approached through an exterior corridor rather than by entering decorative walls and bonking an unrelated core.
</mission>
...
<sequence_position>
  <prompt_id>SF-08</prompt_id>
  <declared_dependencies>SF-01, SF-02</declared_dependencies>
  <roadmap_alignment>World-site foundation, T13, G04/G13, graphics interaction descriptors, M1/M4 visual truth</roadmap_alignment>
...
<problem>
Current entities can reduce complex stations to a central circle or ball collider. The player may pass through visible hulls, collide with empty space, or search inside geometry for a dock trigger. Monumental wrecks, industrial sites, orbital machinery, and physical combat cannot work until geometry and gameplay tell the same truth.
</problem>
...
This is the substrate that lets the world become a place rather than a gallery of meshes around circles. One good station proves the compound-proxy authoring, runtime, debugging, docking, save, and performance path needed by every later site.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Audit current Rapier/custom collision authority, station bounds, docking prompt/corridor logic, authored asset transforms/LODs, place registration, and visual admission.</deliverable>
  <deliverable>Define a data-driven planar collision proxy format supporting circles, capsules, convex polygons or another bounded set appropriate to the live engine, plus sensors/interaction volumes and semantic component IDs.</deliverable>
  <deliverable>Implement entity broad phase plus per-proxy narrow phase through physics authority, including swept/CCD behavior appropriate to player speeds.</deliverable>
  <deliverable>Author proxies for one existing station based on its actual top-down shape, with navigable voids/channels and no unrelated central core.</deliverable>
  <deliverable>Create an exterior approach corridor, capture/berth sensor, velocity/alignment policy, prompt, abort/retry behavior, and safe undock vector.</deliverable>
  <deliverable>Ensure LOD/asset normalization preserves proxy alignment and stable transform; collision does not swap with visual LOD.</deliverable>
  <deliverable>Add debug overlay and authoring validation for proxy bounds, overlaps, channels, dock corridor, and visible silhouette.</deliverable>
  <deliverable>Preserve legacy entities through an explicit compatibility descriptor while preventing new hero sites from silently falling back to one circle.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>No fleet-wide station remaster.</non_goal>
  <non_goal>No first-person interiors or arbitrary mesh collision.</non_goal>
  <non_goal>No hidden teleport into dock from anywhere inside a radius.</non_goal>
  <non_goal>No renderer ownership of simulation collision.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>Physics authority, Rapier body/collider creation, spatial query, station/docking systems, place registration.</anchor>
  <anchor>Authored asset admission, stable-transform verifier, station asset metadata, camera/player route harnesses.</anchor>
...
<implementation_direction>
  <direction>Keep collision proxies in gameplay/asset metadata consumed by physics and render debug, not inferred every frame from mesh triangles.</direction>
  <direction>Validate source-to-runtime transform, scale, axes, pivot, bounds, and semantic IDs at asset admission.</direction>
  <direction>Use swept circle/capsule or appropriate continuous collision for the player against static proxies.</direction>
  <direction>Docking should be a stateful approach/capture process with clear failure reasons, not collision as the success condition.</direction>
  <direction>Measure proxy count, broad-phase candidates, narrow-phase cost, and contact stability.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>At normal play speed, the player cannot fly through the station’s declared solid hull regions and can fly through at least one visibly open channel.</criterion>
  <criterion>No collision occurs in a large visible-empty region formerly occupied by the central proxy.</criterion>
  <criterion>The dock prompt appears only in the exterior corridor under documented alignment/speed conditions; success captures cleanly and undock cannot spawn inside a proxy.</criterion>
  <criterion>Visual, proxy, target, and dock coordinates remain aligned across approach, LOD, frame-origin changes, save/Continue, browser, and Electron.</criterion>
  <criterion>Legacy entities still function under explicit fallback policy.</criterion>
  <criterion>Performance remains within measured budget without using full triangle-mesh collision.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>One larger circle that merely covers more of the station.</failure>
  <failure>Decorative walls with unchanged central collider.</failure>
  <failure>A dock sensor placed inside solid-looking geometry.</failure>
  <failure>Per-frame mesh raycast as the entire physics solution.</failure>
  <failure>Kinematic push/teleport that hides collision tunneling.</failure>
  <failure>Visual acceptance from a Blender screenshot only.</failure>
...
Task-specific required verification:
  <verification>Pure geometry/proxy validation, swept collision fixtures, tunneling, channel, corner, and overlap cases.</verification>
  <verification>Live browser/Electron approach, channel traversal, dock/undock, save/Continue, and high-speed impact routes.</verification>
  <verification>Same-framing visual/proxy overlay captures reviewed by a vision agent.</verification>
  <verification>Physics/profile/allocation checks and launch/sim/save gates.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-08.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-08.yaml`.
...
- [ ] Receipt written to `receipts/SF-08.yaml`.
