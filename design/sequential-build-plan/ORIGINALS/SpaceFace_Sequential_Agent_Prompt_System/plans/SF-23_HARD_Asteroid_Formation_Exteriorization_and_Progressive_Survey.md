...
recommended_agent_profile: "Strong Asteroid Ops systems agent plus frontend/vision agent"
depends_on: ["SF-19", "SF-21"]
roadmap_alignment: "A03/A04/G08/W08; Asteroid Ops signature system"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-23 — [HARD][FULLSTACK][FRONTEND-HARD][VISION-YES] Asteroid Formation Exteriorization and Progressive Survey

<agent_assignment>
  <role>Strong Asteroid Ops systems agent plus frontend/vision agent</role>
...
<mission>
A current asteroid formation is spatially readable at navigation and operation distance, survey reveals structure and uncertainty progressively, and entry into operations is connected to real exterior geometry rather than a generic cluster/menu handoff.
</mission>
...
<sequence_position>
  <prompt_id>SF-23</prompt_id>
  <declared_dependencies>SF-19, SF-21</declared_dependencies>
  <roadmap_alignment>A03/A04/G08/W08; Asteroid Ops signature system</roadmap_alignment>
...
<problem>
The repository already has pure formation/geology and persistence foundations, but invisible data does not make a mining region legible. If formations render as interchangeable rocks and survey is a binary overlay, the player cannot reason about approach corridors, contact, gas/thermal risk, or strategic site choice.
</problem>
...
This is where Asteroid Ops begins to exist in the flight world. The player sees a formation’s shape, scans uncertainty into knowledge, chooses where to enter, and understands that the geometry they alter has persistent production consequences.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Audit A01/A02 formation model/persistence, current asteroid rendering/instances, survey, map, site entry, contact-ring law, geology materials, collision, and performance.</deliverable>
  <deliverable>Create a formation visual adapter that preserves deterministic formation identity and renders distinct silhouettes, density gradients, lanes, exposed geology cues, gas/thermal risk hints, and site-affinity markers at multiple distances.</deliverable>
  <deliverable>Use instancing/HLOD/LOD and authored representative rocks/landmarks without reducing default density or visual bar.</deliverable>
  <deliverable>Implement progressive survey stages: unknown bounds, uncertain archetype/corridors, risk/resource hints, then actionable structural knowledge; preserve confidence/staleness/provenance.</deliverable>
  <deliverable>Connect scanner pulse/flight path and map/Atlas to survey progress; avoid one-click full reveal.</deliverable>
  <deliverable>Implement exterior entry points/corridors bound to formation geometry and site IDs; show operation relationship before opening the Asteroid Ops screen.</deliverable>
  <deliverable>Persist survey/discovery without rerolling formation IDs and safely load old saves.</deliverable>
  <deliverable>Author one Ceres or representative route from arrival→survey→choose entry→enter operation→exit with changed exterior cue.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>No full Asteroid Ops production/heat/signature UI yet.</non_goal>
  <non_goal>No all-galaxy formation remaster.</non_goal>
  <non_goal>No per-rock save bloat.</non_goal>
  <non_goal>No binary resource overlay or giant labels.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>A01/A02 formation and persistence, A03/A04 roadmap, asteroid/site/render/survey owners.</anchor>
  <anchor>Atlas/map discovery confidence, scanner, asset/admission/rock foundry, G08 route fixtures.</anchor>
...
<implementation_direction>
  <direction>Derive visual layout from stable formation data; render state may be cached but not authoritative.</direction>
  <direction>Use semantic zoom and uncertainty styling rather than permanent text walls.</direction>
  <direction>Survey accumulation depends on geometry/time/positions or authored scan actions and emits bounded receipts.</direction>
  <direction>Entry is a selected formation/site relationship with readable corridor, not proximity to arbitrary center.</direction>
  <direction>Keep mining/drill ownership separate from rendering.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>At navigation distance, the formation archetype and approach lanes are readable without labels alone.</criterion>
  <criterion>Survey reveals information in at least three meaningful stages and survives Continue.</criterion>
  <criterion>Player can choose an entry based on newly learned structure/risk.</criterion>
  <criterion>Exterior formation and internal/site identity agree before and after operation.</criterion>
  <criterion>Performance remains within measured sparse/normal/dense budgets with stable transforms.</criterion>
  <criterion>Browser/Electron route and gameplay-scale visual review pass.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>Random rocks around a label.</failure>
  <failure>Survey button instantly revealing all data.</failure>
  <failure>Changing formation every load.</failure>
  <failure>Entry trigger at invisible central circle.</failure>
  <failure>Passing performance by reducing asteroid counts or visual quality.</failure>
...
Task-specific required verification:
  <verification>Formation identity/visual adapter/survey progression/save tests and translation/rotation invariants.</verification>
  <verification>Instance/LOD/stability/perf checks.</verification>
  <verification>Normal browser/Electron survey-entry-exit route and map captures reviewed by vision agent.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-23.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-23.yaml`.
...
- [ ] Receipt written to `receipts/SF-23.yaml`.
