...
recommended_agent_profile: "Top-tier physics/interaction agent; no-vision implementation possible, vision review recommended"
depends_on: ["SF-17", "SF-27", "SF-28"]
roadmap_alignment: "Advanced future Massline; body-to-body interaction; explicitly before any drag-net work"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-29 — [HARD][SYSTEMS][FRONTEND-MEDIUM][VISION-RECOMMENDED] Twin Bridle World-to-World Tether

<agent_assignment>
  <role>Top-tier physics/interaction agent; no-vision implementation possible, vision review recommended</role>
...
<pseudo_skills>
  <skill name="deterministic-gameplay-systems">Design fixed-step, finite, replay-safe kernels and adapters with explicit reference frames.</skill>
  <skill name="physics-authority-integration">Route forces, impulses, constraints, contacts, and body commands through the live Rapier authority.</skill>
  <skill name="input-and-control-engineering">Build bounded intent interpretation, deadbands, hysteresis, immediate override, and telemetry.</skill>
  <skill name="mutation-resistant-testing">Create invariant, contract, differential, and public-route tests that fail on plausible wrong implementations.</skill>
</pseudo_skills>
</agent_assignment>

<mission>
The player can select two eligible world bodies/components and create a tether between them—without tethering the player—causing controlled mutual constraints, rotation, separation, trap, or relocation with readable ownership and counterplay.
</mission>
...
<sequence_position>
  <prompt_id>SF-29</prompt_id>
  <declared_dependencies>SF-17, SF-27, SF-28</declared_dependencies>
  <roadmap_alignment>Advanced future Massline; body-to-body interaction; explicitly before any drag-net work</roadmap_alignment>
...
<dependency_gate>
  <dependency>SF-17</dependency>
  <dependency>SF-27</dependency>
  <dependency>SF-28</dependency>
...
<problem>
A twin bridle can create some of the most interesting emergent physics—tie two enemies together, anchor a payload, rotate a large component, trap a target between bodies—but it introduces target sequencing, authority, force-law, ownership, and UI ambiguity. Building it as “press F twice” without a transaction model would be dangerous.
</problem>
...
This concentrates several desired verbs into one primitive. It lets players invent tactics rather than select telekinesis powers, while remaining visually and physically coherent with the Massline fantasy.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Audit attachment service support for non-player owner/source/target, limits, save, AI/friendly policy, component anchors, target selection receipts, and input transaction semantics.</deliverable>
  <deliverable>Define a two-stage bridle transaction with explicit source selection, target selection, preview line, cancel/timeout, eligibility/revalidation, spool law, ownership/cost, and confirmation.</deliverable>
  <deliverable>Implement world-to-world attachment through the existing physics authority with stable owner/attachment IDs; player is controller/owner but not a body endpoint.</deliverable>
  <deliverable>Support at least three use cases in a bounded fixture: enemy↔enemy disruption, payload/component↔static anchor stabilization/relocation, and target↔hazard/repulsor combination.</deliverable>
  <deliverable>Define mass ratios, max length, reel/pay-out or fixed length, break/overload, obstruction, destruction, source/target death, and cap policy.</deliverable>
  <deliverable>Integrate component selection, preview, HUD transaction state, cable VFX, combat trace, save/Continue, AI counterplay, law/reputation intents, and cleanup.</deliverable>
  <deliverable>Explicitly assess drag-net feasibility after this feature but do not implement it unless the receipt proves a distinct, low-risk need; preserve it as deferred.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>No arbitrary multi-line web, drag net, or tethering dozens of entities.</non_goal>
  <non_goal>No player physically attached as a hidden third endpoint.</non_goal>
  <non_goal>No direct transform parenting.</non_goal>
  <non_goal>No ambiguous reliance on current weapon target.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>Attachment service/catalog and owner semantics, component/target receipts, specialized spool interface.</anchor>
  <anchor>Combat trace, save, AI/faction/law, cable render.</anchor>
...
<implementation_direction>
  <direction>Use a command transaction object with source/target receipts and single authority commit.</direction>
  <direction>Revalidate both endpoints at commit; failure names which endpoint/reason and leaves no partial attachment.</direction>
  <direction>Force law comes from a selected spool definition and respects both body response policies.</direction>
  <direction>Line/control UI shows stage 1 vs stage 2 and supports immediate cancel.</direction>
  <direction>Save either persists the attachment and transaction-free state or safely cancels incomplete transactions.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>Player can deliberately choose endpoint A then B in clutter, preview the exact pair, cancel, and commit.</criterion>
  <criterion>World-to-world line applies physically through authority and does not secretly move/attach the player.</criterion>
  <criterion>Three declared use cases work with deterministic outcomes and counterplay.</criterion>
  <criterion>Endpoint death/destruction/out-of-range/obstruction/break cleanly removes line and transaction state.</criterion>
  <criterion>Caps prevent line explosions and AI/law policies are explicit.</criterion>
  <criterion>Browser/Electron, save/reload, visual clarity, and performance pass.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>Using the current weapon lock as one endpoint without preview.</failure>
  <failure>Creating one attachment then another to the player as an imitation.</failure>
  <failure>Parenting bodies or setting positions.</failure>
  <failure>Partial endpoint state after failure.</failure>
  <failure>Implementing drag net in the same task because it sounds adjacent.</failure>
...
Task-specific required verification:
  <verification>Transaction/eligibility/revalidation/cancel/save tests, force-law/mass/break cases, trace determinism.</verification>
  <verification>Cluttered browser routes for each use case, AI/law response, visual review.</verification>
  <verification>Attachment cap/crowded performance and cleanup.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-29.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-29.yaml`.
...
- [ ] Receipt written to `receipts/SF-29.yaml`.
