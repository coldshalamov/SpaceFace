...
recommended_agent_profile: "Backend/systems agent; Codex/GLM-class no-vision agent is suitable"
depends_on: ["SF-01"]
roadmap_alignment: "T01\u2013T07 enabling evidence; M1 tether/camera; foundational test tooling"
prompt_system_version: "1.0"
terminal_gate: "ready-for-review with receipt; global completion remains lead-owned"
---

# SF-02 — [MEDIUM][SYSTEMS][FRONTEND-LOW][VISION-NO] Deterministic Physics-Control Laboratory and Telemetry

<agent_assignment>
  <role>Backend/systems agent; Codex/GLM-class no-vision agent is suitable</role>
...
<mission>
Developers can reproduce, tune, and compare tether acquisition, orbit, reel/pay-out, release, earned momentum, and controller behavior in a deterministic laboratory before risking the live world.
</mission>
...
<sequence_position>
  <prompt_id>SF-02</prompt_id>
  <declared_dependencies>SF-01</declared_dependencies>
  <roadmap_alignment>T01–T07 enabling evidence; M1 tether/camera; foundational test tooling</roadmap_alignment>
...
<problem>
SpaceFace contains many partial Massline mechanics and checks, but feel defects are difficult to diagnose when input, target geometry, line state, physics commands, camera, and outcomes are observed through scattered probes. Agents can tune one symptom while breaking another because there is no compact fixture that makes intent-versus-motion disagreement obvious.
</problem>
...
A good control laboratory pays for itself repeatedly. It lets later prompts test whether assistance preserves real physics rather than drawing a circle, and makes failures legible to both no-vision systems agents and visual reviewers.
</why_this_is_cool_and_worth_building>
...
<scope>
  ...
  <deliverable>Create one deterministic developer fixture/lab reachable through the repository’s declared debug/developer route without changing normal defaults.</deliverable>
  <deliverable>Include configurable massive static anchor, moving heavy anchor, light towable, hostile flyby target, clutter objects, collision obstacle, and release gates.</deliverable>
  <deliverable>Expose time-series telemetry: raw inputs, inferred intent, candidate scores, selected target, reference frame, rest length, distance, radial/tangential velocity, angular rate, strain/load, controller targets/outputs, actual forces/torques, release prediction, actual exit vector, earned-momentum tag, and camera state.</deliverable>
  <deliverable>Provide pause, single-step, reset, seeded scenario selection, and deterministic JSON trace export.</deliverable>
  <deliverable>Add invariant/property tests and a headless replay path that can compare uninterrupted versus reload behavior.</deliverable>
  <deliverable>Keep developer overlays/panels clearly debug-only and bounded; no normal HUD redesign.</deliverable>
  </mandatory_deliverables>
  <explicit_non_goals>
  <non_goal>Do not implement final orbit assist, target acquisition, or new weapons in this task.</non_goal>
  <non_goal>Do not create a second physics simulation or bypass the live attachment/flight authorities.</non_goal>
  <non_goal>Do not make the lab required for normal player access.</non_goal>
  </explicit_non_goals>
</scope>
...
Use these research anchors as starting points, not brittle file instructions:
  <anchor>Existing Massline orbit telemetry, release rating, target scoring, combat trace, physics authority, Flight V3 diagnostics.</anchor>
  <anchor>Current developer scenario/test conventions under `scripts/`, `test/`, and relevant nested `AGENTS.md`.</anchor>
...
<implementation_direction>
  <direction>Reuse existing Massline telemetry and trace owners; add adapters only where an observable is genuinely missing.</direction>
  <direction>Make scenario definitions plain data and stable-seeded.</direction>
  <direction>Record both semantic values and authority commands so direct velocity writes or hidden kinematic motion are detectable.</direction>
  <direction>Provide reference-frame axes in the trace, not only scalar scores.</direction>
  <direction>Ensure trace collection has bounded memory and an explicit sampling cadence.</direction>
</implementation_direction>

<acceptance_contract>
  <criterion>Identical seed, inputs, and revision produce byte-stable semantic traces within declared floating tolerances.</criterion>
  <criterion>Uninterrupted and save/reload runs agree on gameplay outcomes where persistence is claimed.</criterion>
  <criterion>The lab can demonstrate at least: missed latch, correct latch in clutter, radial tug, stable tangent motion, line overload/break, reel, pay-out placeholder input, and release.</criterion>
  <criterion>Telemetry distinguishes requested intent, controller output, and actual motion.</criterion>
  <criterion>Normal gameplay defaults and route remain unchanged.</criterion>
...
The implementation fails review if any of the following occur:
  <failure>A bespoke fake physics scene disconnected from production systems.</failure>
  <failure>Telemetry that observes only final position and cannot reveal authority commands.</failure>
  <failure>Unbounded per-frame logs or object allocation.</failure>
  <failure>A debug overlay that mutates state to make the maneuver succeed.</failure>
  <failure>Hard-coded one-fixture tolerances that do not generalize across mass ratios and frame rates.</failure>
...
Task-specific required verification:
  <verification>Pure tests for trace schema, finite values, symmetry, deterministic ordering, and bounded buffers.</verification>
  <verification>Headless replay and sim compare using live attachment/flight adapters.</verification>
  <verification>Browser lab capture proving controls and overlay are reachable and reset cleanly.</verification>
  <verification>Performance check with telemetry off and with bounded telemetry on.</verification>
</verification_plan>
...
Write this receipt shape to `receipts/SF-02.yaml`:
...
- Write the machine-readable receipt at `receipts/SF-02.yaml`.
...
- [ ] Receipt written to `receipts/SF-02.yaml`.
