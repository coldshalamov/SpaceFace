...
| 4 — Living-galaxy diversity | Distinct regional ecology, six POI behavior families, deterministic encounter fingerprints, solvable causal contracts, persistent aftermath. | In implementation — six deterministic POI behavior grammars now retain stable zone-bound aftermath across days and migrate prior saves; commit `a7021280`, 8/8 focused tests pass. Station/outpost and Helios civilian slices landed. Ashline commit `5ae73146` remains an isolated HOLD/unwired candidate; `check-m4-ashline-v2` passes with one missing LOD-contact warning, not visual acceptance. Regional ecology, accepted art, and player-route acceptance remain open. |
