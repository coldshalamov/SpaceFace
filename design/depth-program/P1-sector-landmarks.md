...
**Thread:** depth-P1 · **Reads:** `00_DEPTH_PROGRAM.md`, `assets/AGENTS.md` §1-3, `design/spec2/00_MASTER_TASTE.md`, `design/spec3/SPEC3-F9-asset-pipeline.md`, `design/graphics-sprints/00_ORCHESTRATION.md`, `design/graphics-sprints/BLENDER_EXCLUSIVE_LOCK.md` · **Status:** PLAN
...
- **Budget:** `parts_manifest.json` defines `trianglesPerLandmark` (500–1,100,000) and `maxBytesPerLandmark` (65MB) as a class distinct from ordinary parts. Landmarks get generous budgets — use them for silhouette, not for micro-detail (the perf policy forbids solving anything by silent quality cuts, so author *to* the budget).
- **Pre-queued landmark slots:** `assets/QUEUE.md` already reserves hero slots — `landmark_beacon_spire`, `landmark_wreck_cathedral`, `landmark_veil_obelisk`, `landmark_pit_anchor`, `landmark_vault_maw`, `landmark_tower_crown` (10k-tri budget each). **This pipeline actualizes that queue.**

KNOWN BUGS: the AGENTS.md claim that the whole-ship map is empty is stale (Kestrel is wired). No P1-specific bugs known. Perf gospel: do not exceed `trianglesPerLandmark`; merge static detail into few submeshes per material/role (AGENTS.md §6 Performance policy).
...
- **Step 2 — Author the GLB** in Blender per the asset pipeline (`assets/AGENTS.md` §3, `SPEC3-F9`). Hero landmark budget: 8k–15k tris (within `trianglesPerLandmark`). Merge static bolts/ribs/panels into few submeshes per material/animated role. Material convention: `Material_Hull` + `Material_Accent` slots (so faction tint flows — see §4). Filename: `place_landmark_<name>.glb` under `assets/ships/parts/places/`.
...
**Do NOT touch:** `src/render/partsLibrary.js` beyond the `PLACE_FILES` append (the C-thread serialization point — `00_ORCHESTRATION.md` §6). `assets/ships/release/release_manifest.json` (auto-written). The legacy `flight.js` / `ai.js` / `flightDynamics.js` (no effect in normal play — AGENTS.md §5).
...
**First worked example (the template):** priority 1, `place_landmark_wreck_cathedral` at `zone_io_derelict`. A split-hull cruiser silhouette, ~12k tris, `Material_Hull` / `Material_Accent` / `Material_Emissive` slots, placed via the zone's existing POI/wreck anchor. Proves the full pipeline end-to-end.
...
1. Acquire the Blender lock per `BLENDER_EXCLUSIVE_LOCK.md`. If `assets/ships/blender.LOCK` is held by another thread, **STOP and report the owner.**
2. Author `place_landmark_<name>.glb` (8k–15k tris, correct material slots, merged static detail). Iterate ≥20 times for a hero landmark per `QUALITY_RITUAL.md`.
...
- **DON'T** exceed `trianglesPerLandmark` "for detail" — the perf policy (AGENTS.md §6) forbids solving anything by silent quality cuts, and over-budget assets get flagged.
...
>
> Read in order: `AGENTS.md` §1, §3, §5, §6, §10 · `design/spec2/00_MASTER_TASTE.md` · `design/depth-program/00_DEPTH_PROGRAM.md` · `design/depth-program/P1-sector-landmarks.md` (this file) · `assets/AGENTS.md` §1-3 · `design/spec3/SPEC3-F9-asset-pipeline.md` · `design/graphics-sprints/00_ORCHESTRATION.md` · `design/graphics-sprints/BLENDER_EXCLUSIVE_LOCK.md` · `design/graphics-sprints/QUALITY_RITUAL.md`. Then stop reading and do the work.
>
> **Target landmark:** `<LANDMARK_ID>` (e.g. `place_landmark_wreck_cathedral` at `zone_io_derelict`, `sector_io_reach` — see §5 backlog row).
>
> **Lock protocol:** acquire `assets/ships/blender.LOCK` as thread depth-P1. If held, **STOP and report the owner.** Do not edit, delete, or stage any `assets/**` outside your target landmark while another graphics lane is active.
>
> **Do:** author the GLB (≥20 iterations for a hero, per QUALITY_RITUAL), register in all 3 registries (`parts_manifest.json`, auto-written `release_manifest.json`, `partsLibrary.js:59`), wire placement at the named zone, screenshot into `.devshots/`, run the named acceptance checks.
>
...
>
> `git add -N` every new file immediately. Print a 10-line summary: what landmark, where placed, tri/byte budget vs actual, which check scripts green, screenshot paths, any deferred/flagged items.
