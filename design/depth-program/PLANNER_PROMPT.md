...
1. **`design/depth-program/PLANNER_BRIEFING.md`** — your dossier. Live code shapes, the contracts (determinism, single-writer rules), the asset pipeline (registries, silent-fallback trap, material roles, profiling metadata, boot gate), the Wave 4 collision map, and the aesthetic north star. Treat it as routing context, then resolve descriptive claims against the live repo and current checks.
...
- **Asset pipeline:** source manifest + auto-written release manifest + runtime routing, documented material roles, and a boot gate that refuses flight if authored assets are not preloaded. Geometry, texture density, materials, and LOD follow screen-space need and representative profiling; manifest limits are alarms, not quality ceilings. Treat release/build markers as ownership signals only after confirming they belong to live work.
