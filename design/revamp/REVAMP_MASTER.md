...
| 08 auto-target wire | **REVERSED 2026-07-27 — the shipped function had zero callers** | This row used to read DONE and describe `pickMasslineAutoTarget(state,opts)` as live. It never was: it was exported, massline-aware, and called by nothing except `scripts/check-massline-auto-target.mjs`, which is how it stayed "green" for the whole time it did nothing. It was **deleted** on 2026-07-27. The rung's actual outcome — the Massline picking a sensible anchor — is delivered by **scored latch acquisition** instead: `src/combat/masslineTargetScoring.js` (`rankMasslineTargets`, rung 07, which *is* live) consumed through `tetherGameplay._refreshAcquisitionPreview` → `_consumeAcquisitionReceipt`, so preview and latch agree by construction. `rankMasslineTargets`' `opts.isLatched` predicate survives and is used. Treat this row as the standing example of the failure mode this program exists to stop: **a check that exercises a function directly proves the function works, not that anything calls it.** |
