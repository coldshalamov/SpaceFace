...
| T5b | Backend-shaped but blocked | Only `T5b-PKT-RITUAL` remains; blocked because runtime proof needs an onboarding helper seam and `onboarding.init(ctx)` does not store `ctx.helpers`, while `onboarding.js` is no-touch for the packet. |
