...
| INFERENCE / make N missions / develop the game / throw an agent at the world | [`../design/program/INFERENCE_LANES.md`](../design/program/INFERENCE_LANES.md) → copy `design/program/INFERENCE_GOAL.txt`. Bare `INFERENCE` = look at play, fix a real weakness, rotate kinds. Detect is an optional count hint, not the assignment. Thin rows do not count. Captures are not the review |
