...
| Finish the game / what is next for release / the professional bar | **`build_map.md` §15** → `--id PQ-146` or any §15.2 ID; the eight reactivated packets `PQ-026`–`PQ-033` are ready again |
| Independent pass over what just landed / taste, improvements, and bugs before those units are finished | **`build_map.md` §1.7** → `--id PQ-191`. Play the named surfaces; fix real defects; a report with leftovers still on camera is not done |
...
| Frontend looks cheap / make the UI A-list / any menu, HUD or screen redesign | **`design/frontend/direction/FIELD_HARDWARE_PROGRAM.md`** → `--id PQ-194`. Frames under `design/frontend/direction/approved/` outrank every prose doc; 27 hand-off packets under `design/frontend/direction/packets/` (`node scripts/build-ui-packet.mjs P01`). `PQ-187` / Cinematic Minimal is superseded |
