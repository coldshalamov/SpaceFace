...
| Frontend looks cheap / make the UI A-list / any menu, HUD or screen redesign | **`design/frontend/direction/FIELD_HARDWARE_PROGRAM.md`** → `--id PQ-194`. Frames under `design/frontend/direction/approved/` outrank every prose doc; 28 hand-off packets under `design/frontend/direction/packets/` (`node scripts/build-ui-packet.mjs P01`). `PQ-187` / Cinematic Minimal is superseded |
