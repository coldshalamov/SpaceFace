...
3. **Real material, not one gradient on every plate.** Material and light *fell* from 4 to 3
   after the plates got stronger bevels. The same bevel on every panel reads as dark-mode app
   cards, not as a machine. Tether state is still a text line, not an instrument.
...
- **Live stills.** All four pictures on this page come from the real game route (`node
...
- **Accessibility and correctness floors all pass:** 12 px type floor, WCAG contrast, UI a11y,
  ...
  reduced motion. The new plates (weapon nameplate, band key, reticle) have forced-colours and
...
- **`check:baseline`:** 14 of 15 groups pass. In the 15th, two long simulation scenarios timed out
  ...
  95 s).
- **Laws held.** Only `src/ui/` changed (plus this page). There are no gameplay or input
