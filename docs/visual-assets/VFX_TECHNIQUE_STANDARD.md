...
| `field_attractor`; live `well`, `seed`, `anchor_snare` | `shield-response` | Reference field label and live swept force-surface owners in `src/render/forceLanguage/fieldForcePresentation.js` (routed from `src/render/vfx.js`), not Massline cable presentation. |
...
| `field_skim` (data-defined) | `shield-response` | `src/data/fields.js` defines it; the dedicated sheet presentation (parallel intake banks, inward scoops) lives in `src/render/forceLanguage/` since 2026-09-16 — see [`VFX_FORCE_LANGUAGE_STANDARD_2026-09-16.md`](./VFX_FORCE_LANGUAGE_STANDARD_2026-09-16.md). Generic field fallback is still not an accepted sheet recipe. |
