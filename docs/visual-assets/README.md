...
|---|---|
| Blender/glTF/GLB ship, station, place, prop, or VFX-support geometry | `VISUAL_ASSET_PRODUCTION_STANDARD.md`, then `.grok/skills/spaceface-blender-material-truth/SKILL.md` for every form or surfacing change; **and** `ADVANCED_MODEL_TECHNIQUE_CONTRACT.md` plus `MODEL_ADVERSARIAL_REVIEW_WORKFLOW.md` (five-plus full-job cycles, three valid stills, subagent reviews, cleanup). Tier C/D may group a repeated manufactured family |
