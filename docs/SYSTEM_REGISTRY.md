...
>
> Generated: 2026-07-14. Live/legacy note: `flight` and `ai` slots are flag-selected
...
|---|---|---|---|---|---|
| `input` | `systems/input.js` | 645 | 1 | 0 | `ui:setCourse`×1 |
...
| `scanner` | `systems/scanner.js` | 980 | 15 | 7 | `contactHail:availability`×2, `scan:pulse`×1, `scan:weakPoint`×1 |
...
| `flight` | `systems/flightV3.js` (+ legacy) | 1171 | 7 | 4 | `ship:boostStop`×2, `ship:boostStart`×1, `ship:dash`×1 |
| `cruise` | `systems/cruise.js` | 142 | 4 | 4 | `cruise:engaged`×1, `cruise:charging`×1, `cruise:snared`×1 |
| `aiPorts` | `systems/aiPorts.js` | 1105 | 1 | 0 | `ai:encounterCommand`×1 |
| `tumbleStates` | `systems/tumbleStates.js` | 194 | 4 | 2 | `massline:tumbleEnd`×1, `massline:tumbled`×1, `audio:cue`×1 |
| `weapons` | `systems/weapons.js` | 844 | 5 | 0 | `weapons:vent`×2, `combat:fire`×2, `combat:beamStop`×1 |
| `countermeasures` | `systems/countermeasures.js` | 249 | 2 | 0 | `countermeasure:deployed`×1, `audio:cue`×1 |
| `impulseCharges` | `systems/impulseCharges.js` | 458 | 10 | 0 | `audio:cue`×2, `charge:combo`×2, `charge:stuck`×1 |
...
| `physics` | `core/physics.js` | 1134 | 9 | 1 | `projectile:hit`×2, `dock:range`×2, `gate:range`×2 |
| `combat` | `systems/combat.js` | 779 | 20 | 7 | `camera:shake`×6, `player:death`×3, `economy:grantCredits`×3 |
...
| `tetherGameplay` | `systems/tetherGameplay.js` | 718 | 12 | 0 | `tether:releaseRated`×4, `tether:released`×2, `tether:broke`×2 |
...
| `masslineImpactDamage` | `systems/masslineImpactDamage.js` | 111 | 0 | 2 | — |
...
| `mining` | `systems/mining.js` | 1015 | 23 | 5 | `mining:yield`×3, `cargo:full`×2, `mining:start`×1 |
| `fieldDepletion` | `systems/fieldDepletion.js` | 278 | 2 | 2 | `fieldDepletion:changed`×1, `field:depletedChanged`×1 |
| `cargo` | `systems/cargo.js` | 211 | 4 | 4 | `cargo:changed`×1, `cargo:full`×1, `cargo:massSettled`×1 |
| `fragileCargo` | `systems/fragileCargo.js` | 218 | 1 | 2 | `cargo:fragileLost`×1 |
| `automation` | `systems/automation.js` | 1970 | 33 | 7 | `automation:offlineSummary`×5, `asset:deployed`×4, `economy:chargeCredits`×4 |
...
| `economy` | `systems/economy.js` | 1333 | 12 | 18 | `economy:tradeFailed`×2, `credits:changed`×2, `economy:tick`×1 |
| `intervention` | `systems/intervention.js` | 144 | 3 | 1 | `camera:shake`×1, `intervention:available`×1, `intervention:closed`×1 |
| `world` | `systems/world.js` | 2582 | 33 | 13 | `world:residency`×2, `economy:chargeCredits`×2, `jump:chargeAbort`×2 |
| `regionalEcology` | `systems/regionalEcology.js` | 397 | 0 | 0 | — |
| `encounterDirector` | `systems/encounterDirector.js` | 1459 | 15 | 18 | `encounter:resolved`×2, `encounter:telegraph`×1, `encounter:spawned`×1 |
...
| `sectorSim` | `systems/sectorSim.js` | 989 | 9 | 12 | `sectorsim:tick`×1, `sectorsim:fieldAdvanced`×1, `economy:applyTradePressure`×1 |
| `missions` | `systems/missions.js` | 4048 | 73 | 31 | `mission:updated`×37, `faction:repDelta`×5, `nav:waypoint`×4 |
| `careerOrigins` | `careers/origins/careerOrigins.js` | 1243 | 0 | 0 | — |
| `careerLadders` | `careers/ladders/careerLadders.js` | 346 | 0 | 0 | — |
| `liveCareerLadderBranches` | `careers/ladders/liveCareerLadderBranches.js` | 206 | 0 | 0 | — |
| `story` | `systems/story.js` | 1277 | 27 | 24 | `graffiti:show`×4, `hud:phase`×3, `endgame:ineligible`×2 |
...
| `drill` | `systems/drill.js` | 691 | 14 | 0 | `drill:warn`×3, `drill:start`×1, `drill:end`×1 |
| `claims` | `systems/claims.js` | 1186 | 28 | 3 | `economy:chargeCredits`×4, `audio:cue`×4, `economy:grantCredits`×2 |
| `bandRadio` | `systems/bandRadio.js` | 577 | 4 | 0 | `band:bearingRequest`×1, `band:bearingReceipt`×1, `band:status`×1 |
| `onboarding` | `systems/onboarding.js` | 1292 | 2 | 38 | `tutorial:say`×1, `tutorial:finished`×1 |
