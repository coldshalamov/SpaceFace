...
>
> Generated: 2026-07-24. Live/legacy note: `flight` and `ai` slots are flag-selected
...
|---|---|---|---|---|---|
| `input` | `systems/input.js` | 1125 | 0 | 0 | — |
| `autoTargetAssist` | `systems/autoTargetAssist.js` | 160 | 0 | 2 | — |
...
| `travelLanes` | `systems/travelLanes.js` | 542 | 0 | 1 | — |
| `flight` | `systems/flightV3.js` (+ legacy) | 1171 | 7 | 5 | `ship:boostStop`×2, `ship:boostStart`×1, `ship:dash`×1 |
| `cruise` | `systems/cruise.js` | 142 | 4 | 4 | `cruise:engaged`×1, `cruise:charging`×1, `cruise:snared`×1 |
| `aiPorts` | `systems/aiPorts.js` | 1149 | 1 | 0 | `ai:encounterCommand`×1 |
...
| `weapons` | `systems/weapons.js` | 1047 | 14 | 0 | `combat:fire`×3, `weapons:vent`×2, `presentation:vfxCue`×2 |
...
| `fields` | `systems/fields.js` | 506 | 13 | 4 | `audio:cue`×5, `fields:coneToggled`×2, `presentation:vfxCue`×2 |
| `planetRuntime` | `systems/planetRuntime.js` | 558 | 10 | 3 | `planet:plungeStage`×2, `planet:registered`×1, `planet:unregistered`×1 |
| `physics` | `core/physics.js` | 1227 | 9 | 1 | `projectile:hit`×2, `dock:range`×2, `gate:range`×2 |
...
| `tetherGameplay` | `systems/tetherGameplay.js` | 1285 | 21 | 2 | `tether:latchDenied`×5, `tether:releaseRated`×5, `tether:released`×3 |
...
| `masslineThreats` | `systems/masslineThreats.js` | 221 | 1 | 0 | `massline:threat`×1 |
| `masslineImpacts` | `systems/masslineImpacts.js` | 241 | 1 | 0 | `tether:whipImpact`×1 |
| `masslineThrow` | `systems/masslineThrow.js` | 719 | 7 | 1 | `audio:cue`×3, `massline:throw`×1, `presentation:vfxCue`×1 |
...
| `mining` | `systems/mining.js` | 1273 | 33 | 5 | `beam:denied`×4, `mining:yield`×3, `mining:start`×2 |
...
| `asteroidSites` | `systems/asteroidSites.js` | 1571 | 16 | 9 | `site:laneSpilled`×2, `worldSite:operationReceipt`×1, `worldSite:failureReceipt`×1 |
...
| `economy` | `systems/economy.js` | 1709 | 12 | 20 | `economy:tradeFailed`×2, `credits:changed`×2, `economy:tick`×1 |
...
| `npcJobsRuntime` | `systems/npcJobsRuntime.js` | 438 | 0 | 4 | — |
...
| `traffic` | `systems/traffic.js` | 1242 | 5 | 5 | `aiTrader:requestTrade`×1, `freight:arrival`×1, `economy:applyTradePressure`×1 |
...
| `masslineHud` | `ui/masslineHud.js` | 476 | 0 | 0 | — |
| `massSeedHud` | `ui/massSeedHud.js` | 199 | 0 | 0 | — |
| `fieldHud` | `ui/fieldHud.js` | 146 | 0 | 0 | — |
| `planetHud` | `ui/planetHud.js` | 133 | 0 | 0 | — |
