...
## THE DRAWER (REF 44-C as a place you can stand in front of)

There is a records annex on the second tier of the Tycho relay that anyone with a berth-fee receipt can walk into. The annex is not locked. The annex is not restricted. The annex is where REF 44-C lives. Not as a code on a screen. As a drawer.

The drawer is the third one on the left, second row, labeled with the code on a piece of tape that has yellowed and been re-taped twice. The drawer is a physical object in a room anyone can enter. The drawer contains the customs infractions Hale initialed. The drawer contains the contract authorizations Vale signed. The drawer contains the atmospheric allocation appeals the Pit never knew it had. The drawer contains the seal on the evidence that proves humanity is not alone. The drawer contains nineteen years of berth fees that funded nineteen years of air that didn't arrive. The drawer is full. The drawer has been full since year 7. Nobody re-taped the label because nobody needs to — the code is the code and the drawer is where the code goes and everyone who needs to file under the code knows which drawer it is without reading the tape.

The player can open the drawer. The drawer opens. The contents are papers. The papers are the filings. The filings are the crimes. The crimes are lawful. The drawer does not judge the papers. The drawer holds them. The drawer has been holding them for nineteen years. The drawer will hold them for nineteen more. The drawer is the place the system keeps the things it has decided not to resolve, and the system has decided not to resolve most things, and the drawer is the size it is because the system decided a long time ago how much it was going to refuse to resolve, and built a drawer for it, and put the drawer in a room anyone can enter, and labeled it with a code everyone uses, and never locked it, because locking it would be admitting the drawer holds something, and the drawer holds everything, and admitting that would require a bigger drawer.

The player who opens the drawer at B6 does not understand everything in it. The player who opens the drawer at B7, after reading the Kurtz figure's ledger, understands more. The player who opens the drawer on a second playthrough, after reading the operation page, understands that the drawer is also the dead-drop's backup medium and the mole's filing cabinet and the handler's cast list. The drawer is one object. The drawer holds every version of what the system has done. The system filed it all under the same code because the system understood, when it wrote the code, that all of it was one apparatus, and the apparatus needed a drawer, and the drawer needed to be unlocked, because the apparatus does not hide its work — the apparatus files its work in a place anyone can find and trusts that no one will look, because looking costs a thing the system has arranged for no one to have: the time, and the reason, and the hands that don't shake.
