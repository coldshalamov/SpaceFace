...
> player chose. The system filed the choice. The system kept the columns. The next
> cycle's ledger opens with the prior cycle's balance carried forward. That is the
> only kind of after this world has.

---

## The principle: endings are positions, not exits

The five choices at Ashfall Reach are not five doors out of the system. They are five positions the player takes up *inside* the system. The system does not grant exits (Choice C proves this — the wormhole loops, it does not leave). What the system grants is a filing: the player's callsign moves from one column to another, the routing changes, the work changes, the system does not change.

The post-ending game is the same game on a different row of the same ledger. The contracts still post. The graffiti still knows. The air in the Pit is still chalk. The player's relationship to these facts is what the ending determined. The facts themselves were not asked.

---

## CHOICE A — THE CLEAN UNIFORM: the world after the badge

**The player became the institution.** The record was expunged. The tags stabilized. The HUD shows clean IFF, accurate rep, correct filings. Everything looks clean. Everything IS clean, in the sense the system uses the word: the filings match, the columns balance, the signatures are unbroken.

**What changed:** the player signs documents now. The player's name appears above Vale's on authorizations. The player's signature makes things legal. The first offered mission is a ROUTINE CUSTOMS OPERATION. The seized cargo belonged to someone the player worked with. The manifest already assigns it to a private buyer. The player signs. The signature is correct. The signature is the crime. The player is now the person whose initials make the second fine lawful, and the initials are load-bearing, and the player knows this, and the player signs anyway, because signing is the job and the job is what the player chose and the choice is filed and the filing is correct.
...
## CHOICE B — THE SAME SILENCE: the world after the vanishing

**The player became the channel.** Identity disappeared from public records. Name stopped appearing on boards. ROUTING OVERSIGHT jobs unlock — logistics only, no direct action. The cut arrives automatically. The player profits from every war, every shortage, every transit that goes sideways. The player is never on the kill feed.

**What changed:** the player sees the freight moving. The player cannot see themselves. The player's callsign shows up in someone else's graffiti. The HUD stopped showing the player's own rep delta — the player can see the system's rep, can see the factions' rep, cannot see their own, because the player's own rep is now a routing coordinate, not a reputation. Coordinates don't have reputation. Coordinates have throughput.
...
## CHOICE C — THE ONLY HONEST OPTION: the world after the loop

**The player jumped without destination. The jump completed. The player emerged at the Pit's docking bay.** The date is the same as the day the player left. The board shows one entry: CONTRACT 47-A — OPEN / PAYMENT: PENDING. The wormhole is not an exit. The wormhole is the system telling the player they are already in the only place they were ever going to be.

**What changed:** nothing. The campaign reset. The HUD loaded normally. The graffiti at the airlock is the graffiti from the first day. The player has the same ship, the same transponder, the same manifest — including the fragment and the coordinates, if found. The player is running Contract 47-A again. The weight on accept will be 12.4t. The weight on delivery will be 0kg. The HUD will call it STABLE LOAD. The graffiti will say THEY KNEW THE MASS. The player knows. The player knew last time. The player runs it anyway.
...
## CHOICE E — THE NEXT RUN: the world after the acceptance

**The player declined all four.** The player approached the station one final time. A courier who was not there before said the line. The player accepted the payout for 47-A: +1,200cr. CONTRACT 47-A: STATUS: CLOSED. Immediately: CONTRACT 47-B: STATUS: PENDING.

**What changed:** Contract 47-A is closed. This is the only ending where 47-A closes. The payment was 1,200 credits. Meager. Standard transaction. The player is not a Concord administrator. Not a shadow coordinator. Not a witness. Not a loop. The player is a tired pilot in a cheap hull who needs reactor fuel.
...
Each ending's **post-ending actor + durable premise** maps directly to a `postEndingReplayChains.js` chain's `actor` and `durablePremise` fields. The existing chains (CHANNEL ZERO, ECHO 47, THE CURRENT DESK, BROKER DESK 47-B) already reflect this mapping. This doc is the narrative authority for those chains — the source of truth for what the world is after each choice, so that downstream agents authoring new post-ending content have a consistent bed.

The five "what changed / what didn't change" blocks are the world-state contract: any post-ending mission must honor the ending's position (the player's role, what the player can/cannot see, whether 47-A is closed). The chains that violate this contract break the ending's meaning. The chains that honor it extend it.

---

> **Canon refs:** `ENDGAME-B7-REDESIGN.md` (the five choices), `STORY-STRUCTURE.md#thread-b-in-the-endgame` (what Thread B does to each choice), `chapter-07-deep-reach.md#the-five-choices` (the choices as enacted), `DOSTOYEVSKY-LAYER.md` (each choice as a Dosto resolution), `LECARRE-LAYER.md#iii-the-drummer-girl-thread-staged-reveal` (the operation's relationship to each choice), `postEndingReplayChains.js` (the existing chains this doc authorizes).
