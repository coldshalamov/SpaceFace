# Chapter 02 — First Blood (B2)

> **Beat theme:** `crime_without_punishment_system_stolen`. The civilian tag flickers 0.5s; the kill feed overwrites it BOUNTY COLLECTED. The collapse that should come doesn't, because the system logged the killing as a transaction. The absence is the wound. (The central Dosto→SpaceFace bridge.)
> **HUD phase:** 1 (Protective). The lie reads as a sensor lag.

---

## THE BOARD

The contract is on the Cinder board. The board is the only legitimate market in the sector. The contract reads:

```
BOARD: PIRATE INTERDICTION — SECTOR INTERFERENCE. TARGET: UNKNOWN. STANDARD FEE.
...
The target vessel is at the coordinates the board provided. The tag reads UNKNOWN. The player closes to weapons range.

```
HUD: TARGET ACQUIRED. TAG: UNKNOWN.
HUD: IFF: —
```

The IFF line is blank. The blank is not an error. The blank is the system declining to resolve an identity the contract did not require resolved. The contract required HOSTILE. The system provided HOSTILE. The IFF is the field the system did not fill because no one asked.

The player fires. The target's shields drop. The hull breaches. There is a moment — half a second, maybe less — where the tag flickers. The HUD, reading the target's transponder as the hull fails and the encryption drops, briefly resolves an identity it had declined to resolve before.

```
HUD: IFF: CIVILIAN VESSEL — REGISTERED.
```

The tag is on screen for half a second. Then the kill feed updates.

```
KILLFEED: BOUNTY COLLECTED.
HUD: TARGET NEUTRALIZED.
```

The civilian tag is gone. The kill feed does not preserve it. The kill feed prints the current status. The current status is BOUNTY COLLECTED. The prior status — the half-second during which the target was a registered civilian vessel — is in no log the player can open. It happened. The system has one version now. The version is BOUNTY COLLECTED.

## THE GRAFFITI (post-action, at the dock)

The player docks to collect. The bounty is paid. The payment is correct. The board marks the contract closed. The system is satisfied. The system has processed the killing the way it processes the cargo: the weight matched, the seal was Concord, the filing is correct, the file is closed.

The graffiti is in the airlock. It was not there before. It is there now.

```
GRAFFITI: THEY WERE CARRYING MEDICINE.
```

The line is the only voice in the system that refuses the refile. The kill feed said BOUNTY COLLECTED. The graffiti says what the target was carrying. The two filings disagree. The kill feed is the official record. The graffiti is testimony. Testimony has no standing. The graffiti stays anyway, the way graffiti stays — until maintenance paints over it, and then it comes back, in the same hand, in the same place, because everyone who reads it knows how to finish the sentence.

The player does not know what the medicine was. The player may not connect the graffiti to the kill — the kill was a pirate interdiction, the bounty was collected, the contract is closed. The graffiti uses no name. The graffiti uses no date. The graffiti states a fact about cargo. The cargo is gone. The wreckage is salvaged or it isn't. The graffiti doesn't care. The graffiti is for the next person who docks.

## THE KILL FEED AND THE COLLAPSE THAT DIDN'T COME

The player has killed someone. The system logged the killing as a bounty collected. The payment cleared. The contract closed. The civilian tag is in no log. The system has done everything correctly. The player has done everything correctly. The two correct filings sit on top of the half-second where the target was a registered civilian vessel, and the half-second is gone, and the system runs fine.
...
|------|----|-----------------|
| character | `pc_wren` | the killer; the working-register silence |
| character | `npc_rook` | the booth that double-billed the Elroy tag (player doesn't see) |
| character | `npc_elroy` | the target; the report; the unpunished crime's evidence |
| character | `npc_vale` | the intermediary who paid the second tag (player doesn't see) |
| character | `npc_kurtz` | the ledger entry that will name Elroy at B7 (player doesn't see) |
| world | `world_charon` | Cinder, the bounty board |
| chapter-beat | B2 | first-blood; crime-without-punishment-system-stolen |
...
> **Canon refs:** `STORY-SPINE-NARRATIVE-OVERLAY.md#B2`, `NPCs-CANONICAL.md#ROOK` (Elroy double-bill), `NPC-ECOLOGY.md#the-structural-reveal` (Rook + Elroy), `PROTAGONIST.md#voice` (cornered register), `DOSTOYEVSKY-LAYER.md#crime-without-punishment-system-stolen`, `LECARRE-LAYER.md#iii-the-drummer-girl-thread-staged-reveal` (the B2 row of the staged-reveal table).
