...
| SG-05 scenario/mission/dialogue DSL | Current master 47-A scenario contract | Schema skeleton accepted | `src/data/scenarios/47a.scenario.json` now declares the canonical 47-A actors, world facts, presentation event ids, proof metrics, eight beat windows, and four outcome branches. `npm run check:sg05` validates required beats/branches, reference integrity, critical SG-08 cue reservation, and immediate world-fact consequences for every branch. Full DSL runtime, localization, save-node migration, policy runner, content hot reload, and complete branch implementation remain pending. |
...
- SG-05 scenario skeleton gate: `npm run check:sg05`.
