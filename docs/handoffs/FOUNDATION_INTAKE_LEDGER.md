...
| SG-06 layered tactical AI | `origin/sg-06-layered-tactical-ai` at `c543cfb` and external completion report | Pending usable artifact | The refreshed branch is still not a complete SG-06 handoff: it adds only partial `src/ai` inspection/trace exports plus a marker system, with missing director/squad/perception/maneuver/SG-03 action-port modules, no fixtures, no acceptance script, and no handoff package. `npm run check:sg06` now gates this pending state. Do not wire AI-only damage, motion, or hidden-state paths. SG-06 may integrate after a real artifact is available and, for physical parity, after SG-02 lands. |
