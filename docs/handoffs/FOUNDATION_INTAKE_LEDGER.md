...
| SG-06 layered tactical AI | `SpaceFace-SG-06.zip` (`F10CC6B0...20A358D5`) | Production ports landed; live replacement gated | The five-layer director/squad/perception/utility/maneuver stack, SG-03 action adapter, inspection endpoint, trace ring, handoff doc, clean-room ledger, 100-seed acceptance harness, production-shaped `helpers.aiSensors`/`helpers.aiRoster`/`helpers.aiManeuver`/`helpers.aiEncounter` ports, the encounter-sink gate, and the live-shadow SG-03 ActionDef gate are integrated and gated by `npm run check:sg06`. Live replacement of the legacy FSM remains blocked until tacticalAI is proven in the real registry slot with active encounter ownership, legacy weapon-fire cleanup, physical convergence, Massline break telemetry, and same-milestone deletion of superseded intent paths. Do not wire AI-only damage, motion, or hidden-state paths. |
