...
| SG-08 presentation orchestration | `origin/commission/sg-08-presentation-orchestration` plus current master runtime bridge | Phase 0 cue runtime accepted with headless evidence | The received branch contributed the presentation cue schema/validator surface; current master now adds deterministic cue recipes plus a headless `presentationOrchestrator` that maps scenario, tether, shield-collapse, and subsystem-disable events to normalized `presentation:cue` records with recipe lanes, per-tick budgets, and dedupe suppression. `npm run check:presentation`, `npm run check:sg05`, and `node scripts/check-phase0-slice-contract.mjs` prove the 47-A cue contract, including two Phase 0 golden cues at 720 ticks and four semantic cues through `scavenger_arrival`. Render/VFX/audio/UI adapters, accessibility transforms, mix profiling, and visual/audio golden traces remain pending. |
| SG-05 scenario/mission/dialogue DSL | Current master 47-A scenario contract + runtime bridge | Phase 0 live contract + branch policies accepted with headless evidence | `src/data/scenarios/47a.scenario.json` now declares the canonical 47-A actors, world facts, presentation event ids, proof metrics, eight beat windows, and four outcome branches. `npm run check:sg05` validates required beats/branches, reference integrity, critical SG-08 cue reservation, immediate world-fact consequences for every branch, the `sf-sim 47a` runtime bridge, and a four-policy branch replay suite. The bridge loads the validated contract, emits deterministic `scenario:*` trace events, snapshots/saves/reloads scenario state, binds all eight required Phase 0 actors, proves beat progression through `scavenger_arrival`, records hostile fire from a scenario-bound scavenger inside the 90s proof window, and proves escape/surrender/destroy/deliver policies mutate immediate world facts while preserving branch resolution through save/load. Full DSL runtime, localization, content hot reload, AI-driven live actor migration, and richer failure/abandon/aftermath implementation remain pending. |
...
- SG-08 recipe/runtime cue gate: `npm run check:presentation`; 47-A cue replay: `npm run check:sg05` and `node scripts/check-phase0-slice-contract.mjs`.
