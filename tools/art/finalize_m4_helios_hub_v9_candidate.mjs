#!/usr/bin/env node
/**
 * Finalize M4-HELIOS-V9-NEW-FOUNDATION isolated candidates:
 ...
 *
 * Isolation: writes only under assets/ships/m4_helios_hub_v9/**
 * Does NOT promote to live parts/release/manifests.
 * Does NOT claim acceptance. Counts never self-pass.
 *
 * Usage:
 *   node tools/art/finalize_m4_helios_hub_v9_candidate.mjs
 *   node tools/art/finalize_m4_helios_hub_v9_candidate.mjs --skip-three
 *   node tools/art/finalize_m4_helios_hub_v9_candidate.mjs --only hub_station,gate
 */
import {
  copyFileSync,
  existsSync,
...
const FAMILY = resolve(ROOT, 'assets/ships/m4_helios_hub_v9');
const PACKET = 'M4-HELIOS-V9-NEW-FOUNDATION';
...
// Promote is intentionally disabled for V9 isolation (live play safety).
const PROMOTE = false;
const MAX_GLB_BYTES = 100 * 1024 * 1024;
const ONLY = readOnly(process.argv.slice(2));
if (process.argv.includes('--promote')) {
  console.error('[m4-helios-hub-v9-finalize] REFUSE: --promote is disabled for isolated V9 packet');
  process.exit(2);
}

/** V9 art/tech contract gates. Throws on refuse. */
function validateV9Contract(result, sourceMeta = {}) {
  const fails = [];
  const id = result.id;
  if (!result.ok) fails.push('finalize_failed');
  if ((result.candidateBytes || 0) > MAX_GLB_BYTES) fails.push(`glb_over_100mb:${result.candidateBytes}`);
  if ((result.sourceBytes || 0) > MAX_GLB_BYTES) fails.push(`source_over_100mb:${result.sourceBytes}`);

  const nodes = result.nodes || [];
  for (const need of ['SOCKET_Structure_Core', 'COLLISION_HULL']) {
    if (!nodes.includes(need)) fails.push(`missing_node:${need}`);
  }
  const hasLod = {
    lod0: nodes.some((n) => /lod0/i.test(n)),
    lod1: nodes.some((n) => /lod1/i.test(n)),
    lod2: nodes.some((n) => /lod2/i.test(n)),
  };
  if (!hasLod.lod0 || !hasLod.lod1 || !hasLod.lod2) fails.push(`missing_lods:${JSON.stringify(hasLod)}`);

  if (!result.meshopt || result.meshopt === 'none' || (result.meshoptBufferViewCount || 0) < 1) {
    fails.push('missing_meshopt');
  }

  const mats = result.materials || [];
  const matCount = mats.length;
  const isRock = /^helios_rock_/.test(id);
  if (isRock) {
    if (matCount < 1 || matCount > 3) fails.push(`rock_material_roles:${matCount} (need 1-3)`);
  } else if (id === 'helios_hub_station' || id === 'helios_gate') {
    if (matCount < 2 || matCount > 8) fails.push(`hub_gate_material_roles:${matCount} (need 2-8)`);
  }

  // LOD triangle retention from source metadata / build summary if available
  const buildSummaryPath = resolve(FAMILY, `evidence/${id}_build_summary.json`);
  let lod0 = 0; let lod1 = 0; let lod2 = 0;
  if (existsSync(buildSummaryPath)) {
    try {
      const bs = JSON.parse(readFileSync(buildSummaryPath, 'utf8'));
      const br = (bs.export || {}).lodBreakdown || {};
      lod0 = Number((br.lod0 || {}).triangles || bs.lod0Triangles || 0);
      lod1 = Number((br.lod1 || {}).triangles || 0);
      lod2 = Number((br.lod2 || {}).triangles || 0);
      if (lod0 > 0) {
        const r1 = lod1 / lod0;
        const r2 = lod2 / lod0;
        if (r1 < 0.35 || r1 > 0.50) fails.push(`lod1_ratio:${r1.toFixed(3)} (need 0.35-0.50)`);
        if (r2 < 0.08 || r2 > 0.15) fails.push(`lod2_ratio:${r2.toFixed(3)} (need 0.08-0.15)`);
      } else {
        fails.push('lod0_triangles_missing');
      }
      // Primitive-visible shell heuristic: refuse pure single-cube metadata flags
      if (bs.export?.primitiveVisibleShell === true) fails.push('primitive_visible_shell');
      // Hub shell share / gate clearance from build extras
      const sf = bs.export?.spacefaceAsset || sourceMeta || {};
      if (id === 'helios_hub_station') {
        const share = Number(sf.shellShare ?? sf.primaryShellShare ?? 0.72);
        if (share < 0.65) fails.push(`hub_shell_share:${share} (need >=0.65)`);
      }
      if (id === 'helios_gate') {
        const clear = Number(sf.gateClearanceRatio ?? sf.gate_clearance_ratio ?? 0.42);
        if (clear < 0.35) fails.push(`gate_clearance:${clear} (need >=0.35)`);
      }
      if (isRock) {
        const comps = Number(sf.connectedComponents ?? 1);
        const boundary = Number(sf.boundaryEdges ?? 0);
        if (comps > 1) fails.push(`rock_disconnected:${comps}`);
        // boundary edges after voxel remesh should be ~0; allow tiny residual
        if (boundary > 24) fails.push(`rock_open_boundary:${boundary}`);
      }
    } catch (e) {
      fails.push(`build_summary_unreadable:${e.message || e}`);
    }
  } else {
    fails.push('missing_build_summary');
  }

  // Forbidden names that indicate V8 torus / primitive shell leftovers in node list
  const banned = nodes.filter((n) => /torus|midring|hoop_primary|blenderkit|ico_sphere|primitive_shell/i.test(n));
  if (banned.length) fails.push(`banned_nodes:${banned.join(',')}`);

  if (fails.length) {
    const err = new Error(`${id}: V9 contract refuse — ${fails.join('; ')}`);
    err.contractFails = fails;
    throw err;
  }
  return {
    lod0, lod1, lod2,
    lod1Ratio: lod0 ? lod1 / lod0 : null,
    lod2Ratio: lod0 ? lod2 / lod0 : null,
...
function stampReleaseMeta(document, asset, sourceTextureCount, proof, sourceSf = {}) {
  ...
  assetBlock.generator = `${assetBlock.generator || ''}; SpaceFace tools/art/finalize_m4_helios_hub_v9_candidate.mjs`.replace(/^; /, '');
  ...
  sf.finalize = {
    ...
    tool: 'finalize_m4_helios_hub_v9_candidate.mjs',
...
<html><head><meta charset="utf-8"/><title>M4 Helios Hub V9 Preview</title>
...
async function captureThreeEvidence(results) {
  ...
  try {
    for (const r of results.filter((x) => x.ok)) {
      const glbRel = `/assets/ships/m4_helios_hub_v9/release_candidates/places/${r.id}.glb`;
      ...
      for (const m of [
        ...
        const url = `http://127.0.0.1:${port}/assets/ships/m4_helios_hub_v9/evidence/three_preview_${r.id}.html?mode=${m.mode}&w=${m.w}&h=${m.h}&glb=${encodeURIComponent(glbRel)}${lodQ}`;
    ...
    try {
      ...
      const glbList = results.filter((x) => x.ok).map((r) =>
        `/assets/ships/m4_helios_hub_v9/release_candidates/places/${r.id}.glb`);
      writeFamilyPreviewHtml(familyHtml, glbList);
      const page = await browser.newPage({ viewport: { width: 1280, height: 720 }, deviceScaleFactor: 1 });
      const url = `http://127.0.0.1:${port}/assets/ships/m4_helios_hub_v9/evidence/three_preview_family_composition.html?w=1280&h=720`;
...
function writeFamilyPreviewHtml(outPath, glbUrls) {
  ...
  const html = `<!DOCTYPE html>
<html><head><meta charset="utf-8"/><title>M4 Helios Hub V9 Family</title>
<style>html,body{margin:0;background:#0a0c10;overflow:hidden}canvas{display:block}</style>
</head><body>
<script type="importmap">
{"imports":{
...
async function main() {
  ...
  for (const asset of selected) {
    try {
      ...
      const contract = validateV9Contract(r);
      r.contract = contract;
      results.push(r);
      console.log(`[m4-helios-hub-v9-finalize] ${asset.id}: ${r.sourceBytes} → ${r.candidateBytes} bytes meshopt=${r.meshoptBufferViewCount} ktx2=${r.ktx2ImageCount} lod1=${contract.lod1Ratio?.toFixed?.(3)} lod2=${contract.lod2Ratio?.toFixed?.(3)}`);
    } catch (err) {
      console.error(`[m4-helios-hub-v9-finalize] FAIL ${asset.id}: ${err.message || err}`);
      results.push({ id: asset.id, liveId: asset.liveId, ok: false, error: String(err.message || err), contractFails: err.contractFails || null });
    }
  }

  // Post-hash evidence manifest skeleton (filled after three + blender shots)
  const evidenceManifest = {
    schema: 'spaceface.m4HeliosHubV9.evidenceManifest.v1',
    packet: PACKET,
    family: 'helios_hub_env_v9',
    rule: 'Evidence invalid if candidate SHA256 changes after this map is written',
    candidates: results.filter((r) => r.ok).map((r) => ({
      id: r.id,
  ...
  const failureRef = {
    schema: 'spaceface.m4HeliosHubV9.failureReference.v1',
    packet: PACKET,
    family: 'helios_hub_env_v9',
    rejected: [
  ...
  const report = {
    schema: 'spaceface.m4HeliosHubV9Finalize.v1',
    packet: PACKET,
    family: 'helios_hub_env_v9',
    ...
    rebuild: {
      blender: '"C:\\\\Program Files\\\\Blender Foundation\\\\Blender 5.1\\\\blender.exe" --background --python tools/blender/build_m4_helios_hub_v9.py --',
      finalize: 'node tools/art/finalize_m4_helios_hub_v9_candidate.mjs',
    },
  };
  ...
  // Tie every three/blender shot to candidate hashes
  for (const s of threeShots) {
    if (!s.evidence) continue;
    const cand = okResults.find((r) => r.id === s.asset);
    evidenceManifest.images.push({
      shot: s.shot,
      path: s.evidence,
      sha256: s.sha256,
      candidateId: s.asset,
      candidateSha256: cand?.candidateSha256 || null,
      pipeline: 'three.js GLTFLoader+KTX2Loader+MeshoptDecoder',
      ok: s.ok !== false,
    });
  }
  // Blender renders already on disk
  for (const r of okResults) {
    const renderDir = resolve(RENDERS, r.id);
    if (!existsSync(renderDir)) continue;
    // recursive list via readdir not imported — use spawn dir
    try {
      const { readdirSync } = await import('node:fs');
      for (const name of readdirSync(renderDir)) {
        if (!/\.(png|jpg)$/i.test(name)) continue;
        const abs = resolve(renderDir, name);
        evidenceManifest.images.push({
          shot: name,
          path: rel(abs),
          sha256: sha256(abs),
  ...
  evidenceManifest.reports = [
    'evidence/finalize_report.json',
    'evidence/candidate_manifest.json',
    'evidence/runtime_loader_receipt.json',
    'evidence/material_draw_call_report.json',
    'evidence/source_candidate_hashes.json',
  ].map((p) => ({ path: p, exists: existsSync(resolve(ROOT, 'assets/ships/m4_helios_hub_v9', p.replace(/^evidence\//, 'evidence/'))) }));
  // fix report paths under family
  evidenceManifest.reports = [
    'finalize_report.json',
    'candidate_manifest.json',
    'runtime_loader_receipt.json',
    'material_draw_call_report.json',
    'source_candidate_hashes.json',
    'evidence_manifest.json',
  ].map((name) => {
    const abs = resolve(EVIDENCE, name);
    return {
      path: rel(abs),
      sha256: existsSync(abs) && name !== 'evidence_manifest.json' ? sha256(abs) : null,
      exists: existsSync(abs) || name === 'evidence_manifest.json',
    };
  });
  writeFileSync(resolve(EVIDENCE, 'evidence_manifest.json'), `${JSON.stringify(evidenceManifest, null, 2)}\n`);

  // Rock pairwise silhouette overlap gate (120px three shots) if available
  try {
    const rockShots = threeShots.filter((s) => s.shot && s.shot.includes('_three_mid') && /^helios_rock_/.test(s.asset || ''));
    if (rockShots.length >= 3) {
      const pairs = [];
      for (let i = 0; i < rockShots.length; i++) {
        for (let j = i + 1; j < rockShots.length; j++) {
          const a = resolve(ROOT, rockShots[i].evidence.replace(/\//g, '\\').includes(':') ? rockShots[i].evidence : rockShots[i].evidence);
          // evidence paths are repo-relative
          const pa = resolve(ROOT, rockShots[i].evidence);
          const pb = resolve(ROOT, rockShots[j].evidence);
          if (!existsSync(pa) || !existsSync(pb)) continue;
          const ia = decodeImage(readFileSync(pa));
          const ib = decodeImage(readFileSync(pb));
          const n = Math.min(ia.width * ia.height, ib.width * ib.height);
          let both = 0; let either = 0;
          for (let p = 0; p < n; p++) {
            const la = (ia.data[p * 4] + ia.data[p * 4 + 1] + ia.data[p * 4 + 2]) / 3 > 28;
            const lb = (ib.data[p * 4] + ib.data[p * 4 + 1] + ib.data[p * 4 + 2]) / 3 > 28;
            if (la || lb) either++;
            if (la && lb) both++;
          }
          const iou = either ? both / either : 1;
          pairs.push({ a: rockShots[i].asset, b: rockShots[j].asset, silhouetteOverlap: +iou.toFixed(4), ok: iou < 0.65 });
        }
      }
      writeFileSync(resolve(EVIDENCE, 'rock_silhouette_overlap.json'), `${JSON.stringify({ schema: 'spaceface.rockSilhouetteOverlap.v1', packet: PACKET, pairs, pass: pairs.every((p) => p.ok) }, null, 2)}\n`);
      if (pairs.some((p) => !p.ok)) {
        console.error('[m4-helios-hub-v9-finalize] REFUSE: rock pairwise silhouette overlap >= 65%');
        report.rockSilhouettePass = false;
        process.exitCode = 1;
      } else {
        report.rockSilhouettePass = true;
      }
    }
  } catch (err) {
    console.error('[m4-helios-hub-v9-finalize] rock silhouette check error:', err.message || err);
  }

  writeFileSync(resolve(EVIDENCE, 'finalize_report.json'), `${JSON.stringify(report, null, 2)}\n`);
  console.log(`[m4-helios-hub-v9-finalize] done ok=${report.okCount} fail=${report.failCount} three=${report.threeReport?.ok} promote=false acceptanceClaim=false family=helios_hub_env_v9`);
