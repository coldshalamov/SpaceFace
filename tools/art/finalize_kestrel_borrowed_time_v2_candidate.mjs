#!/usr/bin/env node
/** Isolated Kestrel Borrowed Time V2 Meshopt/KTX2 finalizer + Three.js proof. */
import {
...
const SOURCE = resolve(FAMILY, 'source/wholeships/kestrel_borrowed_time_v2.glb');
...
function stamp(document, sourceTextures, proof) {
  ...
  asset.generator = `${asset.generator || ''}; SpaceFace finalize_kestrel_borrowed_time_v2_candidate.mjs`;
  asset.extras = {
    ...(asset.extras || {}),
    assetId: 'SF_WHOLESHIP_KESTREL_BORROWED_TIME_V2',
    partId: 'wholeship_kestrel_borrowed_time_v2',
    spacefaceAsset: {
      ...(asset.extras?.spacefaceAsset || {}),
      contractVersion: 1,
      assetId: 'SF_WHOLESHIP_KESTREL_BORROWED_TIME_V2',
      partId: 'wholeship_kestrel_borrowed_time_v2',
      slot: 'hull', category: 'wholeships', family: 'kestrel_borrowed_time_v2',
      ...
      finalize: { tool: 'finalize_kestrel_borrowed_time_v2_candidate.mjs',
...
async function capture() {
  ...
  const html = resolve(EVIDENCE, 'three_preview_kestrel_v2.html'); writeFileSync(html, previewHtml());
  ...
  try {
    for (const item of [
      ...
      const meta = await page.evaluate(() => window.__SF); const target=resolve(THREE,`kestrel_v2_three_${name}.png`);
      await page.screenshot({path:target}); const dev=resolve(DEVSHOTS,`kestrel_v2_three_${name}.png`); copyFileSync(target,dev);
