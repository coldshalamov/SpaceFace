#!/usr/bin/env node
/**
 * Finalize PROFESSIONAL-HELIOS-HUB-VISUAL-V2-GROK-001 isolated candidates:
 ...
 *
 * Isolation: writes only under assets/ships/m4_helios_hub_v2/**
 ...
 *   node tools/art/finalize_m4_helios_hub_v2_candidate.mjs --skip-three
 *   node tools/art/finalize_m4_helios_hub_v2_candidate.mjs --only hub_station,gate
 */
import {
  copyFileSync,
  existsSync,
...
const FAMILY = resolve(ROOT, 'assets/ships/m4_helios_hub_v2');
const PACKET = 'PROFESSIONAL-HELIOS-HUB-VISUAL-V2-GROK-001';
...
// Promote is intentionally disabled for V2 isolation (live play safety).
const PROMOTE = false;
const ONLY = readOnly(process.argv.slice(2));
if (process.argv.includes('--promote')) {
  console.error('[m4-helios-hub-v2-finalize] REFUSE: --promote is disabled for isolated V2 packet');
  process.exit(2);
}
...
function stampReleaseMeta(document, asset, sourceTextureCount, proof, sourceSf = {}) {
  ...
  assetBlock.generator = `${assetBlock.generator || ''}; SpaceFace tools/art/finalize_m4_helios_hub_v2_candidate.mjs`.replace(/^; /, '');
  ...
  sf.finalize = {
    ...
    tool: 'finalize_m4_helios_hub_v2_candidate.mjs',
...
function promoteToLive() {
  throw new Error('promoteToLive disabled for isolated V2 packet');
}

function runReleaseBuild() {
  return { ok: false, error: 'release rebuild disabled for isolated V2 packet' };
}
...
<html><head><meta charset="utf-8"/><title>M4 Helios Hub V2 Preview</title>
...
async function captureThreeEvidence(results) {
  ...
  try {
    for (const r of results.filter((x) => x.ok)) {
      const glbRel = `/assets/ships/m4_helios_hub_v2/release_candidates/places/${r.id}.glb`;
      ...
      for (const m of [
        ...
        const url = `http://127.0.0.1:${port}/assets/ships/m4_helios_hub_v2/evidence/three_preview_${r.id}.html?mode=${m.mode}&w=${m.w}&h=${m.h}&glb=${encodeURIComponent(glbRel)}`;
...
async function main() {
  ...
  for (const asset of selected) {
    try {
      ...
      results.push(r);
      console.log(`[m4-helios-hub-v2-finalize] ${asset.id}: ${r.sourceBytes} → ${r.candidateBytes} bytes meshopt=${r.meshoptBufferViewCount} ktx2=${r.ktx2ImageCount}`);
    } catch (err) {
      console.error(`[m4-helios-hub-v2-finalize] FAIL ${asset.id}: ${err.message || err}`);
  ...
  const failureRef = {
    schema: 'spaceface.m4HeliosHubV2.failureReference.v1',
    packet: PACKET,
    rejected: [
  ...
  const report = {
    schema: 'spaceface.m4HeliosHubV2Finalize.v1',
    packet: PACKET,
    ...
    threeReport: threeReport.ok
      ...
      : threeReport,
    rebuild: {
      blender: '"C:\\\\Program Files\\\\Blender Foundation\\\\Blender 5.1\\\\blender.exe" --background --python tools/blender/build_m4_helios_hub_v2.py --',
      finalize: 'node tools/art/finalize_m4_helios_hub_v2_candidate.mjs',
    },
  };
  ...
  console.log(`[m4-helios-hub-v2-finalize] done ok=${report.okCount} fail=${report.failCount} promote=false acceptanceClaim=false`);
