"""PQ-050.16 Helios lark MTX builder. Hitch untouched. --mtx-cycle N writes cycle stills."""
...
def role_maps(role, rgb, size=TEX, prefix=None):
    """Unique Helios lark maps. Pale civilian courier, not a Reach tint."""
    ...
    base = write_pixels(f"lark_{prefix}_basecolor", albedo, size, "sRGB")
    orm_img = write_pixels(f"lark_{prefix}_orm", orm, size, "Non-Color")
    nrm_img = write_pixels(f"lark_{prefix}_normal", nrm, size, "Non-Color")
...
def create_materials():
    specs = {
        "Material_Hull": ((0.55, 0.52, 0.46), 0.04, 0.48, "hull", 0.08, None),
        "Material_Armor": ((0.48, 0.46, 0.40), 0.06, 0.46, "armor", 0.04, None),
        "Material_Mechanical": ((0.32, 0.32, 0.32), 0.72, 0.34, "mechanical", 0.0, None),
        "Material_Accent": ((0.72, 0.42, 0.16), 0.04, 0.44, "warning", 0.04, None),
        "Material_Warning": ((0.78, 0.58, 0.12), 0.04, 0.46, "warning", 0.04, None),
...
def sockets():
    ...
    return {
        "SOCKET_Weapon_Front": (5.3, 0.0, 0.25),
        "SOCKET_Mining_Front": (5.9, 0.0, -0.15),
        "SOCKET_Engine_Main": (-5.3, 0.0, 0.08),
        "SOCKET_Trail_Main": (-5.7, 0.0, 0.08),
        "SOCKET_Trail_Port": (-5.5, -0.91, 0.08),
        "SOCKET_Trail_Starboard": (-5.5, 0.91, 0.08),
        "SOCKET_Utility_Dorsal": (0.6, 0.0, 1.17),
        "SOCKET_Cargo_Ventral": (-0.4, 0.0, -0.77),
        "SOCKET_Camera_Focus": (0.8, 0.0, 0.25),
        "SOCKET_RCS_Port": (-1.2, -1.50, 0.15),
        "SOCKET_RCS_Starboard": (-1.2, 1.50, 0.15),
    }
...
def build_lod(lod, mats):
    collection = bpy.data.collections.new(f"LARK_LOD{lod}")
    ...
    root = add_empty(f"LARK_LOD{lod}_ROOT", (0, 0, 0), collection)
    root["spacefaceAsset"] = {
        "assetId": "SF_HELIOS_LARK_V1", "partId": "helios_lark_production_v1",
    ...
    cover_loft_with_plates("LarkSkin", [
        (6.30, 0.24, 0.20, 0.06),
        (3.80, 0.80, 0.48, 0.10),
        (1.20, 1.26, 0.58, 0.08),
        (-1.20, 1.32, 0.62, 0.08),
    ...
    loft_shell("Cabin_P", [
        (2.40, -0.18, -0.62, 0.08, 0.48),
        (1.00, -0.28, -0.82, 0.12, 0.62),
        (-0.80, -0.22, -0.68, 0.04, 0.38),
    ...
    add_box("TransomPlate", (-5.98, 0.0, 0.04), (0.04, 0.38, 0.24), armor, collection, 0.003)
    add_thin_canopy("Canopy", 2.05, 0.0, 0.68, 1.35, 0.32, 0.26, mats, collection)

    for sign, side in ((-1, "Port"), (1, "Starboard")):
        loft_from_rings(f"Winglet_{side}", [
            wing_station(0.55, 1.18 * sign, 0.04, 1.35, 0.16),
            wing_station(-0.15, 1.75 * sign, 0.02, 1.05, 0.10),
            wing_station(-0.85, 2.15 * sign, 0.00, 0.68, 0.06),
        ], hull, collection, 0.004, cap=True)
        add_box(f"WingRoot_{side}", (0.25, 1.18 * sign, 0.06), (0.55, 0.12, 0.08), mech, collection, 0.003)
    ...
    loft_from_rings("TailFin", [
        wing_station(-4.05, 0.0, 0.38, 1.05, 0.07),
        [
            (-3.75, 0.0, 0.50), (-4.05, 0.0, 0.98), (-4.75, 0.0, 1.10), (-5.15, 0.0, 0.92),
            (-5.25, 0.0, 0.66), (-4.85, 0.0, 0.44), (-4.35, 0.0, 0.34), (-3.95, 0.0, 0.36),
        ],
    ], armor, collection, 0.003, cap=True)
    add_service_hatch("Dorsal", (-0.85, 0.0, 0.58), mats, collection, sx=0.26, sy=0.16)
    add_box("CargoDoor", (-0.2, 0.0, -0.48), (0.55, 0.32, 0.03), armor, collection, 0.003)
    add_box("CanopyBrow", (2.65, 0.0, 0.82), (0.12, 0.26, 0.05), armor, collection, 0.002)
    add_box("CanopyRailP", (2.05, -0.30, 0.78), (0.55, 0.02, 0.04), armor, collection, 0.002)
    add_box("CanopyRailS", (2.05, 0.30, 0.78), (0.55, 0.02, 0.04), armor, collection, 0.002)
    add_box("IntakeLip", (6.15, 0.0, 0.04), (0.08, 0.14, 0.08), mech, collection, 0.002)
    add_box("StripeP", (0.4, -1.05, 0.18), (1.8, 0.012, 0.008), accent, collection, 0.001)
    add_box("StripeS", (0.4, 1.05, 0.18), (1.8, 0.012, 0.008), accent, collection, 0.001)
    add_box("CargoRim", (-0.2, 0.0, -0.44), (0.62, 0.38, 0.02), mech, collection, 0.002)
    add_box("AftHatch", (-2.4, 0.28, 0.48), (0.22, 0.14, 0.012), armor, collection, 0.002)
    add_box("WingFenceP", (-0.35, -1.75, 0.10), (0.28, 0.010, 0.08), armor, collection, 0.002)
    add_box("WingFenceS", (-0.35, 1.75, 0.10), (0.28, 0.010, 0.08), armor, collection, 0.002)
    add_box("Keel", (-1.0, 0.0, -0.52), (2.4, 0.08, 0.03), armor, collection, 0.002)
    add_box("TransomBraceP", (-5.75, -0.22, 0.16), (0.14, 0.04, 0.10), mech, collection, 0.002)
    add_box("TransomBraceS", (-5.75, 0.22, 0.16), (0.14, 0.04, 0.10), mech, collection, 0.002)
    add_box("NoseCap", (6.35, 0.0, 0.06), (0.12, 0.12, 0.08), armor, collection, 0.002)
    add_box("DorsalSpine", (-0.2, 0.0, 0.56), (1.8, 0.05, 0.02), armor, collection, 0.002)
    add_box("RegMark", (3.4, 0.42, 0.38), (0.35, 0.08, 0.008), warning, collection, 0.001)
    add_box("WingRootFairP", (0.15, -1.12, 0.02), (0.35, 0.08, 0.06), mech, collection, 0.002)
    add_box("WingRootFairS", (0.15, 1.12, 0.02), (0.35, 0.08, 0.06), mech, collection, 0.002)
    add_panel_seams("Hull", [2.6, 0.6, -1.2, -3.2], 0.85, 0.48, mech, collection)

    if lod == 0:
        add_curve_hose("Hose_Main", [(0.4, 0.35, 0.08), (-2.6, 0.22, 0.10), (-5.4, 0.12, 0.08)], mech, collection, 0.010)
    if CYCLE >= 6 and lod <= 1:
        boolean_cut(hull_obj, "TransomRecess", (-5.85, 0.0, 0.08), (0.12, 0.26, 0.16))
    if CYCLE >= 7:
        add_box("CanopyMullion", (1.55, 0.0, 0.68), (0.016, 0.20, 0.06), armor, collection, 0.002)
        if lod <= 1:
            for i in range(4):
                add_box(f"SpineRad_{i}", (-1.55 + i * 0.16, 0.0, 0.42), (0.010, 0.12, 0.10), mech, collection, 0.001)
    if CYCLE >= 8:
        add_box("DriveBankBand", (-5.15, 0.0, 0.08), (0.045, 0.28, 0.12), armor, collection, 0.003)
    if CYCLE >= 9:
        add_box("ChineCapP", (0.95, -0.72, 0.06), (0.85, 0.024, 0.028), armor, collection, 0.003)
        add_box("ChineCapS", (0.95, 0.72, 0.06), (0.85, 0.024, 0.028), armor, collection, 0.003)
    if CYCLE >= 10:
        add_box("PatchTile2", (0.75, 0.22, 0.58), (0.14, 0.07, 0.006), armor, collection, 0.002)
        add_box("CanopyStayP", (1.25, -0.10, 0.52), (0.12, 0.010, 0.010), mech, collection, 0.002)
        add_box("CanopyStayS", (1.25, 0.10, 0.52), (0.12, 0.010, 0.010), mech, collection, 0.002)
    ...
    bm = bmesh.new()
    for point in [
        (6.3, 0, 0.1), (1.0, -1.4, 0.5), (1.0, 1.4, 0.5),
        (-5.8, -0.6, 0.2), (-5.8, 0.6, 0.2),
        (0.2, -2.2, 0.1), (0.2, 2.2, 0.1),
        (2.0, 0, 0.9),
    ]:
        bm.verts.new(point)
...
def export_lod(collection, lod):
    out = FAMILY / "source" / "wholeships" / f"helios_lark_production_v1_lod{lod}.glb"
...
def render_cycle(collection):
    ...
    camera = setup_studio()
    out = FAMILY / "evidence" / "helios_lark" / "cycles" / f"cycle_{CYCLE:02d}"
...
def main():
    ...
    for lod in (0, 1, 2):
        ...
        if report["hullTriangles"] < 800:
            raise RuntimeError(f"helios_lark lod{lod} hull {report['hullTriangles']} < 800")
        collections.append(collection)
        reports.append(report)
    stills = render_cycle(collections[0])
    report = {
        "schema": "spaceface.heliosLarkMtx.cycle.v1",
        "shipId": "helios_lark",
        "cycle": CYCLE,
        "lods": reports,
        "stills": str(stills.relative_to(FAMILY)).replace("\\", "/"),
    }
    (FAMILY / "evidence" / "helios_lark").mkdir(parents=True, exist_ok=True)
    (FAMILY / "evidence" / "helios_lark" / f"cycle_{CYCLE:02d}.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
