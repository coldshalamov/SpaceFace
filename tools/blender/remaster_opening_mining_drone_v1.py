#!/usr/bin/env python3
"""Build a non-promoting, gameplay-contract-preserving mining drone candidate.
...
def build_lod(lod:int,mats,root) -> None:
    ...
    tooth_count=10 if lod==0 else 6 if lod==1 else 4
    for index in range(tooth_count):
        angle=math.tau*index/tooth_count
        y=math.cos(angle)*0.54
        z=0.56+math.sin(angle)*0.54
        box(f"CutterTooth_{index}",(0.64,0.16,0.12),(3.74,y,z),(angle,0,0),mats["Material_Tool"],lod,"mechanically_indexed_carbide_tooth",root,0.025)
    ...
    if lod<2:
        ...
        for index in range(cable_count):
            ...
            offset=(index//2)*0.14
            beam(f"PowerHarness_{index}",(1.40,side*(0.80-offset),0.42),(2.72,side*(0.78-offset),0.58+offset),0.085,mats["Material_Cable"],lod,"motor_power_harness",root,0.014)
    ...
    if lod==0:
        for index,x in enumerate((0.28,0.72,1.15,1.82)):
            for side in (-1,1):
                cylinder(f"ArmorFastener_{index}_{side}",0.045,0.10,(x,side*0.91,0.78),(math.pi/2,0,0),mats["Material_Mechanical"],lod,"captive_service_fastener",root,10,0.03)
...
def main() -> None:
    ...
    root["spaceface.family"]="opening_route_industrial_props_v1";root["spaceface.surfaceRevision"]="opening_mining_drone_v1"
