"""PQ-050.22 Survey pin MTX builder. Hitch untouched. --mtx-cycle N writes cycle stills."""
...
def role_maps(role, rgb, size=TEX, prefix=None):
    """Unique survey pin maps. Ash-grey surveyor, not rust cutter."""
    ...
    base = write_pixels(f"pin_{prefix}_basecolor", albedo, size, "sRGB")
    orm_img = write_pixels(f"pin_{prefix}_orm", orm, size, "Non-Color")
    nrm_img = write_pixels(f"pin_{prefix}_normal", nrm, size, "Non-Color")
...
def create_materials():
    specs = {
        "Material_Hull": ((0.34, 0.36, 0.32), 0.04, 0.50, "hull", 0.06, None),
        "Material_Armor": ((0.30, 0.32, 0.30), 0.08, 0.48, "armor", 0.03, None),
...
def sockets():
    # Frozen factory survey pin sockets (length 11, halfW 1.1, height 0.55).
    return {
        "SOCKET_Weapon_Front": (4.3, 0.0, 0.25),
        "SOCKET_Mining_Front": (4.9, 0.0, -0.15),
        "SOCKET_Engine_Main": (-4.3, 0.0, 0.08),
        "SOCKET_Trail_Main": (-4.7, 0.0, 0.08),
        "SOCKET_Trail_Port": (-4.5, -0.77, 0.08),
        "SOCKET_Trail_Starboard": (-4.5, 0.77, 0.08),
        "SOCKET_Utility_Dorsal": (0.6, 0.0, 1.10),
        "SOCKET_Cargo_Ventral": (-0.4, 0.0, -0.70),
        "SOCKET_Camera_Focus": (0.8, 0.0, 0.25),
        "SOCKET_RCS_Port": (-1.2, -1.30, 0.15),
        "SOCKET_RCS_Starboard": (-1.2, 1.30, 0.15),
    }
...
def build_lod(lod, mats):
    collection = bpy.data.collections.new(f"PIN_LOD{lod}")
    ...
    root = add_empty(f"PIN_LOD{lod}_ROOT", (0, 0, 0), collection)
    root["spacefaceAsset"] = {
        "assetId": "SF_SURVEY_PIN_V1", "partId": "survey_pin_production_v1",
    ...
    if lod <= 1:
        ...
        boolean_cut(hull_obj, "RadWellS", (-2.45, 0.32, 0.40), (0.38, 0.16, 0.10))
        boolean_cut(hull_obj, "TransomRecess", (-5.15, 0.0, 0.02), (0.12, 0.20, 0.10))
        hull_obj.data.materials.clear()
        hull_obj.data.materials.append(hull)
    inset_large_faces(hull_obj, thickness=0.018, depth=0.010, min_area=0.35)
    cover_loft_with_plates("PinSkin", [
        (5.40, 0.22, 0.16, 0.04),
        (3.20, 0.72, 0.38, 0.08),
        (1.00, 1.05, 0.48, 0.08),
        (-1.20, 1.08, 0.50, 0.08),
    ...
    loft_shell("Deck_P", [
        (2.60, -0.18, -0.55, 0.04, 0.38),
        (0.80, -0.28, -0.72, 0.08, 0.48),
        (-1.00, -0.18, -0.52, 0.02, 0.32),
    ...
    center_loft("Mast_Deck", [
        (1.80, 0.18, 0.32, 0.58),
        (0.40, 0.28, 0.42, 0.72),
        (-0.80, 0.16, 0.28, 0.48),
    ], hull, collection, 0.006)
    add_box("TransomPlate", (-5.08, 0.0, 0.03), (0.03, 0.28, 0.16), armor, collection, 0.003)
    add_thin_canopy("Cabin", 2.85, 0.0, 0.56, 0.72, 0.22, 0.16, mats, collection)
    add_box("CabinBrow", (3.18, 0.0, 0.66), (0.05, 0.16, 0.04), armor, collection, 0.002)

    # Factory mast: dorsal spine, range triangle, two array paddles.
    mast_h = 1.15 if CYCLE >= 2 else 0.85
    add_box("MastBase", (0.15, 0.0, 0.62), (0.22, 0.16, 0.10), mech, collection, 0.003)
    add_cylinder("MastPost", (0.15, 0.0, 0.62 + mast_h * 0.5), 0.035, mast_h, mech, collection, 10, 0.002, rot=(0, 0, 0))
    add_box("MastYard", (0.15, 0.0, 0.62 + mast_h), (0.08, 0.55, 0.04), armor, collection, 0.002)
    add_box("Spine", (-0.85, 0.0, 0.58), (1.35, 0.04, 0.05), armor, collection, 0.002)
    add_sensor_dish("PinDish", (0.15, 0.0, 0.62 + mast_h + 0.08), mats, collection)

    paddle_y = 1.05 if CYCLE >= 3 else 0.82
    for sign, side in ((-1, "Port"), (1, "Starboard")):
        add_box(f"PaddleArm_{side}", (0.55, 0.42 * sign, 0.48), (0.08, 0.28, 0.04), mech, collection, 0.002)
        add_box(f"Paddle_{side}", (0.55, paddle_y * sign, 0.48), (0.55, 0.04, 0.28), mats["Material_Radiator"], collection, 0.002)
        add_box(f"PaddleRib_{side}", (0.55, paddle_y * sign, 0.48), (0.52, 0.012, 0.04), mech, collection, 0.001)
        add_rcs_cluster(side, (-1.2, 1.15 * sign, 0.14), mats, collection, sign=sign)
        add_box(f"SideWellLip_{side}", (0.05, 0.96 * sign, 0.26), (0.58, 0.03, 0.03), armor, collection, 0.002)
        add_box(f"RadCassette_{side}", (-2.45, 0.38 * sign, 0.46), (0.36, 0.14, 0.07), mats["Material_Radiator"], collection, 0.002)
        if lod <= 1:
            for i in range(6):
                add_box(
                    f"RadFin_{side}_{i}",
                    (-2.62 + i * 0.06, 0.38 * sign, 0.50),
                    (0.008, 0.12, 0.12),
                    mech, collection, 0.001,
                )

    if CYCLE >= 4:
        for i, ox in enumerate((-0.35, 0.55)):
            add_cylinder(f"GelDrum_{i}", (ox, 0.28, 0.42), 0.10, 0.18, hull, collection, 12, 0.002)

    if CYCLE >= 5:
        add_cylinder("ColdBoom", (4.85, 0.0, 0.08), 0.035, 1.35, mech, collection, 8, 0.002)
    ...
    add_service_hatch("Dorsal", (-1.55, 0.0, 0.52), mats, collection, sx=0.20, sy=0.12)
    add_box("KeelShoe", (0.0, 0.0, -0.38), (1.15, 0.12, 0.04), armor, collection, 0.003)
    add_box("StripeP", (0.4, -0.92, 0.20), (1.2, 0.010, 0.006), accent, collection, 0.001)
    add_box("StripeS", (0.4, 0.92, 0.20), (1.2, 0.010, 0.006), accent, collection, 0.001)
    add_box("BowCap", (5.38, 0.0, 0.05), (0.08, 0.12, 0.08), armor, collection, 0.002)
    add_panel_seams("Hull", [2.6, 0.6, -1.2, -2.8], 0.72, 0.38, mech, collection)
    ...
    bm = bmesh.new()
    for point in [
        (6.8, 0, 0.1), (3.6, -1.2, 1.4), (3.6, 1.2, 1.4),
        (-6.4, -1.1, 0.2), (-6.4, 1.1, 0.2),
        (0.6, -1.8, 0.5), (0.6, 1.8, 0.5),
        (-1.6, -1.8, 0.5), (-1.6, 1.8, 0.5),
        (0.2, 0, 1.6),
    ]:
        bm.verts.new(point)
...
def export_lod(collection, lod):
    out = FAMILY / "source" / "wholeships" / f"survey_pin_production_v1_lod{lod}.glb"
...
def render_cycle(collection):
    ...
    camera = setup_studio()
    out = FAMILY / "evidence" / "survey_pin" / "cycles" / f"cycle_{CYCLE:02d}"
    out.mkdir(parents=True, exist_ok=True)
    views = {
        "three_quarter": ((9.2, -10.8, 3.6), (0.1, 0, 0.18), 34),
        "starboard": ((0.1, 12.8, 2.6), (0.1, 0, 0.14), 32),
        "rear": ((-9.8, -4.4, 2.8), (-0.3, 0, 0.12), 32),
        "clay_three_quarter": ((9.2, -10.8, 3.6), (0.1, 0, 0.18), 34),
        "grazing_close": ((6.2, -5.2, 1.4), (0.6, 0, 0.22), 46),
        "bay_interior": ((3.4, -2.2, 1.3), (3.4, 0.0, 0.90), 40),
        "drive_rear": ((-8.6, -3.2, 1.1), (-5.8, 0, 0.08), 42),
        "play_size": ((34, -30, 14), (0, 0, 0.10), 48),
        "orm_isolation": ((11.0, -12.8, 4.2), (0.1, 0, 0.20), 32),
        "normal_isolation": ((11.0, -12.8, 4.2), (0.1, 0, 0.20), 32),
        "id_or_material_id": ((11.0, -12.8, 4.2), (0.1, 0, 0.20), 32),
        "material_three_quarter": ((11.0, -12.8, 4.2), (0.1, 0, 0.20), 32),
    }
...
def main():
    ...
    for lod in (0, 1, 2):
        ...
        if report["hullTriangles"] < 800:
            raise RuntimeError(f"survey_pin lod{lod} hull {report['hullTriangles']} < 800")
        collections.append(collection)
        reports.append(report)
    stills = render_cycle(collections[0])
    report = {
        "schema": "spaceface.surveyPinMtx.cycle.v1",
        "shipId": "survey_pin",
        "cycle": CYCLE,
        "lods": reports,
        "stills": str(stills.relative_to(FAMILY)).replace("\\", "/"),
    }
    (FAMILY / "evidence" / "survey_pin").mkdir(parents=True, exist_ok=True)
    (FAMILY / "evidence" / "survey_pin" / f"cycle_{CYCLE:02d}.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
