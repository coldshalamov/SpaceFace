#!/usr/bin/env python3
"""Helios Lark iter14 — clean surface (kill UV-edge spiderweb cracks from iter13).

Keeps iter13 geometry wins (mech mass, hierarchical bays, thick canards, COLLISION rename).
Clears iter13 G7 P1s:
- G3: NO shared-atlas AO nearest-sample wire (that caused spiderweb cracks). Clean soft-edge
  panel masks only; mesh AO saved as evidence only.
- G4: darker mid-grey paint + high-contrast mech; no crack artifacts under Three close.
"""
...
def build_materials() -> dict[str, bpy.types.Material]:
    ...
    def rebuild(name: str, rgba: tuple, rough: float, metal: float, role: str) -> None:
        ...
        for y in range(TEX):
            for x in range(TEX):
                dx = min(x % pw, pw - (x % pw))
                dy = min(y % ph, ph - (y % ph))
                # Soft panel edges only — hard 1px seams spiderweb under Three mip/env
                soft = max(0.0, 1.0 - min(dx, dy) / 5.0) if min(dx, dy) <= 5 else 0.0
                seam = soft * soft  # smooth falloff, no binary edge
                gf = h01(x, y, 11)
                gf2 = h01(x // 4, y // 4, 29)
                # Gentle multi-scale variation (no harsh sin lattice)
                cav = soft * 0.55 + gf2 * 0.12
                dirt = min(1.0, soft * 0.42 + cav * 0.25 + gf2 * 0.06)
    ...
    # Mid warm-grey paint (not near-white plastic) + graphite alloy
    rebuild("Material_Hull", (128, 122, 112, 255), 0.72, 0.02, "hull")
    rebuild("Material_Mechanical", (20, 22, 26, 255), 0.16, 0.96, "mech")
    rebuild("Material_Cyan", (18, 125, 155, 255), 0.26, 0.04, "cyan")
    rebuild("Material_Warm", (105, 54, 24, 255), 0.44, 0.22, "warm")
    rebuild("Material_Glass", (10, 28, 38, 255), 0.04, 0.0, "glass")
    return mats
...
def apply_mesh_ao_to_materials(parts: list[bpy.types.Object], bake_dir: Path) -> None:
    """Bake self-AO as EVIDENCE only. Do NOT nearest-sample onto shared role atlases
    (iter13 spiderweb crack regression under Three close).
    """
    bake_dir.mkdir(parents=True, exist_ok=True)
    targets = [p for p in parts if p.type == "MESH" and any(
        k in p.name.lower() for k in ("fuselage", "engine_housing")
    )]
    for obj in targets[:3]:
        ...
        try:
            img.save()
            px = list(img.pixels)
            mean_ao = sum(px[i] for i in range(0, len(px), 4)) / max(1, len(px) // 4)
            log(f"saved evidence AO {path.name} meanAO={mean_ao:.3f} (not wired to shared atlas)")
        except Exception as e:
