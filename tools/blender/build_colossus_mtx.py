"""PQ-050.08 Colossus MTX builder. Hitch untouched. --mtx-cycle N writes cycle stills."""
...
FAMILY = ROOT / "assets" / "ships" / "fleet_player_bodies_v1" / "colossus"
...
def role_maps(role, rgb, size=TEX, prefix=None):
    """Unique Colossus maps. Not a tint of the shared fleet sheet."""
    ...
    base = write_pixels(f"colossus_{prefix}_basecolor", albedo, size, "sRGB")
    orm_img = write_pixels(f"colossus_{prefix}_orm", orm, size, "Non-Color")
    nrm_img = write_pixels(f"colossus_{prefix}_normal", nrm, size, "Non-Color")
...
def create_materials():
    specs = {
        "Material_Hull": ((0.17, 0.18, 0.21), 0.07, 0.48, "hull", 0.18, None),
        "Material_Armor": ((0.10, 0.11, 0.13), 0.44, 0.38, "armor", 0.08, None),
...
def sockets():
    half, hw, hh = 12.0, 2.9, 1.45
    return {
        "SOCKET_Weapon_Front": (half - 1.5, 0.0, 0.30),
        "SOCKET_Mining_Front": (half - 0.7, 0.0, -0.18),
        "SOCKET_Engine_Main": (-half + 1.5, 0.0, 0.12),
        "SOCKET_Trail_Main": (-half + 1.0, 0.0, 0.12),
        "SOCKET_Trail_Port": (-half + 1.1, -hw * 0.70, 0.12),
        "SOCKET_Trail_Starboard": (-half + 1.1, hw * 0.70, 0.12),
        "SOCKET_Utility_Dorsal": (-1.6, 0.0, hh + 0.95),
        "SOCKET_Cargo_Ventral": (-0.3, 0.0, -hh - 0.12),
...
def build_lod(lod, mats):
    collection = bpy.data.collections.new(f"COLOSSUS_LOD{lod}")
    ...
    half = 12.0
    root = add_empty(f"COLOSSUS_LOD{lod}_ROOT", (0, 0, 0), collection)
    root["spacefaceAsset"] = {
        "assetId": "SF_COLOSSUS_PRODUCTION_V1", "partId": "colossus_production_v1",
        "lod": f"lod{lod}", "slot": "hull", "category": "wholeships",
        "forward": "+X", "embeddedPlume": False,
    }
    # Capital: blunt-wide bow, fat casemate mid, island, four-drive transom, fins.
    hull_obj = loft_from_rings("Pressure_Hull", [
        station_ring(half, 0, 0.18, 0.70, 0.48, flat=0.45, box=0.35, keel=0.55),
        station_ring(half * 0.74, 0, 0.16, 1.52, 0.82, flat=0.55, box=0.50, keel=0.45),
        station_ring(half * 0.42, 0, 0.16, 2.42, 1.12, flat=0.60, box=0.75, keel=0.35),
        station_ring(0.10, 0, 0.14, 2.95, 1.38, flat=0.50, box=0.92, keel=0.28),
        station_ring(-half * 0.28, 0, 0.18, 2.78, 1.42, flat=0.48, box=0.95, keel=0.25),
        station_ring(-half * 0.58, 0, 0.16, 2.18, 1.05, flat=0.40, box=0.95, keel=0.22),
        station_ring(-half + 0.75, 0, 0.14, 1.75, 0.82, flat=0.35, box=0.95, keel=0.18),
    ...
    add_box("Island_Trunk", (-1.75, 0.0, 2.05), (0.64, 0.48, 0.58), hull, collection, 0.010)
    add_box("Island_Door", (-2.28, 0.0, 1.72), (0.04, 0.16, 0.22), mech, collection, 0.003)
    add_box("Island_Shoulder", (-1.60, 0.0, 2.42), (0.88, 0.62, 0.10), armor, collection, 0.006)
    add_box("Island_Bridge", (-1.40, 0.0, 2.72), (0.98, 0.70, 0.22), armor, collection, 0.008)
    add_box("Island_GlassF", (-0.82, 0.0, 2.74), (0.04, 0.52, 0.13), mats["Material_Canopy"], collection, 0.002)
    add_box("Island_GlassP", (-1.35, -0.62, 2.74), (0.34, 0.016, 0.12), mats["Material_Canopy"], collection, 0.002)
    add_box("Island_GlassS", (-1.35, 0.62, 2.74), (0.34, 0.016, 0.12), mats["Material_Canopy"], collection, 0.002)
    add_box("DeckPlate_A", (1.35, 0.0, 1.18), (1.25, 0.88, 0.020), armor, collection, 0.004)
    add_box("DeckPlate_B", (-4.15, 0.0, 1.08), (1.05, 0.72, 0.018), armor, collection, 0.004)
    add_box("DeckPlate_C", (5.35, -0.35, 0.92), (0.70, 0.38, 0.016), armor, collection, 0.003)
    add_box("DeckPlate_D", (-2.85, 0.55, 1.14), (0.62, 0.28, 0.014), armor, collection, 0.003)
    add_box("DeckPlate_E", (2.85, -0.62, 1.10), (0.55, 0.26, 0.014), hull, collection, 0.003)
    add_box("Island_WalkP", (-1.55, -0.82, 1.52), (0.85, 0.08, 0.018), mech, collection, 0.003)
    add_box("Island_WalkS", (-1.55, 0.82, 1.52), (0.85, 0.08, 0.018), mech, collection, 0.003)
    add_box("Island_Mast", (-1.90, 0.0, 3.12), (0.05, 0.05, 0.36), mech, collection, 0.002)
    add_box("Island_Yard", (-1.90, 0.0, 3.32), (0.04, 0.28, 0.03), mech, collection, 0.002)
    add_cylinder("Island_Radar", (-1.35, 0.28, 2.92), 0.14, 0.045, armor, collection, vertices=12, bevel=0.002, rot=(0, math.pi / 2.6, 0))

    drive_ys = ((-2.00, "PortOut"), (2.00, "StbdOut"), (-0.75, "PortIn"), (0.75, "StbdIn"))
    for y, side in drive_ys:
        add_manufactured_drive(side, -10.45, y, lod, mats, collection, scale=0.92, z=0.14)
        add_cylinder(f"DriveCasing_{side}", (-9.35, y, 0.14), 0.36, 0.88, mech, collection, vertices=16, bevel=0.008)
        add_cylinder(f"DriveFairing_{side}", (-8.75, y, 0.14), 0.42, 0.46, armor, collection, vertices=16, bevel=0.008)
        add_box(f"DriveSaddle_{side}", (-9.15, y, -0.22), (0.42, 0.15, 0.09), mech, collection, 0.005)
    add_box("Transom_Plate", (-10.85, 0.0, 0.20), (0.08, 2.45, 0.68), armor, collection, 0.006)
    add_box("Transom_Header", (-9.25, 0.0, 0.72), (0.95, 2.35, 0.06), armor, collection, 0.005)
    add_box("Transom_Hatch", (-9.85, 0.0, 0.56), (0.30, 0.38, 0.03), armor, collection, 0.003)
    add_box("Transom_RibP", (-10.15, -1.15, 0.30), (0.42, 0.05, 0.36), mech, collection, 0.003)
    add_box("Transom_RibS", (-10.15, 1.15, 0.30), (0.42, 0.05, 0.36), mech, collection, 0.003)

    for sign, side in ((-1, "Port"), (1, "Starboard")):
        add_box(f"Casemate_{side}", (0.20, 2.48 * sign, 0.20), (1.85, 0.26, 0.34), armor, collection, 0.010)
        add_box(f"CaseLip_{side}", (0.20, 2.70 * sign, 0.20), (1.90, 0.05, 0.38), mech, collection, 0.004)
        add_cylinder(f"Broad_Fore_{side}", (0.85, 2.80 * sign, 0.20), 0.075, 0.70, mech, collection, vertices=8, bevel=0.003)
        add_cylinder(f"Broad_Mid_{side}", (0.20, 2.80 * sign, 0.20), 0.065, 0.62, mech, collection, vertices=8, bevel=0.003)
        add_cylinder(f"Broad_Aft_{side}", (-0.50, 2.80 * sign, 0.20), 0.075, 0.70, mech, collection, vertices=8, bevel=0.003)
        add_box(f"CaseWalk_{side}", (0.20, 2.35 * sign, 0.42), (1.40, 0.10, 0.018), mech, collection, 0.003)
        add_cylinder(f"GunHouse_{side}", (6.55, 0.62 * sign, 0.08), 0.12, 1.15, mech, collection, vertices=10, bevel=0.005)
    ...
    add_box("Cabin_Shoulder", (3.35, 0.0, 1.15), (0.95, 0.62, 0.06), armor, collection, 0.006)
    add_box("Keel_Spine", (0.10, 0.0, -1.08), (6.8, 0.32, 0.06), mech, collection, 0.01)
    add_box("Repair_Patch", (2.55, -0.78, 1.02), (0.32, 0.16, 0.012), warning, collection, 0.002)
    add_box("Repair_Plate", (4.05, 0.85, 1.08), (0.40, 0.22, 0.016), armor, collection, 0.003)
    add_box("Warn_Chevron", (-4.15, -1.05, 0.92), (0.22, 0.09, 0.010), warning, collection, 0.002)
    add_box("Accent_Flash", (0.65, -2.15, 0.32), (0.50, 0.016, 0.08), accent, collection, 0.002)
    add_sensor_dish("Dorsal", (2.05, 0.52, 1.32), mats, collection)
    add_cylinder("Turret_Ring", (0.55, 0.0, 1.22), 0.36, 0.08, mech, collection, vertices=14, bevel=0.005, rot=(0, 0, 0))
    add_box("Turret_Head", (0.72, 0.0, 1.38), (0.30, 0.18, 0.08), armor, collection, 0.005)
    add_box("Hatch_Lid", (-0.95, 0.45, 0.95), (0.34, 0.22, 0.016), armor, collection, 0.003)
    add_box("Hatch_Hinge", (-1.22, 0.45, 0.96), (0.03, 0.16, 0.016), mech, collection, 0.002)

    if lod == 0:
        add_curve_hose("Hose_Port", [(0.2, -2.35, 0.14), (-3.2, -2.20, 0.16), (-6.8, -2.05, 0.16), (-9.2, -2.00, 0.16)], mech, collection, 0.016)
        add_curve_hose("Hose_Stbd", [(0.2, 2.35, 0.14), (-3.2, 2.20, 0.16), (-6.8, 2.05, 0.16), (-9.2, 2.00, 0.16)], mech, collection, 0.016)
    ...
    bm = bmesh.new()
    for point in [
        (10.6, 0, 0.3), (0, -4.2, 0.8), (0, 4.2, 0.8),
        (-10.2, -3.0, 0.2), (-10.2, 3.0, 0.2),
        (2.0, -2.0, -1.3), (2.0, 2.0, -1.3),
        (-1.6, 0, 2.9), (-7.5, -2.8, 1.1), (-7.5, 2.8, 1.1),
    ]:
        bm.verts.new(point)
...
def export_lod(collection, lod):
    out = FAMILY / "source" / "wholeships" / f"colossus_production_v1_lod{lod}.glb"
...
def render_cycle(collection):
    ...
    camera = setup_studio()
    out = FAMILY / "evidence" / "colossus" / "cycles" / f"cycle_{CYCLE:02d}"
    out.mkdir(parents=True, exist_ok=True)
    views = {
        "three_quarter": ((16.8, -18.8, 8.0), (0, 0, 0.22), 32),
        "starboard": ((0.0, 26.0, 5.0), (0, 0, 0.18), 30),
        "rear": ((-20.5, -7.4, 5.2), (-0.5, 0, 0.14), 32),
        "clay_three_quarter": ((16.8, -18.8, 8.0), (0, 0, 0.22), 32),
        "grazing_close": ((11.5, -9.0, 2.6), (0.5, 0, 0.20), 48),
        "bay_interior": ((0.3, -4.4, 0.9), (0.2, -2.2, 0.18), 46),
        "drive_rear": ((-15.2, -3.8, 1.8), (-9.8, 0, 0.14), 48),
        "play_size": ((60, -54, 28), (0, 0, 0.16), 50),
        "orm_isolation": ((16.8, -18.8, 8.0), (0, 0, 0.22), 32),
        "normal_isolation": ((16.8, -18.8, 8.0), (0, 0, 0.22), 32),
        "id_or_material_id": ((16.8, -18.8, 8.0), (0, 0, 0.22), 32),
        "material_three_quarter": ((16.8, -18.8, 8.0), (0, 0, 0.22), 32),
    }
...
def main():
    ...
    for lod in (0, 1, 2):
        ...
        if report["hullTriangles"] < 800:
            raise RuntimeError(f"colossus lod{lod} hull {report['hullTriangles']} < 800")
        collections.append(collection)
        reports.append(report)
    stills = render_cycle(collections[0])
    report = {
        "schema": "spaceface.colossusMtx.cycle.v1",
        "shipId": "colossus",
        "cycle": CYCLE,
        "lods": reports,
        "stills": str(stills.relative_to(FAMILY)).replace("\\", "/"),
    }
    (FAMILY / "evidence" / "colossus").mkdir(parents=True, exist_ok=True)
    (FAMILY / "evidence" / "colossus" / f"cycle_{CYCLE:02d}.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
