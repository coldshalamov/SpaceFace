"""PQ-050.12 Wasp MTX builder. Hitch untouched. --mtx-cycle N writes cycle stills."""
...
def role_maps(role, rgb, size=TEX, prefix=None):
    """Unique Wasp maps. Not a tint of the shared live Wasp/Hitch sheet."""
    ...
    base = write_pixels(f"wasp_{prefix}_basecolor", albedo, size, "sRGB")
    orm_img = write_pixels(f"wasp_{prefix}_orm", orm, size, "Non-Color")
    nrm_img = write_pixels(f"wasp_{prefix}_normal", nrm, size, "Non-Color")
...
def create_materials():
    specs = {
        "Material_Hull": ((0.10, 0.11, 0.13), 0.08, 0.48, "hull", 0.10, None),
        "Material_Armor": ((0.12, 0.13, 0.15), 0.08, 0.50, "armor", 0.04, None),
        "Material_Mechanical": ((0.30, 0.31, 0.33), 0.78, 0.34, "mechanical", 0.0, None),
        "Material_Accent": ((0.04, 0.58, 0.66), 0.04, 0.42, "accent", 0.08, None),
        ...
        "Material_Canopy": ((0.08, 0.22, 0.24), 0.02, 0.10, "glass", 0.35, None),
...
def sockets():
    return {
        "SOCKET_Weapon_Front": (10.6, 0.0, 0.10),
        "SOCKET_Mining_Front": (10.2, 0.0, -0.35),
        "SOCKET_Engine_Main": (-9.7, 0.0, 0.00),
        "SOCKET_Trail_Main": (-10.0, 0.0, 0.00),
        "SOCKET_Trail_Port": (-9.8, -5.55, 0.00),
        "SOCKET_Trail_Starboard": (-9.8, 5.55, 0.00),
        "SOCKET_Utility_Dorsal": (-1.1, 0.0, 1.50),
...
def build_lod(lod, mats):
    collection = bpy.data.collections.new(f"WASP_LOD{lod}")
    ...
    root = add_empty(f"WASP_LOD{lod}_ROOT", (0, 0, 0), collection)
    root["spacefaceAsset"] = {
        "assetId": "SF_WASP_PRODUCTION_V1", "partId": "wasp_production_v1",
    ...
    for sign, side in ((-1, "Port"), (1, "Starboard")):
        ...
        loft_from_rings(f"Wing_{side}", [
            wing_station(5.15, 2.18 * sign, 0.04, 3.25, 0.70),
            wing_station(3.35, 4.00 * sign, 0.06, 3.55, 0.50),
            wing_station(1.15, 5.02 * sign, 0.04, 2.35, 0.28),
            wing_station(-1.05, 6.40 * sign, 0.03, 3.05, 0.26),
            wing_station(-4.55, 7.48 * sign, 0.04, 2.25, 0.16),
        ], hull, collection, 0.006, cap=True)
        add_box(f"WingSpar_{side}", (0.6, 3.85 * sign, 0.02), (1.85, 0.06, 0.10), mech, collection, 0.003)
        add_box(f"WingFlap_{side}", (-3.4, 6.55 * sign, 0.00), (0.85, 0.55, 0.035), armor, collection, 0.003)
        add_box(f"WingRoot_{side}", (1.55, 2.38 * sign, 0.10), (1.05, 0.22, 0.16), mech, collection, 0.004)
        add_box(f"PylonWeb_{side}", (-1.15, 5.28 * sign, 0.04), (1.65, 0.042, 0.20), armor, collection, 0.003)
        add_box(f"PylonFlange_{side}", (-1.15, 5.28 * sign, 0.18), (1.75, 0.15, 0.028), mech, collection, 0.003)
        add_box(f"PylonFair_{side}", (-2.85, 5.32 * sign, 0.08), (0.55, 0.10, 0.10), mech, collection, 0.003)
        nacelle = loft_from_rings(f"Nacelle_{side}", [
            ellipse_ring(2.90, y, 0.04, 0.58, 0.44, 8),
            ellipse_ring(2.40, y, 0.03, 0.30, 0.24, 8),
            ellipse_ring(1.70, y, 0.02, 0.66, 0.50, 8),
            ellipse_ring(-2.15, y, 0.02, 0.70, 0.54, 8),
            ellipse_ring(-6.20, y, 0.00, 0.58, 0.46, 8),
            ellipse_ring(-9.42, y, 0.00, 0.36, 0.30, 8),
        ], armor, collection, 0.006, cap="front")
        boolean_cut_cyl(nacelle, f"Intake_{side}", (2.55, y, 0.04), 0.22, 1.15)
        boolean_cut(nacelle, f"RailTrench_{side}", (-3.2, y + 0.38 * sign, 0.42), (2.2, 0.045, 0.038))
        ...
        add_box(f"NacellePanel_{side}", (-1.4, y + 0.58 * sign, 0.12), (1.15, 0.018, 0.18), mech, collection, 0.003)
        add_box(f"NacelleRail_{side}", (-3.2, y + 0.38 * sign, 0.38), (2.0, 0.022, 0.012), accent, collection, 0.002)
        add_box(f"WingFence_{side}", (-4.8, 7.15 * sign, 0.10), (0.55, 0.012, 0.16), armor, collection, 0.002)
        add_box(f"GunCheek_{side}", (4.35, 2.72 * sign, 0.16), (0.85, 0.22, 0.12), armor, collection, 0.004)
        add_box(f"GunReceiver_{side}", (5.00, 2.95 * sign, 0.20), (0.38, 0.14, 0.13), mech, collection, 0.004)
        add_box(f"GunTrunnion_{side}", (4.62, 2.95 * sign, 0.14), (0.10, 0.18, 0.09), armor, collection, 0.003)
        add_cylinder(f"GunJacket_{side}", (6.00, 2.95 * sign, 0.20), 0.062, 0.95, armor, collection, 12, 0.002)
        add_cylinder(f"GunBarrel_{side}", (6.90, 2.95 * sign, 0.20), 0.032, 0.70, mech, collection, 10, 0.002)
        add_cylinder(f"GunMuzzle_{side}", (7.30, 2.95 * sign, 0.20), 0.044, 0.07, armor, collection, 10, 0.001)
        add_box(f"GunFeed_{side}", (4.88, 2.95 * sign, 0.34), (0.16, 0.07, 0.05), mech, collection, 0.002)
        add_rcs_cluster(side, (-1.8, 6.85 * sign, 0.08), mats, collection, sign=sign)
        add_box(f"Warn_{side}", (-1.2, 6.25 * sign, 0.22), (0.28, 0.10, 0.02), warning, collection, 0.002)
    ...
    if lod == 0:
        add_curve_hose("Hose_Port", [(0.4, -2.05, 0.12), (-3.6, -3.85, 0.14), (-8.2, -5.25, 0.08)], mech, collection, 0.016)
        add_curve_hose("Hose_Stbd", [(0.4, 2.05, 0.12), (-3.6, 3.85, 0.14), (-8.2, 5.25, 0.08)], mech, collection, 0.016)
    if CYCLE >= 6 and lod <= 1:
        boolean_cut(hull_obj, "TransomRecess", (-9.55, 0.0, -0.02), (0.14, 0.38, 0.22))
    if CYCLE >= 7:
        add_box("CanopyMullion", (4.55, 0.0, 1.12), (0.022, 0.28, 0.08), armor, collection, 0.002)
        if lod <= 1:
            for sign, side in ((-1, "Port"), (1, "Starboard")):
                for i in range(4):
                    add_box(
                        f"WingRad_{side}_{i}",
                        (-2.45 + i * 0.18, 2.35 * sign, 0.16),
                        (0.010, 0.14, 0.12),
                        mech, collection, 0.001,
                    )
    if CYCLE >= 8:
        add_box("NacelleBandX", (-4.15, 0.0, 0.02), (0.06, 0.42, 0.16), armor, collection, 0.003)
    if CYCLE >= 9:
        add_box("ChineCapP", (0.40, -2.18, -0.10), (1.35, 0.032, 0.040), armor, collection, 0.003)
        add_box("ChineCapS", (0.40, 2.18, -0.10), (1.35, 0.032, 0.040), armor, collection, 0.003)
    if CYCLE >= 10:
        add_box("PatchTile2", (1.55, 0.32, 0.94), (0.20, 0.10, 0.008), armor, collection, 0.002)
        add_box("CanopyStayP", (3.85, -0.18, 0.88), (0.16, 0.014, 0.014), mech, collection, 0.002)
        add_box("CanopyStayS", (3.85, 0.18, 0.88), (0.16, 0.014, 0.014), mech, collection, 0.002)
    ...
    for material_name, objects in sorted(groups.items()):
        ...
        if lod == 0 and material_name in {"Material_Hull", "Material_Mechanical"}:
    ...
    bm = bmesh.new()
    for point in [
        (11.0, 0, 0.1), (2.0, -3.2, 0.8), (2.0, 3.2, 0.8),
        (-9.4, -6.2, 0.2), (-9.4, 6.2, 0.2),
        (1.0, -7.6, 0.2), (1.0, 7.6, 0.2),
        (4.4, 0, 1.4),
    ]:
        bm.verts.new(point)
...
def render_cycle(collection):
    ...
    camera = setup_studio()
    out = FAMILY / "evidence" / "wasp" / "cycles" / f"cycle_{CYCLE:02d}"
    out.mkdir(parents=True, exist_ok=True)
    views = {
        "three_quarter": ((16.8, -20.5, 7.4), (0, 0, 0.15), 30),
        "starboard": ((0.0, 28.0, 5.2), (0, 0, 0.12), 28),
        "rear": ((-20.5, -8.8, 5.4), (-0.6, 0, 0.10), 28),
        "clay_three_quarter": ((16.8, -20.5, 7.4), (0, 0, 0.15), 30),
        "grazing_close": ((10.2, -8.4, 2.2), (0.8, 0, 0.18), 46),
        "bay_interior": ((4.5, -4.6, 1.2), (4.4, 0.0, 0.85), 40),
        "drive_rear": ((-14.8, -6.2, 1.8), (-9.4, 0, 0.05), 42),
        "play_size": ((56, -50, 22), (0, 0, 0.10), 48),
        "orm_isolation": ((16.8, -20.5, 7.4), (0, 0, 0.15), 30),
        "normal_isolation": ((16.8, -20.5, 7.4), (0, 0, 0.15), 30),
        "id_or_material_id": ((16.8, -20.5, 7.4), (0, 0, 0.15), 30),
        "material_three_quarter": ((16.8, -20.5, 7.4), (0, 0, 0.15), 30),
    }
...
def main():
    ...
    for lod in (0, 1, 2):
        ...
        if report["hullTriangles"] < 800:
            raise RuntimeError(f"wasp lod{lod} hull {report['hullTriangles']} < 800")
        collections.append(collection)
        reports.append(report)
    stills = render_cycle(collections[0])
    report = {
        "schema": "spaceface.waspMtx.cycle.v1",
        "shipId": "wasp",
        "cycle": CYCLE,
        "lods": reports,
        "stills": str(stills.relative_to(FAMILY)).replace("\\", "/"),
    }
    (FAMILY / "evidence" / "wasp").mkdir(parents=True, exist_ok=True)
    (FAMILY / "evidence" / "wasp" / f"cycle_{CYCLE:02d}.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
