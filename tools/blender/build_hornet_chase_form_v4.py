...
def build_wings(mats):
    ...
    for sign, tag in ((1.0, "Stbd"), (-1.0, "Port")):
        rings = [
            ...
            airfoil(0.85, 1.55 * sign, 0.15, 2.65, 0.64),
            airfoil(0.15, 2.25 * sign, 0.20, 2.05, 0.42),
            airfoil(-0.45, 3.05 * sign, 0.24, 1.35, 0.24),
            airfoil(-0.95, 3.85 * sign, 0.28, 0.68, 0.10),
        ]
...
def build_drives(hull, mats):
    ...
    for sign, tag in ((-1.0, "Port"), (1.0, "Stbd")):
        y = 0.62 * sign
        collar = add_cyl(
            f"LOD0_DriveCollar_{tag}", 0.46, 0.10, (-5.18, y, 0.06),
            ceramic, rotation=(0.0, math.radians(90.0), 0.0), vertices=16,
        )
        floor = add_cyl(
            f"LOD0_DriveFloor_{tag}", 0.40, 0.05, (-4.45, y, 0.02),
            mats["Material_Mechanical"], vertices=16,
        )
        hub = add_cyl(
            f"LOD0_DriveHub_{tag}", 0.10, 0.10, (-4.45, y, 0.08),
            thrust, vertices=10,
        )
        vanes = []
        for index in range(10):
            ...
            vane = add_box(
                ...
                (0.08, 0.026, 0.22),
                (-4.45 + math.cos(angle) * 0.22, y + math.sin(angle) * 0.22, 0.14),
                mats["Material_Mechanical"], 0.001,
                rotation=(0.0, 0.0, angle),
            )
            vanes.append(vane)

        def dorsal():
            return add_cyl(f"DriveDorsal_{tag}", 0.52, 1.15, (-4.45, y, 0.52), hull_mat, vertices=16)

        def aft():
            return add_cyl(
                f"DriveAft_{tag}", 0.42, 1.05, (-5.22, y, 0.08), hull_mat,
                rotation=(0.0, math.radians(90.0), 0.0), vertices=16,
            )

        report.append({"tag": tag, "dorsal": cut(hull, dorsal), "aft": cut(hull, aft)})
        bits.extend([collar, floor, hub, *vanes])
    return bits, report
