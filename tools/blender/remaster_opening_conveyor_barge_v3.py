#!/usr/bin/env python3
"""Conveyor-barge material-truth remaster (v3, non-promoting candidate).

v1 replaced the 2026-07 bevel-only boxes with a manufactured ore-transfer barge
(keel/ribs, deck cradles + containers, drive block with four refractory throats,
glazed tower, forked loading boom).  v2 broke the 46m side sheets into plate courses,
added deck joints/manifolds/louvers, and made the glazed band read.  v3: silhouette
work (rear thrust-frame perimeter, staggered hull storage pods, boom knuckle housings,
bow rake plate) and the LOD diet that brings the candidate inside the 32k part profile.
...
def material(name: str, role: str, maps_root: Path):
    ...
    if emissive_path.exists():
        ...
        emissive_strength = {"barge_nav_optic": 1.6, "barge_bridge_glass": 2.4}.get(role, 1.1)
...
def build_drive_block(lod: int, mats, root) -> None:
    ...
    # Thrust-frame perimeter: the aft plate is framed like a machined mounting face.
    for index, zz in enumerate((0.15, 4.45)):
        box(f"RearFrameRail_{index}", (0.36, 7.0, 0.30), (-2.28, 0, zz), (0, 0, 0), mats["Material_Structure"], lod, "thrust_frame_perimeter_rail", root, 0.06)
    for index, yy in enumerate((-3.35, 3.35)):
        box(f"RearFramePost_{index}", (0.36, 0.30, 4.9), (-2.28, yy, 2.3), (0, 0, 0), mats["Material_Structure"], lod, "thrust_frame_perimeter_post", root, 0.06)
    # Cooling louvers on the block crown and manifolds from the cassette into each housing.
    louver_count = 8 if lod == 0 else 4
    for index in range(louver_count):
        strip_edge(box(f"DriveLouver_{index}", (0.09, 1.9, 0.55), (0.0 + index * 0.48, 0, 4.72), (0, 0, 0), mats["Material_Structure"], lod, "drive_cooling_louver", root, 0.0))
    for index, y in enumerate((-2.2, 2.2)):
        strip_edge(cylinder(f"ManifoldPipe_{index}", 0.22, 2.6, (3.0, y, 4.05), (0, math.pi / 2, 0), mats["Material_Drive"], lod, "drive_throat_manifold_pipe", root, 12, 0.04))
...
def build_deck(lod: int, mats, root) -> None:
    ...
    for side in (-1, 1):
        ...
        # Plate courses: two full-length stringers and six butt seams break the 46m
        # sheet into fastened hull construction instead of one extruded slab.
        box(f"HullStringerLow_{side}", (46.0, 0.22, 0.22), (26.0, side * 4.46, 0.75), (0, 0, 0), mats["Material_Structure"], lod, "longitudinal_hull_stringer", root, 0.05)
        box(f"HullStringerTop_{side}", (46.0, 0.22, 0.22), (26.0, side * 4.46, 3.85), (0, 0, 0), mats["Material_Structure"], lod, "upper_hull_sheer_strake", root, 0.05)
        if lod == 0:
            for index in range(6):
                strip_edge(box(f"ButtSeam_{index}_{side}", (0.10, 0.06, 3.62), (6.0 + index * 7.8, side * 4.505, 2.15), (0, 0, 0), mats["Material_Structure"], lod, "hull_course_butt_seam", root, 0.0))
        if lod == 0:
            for index in range(8):
                strip_edge(cylinder(f"HullFastener_{index}_{side}", 0.07, 0.10, (8.5 + index * 5.2, side * 4.52, 1.9), (math.pi / 2, 0, 0), mats["Material_Structure"], lod, "hull_course_fastener", root, 8, 0.02))
        if lod == 0:
            # Staggered under-deck hardware: magnetized storage/ounce pods bolted to the
            # hull sides at working height, alternating sides like practical retrofits.
            pod_plan = [(8.0, -1), (19.5, 1), (30.0, -1), (41.0, 1)]
            for index, (px, side_sign) in enumerate(pod_plan):
                box(f"HullStoragePod_{index}", (3.2, 0.7, 1.5), (px, side_sign * 4.85, 2.3), (0, 0, 0), mats["Material_Drive"], lod, "bolted_hull_storage_pod", root, 0.08)
                strip_edge(box(f"HullStorageLatch_{index}", (2.2, 0.09, 0.22), (px, side_sign * 5.24, 2.3), (0, 0, 0), mats["Material_Safety"], lod, "storage_pod_latch_strip", root, 0.0))
    # Deck slabs with real negative space between cradle bays, plus the expansion
    # joints that make a 46m deck fabricated rather than one extrusion.
    ...
    if lod == 0:
        for index in range(4):
            strip_edge(box(f"DeckJoint_{index}", (0.12, 8.7, 0.10), (12.65 + index * 10.0, 0, 5.02), (0, 0, 0), mats["Material_Structure"], lod, "deck_expansion_joint", root, 0.0))
    if lod == 0:
        # Center walkway stripe: deck crews walk the paint line between the bays.
        strip_edge(box("DeckWalkwayStripe", (45.5, 0.8, 0.04), (26.0, 0, 5.02), (0, 0, 0), mats["Material_Safety"], lod, "deck_crew_walkway_marking", root, 0.0))
...
def build_tower(lod: int, mats, root) -> None:
    ...
    # Column ribs carry the house's vertical load story to the corners.
    for sx in (40.25, 49.75):
        for index, sy in enumerate((-4.35, 4.35)):
            box(f"HouseColumnRib_{sx}_{index}", (0.45, 0.45, 6.2), (sx, sy, 1.65), (0, 0, 0), mats["Material_Structure"], lod, "machinery_house_column_rib", root, 0.07)
    ...
    # Roof plant: two vent stacks and a converter cover make the crown a plant, not a slab.
    for index in range(2):
        box(f"RoofVent_{index}", (0.8, 0.8, 0.5), (43.6 + index * 1.8, -2.3, 7.38), (0, 0, 0), mats["Material_Drive"], lod, "tower_roof_vent_stack", root, 0.06)
    box("RoofConverterCover", (1.6, 2.2, 0.22), (44.0, 1.2, 7.70), (0, 0, 0), mats["Material_Drive"], lod, "roof_converter_access_cover", root, 0.05)
...
def build_keel_to_bow(lod: int, mats, root) -> None:
    ...
    for side in (-1, 1):
        ...
        # Knuckle housings dress the two prong kinks where rails change course.
        if lod < 2:
            box(f"BoomKnuckle_{side}", (0.85, 0.85, 0.85), (60.4, side * 5.0, 5.15), (0, 0, 0), mats["Material_Drive"], lod, "loading_boom_knuckle_housing", root, 0.10)
    # Bow rake: the foredeck climbs a bulldozer rake plate into the boom root so the
    # bow reads as built structure from every approach angle.
    box("BowRakePlate", (3.2, 6.8, 1.1), (54.0, 0, 5.35), (0, math.radians(-14), 0), mats["Material_Hull"], lod, "bulldozer_rake_bow_plate", root, 0.10)
...
def main() -> None:
    ...
    root["spaceface.surfaceRevision"] = "opening_conveyor_barge_v3"
    ...
    report = {
        "schema": "spaceface.openingConveyorBargeRemaster.v3",
