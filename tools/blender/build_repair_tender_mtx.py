"""PQ-050.20 Repair tender MTX builder. Hitch untouched. --mtx-cycle N writes cycle stills."""
...
def role_maps(role, rgb, size=TEX, prefix=None):
    """Unique repair tender maps. Sage workboat, not rust barge."""
    ...
    base = write_pixels(f"tender_{prefix}_basecolor", albedo, size, "sRGB")
    orm_img = write_pixels(f"tender_{prefix}_orm", orm, size, "Non-Color")
    nrm_img = write_pixels(f"tender_{prefix}_normal", nrm, size, "Non-Color")
...
def create_materials():
    specs = {
        "Material_Hull": ((0.32, 0.34, 0.28), 0.05, 0.52, "hull", 0.05, None),
        "Material_Armor": ((0.26, 0.28, 0.24), 0.08, 0.50, "armor", 0.03, None),
...
def sockets():
    # Frozen factory tender sockets (length 14, halfW 1.6, height 0.8).
    return {
        "SOCKET_Weapon_Front": (5.8, 0.0, 0.25),
        "SOCKET_Mining_Front": (6.4, 0.0, -0.15),
        "SOCKET_Engine_Main": (-5.8, 0.0, 0.08),
        "SOCKET_Trail_Main": (-6.2, 0.0, 0.08),
        "SOCKET_Trail_Port": (-6.0, -1.12, 0.08),
        "SOCKET_Trail_Starboard": (-6.0, 1.12, 0.08),
        "SOCKET_Utility_Dorsal": (0.6, 0.0, 1.35),
        "SOCKET_Cargo_Ventral": (-0.4, 0.0, -0.95),
...
def build_lod(lod, mats):
    collection = bpy.data.collections.new(f"TENDER_LOD{lod}")
    ...
    root = add_empty(f"TENDER_LOD{lod}_ROOT", (0, 0, 0), collection)
    root["spacefaceAsset"] = {
        "assetId": "SF_REPAIR_TENDER_V1", "partId": "repair_tender_production_v1",
    ...
    if lod <= 1:
        ...
        boolean_cut(hull_obj, "RadWellS", (-3.15, 0.52, 0.58), (0.52, 0.26, 0.16))
        boolean_cut(hull_obj, "TransomRecess", (-6.55, 0.0, 0.02), (0.16, 0.30, 0.14))
    ...
    cover_loft_with_plates("TenderSkin", [
        (6.80, 0.48, 0.32, 0.06),
        (4.20, 1.18, 0.62, 0.12),
        (1.40, 1.62, 0.78, 0.12),
        (-1.40, 1.68, 0.80, 0.10),
    ...
    add_box("BridgeBrow", (4.28, 0.0, 1.32), (0.07, 0.24, 0.05), armor, collection, 0.003)
    add_box("CraneBase", (0.15, 0.0, 0.82), (0.28, 0.28, 0.14), mech, collection, 0.003)
    add_cylinder("CranePost", (0.15, 0.0, 1.15), 0.06, 0.55, mech, collection, 10, 0.002, rot=(0, 0, 0))
    add_box("CraneBoom", (0.95, 0.0, 1.42), (1.05, 0.07, 0.07), armor, collection, 0.002)
    add_box("CraneHook", (1.45, 0.0, 1.22), (0.04, 0.04, 0.18), mech, collection, 0.002)
    add_box("ToolPod", (-0.85, 0.55, 0.42), (0.55, 0.22, 0.18), armor, collection, 0.003)
    add_box("Winch", (0.15, 0.22, 0.92), (0.16, 0.12, 0.10), mech, collection, 0.002)

    for sign, side in ((-1, "Port"), (1, "Starboard")):
        add_rcs_cluster(side, (-1.2, 1.75 * sign, 0.16), mats, collection, sign=sign)
        add_box(f"SideWellLip_{side}", (0.10, 1.48 * sign, 0.36), (0.82, 0.04, 0.04), armor, collection, 0.002)
    ...
    add_service_hatch("Dorsal", (-1.85, 0.0, 0.72), mats, collection, sx=0.26, sy=0.16)
    add_box("KeelShoe", (0.1, 0.0, -0.52), (1.6, 0.18, 0.06), armor, collection, 0.003)
    add_box("RepairPlate", (2.15, -0.62, 0.48), (0.32, 0.14, 0.010), warning, collection, 0.002)
    add_box("DeckHatch", (-1.15, 0.0, 0.68), (0.36, 0.22, 0.03), armor, collection, 0.002)
    add_box("ChineStrakeP", (1.0, -1.42, 0.08), (2.2, 0.04, 0.05), armor, collection, 0.003)
    add_box("ChineStrakeS", (1.0, 1.42, 0.08), (2.2, 0.04, 0.05), armor, collection, 0.003)
    add_box("BowCap", (6.85, 0.0, 0.06), (0.12, 0.18, 0.10), armor, collection, 0.003)
    add_box("StripeP", (0.8, -1.42, 0.24), (1.8, 0.012, 0.008), accent, collection, 0.001)
    add_box("StripeS", (0.8, 1.42, 0.24), (1.8, 0.012, 0.008), accent, collection, 0.001)
    add_panel_seams("Hull", [3.6, 0.8, -1.6, -3.8], 1.05, 0.58, mech, collection)

    if lod == 0:
        add_curve_hose("Hose_MainP", [(0.8, -0.55, 0.12), (-2.2, -0.72, 0.12), (-5.4, -0.88, 0.08)], mech, collection, 0.010)
        add_curve_hose("Hose_MainS", [(0.8, 0.55, 0.12), (-2.2, 0.72, 0.12), (-5.4, 0.88, 0.08)], mech, collection, 0.010)
    if CYCLE >= 6 and lod <= 1:
        boolean_cut(hull_obj, "TransomRecess", (-5.95, 0.0, 0.08), (0.12, 0.30, 0.16))
    if CYCLE >= 7:
        add_box("CanopyMullion", (1.65, 0.0, 0.72), (0.018, 0.22, 0.06), armor, collection, 0.002)
        if lod <= 1:
            for i in range(4):
                add_box(f"SpineRad_{i}", (-1.65 + i * 0.16, 0.0, 0.38), (0.010, 0.12, 0.10), mech, collection, 0.001)
    if CYCLE >= 8:
        add_box("DriveBankBand", (-5.25, 0.0, 0.08), (0.045, 0.42, 0.14), armor, collection, 0.003)
    if CYCLE >= 9:
        add_box("ChineCapP", (1.05, -0.78, 0.08), (0.90, 0.026, 0.030), armor, collection, 0.003)
        add_box("ChineCapS", (1.05, 0.78, 0.08), (0.90, 0.026, 0.030), armor, collection, 0.003)
    if CYCLE >= 10:
        add_box("PatchTile2", (0.85, 0.24, 0.62), (0.16, 0.08, 0.006), armor, collection, 0.002)
        add_box("CanopyStayP", (1.35, -0.10, 0.54), (0.12, 0.010, 0.010), mech, collection, 0.002)
        add_box("CanopyStayS", (1.35, 0.10, 0.54), (0.12, 0.010, 0.010), mech, collection, 0.002)
...
def export_lod(collection, lod):
    out = FAMILY / "source" / "wholeships" / f"repair_tender_production_v1_lod{lod}.glb"
...
def render_cycle(collection):
    ...
    camera = setup_studio()
    out = FAMILY / "evidence" / "repair_tender" / "cycles" / f"cycle_{CYCLE:02d}"
    out.mkdir(parents=True, exist_ok=True)
    views = {
        "three_quarter": ((11.2, -13.4, 4.4), (0.1, 0, 0.20), 34),
        "starboard": ((0.2, 16.5, 3.2), (0.1, 0, 0.16), 32),
        "rear": ((-12.4, -5.4, 3.4), (-0.4, 0, 0.14), 32),
        "clay_three_quarter": ((11.2, -13.4, 4.4), (0.1, 0, 0.20), 34),
...
def main():
    ...
    for lod in (0, 1, 2):
        ...
        if report["hullTriangles"] < 800:
            raise RuntimeError(f"repair_tender lod{lod} hull {report['hullTriangles']} < 800")
        collections.append(collection)
        reports.append(report)
    stills = render_cycle(collections[0])
    report = {
        "schema": "spaceface.repairTenderMtx.cycle.v1",
        "shipId": "repair_tender",
        "cycle": CYCLE,
        "lods": reports,
        "stills": str(stills.relative_to(FAMILY)).replace("\\", "/"),
    }
    (FAMILY / "evidence" / "repair_tender").mkdir(parents=True, exist_ok=True)
    (FAMILY / "evidence" / "repair_tender" / f"cycle_{CYCLE:02d}.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
