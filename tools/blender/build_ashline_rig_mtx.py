"""PQ-050.15 Ashline rig MTX builder. Hitch untouched. --mtx-cycle N writes cycle stills."""
...
FAMILY = ROOT / "assets" / "ships" / "fleet_player_bodies_v1" / "ashline_rig"
...
def role_maps(role, rgb, size=TEX, prefix=None):
    """Unique Ashline rig maps. Patched pirate, not a lode tint."""
    ...
    base = write_pixels(f"rig_{prefix}_basecolor", albedo, size, "sRGB")
    orm_img = write_pixels(f"rig_{prefix}_orm", orm, size, "Non-Color")
    nrm_img = write_pixels(f"rig_{prefix}_normal", nrm, size, "Non-Color")
...
def create_materials():
    specs = {
        "Material_Hull": ((0.18, 0.10, 0.08), 0.06, 0.58, "hull", 0.06, None),
...
def sockets():
    # Frozen factory rig sockets (length 15, halfW 2.0, height 0.85).
    return {
        ...
        "SOCKET_Trail_Port": (-6.5, -1.40, 0.08),
        "SOCKET_Trail_Starboard": (-6.5, 1.40, 0.08),
...
def build_lod(lod, mats):
    collection = bpy.data.collections.new(f"RIG_LOD{lod}")
    bpy.context.scene.collection.children.link(collection)
    hull, armor, mech = mats["Material_Hull"], mats["Material_Armor"], mats["Material_Mechanical"]
    warning = mats["Material_Warning"]
    root = add_empty(f"RIG_LOD{lod}_ROOT", (0, 0, 0), collection)
    root["spacefaceAsset"] = {
        "assetId": "SF_ASHLINE_RIG_V1", "partId": "ashline_rig_production_v1",
        "lod": f"lod{lod}", "slot": "hull", "category": "wholeships",
        "forward": "+X", "embeddedPlume": False,
    }
    # 15 m Reach pirate: patched hull, winglets, twin drives, front and rear guns. Not lode.
    hull_obj = loft_from_rings("Pressure_Hull", [
        station_ring(7.5, 0, 0.08, 0.48, 0.32, flat=0.15, box=0.20, keel=0.75),
        station_ring(4.6, 0, 0.12, 1.35, 0.70, flat=0.50, box=0.40, keel=0.55),
        station_ring(1.4, 0, 0.12, 1.95, 0.90, flat=0.40, box=0.80, keel=0.40),
        station_ring(-1.2, 0, 0.10, 2.00, 0.92, flat=0.35, box=0.90, keel=0.32),
        station_ring(-4.2, 0, 0.10, 1.45, 0.72, flat=0.28, box=0.92, keel=0.25),
        station_ring(-7.05, 0, 0.06, 0.76, 0.44, flat=0.22, box=0.92, keel=0.20),
    ...
    add_cylinder("TurretRing", (0.2, 0.0, 1.18), 0.28, 0.08, armor, collection, 14, 0.003, rot=(0, 0, 0))
    add_sensor_dish("Dorsal", (0.55, 0.0, 1.42), mats, collection)

    for sign, side in ((-1, "Port"), (1, "Starboard")):
        loft_from_rings(f"Winglet_{side}", [
            wing_station(0.85, 1.85 * sign, 0.06, 1.55, 0.18),
            wing_station(0.15, 2.55 * sign, 0.02, 1.15, 0.12),
            wing_station(-0.65, 3.05 * sign, -0.02, 0.72, 0.07),
        ], hull, collection, 0.005, cap=True)
        add_box(f"WingRoot_{side}", (0.35, 1.85 * sign, 0.08), (0.65, 0.14, 0.12), mech, collection, 0.003)
        add_box(f"GunCheek_{side}", (4.75, 0.52 * sign, 0.20), (0.65, 0.14, 0.10), armor, collection, 0.004)
        add_box(f"GunReceiver_{side}", (5.35, 0.50 * sign, 0.22), (0.24, 0.09, 0.08), mech, collection, 0.003)
        add_cylinder(f"GunJacket_{side}", (5.95, 0.50 * sign, 0.22), 0.042, 0.62, armor, collection, 10, 0.002)
        add_cylinder(f"GunBarrel_{side}", (6.50, 0.50 * sign, 0.22), 0.022, 0.38, mech, collection, 8, 0.001)
        add_flared_bell(side, -7.25, 1.10 * sign, 0.08, 0.95, mats, collection)
        add_cylinder(f"DriveCoupling_{side}", (-6.95, 1.10 * sign, 0.08), 0.32, 0.16, mech, collection, 14, 0.003)
        add_manufactured_drive(side, -6.55, 1.10 * sign, lod, mats, collection, scale=0.78, z=0.08)
        add_rcs_cluster(side, (-1.2, 2.15 * sign, 0.16), mats, collection, sign=sign)
        add_box(f"Patch_{side}", (0.4, 1.55 * sign, 0.55), (0.28, 0.08, 0.012), warning, collection, 0.002)

    add_box("RearGun", (-5.85, 0.0, 0.62), (0.32, 0.16, 0.12), mech, collection, 0.004)
    add_cylinder("RearBarrel", (-6.35, 0.0, 0.62), 0.028, 0.42, armor, collection, 8, 0.002)
    add_service_hatch("DorsalFore", (1.05, 0.55, 0.88), mats, collection, sx=0.28, sy=0.20)
    add_service_hatch("DorsalAft", (-1.85, -0.35, 0.82), mats, collection, sx=0.26, sy=0.18)
    add_box("Hatch_Yellow", (-0.4, 0.55, 0.88), (0.22, 0.18, 0.016), warning, collection, 0.002)
    add_box("Hatch_Blue", (0.85, -0.62, 0.84), (0.20, 0.16, 0.014), mats["Material_Accent"], collection, 0.002)
    add_box("VentralKeel", (-0.8, 0.0, -0.72), (3.2, 0.14, 0.05), armor, collection, 0.004)
    add_box("ChinPlate", (4.4, 0.0, -0.22), (1.4, 0.48, 0.05), armor, collection, 0.003)
    add_box("ArmorBeltP", (0.4, -1.95, -0.08), (2.1, 0.05, 0.14), armor, collection, 0.003)
    add_box("ArmorBeltS", (0.4, 1.95, -0.08), (2.1, 0.05, 0.14), armor, collection, 0.003)
    add_box("TurretMantlet", (0.35, 0.0, 1.28), (0.22, 0.18, 0.10), armor, collection, 0.003)
    add_box("AftRack", (-3.6, 0.0, 0.72), (0.55, 0.28, 0.08), mech, collection, 0.003)
    for i in range(4):
        add_box(f"Slat_{i}", (0.2 - i * 0.22, -1.88, 0.22), (0.08, 0.012, 0.10), mats["Material_Radiator"], collection, 0.002)
    add_box("BowPatch", (3.6, 0.65, 0.62), (0.35, 0.12, 0.012), warning, collection, 0.002)
    add_box("TransomBraceP", (-6.65, -0.38, 0.20), (0.18, 0.05, 0.14), mech, collection, 0.002)
    add_box("TransomBraceS", (-6.65, 0.38, 0.20), (0.18, 0.05, 0.14), mech, collection, 0.002)
    add_box("WingFenceP", (-0.15, -2.55, 0.14), (0.35, 0.012, 0.10), armor, collection, 0.002)
    add_box("WingFenceS", (-0.15, 2.55, 0.14), (0.35, 0.012, 0.10), armor, collection, 0.002)
    add_box("HardpointP", (0.05, -2.15, -0.14), (0.24, 0.05, 0.05), mech, collection, 0.002)
    add_box("HardpointS", (0.05, 2.15, -0.14), (0.24, 0.05, 0.05), mech, collection, 0.002)
    add_box("CockpitSill", (2.2, 0.0, 0.86), (0.95, 0.42, 0.03), armor, collection, 0.003)
    add_panel_seams("Hull", [2.8, 0.6, -1.4, -3.8], 1.45, 0.72, mech, collection)

    if lod == 0:
        add_curve_hose("Hose_Port", [(0.6, -1.05, 0.10), (-2.8, -1.15, 0.12), (-6.2, -1.05, 0.10)], mech, collection, 0.013)
        add_curve_hose("Hose_Stbd", [(0.6, 1.05, 0.10), (-2.8, 1.15, 0.12), (-6.2, 1.05, 0.10)], mech, collection, 0.013)
    if CYCLE >= 6 and lod <= 1:
        boolean_cut(hull_obj, "TransomRecess", (-6.95, 0.0, 0.10), (0.14, 0.38, 0.20))
    if CYCLE >= 7:
        add_box("CanopyMullion", (1.95, 0.0, 0.88), (0.020, 0.26, 0.07), armor, collection, 0.002)
        if lod <= 1:
            for sign, side in ((-1, "Port"), (1, "Starboard")):
                for i in range(4):
                    add_box(f"FlankRad_{side}_{i}", (-1.85 + i * 0.18, 1.35 * sign, 0.16), (0.012, 0.14, 0.12), mech, collection, 0.001)
    if CYCLE >= 8:
        add_box("DriveBankBand", (-6.05, 0.0, 0.10), (0.055, 0.72, 0.16), armor, collection, 0.003)
    if CYCLE >= 9:
        add_box("ChineCapP", (1.35, -1.25, 0.10), (1.05, 0.032, 0.036), armor, collection, 0.003)
        add_box("ChineCapS", (1.35, 1.25, 0.10), (1.05, 0.032, 0.036), armor, collection, 0.003)
    if CYCLE >= 10:
        add_box("PatchTile2", (0.95, 0.38, 0.82), (0.18, 0.09, 0.008), armor, collection, 0.002)
    ...
    bm = bmesh.new()
    for point in [
        (7.2, 0, 0.1), (1.4, -2.1, 0.7), (1.4, 2.1, 0.7),
        (-6.9, -1.4, 0.2), (-6.9, 1.4, 0.2),
        (0.3, -3.1, 0.1), (0.3, 3.1, 0.1),
        (0.6, 0, 1.5),
    ]:
        bm.verts.new(point)
...
def export_lod(collection, lod):
    out = FAMILY / "source" / "wholeships" / f"ashline_rig_production_v1_lod{lod}.glb"
...
def render_cycle(collection):
    ...
    camera = setup_studio()
    out = FAMILY / "evidence" / "ashline_rig" / "cycles" / f"cycle_{CYCLE:02d}"
    out.mkdir(parents=True, exist_ok=True)
    views = {
        "three_quarter": ((12.4, -14.6, 5.0), (0, 0, 0.16), 32),
        "starboard": ((0.0, 19.0, 3.8), (0, 0, 0.12), 30),
        "rear": ((-14.4, -6.2, 3.8), (-0.5, 0, 0.12), 30),
        "clay_three_quarter": ((12.4, -14.6, 5.0), (0, 0, 0.16), 32),
        "grazing_close": ((7.2, -6.0, 1.8), (0.7, 0, 0.20), 46),
        "bay_interior": ((2.2, -3.2, 1.0), (2.1, 0.0, 0.80), 40),
        "drive_rear": ((-10.4, -4.2, 1.4), (-6.6, 0, 0.08), 42),
        "play_size": ((40, -36, 15), (0, 0, 0.10), 48),
        "orm_isolation": ((12.4, -14.6, 5.0), (0, 0, 0.16), 32),
        "normal_isolation": ((12.4, -14.6, 5.0), (0, 0, 0.16), 32),
        "id_or_material_id": ((12.4, -14.6, 5.0), (0, 0, 0.16), 32),
        "material_three_quarter": ((12.4, -14.6, 5.0), (0, 0, 0.16), 32),
    }
...
def main():
    ...
    for lod in (0, 1, 2):
        ...
        if report["hullTriangles"] < 800:
            raise RuntimeError(f"ashline_rig lod{lod} hull {report['hullTriangles']} < 800")
        collections.append(collection)
        reports.append(report)
    stills = render_cycle(collections[0])
    report = {
        "schema": "spaceface.ashlineRigMtx.cycle.v1",
        "shipId": "ashline_rig",
        "cycle": CYCLE,
        "lods": reports,
        "stills": str(stills.relative_to(FAMILY)).replace("\\", "/"),
    }
    (FAMILY / "evidence" / "ashline_rig").mkdir(parents=True, exist_ok=True)
    (FAMILY / "evidence" / "ashline_rig" / f"cycle_{CYCLE:02d}.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
