...
const out=path.join(root,'.devshots/helios-remaster/additional-fleet');
const registry=JSON.parse(fs.readFileSync(path.join(root,'tools/blender/helios_remaster/additional_fleet_sources.json'),'utf8').replace(/^\uFEFF/,''));
...
function renderBounds(doc,visibleLod='0'){
  const result=new Box3();
  function visit(i,parent,hidden=false){
    const n=doc.nodes[i],local=new Matrix4();
    if(n.matrix)local.fromArray(n.matrix);else local.compose(new Vector3(...(n.translation||[0,0,0])),new Quaternion(...(n.rotation||[0,0,0,1])),new Vector3(...(n.scale||[1,1,1])));
    local.premultiply(parent);
    const nodeLod=n.name?.match(/^LOD([012])_/i)?.[1];
    hidden ||= /COLLISION/i.test(n.name||'')||(nodeLod!==undefined&&nodeLod!==visibleLod)||Boolean(n.extras?.nonRender);
...
for(const [name,pinned] of Object.entries(registry)){
  ...
  const unownedRemasterMeshes=(b.doc.nodes||[]).filter(n=>n.mesh!==undefined&&/Remaster/.test(n.name||'')&&!/^LOD[012]_/.test(n.name||''));
  if(unownedRemasterMeshes.length)throw new Error(`Remaster meshes missing native LOD ownership: ${name}`);
  ...
  const sceneMeta=b.doc.scenes[b.doc.scene||0]?.extras?.spacefaceAsset;
  if(sceneMeta?.assetId!==bm.assetId||sceneMeta?.slot!==bm.slot)throw new Error(`Active scene identity mismatch ${name}`);
  ...
  map.push({asset:name,assetId:am.assetId,sourcePath:pinned.sourcePath,sourceBlob:pinned.sourceBlob,
    ...
    sourceVisibleBounds:renderBounds(a.doc,name.match(/_lod([12])$/)?.[1]||'0'),candidateVisibleBounds:renderBounds(b.doc,name.match(/_lod([12])$/)?.[1]||'0'),
    socketCount:Object.keys(oldSockets).length,maxSocketTransformDelta:maxSocketDelta,lods:deltas});
}
fs.writeFileSync(path.join(out,'promotion.json'),JSON.stringify({schema:'helios.additional-fleet-promotion.v1',note:'Raw source draw counts; final release package batching/cost owned by controller. No source or release files changed by this report.',models:map},null,2));
