...
HONEST = {
    "Material_Hull": {"color": (0.82, 0.83, 0.85), "metallic": 0.06, "roughness": 0.42, "role": "hull"},
...
def build_wings(mats):
    ...
    for sign, tag in ((1.0, "Stbd"), (-1.0, "Port")):
        rings = [
            airfoil(1.55, 1.12 * sign, 0.18, 3.15, 1.15),
...
def build_canopy(hull, mats):
    ...
    def tub():
        return add_box("CanopyTubCut", (2.05, 1.05, 0.85), (3.35, 0.0, 0.85), hull_mat, 0.0)
    ...
    pane = loft_rings(
        "LOD0_Canopy",
        [
            canopy_ring(4.28, 0.22, 0.10, 0.62),
            canopy_ring(3.70, 0.52, 0.38, 0.78),
            canopy_ring(3.10, 0.48, 0.32, 0.72),
            canopy_ring(2.55, 0.22, 0.08, 0.52),
        ],
        glass, 0.003,
    )
    coaming = add_box("LOD0_CanopyCoaming", (2.12, 1.12, 0.04), (3.32, 0.0, 0.52), hull_mat, 0.002)
...
def build_drives(hull, mats):
    ...
    for sign, tag in ((-1.0, "Port"), (1.0, "Stbd")):
        ...
        collar = add_cyl(
            ...
            ceramic, rotation=(0.0, math.radians(58.0), 0.0), vertices=16,
        )
        floor = add_cyl(
        ...
        def aft():
            return add_cyl(
                f"DriveAft_{tag}", 0.48, 1.25, (-5.05, y, 0.28), hull_mat,
                rotation=(0.0, math.radians(58.0), 0.0), vertices=16,
            )

        report.append({"tag": tag, "dorsal": False, "aft": cut(hull, aft)})
