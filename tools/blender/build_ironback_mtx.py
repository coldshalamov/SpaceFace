"""PQ-050.04 Ironback MTX builder. Hitch untouched. --mtx-cycle N writes cycle stills."""
...
def create_materials():
    specs = {
        "Material_Hull": ((0.32, 0.18, 0.10), 0.10, 0.50, "hull", 0.25, None),
...
def build_lod(lod, mats):
    ...
    root["spacefaceAsset"] = {
        "assetId": "SF_IRONBACK_PRODUCTION_V1", "partId": "ironback_production_v1",
    ...
    if lod <= 1:
        ...
        cut_open_bay(hull_obj, "Port", (0.10, -2.28, 0.20), 1.85, 0.55, 0.58, (0, -1, 0), mats, collection, kit="empty", liner=False)
        cut_open_bay(hull_obj, "Starboard", (0.10, 2.28, 0.20), 1.85, 0.55, 0.58, (0, 1, 0), mats, collection, kit="empty", liner=False)
        cut_open_bay(hull_obj, "Hopper", (-0.10, 0.0, 1.32), 2.20, 1.15, 0.58, (0, 0, 1), mats, collection, kit="empty", liner=False)
    ...
    loft_shell("Hopper_ShellP", [
        (3.20, -1.20, -2.05, 0.40, 1.35),
        (0.20, -1.60, -2.55, 0.20, 1.18),
        (-2.80, -1.20, -2.10, 0.16, 1.02),
    ], hull, collection, 0.012)
    loft_shell("Hopper_ShellS", [
        (3.20, 1.20, 2.05, 0.40, 1.35),
        (0.20, 1.60, 2.55, 0.20, 1.18),
        (-2.80, 1.20, 2.10, 0.16, 1.02),
    ], hull, collection, 0.012)
    ...
    add_folded_sheet(
        "Chine_P",
        (2.20, -1.55, 0.10), (-1.80, -1.59, 0.12),
        (-1.80, -1.47, 0.26), (2.20, -1.43, 0.24),
        0.024, armor, collection, 0.004,
    )
    add_folded_sheet(
        "Chine_S",
        (2.20, 1.55, 0.10), (2.20, 1.43, 0.24),
    ...
    for sign, side in ((-1, "Port"), (1, "Starboard")):
        ...
        add_manufactured_drive(side, -7.75, 1.70 * sign, lod, mats, collection, scale=1.05, z=0.10)
    ...
    add_sensor_dish("Dorsal", (2.55, 0.55, 1.45), mats, collection)
    add_cylinder("Turret_Ring", (1.55, 0.0, 1.48), 0.38, 0.10, mech, collection, vertices=14, bevel=0.006, rot=(0, 0, 0))
    add_box("Turret_Head", (1.70, 0.0, 1.64), (0.32, 0.20, 0.10), armor, collection, 0.006)
