"""PQ-050.19 Ore barge MTX builder. Hitch untouched. --mtx-cycle N writes cycle stills."""
...
FAMILY = ROOT / "assets" / "ships" / "fleet_player_bodies_v1" / "ore_barge"
...
def role_maps(role, rgb, size=TEX, prefix=None):
    """Unique ore barge maps. Rust-brown workboat, not Helios pale."""
    ...
    base = write_pixels(f"barge_{prefix}_basecolor", albedo, size, "sRGB")
    orm_img = write_pixels(f"barge_{prefix}_orm", orm, size, "Non-Color")
    nrm_img = write_pixels(f"barge_{prefix}_normal", nrm, size, "Non-Color")
...
def create_materials():
    specs = {
        "Material_Hull": ((0.28, 0.22, 0.14), 0.06, 0.58, "hull", 0.04, None),
        "Material_Armor": ((0.22, 0.18, 0.12), 0.10, 0.54, "armor", 0.02, None),
...
def sockets():
    # Frozen factory ore barge sockets (length 19, halfW 2.4, height 1.2).
    return {
        "SOCKET_Weapon_Front": (8.3, 0.0, 0.25),
        "SOCKET_Mining_Front": (8.9, 0.0, -0.15),
        "SOCKET_Engine_Main": (-8.3, 0.0, 0.08),
        "SOCKET_Trail_Main": (-8.7, 0.0, 0.08),
        "SOCKET_Trail_Port": (-8.5, -1.68, 0.08),
        "SOCKET_Trail_Starboard": (-8.5, 1.68, 0.08),
        "SOCKET_Utility_Dorsal": (0.6, 0.0, 1.75),
        "SOCKET_Cargo_Ventral": (-0.4, 0.0, -1.35),
        "SOCKET_Camera_Focus": (0.8, 0.0, 0.25),
        "SOCKET_RCS_Port": (-1.2, -2.60, 0.15),
        "SOCKET_RCS_Starboard": (-1.2, 2.60, 0.15),
    }
...
def build_lod(lod, mats):
    collection = bpy.data.collections.new(f"BARGE_LOD{lod}")
    ...
    root = add_empty(f"BARGE_LOD{lod}_ROOT", (0, 0, 0), collection)
    root["spacefaceAsset"] = {
        "assetId": "SF_ORE_BARGE_V1", "partId": "ore_barge_production_v1",
        "lod": f"lod{lod}", "slot": "hull", "category": "wholeships",
        "forward": "+X", "embeddedPlume": False,
    }
    # 19 m rust ore barge: wide hard-chine hull, dump well, hoppers, one pod, twin tail drives.
    hull_obj = loft_from_rings("Pressure_Hull", [
        station_ring(9.5, 0, 0.08, 0.62, 0.32, flat=0.40, box=0.45, keel=0.45),
        station_ring(6.4, 0, 0.16, 1.55, 0.70, flat=0.70, box=0.55, keel=0.35),
        station_ring(2.4, 0, 0.22, 2.28, 1.12, flat=0.50, box=0.80, keel=0.28),
        station_ring(-0.8, 0, 0.22, 2.40, 1.22, flat=0.40, box=0.90, keel=0.25),
        station_ring(-4.4, 0, 0.16, 1.95, 0.92, flat=0.32, box=0.92, keel=0.22),
        station_ring(-7.2, 0, 0.10, 1.35, 0.62, flat=0.28, box=0.92, keel=0.20),
        station_ring(-9.05, 0, 0.02, 0.66, 0.38, flat=0.22, box=0.90, keel=0.18),
    ], hull, collection, 0.012, cap="both")
    if lod <= 1:
        cut_open_bay(hull_obj, "Cabin", (5.15, 0.0, 1.15), 1.05, 0.38, 0.28, (0, 0, 1), mats, collection, kit="cockpit", liner=False)
        boolean_cut(hull_obj, "DumpWell", (0.20, 0.0, 1.15), (1.85, 0.95, 0.48))
        boolean_cut(hull_obj, "SideWellP", (0.20, -2.38, 0.28), (1.15, 0.22, 0.28))
        boolean_cut(hull_obj, "SideWellS", (0.20, 2.38, 0.28), (1.15, 0.22, 0.28))
        boolean_cut(hull_obj, "RadWellP", (-4.55, -0.72, 0.72), (0.62, 0.30, 0.18))
        boolean_cut(hull_obj, "RadWellS", (-4.55, 0.72, 0.72), (0.62, 0.30, 0.18))
        boolean_cut(hull_obj, "TransomRecess", (-9.05, 0.0, 0.02), (0.18, 0.38, 0.16))
    ...
    loft_shell("Hopper_P", [
        ...
        (0.20, -1.55, -2.35, 0.12, 0.92),
        (-3.40, -1.20, -2.00, 0.10, 0.78),
    ...
    center_loft("Hopper_Carapace", [
        (3.60, 0.62, 0.72, 1.18),
        (0.20, 0.88, 0.62, 1.12),
        (-3.20, 0.58, 0.52, 0.92),
    ], hull, collection, 0.010)
    add_box("TransomPlate", (-8.98, 0.0, 0.04), (0.05, 0.58, 0.28), armor, collection, 0.003)
    add_box("TransomInner", (-8.86, 0.0, 0.04), (0.05, 0.42, 0.18), mech, collection, 0.002)
    add_box("TransomShelf", (-8.72, 0.0, -0.16), (0.22, 0.78, 0.05), armor, collection, 0.003)
    add_box("RepairPlate", (3.25, -0.95, 0.72), (0.42, 0.18, 0.012), warning, collection, 0.002)
    add_box("CargoDoorSide", (-1.4, 2.28, 0.22), (0.55, 0.04, 0.22), armor, collection, 0.003)
    add_thin_canopy("Cabin", 5.15, 0.0, 1.08, 1.05, 0.36, 0.26, mats, collection)
    add_box("CabinBrow", (5.62, 0.0, 1.24), (0.08, 0.28, 0.06), armor, collection, 0.003)
    add_box("CabinSill", (5.15, 0.0, 1.06), (0.52, 0.38, 0.016), armor, collection, 0.002)
    add_box("WellLipF", (1.95, 0.0, 1.28), (0.05, 0.98, 0.06), warning, collection, 0.002)
    add_box("WellLipA", (-1.55, 0.0, 1.28), (0.05, 0.98, 0.06), warning, collection, 0.002)
    add_box("WellLipP", (0.20, -0.96, 1.28), (1.70, 0.04, 0.06), warning, collection, 0.002)
    add_box("WellLipS", (0.20, 0.96, 1.28), (1.70, 0.04, 0.06), warning, collection, 0.002)
    add_box("WellFloor", (0.20, 0.0, 0.42), (1.65, 0.82, 0.03), mech, collection, 0.003)
    add_box("DumpChute", (0.20, 0.0, 0.18), (0.55, 0.32, 0.18), mech, collection, 0.003)
    if lod == 0:
        for i, ox in enumerate((-0.85, -0.25, 0.35, 0.95)):
            add_box(f"WellSlat_{i}", (ox, 0.0, 0.50), (0.05, 0.62, 0.012), mech, collection, 0.001)

    # One formed cargo pod per side (factory pods=1) plus dump hoppers on the well cheeks.
    for sign, side in ((-1, "Port"), (1, "Starboard")):
        y = 2.05 * sign
        add_cargo_pod(side, -0.15, y, 0.18, 3.40, 0.48, mats, collection, lod)
        loft_from_rings(f"HopperBin_{side}", [
            [
                (1.55, y * 0.62 - 0.28 * sign, 0.85), (1.55, y * 0.62 - 0.28 * sign, 1.55),
                (1.55, y * 0.62 + 0.28 * sign, 1.55), (1.55, y * 0.62 + 0.28 * sign, 0.85),
                (1.55, y * 0.62 + 0.12 * sign, 0.78), (1.55, y * 0.62 - 0.12 * sign, 0.78),
                (1.55, y * 0.62 - 0.20 * sign, 0.82), (1.55, y * 0.62 + 0.20 * sign, 0.82),
            ],
            [
                (-0.15, y * 0.62 - 0.34 * sign, 0.82), (-0.15, y * 0.62 - 0.34 * sign, 1.62),
                (-0.15, y * 0.62 + 0.34 * sign, 1.62), (-0.15, y * 0.62 + 0.34 * sign, 0.82),
                (-0.15, y * 0.62 + 0.14 * sign, 0.74), (-0.15, y * 0.62 - 0.14 * sign, 0.74),
                (-0.15, y * 0.62 - 0.22 * sign, 0.78), (-0.15, y * 0.62 + 0.22 * sign, 0.78),
            ],
        ], hull, collection, 0.008, cap=False)
        add_box(f"HopperLip_{side}", (0.20, y * 0.62 + 0.18 * sign, 1.60), (1.15, 0.03, 0.03), warning, collection, 0.002)
        add_box(f"PodRail_{side}", (-0.15, y, -0.34), (1.8, 0.08, 0.06), mech, collection, 0.003)
        add_box(f"PodLock_{side}", (1.45, y, 0.18), (0.06, 0.10, 0.10), mech, collection, 0.002)
        add_rcs_cluster(side, (-1.2, 2.55 * sign, 0.18), mats, collection, sign=sign)
        add_box(f"SideWellLip_{side}", (0.20, 1.82 * sign, 0.42), (1.00, 0.04, 0.04), armor, collection, 0.002)
        if lod <= 1:
            for i in range(6):
                add_box(
                    f"SideFin_{side}_{i}",
                    (-0.25 + i * 0.14, 1.88 * sign, 0.28),
                    (0.012, 0.10, 0.16),
    ...
    for sign, side in ((-1, "Port"), (1, "Starboard")):
        add_cylinder(f"DriveCoupling_{side}", (-8.30, 1.50 * sign, 0.08), 0.36, 0.18, mech, collection, 16, 0.003)
        add_manufactured_drive(side, -8.00, 1.50 * sign, lod, mats, collection, scale=0.92, z=0.08)
        add_box(f"DriveSaddle_{side}", (-7.90, 1.50 * sign, -0.24), (0.44, 0.14, 0.09), mech, collection, 0.003)
        add_box(f"RadCassette_{side}", (-4.15, 0.62 * sign, 0.68), (0.52, 0.22, 0.10), mats["Material_Radiator"], collection, 0.002)
        add_box(f"RadHeader_{side}", (-4.15, 0.62 * sign, 0.80), (0.54, 0.04, 0.03), mech, collection, 0.002)
        if lod <= 1:
            for i in range(8):
                add_box(
                    f"RadFin_{side}_{i}",
                    (-4.42 + i * 0.08, 0.62 * sign, 0.74),
                    (0.010, 0.18, 0.16),
                    mech, collection, 0.001,
                )

    add_service_hatch("Dorsal", (-2.55, 0.0, 0.78), mats, collection, sx=0.32, sy=0.18)
    add_box("DeckHatchA", (1.85, 0.0, 0.72), (0.55, 0.32, 0.03), armor, collection, 0.003)
    add_box("DeckHatchB", (-0.85, 0.38, 0.70), (0.42, 0.24, 0.03), armor, collection, 0.003)
    add_box("WalkRailP", (0.4, -0.72, 0.68), (2.8, 0.02, 0.03), mech, collection, 0.002)
    add_box("WalkRailS", (0.4, 0.72, 0.68), (2.8, 0.02, 0.03), mech, collection, 0.002)
    add_box("CargoDoor", (-0.4, 0.0, -0.72), (0.62, 0.36, 0.03), armor, collection, 0.003)
    add_box("CargoRim", (-0.4, 0.0, -0.68), (0.70, 0.42, 0.02), mech, collection, 0.002)
    add_box("KeelShoe", (0.2, 0.0, -0.72), (2.2, 0.22, 0.08), armor, collection, 0.004)
    add_box("RepairPlate", (3.15, -0.85, 0.62), (0.42, 0.18, 0.012), warning, collection, 0.002)
    add_box("TransomArmor", (-8.40, 0.0, 0.06), (0.06, 0.46, 0.22), armor, collection, 0.003)
    add_box("ChineStrakeP", (1.8, -1.72, 0.10), (3.2, 0.04, 0.06), armor, collection, 0.003)
    add_box("ChineStrakeS", (1.8, 1.72, 0.10), (3.2, 0.04, 0.06), armor, collection, 0.003)
    add_box("ChinPlate", (6.85, 0.0, -0.18), (1.45, 0.58, 0.05), armor, collection, 0.003)
    add_box("BowCap", (8.85, 0.0, 0.06), (0.16, 0.24, 0.12), armor, collection, 0.003)
    add_box("TransomBraceP", (-8.05, -0.42, 0.22), (0.20, 0.05, 0.14), mech, collection, 0.003)
    add_box("TransomBraceS", (-8.05, 0.42, 0.22), (0.20, 0.05, 0.14), mech, collection, 0.003)
    add_box("TransomShelf", (-8.22, 0.0, -0.18), (0.22, 0.72, 0.05), armor, collection, 0.003)
    add_box("CargoDoorSide", (-1.2, 1.92, 0.18), (0.55, 0.04, 0.22), armor, collection, 0.003)
    add_box("StripeP", (1.4, -1.88, 0.32), (2.8, 0.014, 0.010), accent, collection, 0.001)
    add_box("StripeS", (1.4, 1.88, 0.32), (2.8, 0.014, 0.010), accent, collection, 0.001)
    add_box("RegMark", (5.55, 0.62, 0.72), (0.48, 0.10, 0.008), warning, collection, 0.001)
    add_box("AftHatch", (-3.15, 0.34, 1.00), (0.26, 0.14, 0.012), armor, collection, 0.002)
    add_panel_seams("Hull", [5.2, 1.6, -1.8, -5.2], 1.45, 0.82, mech, collection)

    if lod == 0:
        add_curve_hose("Hose_MainP", [(1.4, -0.95, 0.14), (-3.0, -1.15, 0.16), (-7.2, -1.22, 0.10)], mech, collection, 0.012)
        add_curve_hose("Hose_MainS", [(1.4, 0.95, 0.14), (-3.0, 1.15, 0.16), (-7.2, 1.22, 0.10)], mech, collection, 0.012)
    if CYCLE >= 6 and lod <= 1:
        boolean_cut(hull_obj, "TransomRecess", (-7.85, 0.0, 0.10), (0.16, 0.48, 0.22))
    if CYCLE >= 7:
        add_box("CanopyMullion", (2.35, 0.0, 0.98), (0.022, 0.30, 0.08), armor, collection, 0.002)
        if lod <= 1:
            for sign, side in ((-1, "Port"), (1, "Starboard")):
                for i in range(4):
                    add_box(f"FlankRad_{side}_{i}", (-2.35 + i * 0.20, 1.65 * sign, 0.16), (0.012, 0.16, 0.14), mech, collection, 0.001)
    if CYCLE >= 8:
        add_box("DriveBankBand", (-6.85, 0.0, 0.10), (0.06, 0.95, 0.18), armor, collection, 0.003)
    if CYCLE >= 9:
        add_box("ChineCapP", (1.75, -1.55, 0.12), (1.25, 0.038, 0.042), armor, collection, 0.003)
        add_box("ChineCapS", (1.75, 1.55, 0.12), (1.25, 0.038, 0.042), armor, collection, 0.003)
    if CYCLE >= 10:
        add_box("PatchTile2", (1.15, 0.48, 0.92), (0.22, 0.10, 0.008), armor, collection, 0.002)
        add_box("HouseStayP", (2.05, -0.18, 0.78), (0.16, 0.014, 0.014), mech, collection, 0.002)
        add_box("HouseStayS", (2.05, 0.18, 0.78), (0.16, 0.014, 0.014), mech, collection, 0.002)
    ...
    bm = bmesh.new()
    for point in [
        (8.9, 0, 0.1), (4.8, -1.6, 1.6), (4.8, 1.6, 1.6),
        (-8.4, -1.5, 0.2), (-8.4, 1.5, 0.2),
        (1.0, -2.4, 0.6), (1.0, 2.4, 0.6),
        (-2.0, -2.4, 0.6), (-2.0, 2.4, 0.6),
        (0.2, 0, 1.7),
    ]:
        bm.verts.new(point)
...
def export_lod(collection, lod):
    out = FAMILY / "source" / "wholeships" / f"ore_barge_production_v1_lod{lod}.glb"
...
def render_cycle(collection):
    ...
    camera = setup_studio()
    out = FAMILY / "evidence" / "ore_barge" / "cycles" / f"cycle_{CYCLE:02d}"
    out.mkdir(parents=True, exist_ok=True)
    views = {
        "three_quarter": ((14.6, -17.2, 5.8), (0.2, 0, 0.28), 32),
        "starboard": ((0.4, 22.0, 4.2), (0.2, 0, 0.20), 30),
        "rear": ((-16.6, -7.2, 4.2), (-0.6, 0, 0.18), 30),
        "clay_three_quarter": ((14.6, -17.2, 5.8), (0.2, 0, 0.28), 32),
        "grazing_close": ((7.8, -6.4, 1.8), (0.8, 0, 0.26), 46),
        "bay_interior": ((4.6, -3.0, 1.7), (4.8, 0.0, 1.40), 40),
        "drive_rear": ((-11.0, -4.2, 1.4), (-7.6, 0, 0.08), 42),
        "play_size": ((42, -38, 17), (0, 0, 0.12), 48),
        "orm_isolation": ((13.8, -16.2, 5.4), (0.2, 0, 0.24), 32),
        "normal_isolation": ((13.8, -16.2, 5.4), (0.2, 0, 0.24), 32),
        "id_or_material_id": ((13.8, -16.2, 5.4), (0.2, 0, 0.24), 32),
        "material_three_quarter": ((13.8, -16.2, 5.4), (0.2, 0, 0.24), 32),
    }
...
def main():
    ...
    for lod in (0, 1, 2):
        ...
        if report["hullTriangles"] < 800:
            raise RuntimeError(f"ore_barge lod{lod} hull {report['hullTriangles']} < 800")
        collections.append(collection)
        reports.append(report)
    stills = render_cycle(collections[0])
    report = {
        "schema": "spaceface.oreBargeMtx.cycle.v1",
        "shipId": "ore_barge",
        "cycle": CYCLE,
        "lods": reports,
        "stills": str(stills.relative_to(FAMILY)).replace("\\", "/"),
    }
    (FAMILY / "evidence" / "ore_barge").mkdir(parents=True, exist_ok=True)
    (FAMILY / "evidence" / "ore_barge" / f"cycle_{CYCLE:02d}.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
