...
HONEST = {
    "Material_Hull": {"color": (0.58, 0.59, 0.61), "metallic": 0.08, "roughness": 0.48, "role": "hull"},
    "Material_Armor": {"color": (0.18, 0.19, 0.21), "metallic": 0.20, "roughness": 0.46, "role": "armor"},
...
def build_wings(mats):
    ...
    for sign, tag in ((1.0, "Stbd"), (-1.0, "Port")):
        rings = [
            airfoil(1.55, 1.08 * sign, 0.12, 3.05, 0.80),
            airfoil(0.45, 2.05 * sign, 0.18, 2.25, 0.50),
            airfoil(-0.35, 2.95 * sign, 0.24, 1.45, 0.26),
            airfoil(-0.95, 3.85 * sign, 0.28, 0.70, 0.11),
        ]
        wing = loft_rings(f"LOD0_Wing_{tag}", rings, armor, 0.014)

        def slot_cut(name=f"FlapSlotCut_{tag}", loc=(-0.78, 2.45 * sign, 0.16)):
            return add_box(name, (0.22, 1.55, 0.30), loc, mech, 0.0)
        ...
        flap = loft_rings(
            ...
            [
                ...
                airfoil(-1.22, 3.25 * sign, 0.14, 0.36, 0.06),
            ],
            armor, 0.006,
        )
...
def build_canopy(hull, mats):
    ...
    def tub():
        return add_box("CanopyTubCut", (1.55, 0.70, 0.48), (3.42, 0.0, 0.56), hull_mat, 0.0)

    report["tub"] = cut(hull, tub)
    cavity = add_box("LOD0_CanopyCavity", (1.22, 0.52, 0.07), (3.40, 0.0, 0.24), mech, 0.001)
    pane = loft_rings(
        "LOD0_Canopy",
        [
            canopy_ring(4.22, 0.14, 0.07, 0.44),
            canopy_ring(3.78, 0.34, 0.24, 0.50),
            canopy_ring(3.32, 0.32, 0.20, 0.48),
            canopy_ring(2.92, 0.16, 0.06, 0.42),
        ],
        glass, 0.003,
    )
    coaming = add_box("LOD0_CanopyCoaming", (1.58, 0.82, 0.03), (3.38, 0.0, 0.38), hull_mat, 0.002)
    return [cavity, pane, coaming], report


def build_drives(hull, mats):
    hull_mat = mats["Material_Hull"]
    ceramic = mats["Material_Ceramic"]
    thrust = mats["Material_Thruster"]
    bits = []
    report = []
    for sign, tag in ((-1.0, "Port"), (1.0, "Stbd")):
        y = 0.62 * sign
        collar = add_cyl(
            f"LOD0_DriveCollar_{tag}", 0.46, 0.10, (-5.18, y, 0.14),
            ceramic, rotation=(0.0, math.radians(90.0), 0.0), vertices=16,
        )
        liner = add_cyl(
            f"LOD0_DriveLiner_{tag}", 0.40, 0.85, (-4.55, y, 0.14),
            ceramic, rotation=(0.0, math.radians(90.0), 0.0), vertices=16, cap="NOTHING",
        )
        hub = add_cyl(
            f"LOD0_DriveHub_{tag}", 0.10, 0.18, (-4.15, y, 0.14),
            thrust, rotation=(0.0, math.radians(90.0), 0.0), vertices=10,
        )
        lip = add_cyl(
            f"LOD0_DriveLip_{tag}", 0.48, 0.06, (-4.45, y, 0.52),
            hull_mat, vertices=16,
        )
        vanes = []
        for index in range(10):
            angle = math.tau * index / 10.0
            vane = add_box(
                f"LOD0_DriveVane_{tag}_{index}",
                (0.52, 0.028, 0.13),
                (-4.50, y + math.cos(angle) * 0.22, 0.14 + math.sin(angle) * 0.22),
                ceramic, 0.001,
                rotation=(angle, math.radians(90.0), 0.0),
            )
            vanes.append(vane)

        def dorsal():
            return add_cyl(f"DriveDorsal_{tag}", 0.50, 1.05, (-4.45, y, 0.55), hull_mat, vertices=16)

        def aft():
            return add_cyl(
                f"DriveAft_{tag}", 0.42, 1.05, (-5.22, y, 0.14), hull_mat,
                rotation=(0.0, math.radians(90.0), 0.0), vertices=16,
            )

        report.append({"tag": tag, "dorsal": cut(hull, dorsal), "aft": cut(hull, aft)})
        bits.extend([collar, liner, hub, lip, *vanes])
    return bits, report


def build_radiator(hull, mats):
    """Waist-flank louvers. A roof well photographs as a second eye at 60°."""
    hull_mat = mats["Material_Hull"]
    rad = mats["Material_Radiator"]

    def well():
        return add_box("RadiatorCut", (1.15, 0.10, 0.36), (-0.35, 0.52, 0.12), hull_mat, 0.0)
...
def main():
    ...
    armor_target = next(obj for obj in bpy.data.objects if obj.name.startswith("LOD0_Wing_Stbd"))
    armor_bits = [
        obj for obj in bpy.data.objects
        if obj.type == "MESH" and obj.data.materials and obj.data.materials[0] == mats["Material_Armor"]
        and obj != armor_target and not is_collision(obj)
    ]
    join_into(armor_target, armor_bits)
    canopy_target = next(obj for obj in bpy.data.objects if obj.name == "LOD0_Canopy")
    join_into(canopy_target, [
        obj for obj in bpy.data.objects
        if obj.name == "LOD0_CanopyCavity"
    ])
    hull_bits = [
        obj for obj in bpy.data.objects
    ...
    mech_bits = [
        obj for obj in bpy.data.objects
        if obj.type == "MESH" and obj.data.materials and obj.data.materials[0] == mats["Material_Mechanical"]
        and not is_collision(obj)
    ]
    if mech_bits:
        join_into(mech_bits[0], mech_bits[1:])
