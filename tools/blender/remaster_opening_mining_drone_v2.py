#!/usr/bin/env python3
"""Build a non-promoting, gameplay-contract-preserving mining drone candidate (v2).

Iteration 2 repairs the defects measured on v1's matched preview captures: smooth
unconstructed armor plates, a single-plane cutter crown, no believable propulsion
hardware, one-sided service runs, and an unguarded debris path behind the drum.
...
def rcs_pod(name,x,y,z,side,mat,lod,root) -> None:
    ...
    cylinder(f"{name}_Throat",0.105,0.14,(x,y+side*0.125,z),(math.pi/2,0,0),mat,lod,"rcs_recessed_nozzle_throat",root,16,0.03)
    mouth=cylinder(f"{name}_Mouth",0.085,0.05,(x,y+side*0.1975,z),(math.pi/2,0,0),mat,lod,"rcs_throat_shadow_aperture",root,16,0.01)
    strip_edge(mouth)
...
def build_lod(lod:int,mats,root) -> None:
    ...
    if lod<2:
        for sx in (0.30,1.14):
            for side in (-1,1):
                cylinder(f"BatteryLatch_{sx}_{side}",0.055,0.06,(sx,side*0.872,0.62),(math.pi/2,0,0),mats["Material_Safety"],lod,"quarter_turn_battery_latch",root,10,0.025)
        for sx in (1.62,2.05):
            for side in (-1,1):
                cylinder(f"ControllerLatch_{sx}_{side}",0.050,0.06,(sx,side*0.902,0.67),(math.pi/2,0,0),mats["Material_Safety"],lod,"quarter_turn_controller_latch",root,10,0.025)
    ...
    cylinder("CutterDrum",0.61,0.82,(3.52,0,0.56),(0,math.pi/2,0),mats["Material_Tool"],lod,"rotating_cutter_drum",root,32,0.045)
    cylinder("CutterNose",0.31,0.92,(4.14,0,0.56),(0,math.pi/2,0),mats["Material_Tool"],lod,"indexed_cutter_hub",root,24,0.045)
    ...
    ring_plan=[(12,3.62,0.0),(12,3.74,13.0),(12,3.86,26.0)] if lod==0 else ([(6,3.64,0.0),(6,3.78,30.0)] if lod==1 else [(4,3.68,0.0),(4,3.80,45.0)])
    tooth_index=0
    for teeth,ring_x,offset_deg in ring_plan:
        for index in range(teeth):
            ...
            box(f"CutterTooth_{tooth_index}",(0.10,0.16,0.12),(ring_x,y,z),(0,0,0),mats["Material_Tool"],lod,"wedding_pick_carbide_holder",root,0.02)
            # Conical point aimed outboard of the holder.
            cylinder(f"CutterBit_{tooth_index}",0.045,0.16,(ring_x,math.cos(angle)*0.60,0.56+math.sin(angle)*0.60),(angle-math.pi/2,0,0),mats["Material_Tool"],lod,"brazed_carbide_conical_bit",root,10,0.012)
            tooth_index+=1
    ...
    if lod==0:
        for index,x in enumerate((0.28,0.72,1.15,1.82)):
            for side in (-1,1):
                cylinder(f"ArmorFastener_{index}_{side}",0.045,0.10,(x,side*0.87,0.78),(math.pi/2,0,0),mats["Material_Mechanical"],lod,"captive_service_fastener",root,10,0.03)
...
def main() -> None:
    ...
    root["spaceface.family"]="opening_route_industrial_props_v1";root["spaceface.surfaceRevision"]="opening_mining_drone_v2"
