"""PQ-050.09 Leviathan MTX builder. Hitch untouched. --mtx-cycle N writes cycle stills."""
...
def role_maps(role, rgb, size=TEX, prefix=None):
    """Unique Leviathan maps. Not a tint of the shared fleet sheet."""
    ...
    for y in range(size):
        for x in range(size):
            ...
            stencil = 0.0
            if role == "hull" and 48 <= x <= 210 and 160 <= y <= 300:
                col = x - 48
                row = y - 160
                # Broken spray stencil LV, larger so it reads at play size.
                if 10 <= col <= 28 and 16 <= row <= 118:
                    stencil = 0.92
                if 28 <= col <= 62 and 16 <= row <= 32:
                    stencil = 0.86
                if 28 <= col <= 54 and 58 <= row <= 74:
                    stencil = 0.84
                if 78 <= col <= 100 and 16 <= row <= 118:
                    stencil = 0.90
                if 100 <= col <= 148 and 16 <= row <= 32:
                    stencil = 0.80
                if 100 <= col <= 132 and 96 <= row <= 118:
                    stencil = 0.78
                if gf > 0.88:
                    stencil *= 0.28
            if role == "hull":
    ...
    base = write_pixels(f"leviathan_{prefix}_basecolor", albedo, size, "sRGB")
    orm_img = write_pixels(f"leviathan_{prefix}_orm", orm, size, "Non-Color")
    nrm_img = write_pixels(f"leviathan_{prefix}_normal", nrm, size, "Non-Color")
...
def create_materials():
    specs = {
        "Material_Hull": ((0.09, 0.10, 0.12), 0.06, 0.54, "hull", 0.12, None),
        "Material_Armor": ((0.24, 0.23, 0.22), 0.68, 0.30, "armor", 0.06, None),
        "Material_Mechanical": ((0.38, 0.39, 0.41), 0.90, 0.28, "mechanical", 0.0, None),
        "Material_Accent": ((0.06, 0.42, 0.34), 0.06, 0.36, "accent", 0.12, None),
...
def sockets():
    half, hw, hh = 13.5, 3.2, 1.7
    return {
        "SOCKET_Weapon_Front": (half - 1.6, 0.0, 0.32),
        "SOCKET_Mining_Front": (half - 0.8, 0.0, -0.20),
        "SOCKET_Engine_Main": (-half + 1.6, 0.0, 0.14),
        "SOCKET_Trail_Main": (-half + 1.1, 0.0, 0.14),
        "SOCKET_Trail_Port": (-half + 1.2, -hw * 0.70, 0.14),
        "SOCKET_Trail_Starboard": (-half + 1.2, hw * 0.70, 0.14),
        "SOCKET_Utility_Dorsal": (-1.8, 0.0, hh + 1.10),
        "SOCKET_Cargo_Ventral": (-0.3, 0.0, -hh - 0.14),
        "SOCKET_Camera_Focus": (1.4, 0.0, 0.40),
        "SOCKET_RCS_Port": (-1.6, -hw - 0.20, 0.20),
        "SOCKET_RCS_Starboard": (-1.6, hw + 0.20, 0.20),
    }
...
def build_lod(lod, mats):
    collection = bpy.data.collections.new(f"LEVIATHAN_LOD{lod}")
    ...
    half = 13.5
    root = add_empty(f"LEVIATHAN_LOD{lod}_ROOT", (0, 0, 0), collection)
    root["spacefaceAsset"] = {
        "assetId": "SF_LEVIATHAN_PRODUCTION_V1", "partId": "leviathan_production_v1",
    ...
    if lod <= 1:
        ...
        boolean_cut(hull_obj, "KeelChannel", (0.20, 0.0, -1.42), (8.6, 0.20, 0.16))
        hull_obj.data.materials.clear()
        hull_obj.data.materials.append(hull)
    inset_large_faces(hull_obj, thickness=0.036, depth=0.014, min_area=4.20)

    add_thin_canopy("Bridge", 5.05, 0.0, 1.38, 2.05, 0.58, 0.52, mats, collection)
    # Folded plate island: setbacks, not a cube stack.
    loft_from_rings("Island_House", [
        [(-2.55, -0.78, 1.42), (-0.85, -0.78, 1.42), (-0.85, 0.78, 1.42), (-2.55, 0.78, 1.42)],
        [(-2.45, -0.58, 2.05), (-1.15, -0.58, 2.05), (-1.15, 0.58, 2.05), (-2.45, 0.58, 2.05)],
        [(-2.35, -0.48, 2.48), (-1.05, -0.48, 2.48), (-1.05, 0.48, 2.48), (-2.35, 0.48, 2.48)],
        [(-2.15, -0.72, 2.82), (-0.75, -0.72, 2.82), (-0.75, 0.72, 2.82), (-2.15, 0.72, 2.82)],
        [(-2.05, -0.62, 3.08), (-0.85, -0.62, 3.08), (-0.85, 0.62, 3.08), (-2.05, 0.62, 3.08)],
    ], armor, collection, 0.008, cap=True)
    add_box("Island_Door", (-2.58, 0.0, 1.78), (0.035, 0.18, 0.26), mech, collection, 0.003)
    add_box("Island_GlassF", (-0.70, 0.0, 2.90), (0.016, 0.56, 0.18), mats["Material_Canopy"], collection, 0.002)
    add_box("Island_GlassP", (-1.45, -0.76, 2.90), (0.48, 0.012, 0.16), mats["Material_Canopy"], collection, 0.002)
    add_box("Island_GlassS", (-1.45, 0.76, 2.90), (0.48, 0.012, 0.16), mats["Material_Canopy"], collection, 0.002)
    add_box("Island_Brow", (-0.80, 0.0, 3.08), (0.08, 0.62, 0.05), armor, collection, 0.004)
    add_box("Island_Slit", (-1.45, 0.0, 2.22), (0.55, 0.42, 0.06), mats["Material_Canopy"], collection, 0.002)
    add_box("Island_WalkP", (-1.55, -0.92, 1.58), (0.95, 0.07, 0.016), mech, collection, 0.003)
    add_box("Island_WalkS", (-1.55, 0.92, 1.58), (0.95, 0.07, 0.016), mech, collection, 0.003)
    add_sensor_dish("Island", (-1.15, 0.22, 3.18), mats, collection)
    add_box("Island_Mast", (-1.95, 0.0, 3.42), (0.04, 0.04, 0.28), mech, collection, 0.002)
    add_box("Island_Yard", (-1.95, 0.0, 3.58), (0.03, 0.32, 0.025), mech, collection, 0.002)

    # Overlapping deck plates with real thickness, not a painted grid.
    for i, (px, py, sx, sy, dz) in enumerate((
        (2.15, 0.00, 1.45, 0.92, 0.028),
        (-3.85, 0.00, 1.15, 0.78, 0.026),
        (6.05, -0.42, 0.82, 0.40, 0.022),
        (-2.55, 0.62, 0.72, 0.32, 0.022),
        (3.25, -0.68, 0.68, 0.30, 0.020),
        (0.55, 0.55, 0.88, 0.36, 0.024),
        (-6.15, -0.35, 0.78, 0.34, 0.020),
        (7.45, 0.28, 0.62, 0.28, 0.018),
    )):
        add_box(f"DeckPlate_{i}", (px, py, 1.22 + dz * 0.5), (sx, sy, dz), armor if i % 2 == 0 else hull, collection, 0.003)
    # Short midship rails only. No whole-length chopsticks past the bow.
    add_box("Gunwale_P", (-0.15, -3.05, 1.18), (3.4, 0.050, 0.065), armor, collection, 0.004)
    add_box("Gunwale_S", (-0.15, 3.05, 1.18), (3.4, 0.050, 0.065), armor, collection, 0.004)
    add_box("Stringer_P", (-0.20, -3.18, 0.48), (3.1, 0.034, 0.11), mech, collection, 0.003)
    add_box("Stringer_S", (-0.20, 3.18, 0.48), (3.1, 0.034, 0.11), mech, collection, 0.003)
    ...
    for y, side in drive_ys:
        ...
        add_box(f"DriveHeat_{side}", (-11.25, y, 0.50), (0.34, 0.09, 0.035), ceramic, collection, 0.003)
    add_box("EngineRaft", (-11.55, 0.0, -0.22), (0.85, 2.85, 0.08), armor, collection, 0.006)
    add_box("Transom_Header", (-9.35, 0.0, 0.72), (0.85, 2.05, 0.045), armor, collection, 0.005)
    add_box("Transom_Hatch", (-9.85, 0.0, 0.52), (0.28, 0.34, 0.028), armor, collection, 0.003)
    add_box("Transom_RibP", (-10.45, -1.45, 0.22), (0.42, 0.045, 0.32), mech, collection, 0.003)
    add_box("Transom_RibS", (-10.45, 1.45, 0.22), (0.42, 0.045, 0.32), mech, collection, 0.003)

    for sign, side in ((-1, "Port"), (1, "Starboard")):
        # Flank tubes sit in the shallow well, not a cage.
        add_cylinder(f"Broad_Fore_{side}", (0.75, 3.05 * sign, 0.16), 0.050, 0.42, mech, collection, vertices=10, bevel=0.003)
        add_cylinder(f"Broad_Mid_{side}", (0.15, 3.05 * sign, 0.16), 0.044, 0.36, mech, collection, vertices=10, bevel=0.003)
    ...
    for tag, gx, gy, gz in (("C", 8.15, 0.0, 1.22), ("P", 7.25, -0.62, 1.12), ("S", 7.25, 0.62, 1.12)):
        add_cylinder(f"GunBarbette_{tag}", (gx - 0.42, gy, gz - 0.10), 0.16, 0.08, armor, collection, vertices=12, bevel=0.004, rot=(0, 0, 0))
        add_box(f"GunHouse_{tag}", (gx - 0.28, gy, gz), (0.42, 0.18, 0.16), armor, collection, 0.005)
        add_cylinder(f"GunSleeve_{tag}", (gx + 0.15, gy, gz), 0.055, 0.48, mech, collection, vertices=12, bevel=0.003)
        add_cylinder(f"GunBarrel_{tag}", (gx + 0.85, gy, gz), 0.034, 0.92, mech, collection, vertices=10, bevel=0.002)
        add_box(f"GunMantlet_{tag}", (gx - 0.04, gy, gz), (0.08, 0.14, 0.12), armor, collection, 0.003)
    add_box("Cabin_Shoulder", (3.25, 0.0, 1.28), (1.05, 0.68, 0.055), armor, collection, 0.006)
    add_box("Keel_Shoe", (0.10, 0.0, -1.22), (3.4, 0.18, 0.05), armor, collection, 0.006)
    add_box("Repair_Patch", (2.65, -0.92, 1.16), (0.38, 0.18, 0.014), warning, collection, 0.002)
    add_box("Repair_Plate", (4.15, 0.95, 1.20), (0.44, 0.24, 0.018), armor, collection, 0.003)
    add_box("Warn_Chevron", (-4.05, -1.15, 1.02), (0.28, 0.10, 0.012), warning, collection, 0.002)
    add_box("Accent_Flash", (0.55, -2.35, 0.36), (0.62, 0.018, 0.09), accent, collection, 0.002)
    add_box("Accent_FlashS", (0.55, 2.35, 0.36), (0.62, 0.018, 0.09), accent, collection, 0.002)
    add_sensor_dish("Dorsal", (2.15, 0.58, 1.38), mats, collection)
    add_cylinder("Turret_Ring", (0.65, 0.0, 1.30), 0.38, 0.07, mech, collection, vertices=16, bevel=0.004, rot=(0, 0, 0))
    add_box("Turret_Head", (0.82, 0.0, 1.46), (0.32, 0.18, 0.08), armor, collection, 0.004)
    add_box("Hatch_Lid", (-0.85, 0.52, 1.18), (0.36, 0.24, 0.018), armor, collection, 0.003)
    add_box("Hatch_Hinge", (-1.14, 0.52, 1.20), (0.03, 0.16, 0.016), mech, collection, 0.002)
    # Extra radiator cassette sitting in the aft well.
    for i in range(8):
        add_box(f"RadFin_{i}", (-5.35 - 0.70 + i * 0.20, 0.0, 1.22), (0.014, 0.34, 0.10), mats["Material_Radiator"], collection, 0.002)
    add_cylinder("RadHeader_P", (-5.35, -0.22, 1.28), 0.018, 1.55, mech, collection, 8, 0.002)
    add_cylinder("RadHeader_S", (-5.35, 0.22, 1.28), 0.018, 1.55, mech, collection, 8, 0.002)

    if lod == 0:
        add_curve_hose("Hose_Port", [(0.2, -2.55, 0.18), (-3.4, -2.40, 0.22), (-7.0, -2.25, 0.20), (-10.4, -2.15, 0.16)], mech, collection, 0.022)
        add_curve_hose("Hose_Stbd", [(0.2, 2.55, 0.18), (-3.4, 2.40, 0.22), (-7.0, 2.25, 0.20), (-10.4, 2.15, 0.16)], mech, collection, 0.022)
        add_box("HoseFit_P0", (0.15, -2.35, 0.14), (0.03, 0.03, 0.03), mech, collection, 0.002)
    ...
    bm = bmesh.new()
    for point in [
        (12.0, 0, 0.4), (0, -4.8, 0.9), (0, 4.8, 0.9),
        (-11.6, -3.4, 0.2), (-11.6, 3.4, 0.2),
        (2.2, -2.2, -1.5), (2.2, 2.2, -1.5),
        (-1.8, 0, 3.6), (-8.4, -4.2, 1.4), (-8.4, 4.2, 1.4),
    ]:
        bm.verts.new(point)
...
def export_lod(collection, lod):
    out = FAMILY / "source" / "wholeships" / f"leviathan_production_v1_lod{lod}.glb"
...
def render_cycle(collection):
    ...
    camera = setup_studio()
    out = FAMILY / "evidence" / "leviathan" / "cycles" / f"cycle_{CYCLE:02d}"
    out.mkdir(parents=True, exist_ok=True)
    views = {
        "three_quarter": ((19.4, -22.2, 9.2), (0, 0, 0.28), 30),
        "starboard": ((0.0, 32.0, 6.2), (0, 0, 0.28), 28),
        "rear": ((-24.0, -9.0, 6.2), (-0.6, 0, 0.22), 30),
        "clay_three_quarter": ((19.4, -22.2, 9.2), (0, 0, 0.28), 30),
        "grazing_close": ((12.8, -10.0, 2.8), (0.6, 0, 0.22), 46),
        "bay_interior": ((0.20, -7.4, 0.42), (0.20, -2.85, 0.18), 38),
        "drive_rear": ((-17.4, -4.6, 2.1), (-11.2, 0, 0.14), 46),
        "play_size": ((66, -58, 30), (0, 0, 0.18), 50),
        "orm_isolation": ((18.8, -21.0, 8.8), (0, 0, 0.28), 30),
        "normal_isolation": ((18.8, -21.0, 8.8), (0, 0, 0.28), 30),
        "id_or_material_id": ((18.8, -21.0, 8.8), (0, 0, 0.28), 30),
        "material_three_quarter": ((18.8, -21.0, 8.8), (0, 0, 0.28), 30),
    }
...
def main():
    ...
    for lod in (0, 1, 2):
        ...
        if report["hullTriangles"] < 800:
            raise RuntimeError(f"leviathan lod{lod} hull {report['hullTriangles']} < 800")
        collections.append(collection)
        reports.append(report)
    stills = render_cycle(collections[0])
    report = {
        "schema": "spaceface.leviathanMtx.cycle.v1",
        "shipId": "leviathan",
        "cycle": CYCLE,
        "lods": reports,
        "stills": str(stills.relative_to(FAMILY)).replace("\\", "/"),
    }
    (FAMILY / "evidence" / "leviathan").mkdir(parents=True, exist_ok=True)
    (FAMILY / "evidence" / "leviathan" / f"cycle_{CYCLE:02d}.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
