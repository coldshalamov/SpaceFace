"""PQ-050.21 Salvage cutter MTX builder. Hitch untouched. --mtx-cycle N writes cycle stills."""
...
FAMILY = ROOT / "assets" / "ships" / "fleet_player_bodies_v1" / "salvage_cutter"
...
def role_maps(role, rgb, size=TEX, prefix=None):
    """Unique salvage cutter maps. Soot-brown wreck breaker, not sage tender."""
    ...
    base = write_pixels(f"cutter_{prefix}_basecolor", albedo, size, "sRGB")
    orm_img = write_pixels(f"cutter_{prefix}_orm", orm, size, "Non-Color")
    nrm_img = write_pixels(f"cutter_{prefix}_normal", nrm, size, "Non-Color")
...
def create_materials():
    specs = {
        "Material_Hull": ((0.26, 0.22, 0.16), 0.05, 0.58, "hull", 0.04, None),
        "Material_Armor": ((0.22, 0.18, 0.14), 0.10, 0.54, "armor", 0.03, None),
...
def sockets():
    # Frozen factory cutter sockets (length 15, halfW 1.8, height 0.85).
    return {
        ...
        "SOCKET_Trail_Port": (-6.5, -1.26, 0.08),
        "SOCKET_Trail_Starboard": (-6.5, 1.26, 0.08),
        "SOCKET_Utility_Dorsal": (0.6, 0.0, 1.40),
        "SOCKET_Cargo_Ventral": (-0.4, 0.0, -1.00),
        "SOCKET_Camera_Focus": (0.8, 0.0, 0.25),
        "SOCKET_RCS_Port": (-1.2, -2.00, 0.15),
        "SOCKET_RCS_Starboard": (-1.2, 2.00, 0.15),
    }
...
def build_lod(lod, mats):
    collection = bpy.data.collections.new(f"CUTTER_LOD{lod}")
    ...
    root = add_empty(f"CUTTER_LOD{lod}_ROOT", (0, 0, 0), collection)
    root["spacefaceAsset"] = {
        "assetId": "SF_SALVAGE_CUTTER_V1", "partId": "salvage_cutter_production_v1",
        "lod": f"lod{lod}", "slot": "hull", "category": "wholeships",
        "forward": "+X", "embeddedPlume": False,
    }
    # 15 m soot-brown salvage cutter: patched barge, scrap cradle, two cutter arms, twin tail drives.
    hull_obj = loft_from_rings("Pressure_Hull", [
        station_ring(7.5, 0, 0.04, 0.48, 0.30, flat=0.30, box=0.40, keel=0.50),
        station_ring(5.0, 0, 0.12, 1.18, 0.58, flat=0.65, box=0.50, keel=0.40),
        station_ring(1.8, 0, 0.14, 1.72, 0.85, flat=0.45, box=0.80, keel=0.30),
        station_ring(-1.4, 0, 0.12, 1.80, 0.90, flat=0.35, box=0.90, keel=0.25),
        station_ring(-4.2, 0, 0.10, 1.38, 0.68, flat=0.28, box=0.92, keel=0.22),
        station_ring(-7.05, 0, 0.02, 0.60, 0.38, flat=0.22, box=0.90, keel=0.18),
    ], hull, collection, 0.012, cap="both")
    if lod <= 1:
        cut_open_bay(hull_obj, "Cabin", (4.05, 0.0, 1.02), 0.92, 0.32, 0.26, (0, 0, 1), mats, collection, kit="cockpit", liner=False)
        boolean_cut(hull_obj, "ScrapWell", (0.15, 0.0, 1.05), (1.55, 0.78, 0.42))
        boolean_cut(hull_obj, "SideWellP", (0.15, -1.78, 0.24), (1.00, 0.22, 0.26))
        boolean_cut(hull_obj, "SideWellS", (0.15, 1.78, 0.24), (1.00, 0.22, 0.26))
        boolean_cut(hull_obj, "RadWellP", (-3.45, -0.58, 0.62), (0.54, 0.26, 0.16))
        boolean_cut(hull_obj, "RadWellS", (-3.45, 0.58, 0.62), (0.54, 0.26, 0.16))
        boolean_cut(hull_obj, "TransomRecess", (-7.05, 0.0, 0.02), (0.16, 0.32, 0.14))
    ...
    add_box("CabinSill", (4.05, 0.0, 0.88), (0.48, 0.32, 0.014), armor, collection, 0.002)

    # Open-backed scrap cradle: well, lips, floor, slats.
    add_box("WellLipF", (1.62, 0.0, 1.12), (0.05, 0.82, 0.06), warning, collection, 0.002)
    add_box("WellLipA", (-1.38, 0.0, 1.12), (0.05, 0.82, 0.06), warning, collection, 0.002)
    add_box("WellLipP", (0.15, -0.82, 1.12), (1.42, 0.04, 0.06), warning, collection, 0.002)
    add_box("WellLipS", (0.15, 0.82, 1.12), (1.42, 0.04, 0.06), warning, collection, 0.002)
    add_box("WellFloor", (0.15, 0.0, 0.36), (1.40, 0.70, 0.03), mech, collection, 0.003)
    add_box("WellWallP", (0.15, -0.72, 0.72), (1.36, 0.03, 0.34), armor, collection, 0.002)
    add_box("WellWallS", (0.15, 0.72, 0.72), (1.36, 0.03, 0.34), armor, collection, 0.002)
    add_box("WellWallA", (-1.28, 0.0, 0.72), (0.03, 0.68, 0.34), armor, collection, 0.002)
    if lod == 0:
        for i, ox in enumerate((-0.85, -0.25, 0.35, 0.95)):
            add_box(f"WellSlat_{i}", (ox, 0.0, 0.44), (0.04, 0.56, 0.012), mech, collection, 0.001)

    # Two factory cutter arms: pad, hat boom, knuckle, carbide drum.
    ...
    drum_r = 0.48 if CYCLE >= 2 else 0.40
    tooth_n = 14 if CYCLE >= 2 else 10
    for sign, side in ((-1, "Port"), (1, "Starboard")):
        y = 1.55 * sign
        loft_from_rings(f"ArmPad_{side}", [
            ellipse_ring(2.25, y * 0.86, 0.10, 0.36, 0.20, 12),
            ellipse_ring(2.95, y, 0.02, 0.28, 0.18, 12),
            ellipse_ring(3.55, y, -0.06, 0.20, 0.15, 12),
        ], hull, collection, 0.008)
        loft_from_rings(f"ArmBoom_{side}", [
            hat_ring(3.55, y, -0.08, 0.26, 0.20),
            hat_ring(4.65, y, -0.16, 0.22, 0.16),
            hat_ring(5.65, y, -0.24, 0.18, 0.14),
        ], armor, collection, 0.004, cap=True)
        add_cylinder(f"ArmKnuckle_{side}", (3.55, y, -0.08), 0.14, 0.18, mech, collection, 12, 0.003)
        add_cylinder(f"ArmWrist_{side}", (5.65, y, -0.26), 0.12, 0.15, mech, collection, 12, 0.003)
        add_cylinder(f"Ram_{side}", (4.55, y + 0.18 * sign, 0.06), 0.038, 1.45, mech, collection, 8, 0.002)
        add_box(f"ArmHazard_{side}", (4.65, y, 0.02), (0.26, 0.06, 0.03), warning, collection, 0.002)
        add_cylinder(f"Drum_{side}", (6.35, y, -0.32), drum_r, 0.36, armor, collection, 20, 0.004, rot=(0, 0, math.pi / 2))
        add_cylinder(f"DrumHub_{side}", (6.35, y, -0.32), 0.11, 0.42, mech, collection, 10, 0.003, rot=(0, 0, math.pi / 2))
        if lod == 0:
            for i in range(tooth_n):
                ang = math.tau * i / tooth_n
                add_box(
                    f"Tooth_{side}_{i}",
                    (6.35 + math.cos(ang) * (drum_r + 0.02), y, -0.32 + math.sin(ang) * (drum_r + 0.02)),
                    (0.050, 0.038, 0.070),
                    mech, collection, 0.002,
                )
        add_rcs_cluster(side, (-1.2, 1.95 * sign, 0.16), mats, collection, sign=sign)
        add_box(f"SideWellLip_{side}", (0.15, 1.62 * sign, 0.38), (0.88, 0.04, 0.04), armor, collection, 0.002)

    if CYCLE >= 3:
        for i, (lx, ly) in enumerate(((1.15, -0.55), (0.15, 0.0), (1.15, 0.55))):
            add_cylinder(f"LampPost_{i}", (lx, ly, 1.22), 0.03, 0.28, mech, collection, 8, 0.002, rot=(0, 0, 0))
            add_cylinder(f"LampHood_{i}", (lx, ly, 1.42), 0.14, 0.06, armor, collection, 12, 0.002, rot=(0, 0, 0))
            add_cylinder(f"LampLens_{i}", (lx, ly, 1.36), 0.08, 0.03, mats["Material_Canopy"], collection, 10, 0.001, rot=(0, 0, 0))

    if CYCLE >= 4:
        for i, ox in enumerate((-0.55, 0.35, 1.15)):
            add_cylinder(f"ScrapDrum_{i}", (ox, 0.0, 1.28), 0.16, 0.28, hull, collection, 14, 0.003)
            add_cylinder(f"ScrapBand_{i}", (ox, 0.0, 1.28), 0.17, 0.04, mech, collection, 12, 0.002)

    # Twin axial bells at factory drive stations (-6.2, ±1.1).
    for sign, side in ((-1, "Port"), (1, "Starboard")):
        add_flared_bell(side, -6.85, 1.10 * sign, 0.08, 0.95, mats, collection)
        add_cylinder(f"DriveCoupling_{side}", (-6.50, 1.10 * sign, 0.08), 0.30, 0.16, mech, collection, 14, 0.003)
        add_manufactured_drive(side, -6.20, 1.10 * sign, lod, mats, collection, scale=0.78, z=0.08)
        add_box(f"DriveSaddle_{side}", (-6.10, 1.10 * sign, -0.22), (0.36, 0.12, 0.08), mech, collection, 0.003)
        add_box(f"RadCassette_{side}", (-4.35, 0.68 * sign, 0.70), (0.52, 0.22, 0.10), mats["Material_Radiator"], collection, 0.002)
        add_box(f"RadHeader_{side}", (-4.35, 0.68 * sign, 0.82), (0.54, 0.04, 0.03), mech, collection, 0.002)
        if lod <= 1:
            for i in range(8):
                add_box(
                    f"RadFin_{side}_{i}",
                    (-4.62 + i * 0.08, 0.68 * sign, 0.76),
                    (0.010, 0.18, 0.16),
                    mech, collection, 0.001,
                )

    add_service_hatch("Dorsal", (-2.05, 0.0, 0.78), mats, collection, sx=0.26, sy=0.16)
    add_box("KeelShoe", (0.1, 0.0, -0.58), (1.7, 0.18, 0.06), armor, collection, 0.003)
    add_box("RepairPlate", (2.35, -0.72, 0.52), (0.36, 0.16, 0.010), warning, collection, 0.002)
    add_box("BrightPatch", (1.15, 0.68, 0.54), (0.28, 0.14, 0.010), accent, collection, 0.002)
    add_box("ChineStrakeP", (1.1, -1.58, 0.08), (2.4, 0.04, 0.05), armor, collection, 0.003)
    add_box("ChineStrakeS", (1.1, 1.58, 0.08), (2.4, 0.04, 0.05), armor, collection, 0.003)
    add_box("BowCap", (7.35, 0.0, 0.06), (0.12, 0.18, 0.10), armor, collection, 0.003)
    add_box("StripeP", (0.8, -1.58, 0.26), (1.8, 0.012, 0.008), accent, collection, 0.001)
    add_box("StripeS", (0.8, 1.58, 0.26), (1.8, 0.012, 0.008), accent, collection, 0.001)
    add_panel_seams("Hull", [3.8, 0.8, -1.8, -4.0], 1.15, 0.62, mech, collection)

    if CYCLE >= 5:
        add_box("PatchTile", (-0.85, -0.62, 0.56), (0.22, 0.12, 0.008), armor, collection, 0.002)
        add_cylinder("TetherReelP", (-1.85, -1.35, 0.42), 0.12, 0.18, mech, collection, 12, 0.003)
        add_cylinder("TetherReelS", (-1.85, 1.35, 0.42), 0.12, 0.18, mech, collection, 12, 0.003)
    ...
    bm = bmesh.new()
    for point in [
        (9.4, 0, 0.1), (5.2, -1.8, 1.6), (5.2, 1.8, 1.6),
        (-8.8, -1.6, 0.2), (-8.8, 1.6, 0.2),
        (1.0, -2.6, 0.6), (1.0, 2.6, 0.6),
...
def export_lod(collection, lod):
    out = FAMILY / "source" / "wholeships" / f"salvage_cutter_production_v1_lod{lod}.glb"
...
def render_cycle(collection):
    ...
    camera = setup_studio()
    out = FAMILY / "evidence" / "salvage_cutter" / "cycles" / f"cycle_{CYCLE:02d}"
    out.mkdir(parents=True, exist_ok=True)
    views = {
        "three_quarter": ((12.0, -14.2, 4.6), (0.1, 0, 0.20), 34),
        "starboard": ((0.2, 17.4, 3.3), (0.1, 0, 0.16), 32),
        "rear": ((-13.2, -5.8, 3.5), (-0.4, 0, 0.14), 32),
        "clay_three_quarter": ((12.0, -14.2, 4.6), (0.1, 0, 0.20), 34),
        "grazing_close": ((8.2, -6.8, 1.8), (0.8, 0, 0.26), 46),
        "bay_interior": ((4.8, -3.2, 1.8), (4.9, 0.0, 1.20), 40),
        "drive_rear": ((-11.6, -4.4, 1.4), (-8.0, 0, 0.08), 42),
        "play_size": ((44, -40, 18), (0, 0, 0.12), 48),
        "orm_isolation": ((14.6, -17.0, 5.6), (0.2, 0, 0.24), 32),
        "normal_isolation": ((14.6, -17.0, 5.6), (0.2, 0, 0.24), 32),
        "id_or_material_id": ((14.6, -17.0, 5.6), (0.2, 0, 0.24), 32),
        "material_three_quarter": ((14.6, -17.0, 5.6), (0.2, 0, 0.24), 32),
    }
...
def main():
    ...
    for lod in (0, 1, 2):
        ...
        if report["hullTriangles"] < 800:
            raise RuntimeError(f"salvage_cutter lod{lod} hull {report['hullTriangles']} < 800")
        collections.append(collection)
        reports.append(report)
    stills = render_cycle(collections[0])
    report = {
        "schema": "spaceface.salvageCutterMtx.cycle.v1",
        "shipId": "salvage_cutter",
        "cycle": CYCLE,
        "lods": reports,
        "stills": str(stills.relative_to(FAMILY)).replace("\\", "/"),
    }
    (FAMILY / "evidence" / "salvage_cutter").mkdir(parents=True, exist_ok=True)
    (FAMILY / "evidence" / "salvage_cutter" / f"cycle_{CYCLE:02d}.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
