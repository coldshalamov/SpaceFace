...
HONEST = {
    "Material_Hull": {"color": (0.91, 0.92, 0.94), "metallic": 0.04, "roughness": 0.38, "role": "hull"},
...
def build_drives(hull, mats):
    ...
    for sign, tag in ((-1.0, "Port"), (1.0, "Stbd")):
        ...
        def well():
            return add_cyl(f"DriveWell_{tag}", 0.58, 1.35, (-4.35, y, 0.55), hull_mat, vertices=16)

        def aft():
            return add_cyl(
                f"DriveAft_{tag}", 0.46, 1.10, (-5.20, y, 0.18), hull_mat,
                rotation=(0.0, math.radians(90.0), 0.0), vertices=16,
            )

        report.append({"tag": tag, "dorsal": cut(hull, well), "aft": cut(hull, aft)})
