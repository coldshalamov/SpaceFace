...
HONEST = {
    "Material_Hull": {"color": (0.97, 0.97, 0.98), "metallic": 0.0, "roughness": 0.40, "role": "hull"},
...
def build_canopy(hull, mats):
    ...
    def tub():
        return add_box("CanopyTubCut", (1.95, 1.15, 0.90), (3.35, 0.0, 0.88), hull_mat, 0.0)

    report["tub"] = cut(hull, tub)
    cavity = add_box("LOD0_CanopyCavity", (1.72, 0.92, 0.08), (3.35, 0.0, 0.22), mech, 0.001)
    pane = add_box("LOD0_Canopy", (1.70, 0.90, 0.035), (3.35, 0.0, 0.86), glass, 0.002)
    frame = add_box("LOD0_CanopyFrame", (2.02, 1.18, 0.07), (3.35, 0.0, 0.90), armor, 0.002)

    def frame_hole():
        return add_box("CanopyFrameCut", (1.68, 0.86, 0.22), (3.35, 0.0, 0.90), armor, 0.0)

    cut(frame, frame_hole)
    return [cavity, pane, frame], report
...
def build_drives(hull, mats):
    ...
    for sign, tag in ((-1.0, "Port"), (1.0, "Stbd")):
        y = 0.70 * sign
        floor = add_cyl(
            f"LOD0_DriveFloor_{tag}", 0.48, 0.04, (-4.40, y, 0.04),
            mats["Material_Mechanical"], vertices=16,
        )

        def well():
            return add_cyl(f"DriveWell_{tag}", 0.62, 1.40, (-4.40, y, 0.58), hull_mat, vertices=16)
