"""PQ-050.18 Helios span MTX builder. Hitch untouched. --mtx-cycle N writes cycle stills."""
...
FAMILY = ROOT / "assets" / "ships" / "fleet_player_bodies_v1" / "helios_span"
...
def role_maps(role, rgb, size=TEX, prefix=None):
    """Unique Helios span maps. Pale civilian hauler, not Cradle, Lark, or Reach."""
    ...
    base = write_pixels(f"span_{prefix}_basecolor", albedo, size, "sRGB")
    orm_img = write_pixels(f"span_{prefix}_orm", orm, size, "Non-Color")
    nrm_img = write_pixels(f"span_{prefix}_normal", nrm, size, "Non-Color")
...
def create_materials():
    specs = {
        "Material_Hull": ((0.48, 0.46, 0.40), 0.04, 0.50, "hull", 0.06, None),
        "Material_Armor": ((0.40, 0.38, 0.32), 0.08, 0.48, "armor", 0.03, None),
...
def sockets():
    # Frozen factory span sockets (length 18, halfW 2.0, height 1.05).
    return {
        "SOCKET_Weapon_Front": (7.8, 0.0, 0.25),
        "SOCKET_Mining_Front": (8.4, 0.0, -0.15),
        "SOCKET_Engine_Main": (-7.8, 0.0, 0.08),
        "SOCKET_Trail_Main": (-8.2, 0.0, 0.08),
        "SOCKET_Trail_Port": (-8.0, -1.40, 0.08),
        "SOCKET_Trail_Starboard": (-8.0, 1.40, 0.08),
        "SOCKET_Utility_Dorsal": (0.6, 0.0, 1.60),
        "SOCKET_Cargo_Ventral": (-0.4, 0.0, -1.20),
        "SOCKET_Camera_Focus": (0.8, 0.0, 0.25),
        "SOCKET_RCS_Port": (-1.2, -2.20, 0.15),
        "SOCKET_RCS_Starboard": (-1.2, 2.20, 0.15),
    }
...
def build_lod(lod, mats):
    collection = bpy.data.collections.new(f"SPAN_LOD{lod}")
    ...
    root = add_empty(f"SPAN_LOD{lod}_ROOT", (0, 0, 0), collection)
    root["spacefaceAsset"] = {
        "assetId": "SF_HELIOS_SPAN_V1", "partId": "helios_span_production_v1",
        "lod": f"lod{lod}", "slot": "hull", "category": "wholeships",
        "forward": "+X", "embeddedPlume": False,
    }
    # 18 m pale civilian hauler: long barge, raised bridge, two cargo pods, twin tail drives.
    hull_obj = loft_from_rings("Pressure_Hull", [
        station_ring(9.0, 0, 0.08, 0.48, 0.26, flat=0.40, box=0.40, keel=0.50),
        station_ring(6.4, 0, 0.14, 1.22, 0.58, flat=0.70, box=0.55, keel=0.40),
        station_ring(2.6, 0, 0.14, 1.88, 0.68, flat=0.35, box=0.85, keel=0.30),
        station_ring(-0.6, 0, 0.14, 2.00, 0.70, flat=0.30, box=0.90, keel=0.25),
        station_ring(-4.0, 0, 0.12, 1.68, 0.62, flat=0.28, box=0.92, keel=0.22),
        station_ring(-6.8, 0, 0.08, 1.15, 0.50, flat=0.25, box=0.92, keel=0.20),
        station_ring(-8.55, 0, 0.02, 0.56, 0.32, flat=0.22, box=0.90, keel=0.18),
    ], hull, collection, 0.012, cap="both")
    if lod <= 1:
        cut_open_bay(hull_obj, "Bridge", (4.85, 0.0, 1.42), 1.05, 0.38, 0.28, (0, 0, 1), mats, collection, kit="cockpit", liner=False)
        boolean_cut(hull_obj, "SideWellP", (0.20, -2.00, 0.28), (1.15, 0.22, 0.28))
        boolean_cut(hull_obj, "SideWellS", (0.20, 2.00, 0.28), (1.15, 0.22, 0.28))
        boolean_cut(hull_obj, "RadWellP", (-4.15, -0.62, 0.78), (0.62, 0.30, 0.18))
        boolean_cut(hull_obj, "RadWellS", (-4.15, 0.62, 0.78), (0.62, 0.30, 0.18))
        boolean_cut(hull_obj, "TransomRecess", (-8.55, 0.0, 0.02), (0.18, 0.36, 0.16))
        hull_obj.data.materials.clear()
        hull_obj.data.materials.append(hull)
    inset_large_faces(hull_obj, thickness=0.028, depth=0.014, min_area=0.70)
    cover_loft_with_plates("SpanSkin", [
        (8.80, 0.48, 0.32, 0.06),
        (5.60, 1.35, 0.72, 0.12),
        (2.00, 1.95, 1.05, 0.16),
        (-1.40, 2.05, 1.08, 0.14),
    ...
    loft_shell("Hold_P", [
        (4.20, -1.05, -1.85, -0.16, 0.42),
        (0.20, -1.35, -2.15, -0.20, 0.46),
        (-4.00, -1.05, -1.75, -0.14, 0.36),
    ...
    add_box("TransomPlate", (-8.48, 0.0, 0.04), (0.05, 0.55, 0.28), armor, collection, 0.003)
    add_box("TransomInner", (-8.36, 0.0, 0.04), (0.05, 0.40, 0.18), mech, collection, 0.002)
    add_box("BridgePedestal", (4.85, 0.0, 1.28), (0.78, 0.44, 0.48), armor, collection, 0.006)
    add_thin_canopy("Bridge", 4.85, 0.0, 1.68, 1.05, 0.36, 0.32, mats, collection)
    add_box("BridgeBrow", (5.32, 0.0, 1.88), (0.08, 0.28, 0.06), armor, collection, 0.003)
    add_box("BridgeSill", (4.85, 0.0, 1.66), (0.52, 0.38, 0.016), armor, collection, 0.002)
    add_box("BridgeFrameP", (4.85, -0.32, 1.82), (0.40, 0.018, 0.12), armor, collection, 0.002)
    add_box("BridgeFrameS", (4.85, 0.32, 1.82), (0.40, 0.018, 0.12), armor, collection, 0.002)
    add_box("BridgeWingP", (4.85, -0.48, 1.72), (0.22, 0.08, 0.04), armor, collection, 0.002)
    add_box("BridgeWingS", (4.85, 0.48, 1.72), (0.22, 0.08, 0.04), armor, collection, 0.002)
    add_sensor_dish("Bridge", (4.55, 0.22, 2.08), mats, collection)

    # Two formed cargo pods per side (factory pods=2 rows). Not a miner well.
    for sign, side in ((-1, "Port"), (1, "Starboard")):
        y = 1.72 * sign
        add_cargo_pod(side, -0.35, y, 0.22, 4.40, 0.46, mats, collection, lod)
        add_box(f"PodRail_{side}", (-0.35, y, -0.30), (2.2, 0.08, 0.06), mech, collection, 0.003)
        add_box(f"PodLockF_{side}", (1.75, y, 0.22), (0.06, 0.10, 0.10), mech, collection, 0.002)
        add_box(f"PodLockA_{side}", (-2.45, y, 0.22), (0.06, 0.10, 0.10), mech, collection, 0.002)
        add_rcs_cluster(side, (-1.2, 2.15 * sign, 0.18), mats, collection, sign=sign)
...
def export_lod(collection, lod):
    out = FAMILY / "source" / "wholeships" / f"helios_span_production_v1_lod{lod}.glb"
...
def render_cycle(collection):
    ...
    camera = setup_studio()
    out = FAMILY / "evidence" / "helios_span" / "cycles" / f"cycle_{CYCLE:02d}"
    out.mkdir(parents=True, exist_ok=True)
    views = {
        "three_quarter": ((13.8, -16.2, 5.4), (0.2, 0, 0.24), 32),
        "starboard": ((0.4, 20.5, 3.8), (0.2, 0, 0.18), 30),
        "rear": ((-15.8, -6.8, 4.0), (-0.6, 0, 0.16), 30),
        "clay_three_quarter": ((13.8, -16.2, 5.4), (0.2, 0, 0.24), 32),
...
def main():
    ...
    for lod in (0, 1, 2):
        ...
        if report["hullTriangles"] < 800:
            raise RuntimeError(f"helios_span lod{lod} hull {report['hullTriangles']} < 800")
        collections.append(collection)
        reports.append(report)
    stills = render_cycle(collections[0])
    report = {
        "schema": "spaceface.heliosSpanMtx.cycle.v1",
        "shipId": "helios_span",
        "cycle": CYCLE,
        "lods": reports,
        "stills": str(stills.relative_to(FAMILY)).replace("\\", "/"),
    }
    (FAMILY / "evidence" / "helios_span").mkdir(parents=True, exist_ok=True)
    (FAMILY / "evidence" / "helios_span" / f"cycle_{CYCLE:02d}.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
