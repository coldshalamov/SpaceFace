"""PQ-050.05 Bastion MTX builder. Hitch untouched. --mtx-cycle N writes cycle stills."""
...
FAMILY = ROOT / "assets" / "ships" / "fleet_player_bodies_v1" / "bastion"
...
def role_maps(role, rgb, size=TEX, prefix=None):
    ...
    base = write_pixels(f"bastion_{prefix}_basecolor", albedo, size, "sRGB")
    orm_img = write_pixels(f"bastion_{prefix}_orm", orm, size, "Non-Color")
    nrm_img = write_pixels(f"bastion_{prefix}_normal", nrm, size, "Non-Color")
...
def create_materials():
    specs = {
        "Material_Hull": ((0.16, 0.18, 0.22), 0.10, 0.44, "hull", 0.40, None),
...
def build_lod(lod, mats):
    collection = bpy.data.collections.new(f"BASTION_LOD{lod}")
    ...
    half = 9.25
    root = add_empty(f"BASTION_LOD{lod}_ROOT", (0, 0, 0), collection)
    root["spacefaceAsset"] = {
        "assetId": "SF_BASTION_PRODUCTION_V1", "partId": "bastion_production_v1",
        "lod": f"lod{lod}", "slot": "hull", "category": "wholeships",
        "forward": "+X", "embeddedPlume": False,
    }
    # C6 corvette hull: knife → canopy shoulder → casemate waist → tower → transom.
    hull_obj = loft_from_rings("Pressure_Hull", [
        station_ring(half, 0, 0.08, 0.26, 0.20, flat=0.08, box=0.10, keel=0.95),
        station_ring(7.20, 0, 0.14, 0.66, 0.46, flat=0.25, box=0.22, keel=0.85),
        station_ring(4.80, 0, 0.24, 1.12, 0.92, flat=0.85, box=0.40, keel=0.70),
        station_ring(1.80, 0, 0.14, 1.65, 0.72, flat=0.30, box=0.78, keel=0.55),
        station_ring(-0.80, 0, 0.18, 1.95, 1.05, flat=0.35, box=0.88, keel=0.40),
        station_ring(-3.20, 0, 0.22, 1.75, 1.18, flat=0.40, box=0.95, keel=0.32),
        station_ring(-5.80, 0, 0.16, 1.32, 0.88, flat=0.32, box=0.96, keel=0.25),
        station_ring(-8.40, 0, 0.12, 0.88, 0.62, flat=0.30, box=0.95, keel=0.20),
    ], hull, collection, 0.018)
    if lod <= 1:
        cut_open_bay(hull_obj, "Cockpit", (4.10, 0.0, 1.12), 1.25, 0.52, 0.50, (0, 0, 1), mats, collection, kit="cockpit", liner=False)
        cut_open_bay(hull_obj, "Port", (0.20, -1.72, 0.18), 1.75, 0.42, 0.50, (0, -1, 0), mats, collection, kit="radiator")
        cut_open_bay(hull_obj, "Starboard", (0.20, 1.72, 0.18), 1.75, 0.42, 0.50, (0, 1, 0), mats, collection, kit="rack")
        cut_open_bay(hull_obj, "TowerWell", (-2.40, 0.0, 1.18), 1.05, 0.62, 0.38, (0, 0, 1), mats, collection, kit="radiator")
        boolean_cut(hull_obj, "TransomRecess", (-8.20, 0.0, 0.10), (0.20, 0.42, 0.26))
    ...
    add_thin_canopy("Canopy", 4.10, 0.0, 1.02, 1.20, 0.52, 0.34, mats, collection)
    add_box("TransomPlate", (-8.08, 0.0, 0.10), (0.04, 0.48, 0.28), armor, collection, 0.003)
    add_box("Tower_Base", (-2.40, 0.0, 1.18), (0.92, 0.62, 0.24), armor, collection, 0.012)
    add_box("Tower_Trunk", (-2.45, 0.0, 1.78), (0.52, 0.36, 0.42), hull, collection, 0.010)
    add_box("Tower_Shoulder", (-2.35, 0.0, 2.08), (0.68, 0.48, 0.10), armor, collection, 0.006)
    add_box("Tower_Bridge", (-2.20, 0.0, 2.32), (0.78, 0.58, 0.18), armor, collection, 0.008)
    add_box("Tower_Glass", (-1.78, 0.0, 2.34), (0.04, 0.46, 0.11), mats["Material_Canopy"], collection, 0.003)
    add_box("Tower_Mast", (-2.55, 0.0, 2.72), (0.05, 0.05, 0.32), mech, collection, 0.003)
    add_box("Tower_Yard", (-2.55, 0.0, 2.88), (0.04, 0.22, 0.03), mech, collection, 0.002)
    add_cylinder("Tower_Radar", (-2.10, 0.22, 2.52), 0.12, 0.04, armor, collection, vertices=12, bevel=0.002, rot=(0, math.pi / 2.6, 0))
    for sign, side in ((-1, "Port"), (1, "Starboard")):
        loft_from_rings(f"DriveHouse_{side}", [
            diamond_ring(-6.20, 1.50 * sign, 0.10, 0.32, 0.26),
            diamond_ring(-7.20, 1.50 * sign, 0.10, 0.42, 0.34),
        ...
        add_cylinder(f"Broad_Fore_{side}", (0.75, 2.18 * sign, 0.18), 0.07, 0.62, mech, collection, vertices=8, bevel=0.003)
        add_cylinder(f"Broad_Aft_{side}", (-0.45, 2.18 * sign, 0.18), 0.07, 0.62, mech, collection, vertices=8, bevel=0.003)
        add_cylinder(f"Broad_Mid_{side}", (0.15, 2.18 * sign, 0.32), 0.05, 0.42, mech, collection, vertices=8, bevel=0.002)
        add_box(f"DorsalTile_{side}", (1.05, 0.55 * sign, 0.88), (0.75, 0.28, 0.018), armor, collection, 0.004)
        add_box(f"BowCheek_{side}", (5.85, 0.72 * sign, 0.14), (0.95, 0.06, 0.18), armor, collection, 0.006)
    ...
    add_box("DeckPlate_D", (2.05, -0.22, 0.86), (0.55, 0.24, 0.014), armor, collection, 0.003)
    add_box("CaseDeck_P", (0.15, -1.35, 0.42), (1.10, 0.18, 0.04), armor, collection, 0.004)
    add_box("CaseDeck_S", (0.15, 1.35, 0.42), (1.10, 0.18, 0.04), armor, collection, 0.004)
    ...
    add_box("Warn_Chevron", (-3.55, -0.72, 0.72), (0.22, 0.10, 0.01), warning, collection, 0.002)
    add_box("Accent_Flash", (0.55, -1.42, 0.28), (0.50, 0.016, 0.08), accent, collection, 0.002)
    add_sensor_dish("Dorsal", (1.85, 0.38, 1.05), mats, collection)
    add_cylinder("Turret_Ring", (0.55, 0.0, 0.98), 0.32, 0.08, mech, collection, vertices=14, bevel=0.006, rot=(0, 0, 0))
    add_box("Turret_Head", (0.70, 0.0, 1.12), (0.28, 0.16, 0.08), armor, collection, 0.006)
    ...
    if CYCLE >= 7:
        add_box("TowerMullion", (-2.15, 0.0, 2.38), (0.03, 0.36, 0.10), armor, collection, 0.002)
        if lod <= 1:
            for sign, side in ((-1, "Port"), (1, "Starboard")):
                for i in range(5):
                    add_box(
                        f"RadFin_{side}_{i}",
                        (-0.20 + i * 0.18, 1.58 * sign, 0.20),
                        (0.012, 0.14, 0.16),
                        mech, collection, 0.001,
                    )
    if CYCLE >= 8:
        add_box("DriveHouseBandP", (-7.15, -1.50, 0.10), (0.06, 0.38, 0.26), armor, collection, 0.003)
        add_box("DriveHouseBandS", (-7.15, 1.50, 0.10), (0.06, 0.38, 0.26), armor, collection, 0.003)
    if CYCLE >= 9:
        add_box("ChineCapP", (2.80, -1.35, 0.12), (1.35, 0.035, 0.045), armor, collection, 0.003)
        add_box("ChineCapS", (2.80, 1.35, 0.12), (1.35, 0.035, 0.045), armor, collection, 0.003)
    if CYCLE >= 10:
        add_box("PatchTile2", (1.25, 0.48, 0.82), (0.22, 0.10, 0.008), armor, collection, 0.002)
        add_box("TowerStayP", (-2.85, -0.18, 2.05), (0.22, 0.02, 0.02), mech, collection, 0.002)
        add_box("TowerStayS", (-2.85, 0.18, 2.05), (0.22, 0.02, 0.02), mech, collection, 0.002)
    if lod == 0:
        add_curve_hose("Hose_Port", [(0.2, -1.55, 0.10), (-2.6, -1.52, 0.12), (-5.6, -1.50, 0.14), (-8.0, -1.50, 0.16)], mech, collection, 0.016)
        add_curve_hose("Hose_Stbd", [(0.2, 1.55, 0.10), (-2.6, 1.52, 0.12), (-5.6, 1.50, 0.14), (-8.0, 1.50, 0.16)], mech, collection, 0.016)
        add_box("HoseFit_P0", (0.15, -1.55, 0.10), (0.03, 0.03, 0.03), mech, collection, 0.002)
        add_box("HoseFit_P1", (-7.95, -1.50, 0.16), (0.03, 0.03, 0.03), mech, collection, 0.002)
        add_box("HoseFit_S0", (0.15, 1.55, 0.10), (0.03, 0.03, 0.03), mech, collection, 0.002)
        add_box("HoseFit_S1", (-7.95, 1.50, 0.16), (0.03, 0.03, 0.03), mech, collection, 0.002)
    ...
    bm = bmesh.new()
    for point in [
        (8.0, 0, 0.2), (0, -3.2, 0.6), (0, 3.2, 0.6),
        (-7.6, -2.2, 0.2), (-7.6, 2.2, 0.2),
        (2.0, -1.6, -1.1), (2.0, 1.6, -1.1),
    ]:
        bm.verts.new(point)
...
def export_lod(collection, lod):
    out = FAMILY / "source" / "wholeships" / f"bastion_production_v1_lod{lod}.glb"
...
def render_cycle(collection):
    ...
    camera = setup_studio()
    out = FAMILY / "evidence" / "bastion" / "cycles" / f"cycle_{CYCLE:02d}"
    out.mkdir(parents=True, exist_ok=True)
    views = {
        "three_quarter": ((13.5, -15.5, 6.8), (0, 0, 0.12), 36),
        "starboard": ((0.0, 24.0, 4.2), (0, 0, 0.12), 30),
...
def main():
    ...
    for lod in (0, 1, 2):
        ...
        if report["hullTriangles"] < 800:
            raise RuntimeError(f"bastion lod{lod} hull {report['hullTriangles']} < 800")
        collections.append(collection)
        reports.append(report)
    stills = render_cycle(collections[0])
    report = {
        "schema": "spaceface.bastionMtx.cycle.v1",
        "shipId": "bastion",
        "cycle": CYCLE,
        "lods": reports,
        "stills": str(stills.relative_to(FAMILY)).replace("\\", "/"),
    }
    (FAMILY / "evidence" / "bastion").mkdir(parents=True, exist_ok=True)
    (FAMILY / "evidence" / "bastion" / f"cycle_{CYCLE:02d}.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
