"""PQ-050.11 Mule MTX builder. Hitch untouched. --mtx-cycle N writes cycle stills."""
...
FAMILY = ROOT / "assets" / "ships" / "fleet_player_bodies_v1" / "mule"
...
def role_maps(role, rgb, size=TEX, prefix=None):
    """Unique Mule maps. Not a tint of the shared Wasp/Hitch sheet."""
    ...
    base = write_pixels(f"mule_{prefix}_basecolor", albedo, size, "sRGB")
    orm_img = write_pixels(f"mule_{prefix}_orm", orm, size, "Non-Color")
    nrm_img = write_pixels(f"mule_{prefix}_normal", nrm, size, "Non-Color")
...
def create_materials():
    specs = {
        "Material_Hull": ((0.20, 0.22, 0.16), 0.08, 0.54, "hull", 0.08, None),
...
def sockets():
    return {
        "SOCKET_Weapon_Front": (7.6, 0.0, 0.28),
        "SOCKET_Mining_Front": (8.2, 0.0, -0.18),
        "SOCKET_Engine_Main": (-7.6, 0.0, 0.08),
        "SOCKET_Trail_Main": (-8.0, 0.0, 0.08),
        "SOCKET_Trail_Port": (-7.6, -1.45, 0.08),
        "SOCKET_Trail_Starboard": (-7.6, 1.45, 0.08),
        "SOCKET_Utility_Dorsal": (-1.2, 0.0, 1.85),
        "SOCKET_Cargo_Ventral": (0.2, 0.0, -1.25),
        "SOCKET_Camera_Focus": (1.2, 0.0, 0.35),
        "SOCKET_RCS_Port": (-1.4, -2.15, 0.18),
        "SOCKET_RCS_Starboard": (-1.4, 2.15, 0.18),
    }
...
def build_lod(lod, mats):
    collection = bpy.data.collections.new(f"MULE_LOD{lod}")
    ...
    half = 9.0
    root = add_empty(f"MULE_LOD{lod}_ROOT", (0, 0, 0), collection)
    root["spacefaceAsset"] = {
        "assetId": "SF_MULE_PRODUCTION_V1", "partId": "mule_production_v1",
        "lod": f"lod{lod}", "slot": "hull", "category": "wholeships",
        "forward": "+X", "embeddedPlume": False,
    }
    # Hauler: blunt bow, fat cargo mid, twin-drive transom. Not Atlas (no four-drive / three-pod).
    hull_obj = loft_from_rings("Pressure_Hull", [
        barge_ring(half, 0, 0.08, 0.72, 0.50, 0.42),
        barge_ring(half * 0.80, 0, 0.08, 1.18, 0.72, 0.60),
        barge_ring(half * 0.48, 0, 0.08, 1.72, 0.98, 0.82),
        barge_ring(half * 0.08, 0, 0.08, 2.02, 1.18, 1.02),
        barge_ring(-half * 0.22, 0, 0.08, 1.95, 1.10, 0.95),
        barge_ring(-half * 0.58, 0, 0.08, 1.62, 0.88, 0.76),
        barge_ring(-half + 0.50, 0, 0.06, 1.22, 0.68, 0.54),
    ], hull, collection, 0.012)
    if lod <= 1:
        cut_open_bay(hull_obj, "Bridge", (3.15, 0.0, 1.18), 1.55, 0.72, 0.40, (0, 0, 1), mats, collection, kit="cockpit")
    ...
    add_box("Bridge_Glass", (3.15, 0.0, 1.58), (0.04, 0.42, 0.14), mats["Material_Canopy"], collection, 0.002)
    add_box("Bridge_Walk", (2.45, 0.0, 1.28), (0.70, 0.68, 0.04), mech, collection, 0.003)
    ...
    for y, side in ((-1.45, "Port"), (1.45, "Starboard")):
        add_hollow_bell(side, -7.85, y, 0.08, 1.12, mats, collection)
        ...
        add_rcs_cluster(side, (-1.40, 2.05 * (1 if y > 0 else -1), 0.18), mats, collection, sign=1 if y > 0 else -1)
    add_box("Transom_Header", (-5.85, 0.0, 0.62), (0.55, 1.65, 0.04), armor, collection, 0.004)
    add_cylinder("RearGun_P", (-7.15, -0.55, 0.42), 0.045, 0.55, mech, collection, 10, 0.003)
    add_cylinder("RearGun_S", (-7.15, 0.55, 0.42), 0.045, 0.55, mech, collection, 10, 0.003)
    add_box("Hatch_Lid", (-0.65, 0.38, 1.10), (0.34, 0.20, 0.018), armor, collection, 0.003)
    ...
    if lod == 0:
        add_curve_hose("Hose_Port", [(0.2, -1.55, 0.12), (-3.2, -1.50, 0.14), (-6.4, -1.48, 0.12)], mech, collection, 0.018)
        add_curve_hose("Hose_Stbd", [(0.2, 1.55, 0.12), (-3.2, 1.50, 0.14), (-6.4, 1.48, 0.12)], mech, collection, 0.018)
    if CYCLE >= 6 and lod <= 1:
        boolean_cut(hull_obj, "TransomRecess", (-8.25, 0.0, 0.08), (0.18, 0.68, 0.26))
    if CYCLE >= 7:
        add_box("BridgeMullion", (3.15, 0.0, 1.58), (0.025, 0.32, 0.10), armor, collection, 0.002)
        if lod <= 1:
            for sign, side in ((-1, "Port"), (1, "Starboard")):
                for i in range(4):
                    add_box(
                        f"FlankRad_{side}_{i}",
                        (-2.85 + i * 0.20, 1.78 * sign, 0.18),
                        (0.012, 0.16, 0.16),
                        mech, collection, 0.001,
                    )
    if CYCLE >= 8:
        add_box("DriveBankBand", (-6.75, 0.0, 0.08), (0.06, 1.65, 0.22), armor, collection, 0.003)
    if CYCLE >= 9:
        add_box("ChineCapP", (2.15, -1.72, 0.18), (1.25, 0.040, 0.048), armor, collection, 0.003)
        add_box("ChineCapS", (2.15, 1.72, 0.18), (1.25, 0.040, 0.048), armor, collection, 0.003)
    if CYCLE >= 10:
        add_box("PatchTile2", (1.45, 0.42, 1.16), (0.24, 0.12, 0.010), armor, collection, 0.002)
        add_box("HouseStayP", (2.15, -0.22, 1.42), (0.20, 0.016, 0.016), mech, collection, 0.002)
        add_box("HouseStayS", (2.15, 0.22, 1.42), (0.20, 0.016, 0.016), mech, collection, 0.002)
    ...
    bm = bmesh.new()
    for point in [
        (8.8, 0, 0.2), (0, -2.6, 0.7), (0, 2.6, 0.7),
        (-7.8, -1.8, 0.1), (-7.8, 1.8, 0.1),
        (0.2, -2.4, -0.8), (0.2, 2.4, -0.8),
        (2.6, 0, 1.7),
    ]:
        bm.verts.new(point)
...
def render_cycle(collection):
    ...
    camera = setup_studio()
    out = FAMILY / "evidence" / "mule" / "cycles" / f"cycle_{CYCLE:02d}"
    out.mkdir(parents=True, exist_ok=True)
    views = {
        "three_quarter": ((14.8, -16.2, 6.8), (0, 0, 0.18), 32),
        "starboard": ((0.0, 22.0, 4.4), (0, 0, 0.15), 30),
        "rear": ((-16.8, -6.2, 4.2), (-0.4, 0, 0.12), 32),
        "clay_three_quarter": ((14.8, -16.2, 6.8), (0, 0, 0.18), 32),
        "grazing_close": ((9.2, -7.2, 2.0), (0.4, 0, 0.15), 48),
        "bay_interior": ((3.1, -4.2, 1.3), (3.0, 0.0, 1.1), 40),
        "drive_rear": ((-11.8, -3.2, 1.5), (-7.4, 0, 0.08), 48),
        "play_size": ((48, -42, 20), (0, 0, 0.12), 50),
        "orm_isolation": ((14.8, -16.2, 6.8), (0, 0, 0.18), 32),
        "normal_isolation": ((14.8, -16.2, 6.8), (0, 0, 0.18), 32),
        "id_or_material_id": ((14.8, -16.2, 6.8), (0, 0, 0.18), 32),
        "material_three_quarter": ((14.8, -16.2, 6.8), (0, 0, 0.18), 32),
    }
...
def main():
    ...
    for lod in (0, 1, 2):
        ...
        if report["hullTriangles"] < 800:
            raise RuntimeError(f"mule lod{lod} hull {report['hullTriangles']} < 800")
        collections.append(collection)
        reports.append(report)
    stills = render_cycle(collections[0])
    report = {
        "schema": "spaceface.muleMtx.cycle.v1",
        "shipId": "mule",
        "cycle": CYCLE,
        "lods": reports,
        "stills": str(stills.relative_to(FAMILY)).replace("\\", "/"),
    }
    (FAMILY / "evidence" / "mule").mkdir(parents=True, exist_ok=True)
    (FAMILY / "evidence" / "mule" / f"cycle_{CYCLE:02d}.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
