"""PQ-050.14 Ashline lode MTX builder. Hitch untouched. --mtx-cycle N writes cycle stills."""
...
FAMILY = ROOT / "assets" / "ships" / "fleet_player_bodies_v1" / "ashline_lode"
...
def role_maps(role, rgb, size=TEX, prefix=None):
    """Unique Ashline lode maps. Rust-red Reach bruiser, not a dart tint."""
    ...
    base = write_pixels(f"lode_{prefix}_basecolor", albedo, size, "sRGB")
    orm_img = write_pixels(f"lode_{prefix}_orm", orm, size, "Non-Color")
    nrm_img = write_pixels(f"lode_{prefix}_normal", nrm, size, "Non-Color")
...
def create_materials():
    specs = {
        "Material_Hull": ((0.24, 0.12, 0.08), 0.06, 0.56, "hull", 0.06, None),
...
def sockets():
    # Frozen factory lode sockets (length 16, halfW 2.1, height 0.95).
    return {
        "SOCKET_Weapon_Front": (6.8, 0.0, 0.25),
        "SOCKET_Mining_Front": (7.4, 0.0, -0.15),
        "SOCKET_Engine_Main": (-6.8, 0.0, 0.08),
        "SOCKET_Trail_Main": (-7.2, 0.0, 0.08),
        "SOCKET_Trail_Port": (-7.0, -1.47, 0.08),
        "SOCKET_Trail_Starboard": (-7.0, 1.47, 0.08),
        "SOCKET_Utility_Dorsal": (0.6, 0.0, 1.50),
        "SOCKET_Cargo_Ventral": (-0.4, 0.0, -1.10),
        "SOCKET_Camera_Focus": (0.8, 0.0, 0.25),
        "SOCKET_RCS_Port": (-1.2, -2.30, 0.15),
        "SOCKET_RCS_Starboard": (-1.2, 2.30, 0.15),
    }
...
def build_lod(lod, mats):
    collection = bpy.data.collections.new(f"LODE_LOD{lod}")
    bpy.context.scene.collection.children.link(collection)
    hull, armor, mech = mats["Material_Hull"], mats["Material_Armor"], mats["Material_Mechanical"]
    warning = mats["Material_Warning"]
    root = add_empty(f"LODE_LOD{lod}_ROOT", (0, 0, 0), collection)
    root["spacefaceAsset"] = {
        "assetId": "SF_ASHLINE_LODE_V1", "partId": "ashline_lode_production_v1",
        "lod": f"lod{lod}", "slot": "hull", "category": "wholeships",
        "forward": "+X", "embeddedPlume": False,
    }
    # 16 m Reach bruiser: blunt prow, thick waist, tower, twin tail drives. Not dart.
    hull_obj = loft_from_rings("Pressure_Hull", [
        station_ring(8.0, 0, 0.08, 0.55, 0.38, flat=0.20, box=0.25, keel=0.70),
        station_ring(5.0, 0, 0.12, 1.45, 0.78, flat=0.55, box=0.45, keel=0.55),
        station_ring(1.6, 0, 0.12, 2.05, 1.00, flat=0.45, box=0.80, keel=0.40),
        station_ring(-1.4, 0, 0.10, 2.10, 1.05, flat=0.40, box=0.90, keel=0.32),
        station_ring(-4.6, 0, 0.10, 1.55, 0.82, flat=0.32, box=0.92, keel=0.25),
        station_ring(-7.55, 0, 0.06, 0.82, 0.48, flat=0.25, box=0.92, keel=0.20),
    ...
    cover_loft_with_plates("LodeSkin", [
        (7.80, 0.60, 0.42, 0.08),
        (5.00, 1.50, 0.80, 0.12),
        (1.60, 2.10, 1.02, 0.12),
        (-1.40, 2.14, 1.08, 0.10),
    ...
    add_box("TowerGlass", (1.05, 0.0, 1.72), (0.32, 0.32, 0.16), mats["Material_Canopy"], collection, 0.004)
    add_box("TowerBrow", (1.35, 0.0, 1.62), (0.22, 0.36, 0.06), armor, collection, 0.003)
    add_box("ArmorBeltP", (0.6, -2.08, -0.12), (2.4, 0.06, 0.18), armor, collection, 0.004)
    add_box("ArmorBeltS", (0.6, 2.08, -0.12), (2.4, 0.06, 0.18), armor, collection, 0.004)
    add_sensor_dish("Dorsal", (0.4, 0.0, 1.85), mats, collection)

    for sign, side in ((-1, "Port"), (1, "Starboard")):
        add_box(f"Shoulder_{side}", (1.4, 1.95 * sign, 0.22), (1.15, 0.22, 0.28), armor, collection, 0.006)
        add_box(f"GunCheek_{side}", (5.15, 0.55 * sign, 0.22), (0.72, 0.16, 0.12), armor, collection, 0.004)
        ...
        add_box(f"Warn_{side}", (0.2, 2.05 * sign, 0.28), (0.22, 0.08, 0.02), warning, collection, 0.002)

    add_box("Hatch_Lid", (-0.55, 0.28, 1.02), (0.26, 0.14, 0.014), armor, collection, 0.002)
    add_box("Repair_Patch", (1.15, -0.55, 0.92), (0.22, 0.10, 0.010), warning, collection, 0.002)
    add_box("VentralKeel", (-1.0, 0.0, -0.82), (3.6, 0.16, 0.06), armor, collection, 0.004)
    add_box("ChinPlate", (4.8, 0.0, -0.28), (1.6, 0.55, 0.06), armor, collection, 0.004)
    add_box("CasemateP", (2.2, -1.85, 0.42), (0.85, 0.18, 0.16), armor, collection, 0.004)
    add_box("CasemateS", (2.2, 1.85, 0.42), (0.85, 0.18, 0.16), armor, collection, 0.004)
    add_box("TransomBraceP", (-7.15, -0.42, 0.22), (0.22, 0.06, 0.18), mech, collection, 0.003)
    add_box("TransomBraceS", (-7.15, 0.42, 0.22), (0.22, 0.06, 0.18), mech, collection, 0.003)
    add_box("AftShoulderP", (-4.4, -1.55, 0.28), (0.95, 0.16, 0.14), armor, collection, 0.004)
    add_box("AftShoulderS", (-4.4, 1.55, 0.28), (0.95, 0.16, 0.14), armor, collection, 0.004)
    add_box("BowCheekP", (5.4, -0.85, 0.08), (1.05, 0.12, 0.10), armor, collection, 0.003)
    add_box("BowCheekS", (5.4, 0.85, 0.08), (1.05, 0.12, 0.10), armor, collection, 0.003)
    add_box("TowerDeck", (0.55, 0.0, 1.12), (1.35, 0.62, 0.04), mech, collection, 0.003)
    add_service_hatch("Dorsal", (-1.6, 0.0, 1.02), mats, collection, sx=0.34, sy=0.22)
    add_panel_seams("Hull", [3.2, 0.8, -1.6, -4.2], 1.55, 0.82, mech, collection)

    if lod == 0:
        add_curve_hose("Hose_Port", [(0.8, -1.15, 0.12), (-3.2, -1.25, 0.14), (-6.8, -1.15, 0.10)], mech, collection, 0.014)
        add_curve_hose("Hose_Stbd", [(0.8, 1.15, 0.12), (-3.2, 1.25, 0.14), (-6.8, 1.15, 0.10)], mech, collection, 0.014)
    if CYCLE >= 6 and lod <= 1:
        boolean_cut(hull_obj, "TransomRecess", (-7.55, 0.0, 0.10), (0.16, 0.42, 0.22))
    if CYCLE >= 7:
        add_box("CanopyMullion", (2.15, 0.0, 0.92), (0.022, 0.28, 0.08), armor, collection, 0.002)
        if lod <= 1:
            for sign, side in ((-1, "Port"), (1, "Starboard")):
                for i in range(4):
                    add_box(f"FlankRad_{side}_{i}", (-2.15 + i * 0.18, 1.55 * sign, 0.16), (0.012, 0.14, 0.12), mech, collection, 0.001)
    if CYCLE >= 8:
        add_box("DriveBankBand", (-6.55, 0.0, 0.10), (0.06, 0.85, 0.18), armor, collection, 0.003)
    if CYCLE >= 9:
        add_box("ChineCapP", (1.55, -1.45, 0.12), (1.15, 0.036, 0.040), armor, collection, 0.003)
        add_box("ChineCapS", (1.55, 1.45, 0.12), (1.15, 0.036, 0.040), armor, collection, 0.003)
    if CYCLE >= 10:
        add_box("PatchTile2", (1.05, 0.42, 0.88), (0.20, 0.10, 0.008), armor, collection, 0.002)
        add_box("HouseStayP", (1.85, -0.16, 0.72), (0.16, 0.014, 0.014), mech, collection, 0.002)
        add_box("HouseStayS", (1.85, 0.16, 0.72), (0.16, 0.014, 0.014), mech, collection, 0.002)
    ...
    bm = bmesh.new()
    for point in [
        (7.7, 0, 0.1), (1.6, -2.2, 0.8), (1.6, 2.2, 0.8),
        (-7.4, -1.5, 0.2), (-7.4, 1.5, 0.2),
        (0.4, -2.4, -0.7), (0.4, 2.4, -0.7),
        (0.8, 0, 1.8),
    ]:
        bm.verts.new(point)
...
def export_lod(collection, lod):
    out = FAMILY / "source" / "wholeships" / f"ashline_lode_production_v1_lod{lod}.glb"
...
def render_cycle(collection):
    ...
    camera = setup_studio()
    out = FAMILY / "evidence" / "ashline_lode" / "cycles" / f"cycle_{CYCLE:02d}"
    out.mkdir(parents=True, exist_ok=True)
    views = {
        "three_quarter": ((13.2, -15.4, 5.4), (0, 0, 0.18), 32),
        "starboard": ((0.0, 20.5, 4.0), (0, 0, 0.14), 30),
        "rear": ((-15.4, -6.6, 4.2), (-0.6, 0, 0.12), 30),
        "clay_three_quarter": ((13.2, -15.4, 5.4), (0, 0, 0.18), 32),
        "grazing_close": ((7.8, -6.4, 2.0), (0.8, 0, 0.22), 46),
        "bay_interior": ((2.6, -3.4, 1.1), (2.5, 0.0, 0.90), 40),
        "drive_rear": ((-11.2, -4.6, 1.5), (-7.2, 0, 0.08), 42),
        "play_size": ((42, -38, 16), (0, 0, 0.10), 48),
        "orm_isolation": ((13.2, -15.4, 5.4), (0, 0, 0.18), 32),
        "normal_isolation": ((13.2, -15.4, 5.4), (0, 0, 0.18), 32),
        "id_or_material_id": ((13.2, -15.4, 5.4), (0, 0, 0.18), 32),
        "material_three_quarter": ((13.2, -15.4, 5.4), (0, 0, 0.18), 32),
    }
...
def main():
    ...
    for lod in (0, 1, 2):
        ...
        if report["hullTriangles"] < 800:
            raise RuntimeError(f"ashline_lode lod{lod} hull {report['hullTriangles']} < 800")
        collections.append(collection)
        reports.append(report)
    stills = render_cycle(collections[0])
    report = {
        "schema": "spaceface.ashlineLodeMtx.cycle.v1",
        "shipId": "ashline_lode",
        "cycle": CYCLE,
        "lods": reports,
        "stills": str(stills.relative_to(FAMILY)).replace("\\", "/"),
    }
    (FAMILY / "evidence" / "ashline_lode").mkdir(parents=True, exist_ok=True)
    (FAMILY / "evidence" / "ashline_lode" / f"cycle_{CYCLE:02d}.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
