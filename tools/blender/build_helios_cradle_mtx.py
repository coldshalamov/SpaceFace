"""PQ-050.17 Helios cradle MTX builder. Hitch untouched. --mtx-cycle N writes cycle stills."""
...
FAMILY = ROOT / "assets" / "ships" / "fleet_player_bodies_v1" / "helios_cradle"
...
def role_maps(role, rgb, size=TEX, prefix=None):
    """Unique Helios cradle maps. Pale dirty civilian miner, not Lark or Reach."""
    ...
    for y in range(size):
        for x in range(size):
            ...
            stencil = 0.0
            if role == "hull" and 28 <= x <= 220 and 40 <= y <= 140:
                col = x - 28
                row = y - 40
                stripe = (row // 12) % 2 == 0
                if stripe and 8 <= col <= 170:
                    stencil = 0.42
                if gf > 0.88:
                    stencil *= 0.30
            if role == "hull":
    ...
    base = write_pixels(f"cradle_{prefix}_basecolor", albedo, size, "sRGB")
    orm_img = write_pixels(f"cradle_{prefix}_orm", orm, size, "Non-Color")
    nrm_img = write_pixels(f"cradle_{prefix}_normal", nrm, size, "Non-Color")
...
def create_materials():
    specs = {
        "Material_Hull": ((0.42, 0.40, 0.34), 0.04, 0.52, "hull", 0.06, None),
        "Material_Armor": ((0.36, 0.34, 0.28), 0.08, 0.50, "armor", 0.03, None),
        "Material_Mechanical": ((0.28, 0.28, 0.28), 0.74, 0.36, "mechanical", 0.0, None),
        "Material_Accent": ((0.80, 0.62, 0.10), 0.04, 0.46, "warning", 0.04, None),
        "Material_Warning": ((0.84, 0.56, 0.08), 0.04, 0.48, "warning", 0.04, None),
...
def sockets():
    # Frozen factory cradle sockets (length 15, halfW 1.9, height 0.95).
    return {
        "SOCKET_Weapon_Front": (6.3, 0.0, 0.25),
        "SOCKET_Mining_Front": (6.9, 0.0, -0.15),
        "SOCKET_Engine_Main": (-6.3, 0.0, 0.08),
        "SOCKET_Trail_Main": (-6.7, 0.0, 0.08),
        "SOCKET_Trail_Port": (-6.5, -1.33, 0.08),
        "SOCKET_Trail_Starboard": (-6.5, 1.33, 0.08),
        "SOCKET_Utility_Dorsal": (0.6, 0.0, 1.50),
        "SOCKET_Cargo_Ventral": (-0.4, 0.0, -1.10),
        "SOCKET_Camera_Focus": (0.8, 0.0, 0.25),
        "SOCKET_RCS_Port": (-1.2, -2.10, 0.15),
        "SOCKET_RCS_Starboard": (-1.2, 2.10, 0.15),
    }
...
def build_lod(lod, mats):
    collection = bpy.data.collections.new(f"CRADLE_LOD{lod}")
    ...
    root = add_empty(f"CRADLE_LOD{lod}_ROOT", (0, 0, 0), collection)
    root["spacefaceAsset"] = {
        "assetId": "SF_HELIOS_CRADLE_V1", "partId": "helios_cradle_production_v1",
        "lod": f"lod{lod}", "slot": "hull", "category": "wholeships",
        "forward": "+X", "embeddedPlume": False,
    }
    # 15 m pale civilian miner. Stations change: chisel bow, cabin shoulder, tall hopper waist, pinched transom.
    hull_obj = loft_from_rings("Pressure_Hull", [
        station_ring(7.5, 0, 0.02, 0.30, 0.20, flat=0.15, box=0.20, keel=0.70),
        station_ring(5.8, 0, 0.08, 0.82, 0.42, flat=0.45, box=0.40, keel=0.55),
        station_ring(4.0, 0, 0.28, 1.22, 0.78, flat=0.85, box=0.50, keel=0.40),
        station_ring(1.6, 0, 0.40, 1.82, 1.32, flat=0.55, box=0.80, keel=0.28),
        station_ring(-1.0, 0, 0.36, 1.90, 1.22, flat=0.45, box=0.88, keel=0.25),
        station_ring(-3.6, 0, 0.16, 1.38, 0.72, flat=0.30, box=0.92, keel=0.22),
        station_ring(-7.05, 0, 0.00, 0.52, 0.34, flat=0.22, box=0.90, keel=0.18),
    ], hull, collection, 0.012, cap="both")
    if lod <= 1:
        cut_open_bay(hull_obj, "Cockpit", (3.85, 0.0, 0.98), 0.95, 0.32, 0.30, (0, 0, 1), mats, collection, kit="cockpit", liner=False)
        boolean_cut(hull_obj, "SideWellP", (0.10, -1.92, 0.28), (0.95, 0.22, 0.28))
        boolean_cut(hull_obj, "SideWellS", (0.10, 1.92, 0.28), (0.95, 0.22, 0.28))
        boolean_cut(hull_obj, "HopperWell", (0.20, 0.0, 1.18), (1.45, 0.72, 0.48))
        boolean_cut(hull_obj, "RadWellP", (-3.35, -0.58, 0.72), (0.55, 0.28, 0.18))
        boolean_cut(hull_obj, "RadWellS", (-3.35, 0.58, 0.72), (0.55, 0.28, 0.18))
        boolean_cut(hull_obj, "TransomRecess", (-7.05, 0.0, 0.02), (0.18, 0.32, 0.16))
    ...
    add_box("CabinBrow", (4.30, 0.0, 1.00), (0.07, 0.24, 0.05), armor, collection, 0.003)
    add_box("CabinSill", (3.85, 0.0, 0.84), (0.48, 0.32, 0.014), armor, collection, 0.002)
    add_box("CabinFrameP", (3.85, -0.28, 0.96), (0.38, 0.018, 0.10), armor, collection, 0.002)
    add_box("CabinFrameS", (3.85, 0.28, 0.96), (0.38, 0.018, 0.10), armor, collection, 0.002)

    # Hopper well: deep pit, hazard lip, five-wall liner, slat grate.
    add_box("WellLipF", (1.58, 0.0, 1.12), (0.05, 0.86, 0.06), warning, collection, 0.002)
    add_box("WellLipA", (-1.38, 0.0, 1.12), (0.05, 0.86, 0.06), warning, collection, 0.002)
    add_box("WellLipP", (0.10, -0.84, 1.12), (1.42, 0.04, 0.06), warning, collection, 0.002)
    add_box("WellLipS", (0.10, 0.84, 1.12), (1.42, 0.04, 0.06), warning, collection, 0.002)
    add_box("WellFloor", (0.10, 0.0, 0.28), (1.38, 0.72, 0.03), mech, collection, 0.003)
    add_box("WellWallP", (0.10, -0.74, 0.68), (1.36, 0.03, 0.38), armor, collection, 0.002)
    add_box("WellWallS", (0.10, 0.74, 0.68), (1.36, 0.03, 0.38), armor, collection, 0.002)
    add_box("WellWallA", (-1.28, 0.0, 0.68), (0.03, 0.72, 0.38), armor, collection, 0.002)
    add_box("WellWallF", (1.48, 0.0, 0.68), (0.03, 0.72, 0.38), armor, collection, 0.002)
    if lod == 0:
        for i, ox in enumerate((-0.85, -0.35, 0.15, 0.65)):
            add_box(f"WellSlat_{i}", (ox, 0.0, 0.36), (0.04, 0.58, 0.012), mech, collection, 0.001)

    # Open dump bins standing proud of the hopper castle, not flush deck holes.
    for sign, side in ((-1, "Port"), (1, "Starboard")):
        y = 1.35 * sign
        loft_from_rings(f"Bin_{side}", [
            [
                (1.45, y - 0.38 * sign, 0.95), (1.45, y - 0.38 * sign, 1.62),
                (1.45, y - 0.06 * sign, 1.62), (1.45, y + 0.06 * sign, 1.62),
                (1.45, y + 0.38 * sign, 1.62), (1.45, y + 0.38 * sign, 0.95),
                (1.45, y + 0.16 * sign, 0.88), (1.45, y - 0.16 * sign, 0.88),
            ],
            [
                (0.15, y - 0.46 * sign, 0.92), (0.15, y - 0.46 * sign, 1.72),
                (0.15, y - 0.08 * sign, 1.72), (0.15, y + 0.08 * sign, 1.72),
                (0.15, y + 0.46 * sign, 1.72), (0.15, y + 0.46 * sign, 0.92),
                (0.15, y + 0.18 * sign, 0.84), (0.15, y - 0.18 * sign, 0.84),
            ],
            [
                (-1.15, y - 0.36 * sign, 0.94), (-1.15, y - 0.36 * sign, 1.58),
                (-1.15, y - 0.06 * sign, 1.58), (-1.15, y + 0.06 * sign, 1.58),
                (-1.15, y + 0.36 * sign, 1.58), (-1.15, y + 0.36 * sign, 0.94),
                (-1.15, y + 0.14 * sign, 0.88), (-1.15, y - 0.14 * sign, 0.88),
            ],
        ], hull, collection, 0.008, cap=False)
        add_box(f"BinLip_{side}", (0.15, y + 0.20 * sign, 1.70), (1.15, 0.035, 0.025), warning, collection, 0.002)
        add_box(f"BinChute_{side}", (0.15, y * 0.48, 1.05), (0.32, 0.16, 0.08), mech, collection, 0.003)
        add_box(f"BinFootF_{side}", (1.20, y, 0.86), (0.12, 0.16, 0.08), mech, collection, 0.003)
        add_box(f"BinFootA_{side}", (-0.90, y, 0.86), (0.12, 0.16, 0.08), mech, collection, 0.003)
        add_box(f"BinLid_{side}", (0.15, y + 0.08 * sign, 1.78), (0.55, 0.22, 0.03), armor, collection, 0.003, rot=(0, 0, 0.35 * sign))

    # Two cutter arms: hull pad, hat boom, knuckle, rams, carbide drum.
    def hat_ring(x, y, z, w, h):
        return [
            (x, y - w, z - h * 0.15),
            (x, y - w, z + h),
            (x, y - w * 0.32, z + h),
    ...
    for sign, side in ((-1, "Port"), (1, "Starboard")):
        y = 1.58 * sign
        loft_from_rings(f"ArmPad_{side}", [
            ellipse_ring(2.15, y * 0.88, 0.12, 0.38, 0.22, 12),
            ellipse_ring(2.85, y, 0.04, 0.30, 0.20, 12),
            ellipse_ring(3.45, y, -0.04, 0.22, 0.16, 12),
        ], hull, collection, 0.008)
        loft_from_rings(f"ArmBoom_{side}", [
            hat_ring(3.45, y, -0.06, 0.28, 0.22),
            hat_ring(4.55, y, -0.14, 0.24, 0.18),
            hat_ring(5.55, y, -0.22, 0.20, 0.15),
        ], armor, collection, 0.004, cap=True)
        add_cylinder(f"ArmKnuckle_{side}", (3.45, y, -0.06), 0.15, 0.20, mech, collection, 12, 0.003)
        add_cylinder(f"ArmWrist_{side}", (5.55, y, -0.24), 0.13, 0.16, mech, collection, 12, 0.003)
        add_cylinder(f"Ram_{side}", (4.45, y + 0.20 * sign, 0.08), 0.040, 1.55, mech, collection, 8, 0.002)
        add_cylinder(f"RamB_{side}", (4.35, y - 0.12 * sign, 0.02), 0.028, 1.35, mech, collection, 8, 0.002)
        add_box(f"ArmHazard_{side}", (4.55, y, 0.04), (0.28, 0.07, 0.03), warning, collection, 0.002)
        add_cylinder(f"Drum_{side}", (6.25, y, -0.30), 0.42, 0.38, armor, collection, 20, 0.004, rot=(0, 0, math.pi / 2))
        add_cylinder(f"DrumHub_{side}", (6.25, y, -0.30), 0.12, 0.44, mech, collection, 10, 0.003, rot=(0, 0, math.pi / 2))
        add_cylinder(f"DrumGuard_{side}", (6.05, y, -0.08), 0.18, 0.10, armor, collection, 12, 0.003)
        if lod == 0:
            for i in range(12):
                ang = math.tau * i / 12
                add_box(
                    f"Tooth_{side}_{i}",
                    (6.25 + math.cos(ang) * 0.40, y, -0.30 + math.sin(ang) * 0.40),
                    (0.055, 0.042, 0.075),
                    mech, collection, 0.002,
                )
            add_curve_hose(
                f"Hose_{side}",
                [(2.45, y * 0.70, 0.18), (3.85, y * 0.92, 0.08), (5.35, y, -0.08)],
                mech, collection, 0.014,
            )
        add_rcs_cluster(side, (-1.2, 2.05 * sign, 0.18), mats, collection, sign=sign)
        add_box(f"Cheek_{side}", (4.65, 0.78 * sign, 0.10), (0.72, 0.10, 0.08), armor, collection, 0.003)

    # Twin axial bells at factory drive stations (-6.2, ±1.2). Hollow, no glow disk.
    for sign, side in ((-1, "Port"), (1, "Starboard")):
        add_flared_bell(side, -6.95, 1.20 * sign, 0.08, 1.05, mats, collection)
        add_cylinder(f"DriveCoupling_{side}", (-6.50, 1.20 * sign, 0.08), 0.34, 0.18, mech, collection, 16, 0.003)
        add_manufactured_drive(side, -6.20, 1.20 * sign, lod, mats, collection, scale=0.82, z=0.08)
        add_box(f"DriveSaddle_{side}", (-6.10, 1.20 * sign, -0.24), (0.42, 0.14, 0.09), mech, collection, 0.003)
        add_box(f"RadCassette_{side}", (-3.35, 0.58 * sign, 0.62), (0.48, 0.22, 0.10), mats["Material_Radiator"], collection, 0.002)
        add_box(f"RadHeader_{side}", (-3.35, 0.58 * sign, 0.74), (0.50, 0.04, 0.03), mech, collection, 0.002)
        if lod <= 1:
            for i in range(8):
                add_box(
                    f"RadFin_{side}_{i}",
                    (-3.62 + i * 0.08, 0.58 * sign, 0.68),
                    (0.010, 0.18, 0.16),
                    mech, collection, 0.001,
                )

    add_service_hatch("Dorsal", (-2.15, 0.0, 0.98), mats, collection, sx=0.30, sy=0.18)
    add_box("CargoDoor", (-0.4, 0.0, -0.62), (0.55, 0.32, 0.03), armor, collection, 0.003)
    add_box("CargoRim", (-0.4, 0.0, -0.58), (0.62, 0.38, 0.02), mech, collection, 0.002)
    add_box("DumpChute", (0.15, 0.0, 0.08), (0.48, 0.24, 0.28), mech, collection, 0.004)
    add_box("DumpLip", (0.15, 0.0, -0.18), (0.36, 0.18, 0.05), armor, collection, 0.003)
    add_box("RepairPlate", (2.15, -0.72, 0.55), (0.38, 0.18, 0.012), warning, collection, 0.002)
    add_box("TransomArmor", (-6.92, 0.0, 0.06), (0.06, 0.42, 0.22), armor, collection, 0.003)
    add_box("ChineStrakeP", (1.4, -1.55, 0.08), (2.4, 0.04, 0.06), armor, collection, 0.003)
    add_box("ChineStrakeS", (1.4, 1.55, 0.08), (2.4, 0.04, 0.06), armor, collection, 0.003)
    for sign, side in ((-1, "Port"), (1, "Starboard")):
        add_box(f"SideWellLip_{side}", (0.10, 1.72 * sign, 0.42), (0.85, 0.04, 0.04), armor, collection, 0.002)
        if lod <= 1:
            for i in range(6):
                add_box(
                    f"SideFin_{side}_{i}",
                    (-0.28 + i * 0.12, 1.78 * sign, 0.28),
                    (0.012, 0.10, 0.16),
                    mech, collection, 0.001,
                )
    add_box("ChinPlate", (5.15, 0.0, -0.22), (1.35, 0.55, 0.05), armor, collection, 0.003)
    add_box("BowCap", (7.35, 0.0, 0.08), (0.14, 0.22, 0.12), armor, collection, 0.003)
    add_box("TransomBraceP", (-6.65, -0.38, 0.22), (0.18, 0.05, 0.14), mech, collection, 0.003)
    add_box("TransomBraceS", (-6.65, 0.38, 0.22), (0.18, 0.05, 0.14), mech, collection, 0.003)
    add_box("StripeP", (1.2, -1.72, 0.28), (2.2, 0.014, 0.010), accent, collection, 0.001)
    add_box("StripeS", (1.2, 1.72, 0.28), (2.2, 0.014, 0.010), accent, collection, 0.001)
    add_box("RegMark", (4.15, 0.55, 0.62), (0.42, 0.10, 0.008), warning, collection, 0.001)
    add_box("AftHatch", (-2.55, 0.32, 0.92), (0.24, 0.14, 0.012), armor, collection, 0.002)
    add_sensor_dish("Dorsal", (-2.35, 0.0, 1.18), mats, collection)
    add_panel_seams("Hull", [3.6, 1.0, -1.4, -4.0], 1.35, 0.78, mech, collection)

    if lod == 0:
        add_curve_hose("Hose_MainP", [(1.2, -0.85, 0.12), (-2.4, -1.05, 0.14), (-5.8, -1.12, 0.10)], mech, collection, 0.012)
        add_curve_hose("Hose_MainS", [(1.2, 0.85, 0.12), (-2.4, 1.05, 0.14), (-5.8, 1.12, 0.10)], mech, collection, 0.012)
    if CYCLE >= 6 and lod <= 1:
        boolean_cut(hull_obj, "TransomRecess", (-6.45, 0.0, 0.10), (0.14, 0.38, 0.18))
    if CYCLE >= 7:
        add_box("CanopyMullion", (1.85, 0.0, 0.88), (0.020, 0.26, 0.07), armor, collection, 0.002)
        if lod <= 1:
            for sign, side in ((-1, "Port"), (1, "Starboard")):
                for i in range(4):
                    add_box(f"FlankRad_{side}_{i}", (-1.85 + i * 0.18, 1.35 * sign, 0.16), (0.012, 0.14, 0.12), mech, collection, 0.001)
    if CYCLE >= 8:
        add_box("DriveBankBand", (-5.65, 0.0, 0.10), (0.055, 0.72, 0.16), armor, collection, 0.003)
    if CYCLE >= 9:
        add_box("ChineCapP", (1.45, -1.25, 0.10), (1.05, 0.032, 0.036), armor, collection, 0.003)
        add_box("ChineCapS", (1.45, 1.25, 0.10), (1.05, 0.032, 0.036), armor, collection, 0.003)
    if CYCLE >= 10:
        add_box("PatchTile2", (1.05, 0.38, 0.82), (0.18, 0.09, 0.008), armor, collection, 0.002)
        add_box("HouseStayP", (1.65, -0.14, 0.68), (0.14, 0.012, 0.012), mech, collection, 0.002)
        add_box("HouseStayS", (1.65, 0.14, 0.68), (0.14, 0.012, 0.012), mech, collection, 0.002)
    ...
    bm = bmesh.new()
    for point in [
        (7.4, 0, 0.1), (2.4, -1.9, 0.8), (2.4, 1.9, 0.8),
        (-6.9, -1.4, 0.2), (-6.9, 1.4, 0.2),
        (0.3, -2.2, -0.8), (0.3, 2.2, -0.8),
        (6.2, -1.6, -0.3), (6.2, 1.6, -0.3),
        (0.1, 0, 1.6), (0.1, -1.7, 1.3), (0.1, 1.7, 1.3),
    ]:
        bm.verts.new(point)
...
def export_lod(collection, lod):
    out = FAMILY / "source" / "wholeships" / f"helios_cradle_production_v1_lod{lod}.glb"
...
def render_cycle(collection):
    ...
    camera = setup_studio()
    out = FAMILY / "evidence" / "helios_cradle" / "cycles" / f"cycle_{CYCLE:02d}"
    out.mkdir(parents=True, exist_ok=True)
    views = {
        "three_quarter": ((11.2, -13.2, 4.6), (0.2, 0, 0.22), 34),
        "starboard": ((0.3, 17.4, 3.4), (0.2, 0, 0.16), 32),
        "rear": ((-13.2, -5.8, 3.6), (-0.5, 0, 0.16), 32),
        "clay_three_quarter": ((11.2, -13.2, 4.6), (0.2, 0, 0.22), 34),
        "grazing_close": ((6.6, -5.6, 1.7), (0.7, 0, 0.24), 46),
        "bay_interior": ((0.2, -2.8, 1.5), (0.1, 0.0, 0.78), 40),
        "drive_rear": ((-9.2, -3.8, 1.3), (-6.3, 0, 0.08), 42),
        "play_size": ((36, -32, 15), (0, 0, 0.12), 48),
        "orm_isolation": ((11.2, -13.2, 4.6), (0.2, 0, 0.22), 34),
        "normal_isolation": ((11.2, -13.2, 4.6), (0.2, 0, 0.22), 34),
        "id_or_material_id": ((11.2, -13.2, 4.6), (0.2, 0, 0.22), 34),
        "material_three_quarter": ((11.2, -13.2, 4.6), (0.2, 0, 0.22), 34),
    }
...
def main():
    ...
    for lod in (0, 1, 2):
        ...
        if report["hullTriangles"] < 800:
            raise RuntimeError(f"helios_cradle lod{lod} hull {report['hullTriangles']} < 800")
        collections.append(collection)
        reports.append(report)
    stills = render_cycle(collections[0])
    report = {
        "schema": "spaceface.heliosCradleMtx.cycle.v1",
        "shipId": "helios_cradle",
        "cycle": CYCLE,
        "lods": reports,
        "stills": str(stills.relative_to(FAMILY)).replace("\\", "/"),
    }
    (FAMILY / "evidence" / "helios_cradle").mkdir(parents=True, exist_ok=True)
    (FAMILY / "evidence" / "helios_cradle" / f"cycle_{CYCLE:02d}.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
