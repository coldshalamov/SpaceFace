...
def build_lod(lod, mats):
    ...
    hull_obj = loft_from_rings("Pressure_Hull", [
        station_ring(half, 0, 0.32, 0.50, 0.70, flat=0.75, box=0.50, keel=0.35),
        station_ring(half * 0.78, 0, 0.36, 1.05, 0.95, flat=0.90, box=0.55, keel=0.30),
        station_ring(half * 0.54, 0, 0.02, 2.10, 0.48, flat=0.20, box=0.88, keel=0.25),
        station_ring(half * 0.12, 0, -0.06, 2.45, 0.32, flat=0.12, box=0.95, keel=0.22),
        station_ring(-half * 0.22, 0, -0.04, 2.42, 0.34, flat=0.12, box=0.95, keel=0.22),
        station_ring(-half * 0.50, 0, 0.10, 2.00, 0.58, flat=0.25, box=0.90, keel=0.22),
        station_ring(-half * 0.76, 0, 0.22, 1.55, 0.88, flat=0.40, box=0.95, keel=0.20),
        station_ring(-half + 0.58, 0, 0.18, 1.40, 0.72, flat=0.35, box=0.95, keel=0.18),
    ], hull, collection, 0.014)
    if lod <= 1:
        cut_open_bay(hull_obj, "Bridge", (6.70, 0.0, 1.22), 1.85, 0.78, 0.45, (0, 0, 1), mats, collection, kit="cockpit", liner=False)
        cut_open_bay(hull_obj, "CargoWell", (0.20, 0.0, 0.38), 6.40, 1.28, 0.42, (0, 0, 1), mats, collection, kit="empty", liner=False)
        cut_open_bay(hull_obj, "Port", (0.20, -2.28, 0.10), 2.10, 0.42, 0.48, (0, -1, 0), mats, collection, kit="empty", liner=False)
        cut_open_bay(hull_obj, "Starboard", (0.20, 2.28, 0.10), 2.10, 0.42, 0.48, (0, 1, 0), mats, collection, kit="empty", liner=False)
    ...
    loft_shell("Hold_FlankP", [
        (5.20, -1.40, -2.22, -0.18, 0.42),
        (0.20, -1.70, -2.48, -0.22, 0.38),
        (-4.80, -1.40, -2.18, -0.16, 0.32),
    ], hull, collection, 0.012)
    loft_shell("Hold_FlankS", [
        (5.20, 1.40, 2.22, -0.18, 0.42),
        (0.20, 1.70, 2.48, -0.22, 0.38),
        (-4.80, 1.40, 2.18, -0.16, 0.32),
    ], hull, collection, 0.012)
    center_loft("Hold_Carapace", [
        (4.80, 0.72, 0.08, 0.52),
        (0.20, 1.05, 0.12, 0.58),
        (-4.40, 0.68, 0.08, 0.46),
    ...
    cover_loft_with_plates("AtlasSkin", [
        (10.80, 0.56, 0.72, 0.32),
        (8.60, 1.10, 0.96, 0.36),
        (5.90, 2.14, 0.50, 0.04),
        (1.30, 2.48, 0.34, -0.04),
        (-2.40, 2.46, 0.36, -0.02),
    ...
    if CYCLE >= 6 and lod <= 1:
        boolean_cut(hull_obj, "TransomRecess", (-10.35, 0.0, 0.16), (0.18, 0.72, 0.28))
    if CYCLE >= 7:
        add_box("HouseMullion", (6.55, 0.0, 1.42), (0.03, 0.38, 0.10), armor, collection, 0.002)
        if lod <= 1:
            for sign, side in ((-1, "Port"), (1, "Starboard")):
                for i in range(5):
                    add_box(
                        f"RadFin_{side}_{i}",
                        (-0.40 + i * 0.22, 2.12 * sign, 0.16),
                        (0.014, 0.16, 0.18),
                        mech, collection, 0.001,
                    )
    if False:  # C14 drop kit-on-loaf extras (was CYCLE >= 8)
        add_box("DriveBankBand", (-8.85, 0.0, 0.14), (0.06, 2.10, 0.22), armor, collection, 0.003)
    if False:  # C14 drop kit-on-loaf extras (was CYCLE >= 9)
        add_box("ChineCapP", (4.20, -1.85, 0.22), (1.65, 0.045, 0.055), armor, collection, 0.003)
        add_box("ChineCapS", (4.20, 1.85, 0.22), (1.65, 0.045, 0.055), armor, collection, 0.003)
    if False:  # C14 drop kit-on-loaf extras (was CYCLE >= 10)
        add_box("PatchTile2", (3.10, 0.72, 0.68), (0.26, 0.12, 0.010), armor, collection, 0.002)
        add_box("HouseStayP", (5.85, -0.22, 1.55), (0.22, 0.02, 0.02), mech, collection, 0.002)
        add_box("HouseStayS", (5.85, 0.22, 1.55), (0.22, 0.02, 0.02), mech, collection, 0.002)
