...
def build_lod(lod, mats):
    ...
    hull_obj = loft_from_rings("Pressure_Hull", [
        station_ring(half, 0, 0.08, 0.72, 0.52, flat=0.55, box=0.50, keel=0.45),
        station_ring(half * 0.78, 0, 0.10, 1.22, 0.80, flat=0.65, box=0.60, keel=0.38),
        station_ring(half * 0.48, 0, 0.08, 1.82, 1.12, flat=0.70, box=0.78, keel=0.30),
        station_ring(half * 0.10, 0, 0.06, 2.22, 1.32, flat=0.50, box=0.92, keel=0.25),
        station_ring(-half * 0.22, 0, 0.08, 2.08, 1.22, flat=0.45, box=0.92, keel=0.22),
        station_ring(-half * 0.55, 0, 0.10, 1.68, 1.00, flat=0.40, box=0.95, keel=0.20),
    ...
    loft_shell("Hold_FlankP", [
        (2.80, -1.20, -1.95, -0.28, 0.42),
        (-0.20, -1.50, -2.18, -0.42, 0.38),
        (-3.20, -1.20, -1.85, -0.22, 0.32),
    ...
    add_folded_sheet(
        "Chine_P",
        (2.20, -1.90, 0.10), (-1.80, -1.94, 0.12),
        (-1.80, -1.82, 0.26), (2.20, -1.78, 0.24),
        0.024, armor, collection, 0.004,
    )
    add_folded_sheet(
        "Chine_S",
        (2.20, 1.90, 0.10), (2.20, 1.78, 0.24),
    ...
    if CYCLE >= 6 and lod <= 1:
        boolean_cut(hull_obj, "TransomRecess", (-6.95, 0.0, 0.08), (0.18, 0.62, 0.26))
    if CYCLE >= 7:
        add_box("CanopyMullion", (2.25, 0.0, 1.22), (0.025, 0.36, 0.10), armor, collection, 0.002)
        if lod <= 1:
            for sign, side in ((-1, "Port"), (1, "Starboard")):
                for i in range(4):
                    add_box(
                        f"FlankRad_{side}_{i}",
                        (-2.15 + i * 0.20, 2.12 * sign, 0.18),
                        (0.012, 0.16, 0.16),
