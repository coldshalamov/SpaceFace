#!/usr/bin/env python3
"""Helios Lark iter13 — hard-surface bar pass (G2/G3/G4 after iter12 G7 reject).

Keeps continuous loft + cylindrical nacelles + glass dome.
Clears residual G7 P1s from iter12:
- G2: thicker canards, hierarchical panel bays (not formula torus rings), larger mech mass,
  distinct edge-radius families, COLLISION mesh data renamed (no Cube.008)
- G3: mesh AO baked onto shared role ORM/baseColor after authoring (wired, not evidence-only)
- G4: darker paint / higher mech metal response under Three RoomEnvironment (wasp-class contrast)
- G5/G6: LOD ladder + lit EEVEE + RC Three captures (pipeline same as iter12)
"""
...
def log(msg: str) -> None:
    print(f"[lark-iter13] {msg}", flush=True)
...
def make_fuselage_loft(coll: bpy.types.Collection, mat: bpy.types.Material) -> bpy.types.Object:
    ...
    # Hierarchical service bays / access panels (designed, not formula rings)
    for size, loc in (
        ((1.9, 0.28, 1.05), (-1.0, 0.9, 0.0)),       # dorsal service
        ((1.5, 0.24, 0.9), (2.5, 0.92, 0.0)),        # canopy-adjacent bay
        ((1.35, 0.7, 0.22), (-2.2, 0.15, 0.98)),     # port side access
        ((1.35, 0.7, 0.22), (-2.2, 0.15, -0.98)),    # stbd side access
        ((1.1, 0.22, 0.8), (4.3, 0.8, 0.0)),         # forward bay
        ((0.7, 0.55, 0.55), (-4.6, 0.0, 0.0)),       # aft ring access
        ((2.0, 0.2, 0.55), (0.5, -0.95, 0.0)),       # ventral cargo interface
    ):
    ...
    # Few intentional manufacturing joints (not 9 equispaced tori)
    for gx, minor in ((-5.2, 0.04), (-3.0, 0.032), (1.5, 0.03), (5.0, 0.028)):
        ry = 0.95
    ...
    # Cast hull edge family (wider than sheet metal)
    bevel_angle(obj, width=0.055, segments=3, angle=24.0)
    mark_sharp(obj, 26.0)
    unwrap_hard(obj)
...
def make_wing(coll, mat, z_sign: float) -> bpy.types.Object:
    ...
    y = 0.0
    ht, htip = 0.16, 0.08  # thicker plates — not paper thin
    pts = [
        (root_le, y + ht, z0),
        (root_te, y + ht, z0),
        (root_te, y - ht, z0),
...
def build_materials() -> dict[str, bpy.types.Material]:
    ...
    # Colors for Three RoomEnvironment: mid ivory paint (not white), graphite alloy
    rebuild("Material_Hull", (142, 136, 126, 255), 0.70, 0.02, "hull")
    rebuild("Material_Mechanical", (22, 24, 28, 255), 0.18, 0.95, "mech")
    rebuild("Material_Cyan", (20, 120, 150, 255), 0.28, 0.04, "cyan")
    rebuild("Material_Warm", (100, 52, 24, 255), 0.45, 0.2, "warm")
    rebuild("Material_Glass", (12, 30, 40, 255), 0.04, 0.0, "glass")
    return mats


def _mat_images(mat: bpy.types.Material) -> dict[str, bpy.types.Image]:
    ...
    for node in mat.node_tree.nodes:
        if node.type != "TEX_IMAGE" or not node.image:
            continue
        n = node.image.name or ""
        if "_baseColor" in n or "baseColor" in n:
            out["base"] = node.image
        elif "_orm" in n or "orm" in n.lower():
            out["orm"] = node.image
        elif "_normal" in n or "normal" in n.lower():
            out["normal"] = node.image
    return out


def apply_mesh_ao_to_materials(parts: list[bpy.types.Object], bake_dir: Path) -> None:
    """Bake self-AO and WIRE it into shared role baseColor + ORM (not evidence-only)."""
    bake_dir.mkdir(parents=True, exist_ok=True)
    # One representative mesh per role material so shared atlas gets mesh-derived contact
    role_pick = {
        "Material_Hull": None,
        "Material_Mechanical": None,
    }
    for p in parts:
        if p.type != "MESH" or not p.data.materials:
            continue
        mname = p.data.materials[0].name if p.data.materials[0] else ""
        if mname in role_pick and role_pick[mname] is None:
            if mname == "Material_Hull" and "fuselage" in p.name.lower():
                role_pick[mname] = p
            elif mname == "Material_Mechanical" and "engine_housing" in p.name.lower():
                role_pick[mname] = p
    # fallbacks
    for p in parts:
        if p.type != "MESH" or not p.data.materials:
    ...
    for mat_name, obj in role_pick.items():
        if obj is None:
            continue
        img = bake_self_ao(obj, f"AO_{obj.name}", size=TEX)
        if img is None:
            continue
        ...
        try:
            ao_px = list(img.pixels)
            acc = sum(ao_px[i] for i in range(0, len(ao_px), 4))
            n = max(1, len(ao_px) // 4)
            mean_ao = acc / n
            log(f"  {obj.name} meanAO={mean_ao:.3f} → wiring into {mat_name}")
        except Exception:
            ao_px = None
            mean_ao = 0.5
        mat = obj.data.materials[0]
        imgs = _mat_images(mat)
        if not ao_px or "orm" not in imgs:
            continue
        # Resize-agnostic: sample AO by UV-normalized coords if sizes differ
        ao_w = img.size[0]
        ao_h = img.size[1]
        orm = imgs["orm"]
        base = imgs.get("base")
        ow, oh = orm.size[0], orm.size[1]
        orm_px = list(orm.pixels)
        base_px = list(base.pixels) if base else None
        for y in range(oh):
            for x in range(ow):
                # nearest sample from AO bake (shared UV space imperfect but mesh-derived)
                ax = min(ao_w - 1, int(x * ao_w / ow))
                ay = min(ao_h - 1, int(y * ao_h / oh))
                ai = (ay * ao_w + ax) * 4
                ao = ao_px[ai]
                # stretch contrast so recesses read under Three
                ao2 = max(0.15, min(1.0, (ao - 0.25) * 1.35 + 0.25))
                oi = (y * ow + x) * 4
                # ORM R = AO; keep G rough B metal
                orm_px[oi] = ao2
                if base_px is not None:
                    # darken albedo in contact / recess zones
                    k = 0.55 + 0.45 * ao2
                    base_px[oi] *= k
                    base_px[oi + 1] *= k
                    base_px[oi + 2] *= k
        orm.pixels = orm_px
        orm.pack()
        if base and base_px is not None:
            base.pixels = base_px
            base.pack()
        log(f"  wired mesh AO into {mat_name} ORM+base")
...
def build_ship(mats: dict) -> list[bpy.types.Object]:
    ...
    # Large dorsal mechanical service rail (wasp-class graphite mass, not thin strip)
    spine = F.make_box("Hull_Spine", (11.0, 0.28, 0.48), (-0.3, 0.95, 0.0), mech, coll, detail=1)
    bevel_angle(spine, 0.01, 2, 36.0)
    mark_sharp(spine, 34.0)
    unwrap_hard(spine)
    parts.append(spine)
    # Forward mechanical canopy coaming (extends frame contrast)
    coam = F.make_box("Canopy_Coaming", (2.6, 0.2, 1.15), (2.7, 0.82, 0.0), mech, coll, detail=1)
    bevel_angle(coam, 0.01, 2, 36.0)
    unwrap_hard(coam)
    parts.append(coam)
    # Side mechanical radiator plates
    for z_sign in (-1.0, 1.0):
        rad = F.make_box(
            f"Radiator_{'P' if z_sign < 0 else 'S'}",
            (3.2, 0.55, 0.12),
            (-0.5, 0.15, z_sign * 1.05),
            mech,
            coll,
            detail=1,
        )
        bevel_angle(rad, 0.008, 2, 40.0)
        unwrap_hard(rad)
        parts.append(rad)

    for z_sign in (-1.0, 1.0):
        parts.append(make_wing(coll, hull, z_sign))
        # Root fairing block (attachment mass) — mechanical joint, not paint-only
        root = F.make_box(
...
def main() -> int:
    ...
    if collision is not None:
        # Avoid Cube.00N mesh data names in glTF extract logs
        try:
    ...
    try:
        ...
        strong = {
            "Material_Hull": {"baseColorFactor": [0.58, 0.55, 0.50, 1.0], "roughnessFactor": 0.74, "metallicFactor": 0.02},
            "Material_Mechanical": {"baseColorFactor": [0.10, 0.11, 0.13, 1.0], "roughnessFactor": 0.18, "metallicFactor": 0.96},
            "Material_Cyan": {"baseColorFactor": [0.10, 0.50, 0.62, 1.0], "roughnessFactor": 0.28, "metallicFactor": 0.04},
            "Material_Warm": {"baseColorFactor": [0.40, 0.20, 0.09, 1.0], "roughnessFactor": 0.45, "metallicFactor": 0.20},
            "Material_Glass": {"baseColorFactor": [0.06, 0.14, 0.20, 0.30], "roughnessFactor": 0.04, "metallicFactor": 0.0},
        }
