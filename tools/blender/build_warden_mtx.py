...
def build_lod(lod, mats):
    ...
    hull_obj = loft_from_rings("Pressure_Hull", [
        barge_ring(half, 0, 0.12, 0.38, 0.32, 0.28),
        barge_ring(half * 0.68, 0, 0.14, 1.05, 0.52, 0.50),
        barge_ring(half * 0.38, 0, 0.12, 2.15, 0.82, 0.78),
        barge_ring(0.05, 0, 0.10, 2.48, 0.95, 0.90),
        barge_ring(-half * 0.30, 0, 0.12, 2.22, 0.88, 0.82),
        barge_ring(-half * 0.60, 0, 0.12, 1.72, 0.72, 0.66),
        barge_ring(-half + 0.65, 0, 0.10, 1.32, 0.52, 0.48),
    ], hull, collection, 0.014)
    if lod <= 1:
        cut_open_bay(hull_obj, "Bridge", (3.85, 0.0, 1.05), 1.55, 0.72, 0.40, (0, 0, 1), mats, collection, kit="cockpit")
    ...
    for y, side in drive_ys:
        add_hollow_bell(side, -8.95, y, 0.12, 1.05, mats, collection)
