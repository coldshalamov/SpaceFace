"""PQ-050.07 Warden MTX builder. Hitch untouched. --mtx-cycle N writes cycle stills."""
...
FAMILY = ROOT / "assets" / "ships" / "fleet_player_bodies_v1" / "warden"
...
def role_maps(role, rgb, size=TEX, prefix=None):
    """Unique Warden maps. Not a tint of the shared fleet sheet."""
    ...
    base = write_pixels(f"warden_{prefix}_basecolor", albedo, size, "sRGB")
    orm_img = write_pixels(f"warden_{prefix}_orm", orm, size, "Non-Color")
    nrm_img = write_pixels(f"warden_{prefix}_normal", nrm, size, "Non-Color")
...
def create_materials():
    specs = {
        "Material_Hull": ((0.16, 0.17, 0.19), 0.08, 0.46, "hull", 0.22, None),
        "Material_Armor": ((0.08, 0.09, 0.10), 0.48, 0.36, "armor", 0.10, None),
...
def sockets():
    half, hw, hh = 10.0, 2.5, 1.2
    return {
        "SOCKET_Weapon_Front": (half - 1.3, 0.0, 0.26),
        "SOCKET_Mining_Front": (half - 0.6, 0.0, -0.16),
        "SOCKET_Engine_Main": (-half + 1.3, 0.0, 0.10),
        "SOCKET_Trail_Main": (-half + 0.9, 0.0, 0.10),
        "SOCKET_Trail_Port": (-half + 1.0, -hw * 0.70, 0.10),
        "SOCKET_Trail_Starboard": (-half + 1.0, hw * 0.70, 0.10),
        "SOCKET_Utility_Dorsal": (0.4, 0.0, hh + 0.70),
        "SOCKET_Cargo_Ventral": (-0.3, 0.0, -hh - 0.12),
        "SOCKET_Camera_Focus": (1.0, 0.0, 0.30),
        "SOCKET_RCS_Port": (-1.3, -hw - 0.18, 0.16),
        "SOCKET_RCS_Starboard": (-1.3, hw + 0.18, 0.16),
    }
...
def build_lod(lod, mats):
    collection = bpy.data.collections.new(f"WARDEN_LOD{lod}")
    ...
    half = 10.0
    root = add_empty(f"WARDEN_LOD{lod}_ROOT", (0, 0, 0), collection)
    root["spacefaceAsset"] = {
        "assetId": "SF_WARDEN_PRODUCTION_V1", "partId": "warden_production_v1",
        "lod": f"lod{lod}", "slot": "hull", "category": "wholeships",
        "forward": "+X", "embeddedPlume": False,
    }
    # Gunship: aggressive bow, wide casemate mid, tower, three-drive transom.
    hull_obj = loft_from_rings("Pressure_Hull", [
        station_ring(half, 0, 0.12, 0.36, 0.30, flat=0.15, box=0.20, keel=0.85),
        station_ring(half * 0.68, 0, 0.16, 1.02, 0.58, flat=0.40, box=0.40, keel=0.70),
        station_ring(half * 0.38, 0, 0.14, 2.12, 0.88, flat=0.55, box=0.75, keel=0.50),
        station_ring(0.05, 0, 0.12, 2.50, 1.05, flat=0.45, box=0.90, keel=0.38),
        station_ring(-half * 0.30, 0, 0.16, 2.22, 1.12, flat=0.40, box=0.95, keel=0.32),
        station_ring(-half * 0.60, 0, 0.14, 1.75, 0.88, flat=0.35, box=0.95, keel=0.25),
        station_ring(-half + 0.65, 0, 0.12, 1.35, 0.68, flat=0.32, box=0.95, keel=0.20),
    ], hull, collection, 0.014)
    if lod <= 1:
        cut_open_bay(hull_obj, "Bridge", (3.85, 0.0, 1.18), 1.55, 0.62, 0.42, (0, 0, 1), mats, collection, kit="cockpit", liner=False)
        cut_open_bay(hull_obj, "TowerWell", (-1.85, 0.0, 1.08), 1.15, 0.68, 0.36, (0, 0, 1), mats, collection, kit="empty", liner=False)
        cut_open_bay(hull_obj, "Port", (0.15, -2.22, 0.16), 2.15, 0.46, 0.50, (0, -1, 0), mats, collection, kit="empty", liner=False)
        cut_open_bay(hull_obj, "Starboard", (0.15, 2.22, 0.16), 2.15, 0.46, 0.50, (0, 1, 0), mats, collection, kit="empty", liner=False)
    ...
    loft_shell("Casemate_P", [
        (3.40, -1.40, -2.18, -0.06, 0.42),
        (0.20, -1.70, -2.38, -0.10, 0.48),
        (-2.80, -1.40, -2.10, -0.06, 0.38),
    ...
    cover_loft_with_plates("WardenSkin", [
        (9.80, 0.55, 0.40, 0.10),
        (7.20, 1.15, 0.68, 0.14),
        (4.00, 1.72, 0.88, 0.14),
        (0.80, 1.95, 0.82, 0.12),
    ...
    add_box("Tower_Trunk", (-1.90, 0.0, 1.68), (0.52, 0.38, 0.44), hull, collection, 0.008)
    add_box("Tower_Shoulder", (-1.80, 0.0, 2.08), (0.70, 0.50, 0.08), armor, collection, 0.006)
    add_box("Tower_Bridge", (-1.65, 0.0, 2.34), (0.82, 0.58, 0.18), armor, collection, 0.006)
    ...
    drive_ys = ((-1.80, "Port"), (1.80, "Starboard"), (0.00, "Center"))
    for y, side in drive_ys:
        add_manufactured_drive(side, -8.85, y, lod, mats, collection, scale=0.88, z=0.12)
        add_cylinder(f"DriveCasing_{side}", (-7.85, y, 0.12), 0.34, 0.80, mech, collection, vertices=16, bevel=0.008)
        add_cylinder(f"DriveFairing_{side}", (-7.35, y, 0.12), 0.40, 0.42, armor, collection, vertices=16, bevel=0.008)
        add_box(f"DriveSaddle_{side}", (-7.75, y, -0.20), (0.38, 0.14, 0.08), mech, collection, 0.005)
    add_box("Transom_Plate", (-9.15, 0.0, 0.18), (0.07, 2.05, 0.58), armor, collection, 0.006)
    add_box("Transom_Header", (-7.85, 0.0, 0.62), (0.85, 1.95, 0.05), armor, collection, 0.005)
    add_box("Transom_Hatch", (-8.35, 0.0, 0.50), (0.26, 0.32, 0.03), armor, collection, 0.003)
    add_box("Transom_RibP", (-8.55, -0.95, 0.28), (0.38, 0.04, 0.32), mech, collection, 0.003)
    add_box("Transom_RibS", (-8.55, 0.95, 0.28), (0.38, 0.04, 0.32), mech, collection, 0.003)
    add_box("Transom_HeatP", (-8.75, -1.80, 0.32), (0.22, 0.18, 0.04), ceramic, collection, 0.003)
    add_box("Transom_HeatS", (-8.75, 1.80, 0.32), (0.22, 0.18, 0.04), ceramic, collection, 0.003)

    for sign, side in ((-1, "Port"), (1, "Starboard")):
        add_box(f"Casemate_{side}", (0.15, 2.05 * sign, 0.18), (1.55, 0.22, 0.28), armor, collection, 0.010)
        add_box(f"CaseLip_{side}", (0.15, 2.24 * sign, 0.18), (1.60, 0.04, 0.32), mech, collection, 0.004)
        add_cylinder(f"Broad_Fore_{side}", (0.70, 2.32 * sign, 0.18), 0.07, 0.62, mech, collection, vertices=8, bevel=0.003)
        add_cylinder(f"Broad_Aft_{side}", (-0.40, 2.32 * sign, 0.18), 0.07, 0.62, mech, collection, vertices=8, bevel=0.003)
        add_cylinder(f"GunHouse_{side}", (5.55, 0.55 * sign, 0.06), 0.11, 1.05, mech, collection, vertices=10, bevel=0.005)
    ...
    add_box("Keel_Spine", (0.10, 0.0, -0.88), (5.4, 0.26, 0.05), mech, collection, 0.01)
    add_box("Repair_Patch", (2.15, -0.62, 0.82), (0.28, 0.14, 0.012), warning, collection, 0.002)
    add_box("Repair_Plate", (3.35, 0.72, 0.88), (0.36, 0.20, 0.016), armor, collection, 0.003)
    ...
    add_box("Hatch_Lid", (-0.85, 0.38, 0.78), (0.30, 0.20, 0.016), armor, collection, 0.003)
    add_box("Hatch_Hinge", (-1.10, 0.38, 0.79), (0.03, 0.14, 0.016), mech, collection, 0.002)

    if lod == 0:
        add_curve_hose("Hose_Port", [(0.2, -1.95, 0.12), (-2.8, -1.90, 0.14), (-5.8, -1.82, 0.14), (-7.8, -1.80, 0.14)], mech, collection, 0.015)
        add_curve_hose("Hose_Stbd", [(0.2, 1.95, 0.12), (-2.8, 1.90, 0.14), (-5.8, 1.82, 0.14), (-7.8, 1.80, 0.14)], mech, collection, 0.015)
        add_box("HoseFit_P0", (0.15, -1.95, 0.12), (0.03, 0.03, 0.03), mech, collection, 0.002)
    ...
    bm = bmesh.new()
    for point in [
        (8.8, 0, 0.3), (0, -3.6, 0.7), (0, 3.6, 0.7),
        (-8.4, -2.5, 0.2), (-8.4, 2.5, 0.2),
        (2.0, -1.8, -1.1), (2.0, 1.8, -1.1),
        (-1.8, 0, 2.4),
    ]:
        bm.verts.new(point)
...
def export_lod(collection, lod):
    out = FAMILY / "source" / "wholeships" / f"warden_production_v1_lod{lod}.glb"
...
def render_cycle(collection):
    ...
    camera = setup_studio()
    out = FAMILY / "evidence" / "warden" / "cycles" / f"cycle_{CYCLE:02d}"
    out.mkdir(parents=True, exist_ok=True)
    views = {
        "three_quarter": ((13.4, -15.2, 6.6), (0, 0, 0.16), 36),
        "starboard": ((0.0, 20.0, 3.8), (0, 0, 0.14), 34),
        "rear": ((-16.2, -6.0, 4.1), (-0.4, 0, 0.12), 36),
        "clay_three_quarter": ((14.2, -16.2, 7.0), (0, 0, 0.16), 34),
        "grazing_close": ((9.6, -7.8, 2.2), (0.4, 0, 0.16), 48),
        "bay_interior": ((0.2, -3.8, 0.8), (0.15, -1.8, 0.16), 46),
        "drive_rear": ((-12.8, -3.2, 1.5), (-8.4, 0, 0.12), 48),
        "play_size": ((52, -46, 24), (0, 0, 0.12), 50),
        "orm_isolation": ((14.2, -16.2, 7.0), (0, 0, 0.16), 34),
        "normal_isolation": ((14.2, -16.2, 7.0), (0, 0, 0.16), 34),
        "id_or_material_id": ((14.2, -16.2, 7.0), (0, 0, 0.16), 34),
        "material_three_quarter": ((14.2, -16.2, 7.0), (0, 0, 0.16), 34),
    }
...
def main():
    ...
    for lod in (0, 1, 2):
        ...
        if report["hullTriangles"] < 800:
            raise RuntimeError(f"warden lod{lod} hull {report['hullTriangles']} < 800")
        collections.append(collection)
        reports.append(report)
    stills = render_cycle(collections[0])
    report = {
        "schema": "spaceface.wardenMtx.cycle.v1",
        "shipId": "warden",
        "cycle": CYCLE,
        "lods": reports,
        "stills": str(stills.relative_to(FAMILY)).replace("\\", "/"),
    }
    (FAMILY / "evidence" / "warden").mkdir(parents=True, exist_ok=True)
    (FAMILY / "evidence" / "warden" / f"cycle_{CYCLE:02d}.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
