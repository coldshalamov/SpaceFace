...
import { bindStuntReceiptToasts } from '../src/ui/toasts.js';
...
test('VERB-05 distinct episodes each earn one receipt; nameless payloads post none', () => {
  const bus = fakeBus();
  bindStuntReceiptToasts(bus);
  ...
  bus.emit('stunt:trickDetected', { trickId: 'c', episodeId: 'ep-c', tick: 3 });
...
test('VERB-05 a stunt receipt survives combat quiet; ordinary lines still drop', () => {
  assert.equal(
    admitReceipt({ text: 'Bulldozer', kind: 'success', channel: 'stunt', combat: true }).admit,
    true,
  );
  assert.equal(
  ...
  // Chatter channels remain refused — the stunt exemption is not a chatter backdoor.
  assert.equal(
