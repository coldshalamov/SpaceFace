...
test('production init + update order lengths match the live browser baseline', () => {
  assert.equal(PRODUCTION_INIT_ORDER.length, 140);
  ...
  const fuelStackIndex = PRODUCTION_UPDATE_ORDER.indexOf('fuelStack');
  ...
  assert.ok(worldIndex < fuelStackIndex);
  assert.ok(fuelStackIndex < heistFacilitiesIndex);
...
test('browser production system set is unchanged vs production manifest constants', () => {
  ...
  assert.equal(registry.systems.length, 140);
