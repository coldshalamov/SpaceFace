...
test('seed 4242: one Helios starter miner cuts one rock with the live beam, then stops', () => {
  ...
  // This focused host has no flight/physics. Begin the work phase at the face; production
  // flight covers the physical arrival gate, while this fixture owns the bounded live beam.
  miner.pos.x = rock.pos.x + (rock.radius || 6) + 25;
  miner.pos.z = rock.pos.z;
  miner.vel = { x: 0, z: 0 };
  const job = sim.state.npcJobs.byId[miner.data.jobId].job;
  job.phase = 'work';
  job.routeIndex = 1;
  job.progress = 0;
