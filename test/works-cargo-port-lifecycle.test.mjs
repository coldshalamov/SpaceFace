// PQ-131.09 — authored Cargo Port transaction and lifecycle.
//
// These tests exercise the exact pure controller used by asteroidRenderer3d. They prove that the
// procedural port is an error path rather than a hidden second construction, and that async
...
function harness(load) {
  ...
  let closed = false;
  const lifecycle = createCargoPortMountLifecycle({
    load,
...
test('successful authored coverage never constructs or retains the procedural fallback', async () => {
  const h = harness(async () => source('cargo_port'));
  const result = await h.lifecycle.rebuild();

  assert.equal(result.status, 'authored');
  assert.deepEqual(h.mounted, ['cargo_port']);
  ...
  assert.equal(h.releases.get('cargo_port'), 1);
...
test('authored and fallback are never standing together across a recovery', async () => {
  let mode = 'ok';
  const h = harness(async () => (mode === 'ok' ? source('cargo_port') : null));
...
test('dynamic pod/crate/lamp materials are isolated per mounted port', () => {
  ...
  assert.notEqual(a.material, b.material, 'each port owns its dynamic state material');

  a.material.color.setHex(0xffb648);
  a.material.emissive.setHex(0xff6242);
  assert.equal(b.material.color.getHex(), 0x6d5a45, 'one port state cannot repaint another');
...
test('the standing authored Cargo group swaps LOD in place across work and site registers', async () => {
  ...
  const blueprint = {
    ...
    primitives: [
      { name: 'LOD0_cargo_port', geometry: lod0, material, matrix: identity.clone(), tags: { lod: 'lod0' } },
      { name: 'LOD1_cargo_port', geometry: lod1, material, matrix: identity.clone(), tags: { lod: 'lod1' } },
    ],
    markers: [],
  };
  let leaseReleases = 0;
  const loader = createWorksPartLoader({
    ...
    registry: {
      cargo_port: Object.freeze({
        lod0: '/cargo.glb', lod1: null, slot: 'place',
        hooks: ['crate_0', 'crate_1', 'crate_2', 'crate_3', 'crate_4', 'cradle', 'pod_root', 'pod_thruster'],
      }),
    },
    lease: {
      isActive: () => true,
  ...
  const group = await loader.loadWorksPart('cargo_port');
  const visibility = () => Object.fromEntries(
    group.children.filter((child) => child.isMesh).map((mesh) => [mesh.name, mesh.visible]),
  );
  assert.deepEqual(visibility(), { LOD0_cargo_port: true, LOD1_cargo_port: false });
  ...
  assert.deepEqual(visibility(), { LOD0_cargo_port: false, LOD1_cargo_port: true });
