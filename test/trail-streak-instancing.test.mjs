...
// Energy-card culling contract: a bolt outside the padded live frame is not drawn.
// Player-owned rounds follow the same rule. The body keeps simulating; only the
// picture is limited to the frame. An on-frame round, whoever fired it, is drawn.
...
const offTableX = tableVfxDrawWuFromState({}) + 220;
const ownBolt = {
...
const onFrameBolt = {
  id: 32, type: 'projectile', alive: true,
  pos: { x: 24, z: 6 }, vel: { x: 280, z: 0 }, rot: 0, radius: 1.2, ownerId: 9,
  data: { weaponId: 'wpn_railgun_m', kind: 'bullet', damageType: 'kinetic' },
};
const cullState = {
  playerId: 1,
  entities: new Map([[1, cullPlayer], [30, ownBolt], [31, strayBolt], [32, onFrameBolt]]),
  entityList: [cullPlayer, ownBolt, strayBolt, onFrameBolt],
...
assert(cullBolts && cullBolts.byEntity.has(32),
  'a round on the frame must be drawn');
assert(!cullBolts.byEntity.has(30),
  'a player-owned round past the frame is not drawn');
assert(!cullBolts.byEntity.has(31),
