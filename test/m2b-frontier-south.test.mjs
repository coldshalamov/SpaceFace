// Focused contract for M2b SOUTH/EAST frontier cluster (additive, self-contained).
...
import {
  ...
  SOUTH_ANCHORS,
  SOUTH_ZONES,
  SOUTH_GATE_DESCRIPTORS,
  FROZEN_ORIGINAL_SECTOR_IDS,
  LATTICE_WU,
} from '../src/data/frontierRegions/south.js';

const __dirname = dirname(fileURLToPath(import.meta.url));
const SOUTH_SRC = join(__dirname, '..', 'src', 'data', 'frontierRegions', 'south.js');
...
const ASSIGNED = Object.freeze({
  sector_eunomia_gulf: { x: 10, y: 14 },
  sector_sedna_dark: { x: 5, y: 15 },
  sector_dione_lane: { x: 8, y: -1 },
});
...
function sectorById(id) {
  return SOUTH_SECTORS.find((s) => s.id === id);
}

test('cluster exports the assigned SOUTH/EAST ids only', () => {
  assert.equal(SOUTH_CLUSTER.id, 'frontier_cluster_south');
  assert.equal(LATTICE_WU, 4096);
  assert.equal(SOUTH_CLUSTER.latticeWu, 4096);
  assert.deepEqual([...SOUTH_SECTOR_IDS], Object.keys(ASSIGNED));
  assert.equal(SOUTH_SECTORS.length, 3);
  for (const id of SOUTH_SECTOR_IDS) {
    assert.ok(sectorById(id), `missing sector card ${id}`);
    assert.ok(SOUTH_ORIGINS[id], `missing origin ${id}`);
    assert.ok(SOUTH_ANCHORS[id], `missing anchors ${id}`);
    assert.ok(SOUTH_ZONES[id], `missing zones ${id}`);
  }
});
...
test('frozen original 10 are preserved and non-overlapping', () => {
  ...
  for (const id of SOUTH_SECTOR_IDS) {
    assert.equal(frozen.has(id), false, `SOUTH id collides with original 10: ${id}`);
  }
  assert.deepEqual(SOUTH_CLUSTER.frozenOriginalSectorIds, FROZEN_ORIGINAL_SECTOR_IDS);
});

test('4096-lattice origins match assigned origin cells', () => {
  for (const [id, cell] of Object.entries(ASSIGNED)) {
    assert.deepEqual(SOUTH_ORIGIN_CELLS[id], cell);
    assert.deepEqual(SOUTH_ORIGINS[id], {
      x: cell.x * LATTICE_WU,
      z: cell.y * LATTICE_WU,
    });
...
test('sector cards use existing vocabularies and stable ids', () => {
  ...
  for (const sector of SOUTH_SECTORS) {
  ...
  for (const id of SOUTH_SECTOR_IDS) {
    ...
    assert.ok(SOUTH_ZONES[id].length >= 1, `${id} needs >=1 named zone`);
  }
});
...
test('anchors cover stations, gates, fields, POIs with finite local XZ', () => {
  for (const id of SOUTH_SECTOR_IDS) {
    ...
    const anchors = SOUTH_ANCHORS[id];
...
test('internal gate descriptors are reciprocal; externals flagged for integration', () => {
  ...
  for (const id of SOUTH_SECTOR_IDS) {
    for (const g of SOUTH_ANCHORS[id].gates) {
      if (SOUTH_SECTOR_IDS.includes(g.to)) {
        internalPairs.add(`${id}->${g.to}`);
        assert.notEqual(g.external, true, `internal gate marked external: ${id}->${g.to}`);
        const back = SOUTH_ANCHORS[g.to].gates.find((x) => x.to === id);
  ...
  const descInternal = SOUTH_GATE_DESCRIPTORS.filter((d) => d.internal);
  ...
  for (const key of internalPairs) {
    ...
    assert.ok(
      SOUTH_GATE_DESCRIPTORS.some((d) => d.from === from && d.to === to && d.internal === true),
...
test('source is deterministic pure data (no Math.random, no imports)', () => {
  const src = readFileSync(SOUTH_SRC, 'utf8');
...
test('cluster record is self-contained and frozen at the export surface', () => {
  assert.equal(SOUTH_CLUSTER.sectors, SOUTH_SECTORS);
  assert.equal(SOUTH_CLUSTER.anchors, SOUTH_ANCHORS);
  assert.equal(SOUTH_CLUSTER.zones, SOUTH_ZONES);
  assert.equal(SOUTH_CLUSTER.origins, SOUTH_ORIGINS);
  assert.equal(SOUTH_CLUSTER.gateDescriptors, SOUTH_GATE_DESCRIPTORS);
  assert.ok(Object.isFrozen(SOUTH_CLUSTER));
  assert.ok(Object.isFrozen(SOUTH_SECTOR_IDS));
  assert.ok(Object.isFrozen(FROZEN_ORIGINAL_SECTOR_IDS));
  assert.ok(Object.isFrozen(SOUTH_ORIGINS));
  assert.ok(Object.isFrozen(SOUTH_ANCHORS));
  assert.ok(Object.isFrozen(SOUTH_ZONES));
});
