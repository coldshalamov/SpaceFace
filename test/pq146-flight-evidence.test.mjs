...
function evasion({hostile=true,retain=true,damage=false}={}) {
  const ship=(id,x,z)=>({id,type:'ship',alive:true,team:id===0?0:1,pos:{x,z},vel:{x:100,z:0},mass:18,radius:6,hull:100,hullMax:100,data:{defId:'ship_kestrel',encounter:{id:'escape-test'}}});
  ...
  for(let tick=0;tick<180;tick++) {
    state.tick=tick;
    if(tick===2){const before={...player.vel};player.vel={x:150,z:45};observeAppliedImpulse(player,before,player.vel,{actorId:0},tick,'impulse_charge',state);}
