// PQ-027.01 — authored Pallas debris current carries loose mass; the reef pinballs.
// The current moves mines and pods; Rapier contacts are the cascade. Never a hull-drain aura.
...
function insidePose() {
  ...
  const origin = PALLAS_REEF_FIELD.center;
  return {
    x: origin.x + dir.x * 110,
    z: origin.z + dir.z * 110,
  };
}
...
async function runReef({
  ...
  const pose = playerInVolume ? insidePose() : spectatorPose();
  ...
  try {
    ...
    host.bus.on('hazard:enter', (payload) => {
      if (payload && payload.zoneId === PALLAS_REEF_FIELD.id && payload.zoneType === 'debris_current') {
        hazardEnter += 1;
      }
    });
    ...
    const language = hazardLanguageFor('debris_current');
    const phaseAt = freezeTime ? simTime : host.state.simTime;
    return {
      phase: pallasReefPhase(freezeTime ? simTime : phaseAt).phase,
      fieldsEnabled,
      ...
      playerInside: pointInsidePallasReef(host.player.pos),
      proof: host.proof(),
    };
  } finally {
    host.dispose();
...
test('PQ-027.01 before: fields off, the reef string does not cascade', LONG, async () => {
  const result = await runReef({
    simTime: surgeSimTime(),
    ticks: SURGE_TICKS,
    fieldsEnabled: false,
  });
  console.log(formatRow(result));
  assert.equal(result.proof.backend, 'rapier-dynamic', 'before stays on Rapier');
  assert.equal(result.cascadedMines, 0, 'without the current the mines are furniture');
  assert.equal(result.cascadePairs, 0, 'before: no pinball');
  assert.equal(result.carried, 0, 'before: nothing rides');
});

test('PQ-027.01 a shoved reef mine pinballs the Pallas string', LONG, async () => {
  const language = hazardLanguageFor('debris_current');
  assert.ok(language, 'reef speaks existing debris_current language');
  assert.ok(language.counterplay.includes('time'), 'counterplay is the calm window');
...
test('PQ-027.01 warning telegraphs the current before it carries', LONG, async () => {
  const result = await runReef({
    simTime: warningSimTime(),
    ticks: TELEGRAPH_TICKS,
    playerInVolume: true,
  });
  console.log(formatRow(result));
  assert.equal(result.phase, 'warning', 'telegraph is the warning window');
  assert.ok(result.playerInside, 'player stands in the authored current');
  assert.ok(result.hazardEnter >= 1, 'warning must speak before the surge');
  assert.equal(result.language, '⇢', 'reef reuses debris_current, not a new name');
  assert.equal(result.cascadedMines, 0, 'warning writes no pinball');
  assert.equal(result.carried, 0, 'warning writes no carry');
});
