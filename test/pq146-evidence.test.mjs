...
async function run({shove=true,alreadyDoomed=false}={}){
  const player=body(0,'ship',-100,-100),victim=body(1,'ship',-100,alreadyDoomed?0:30,80),rock=body(2,'asteroid',0,0);
  ...
  if(shove)owner.applyImpulse({entityId:1,impulse:{x:16*20,y:0,z:alreadyDoomed?0:-16*30},tick:0,reason:'weapon_hit',provenance:{actorId:0,weaponId:'wpn_concussion_cannon_m'}});
  ...
  return {state,owner,kernel,grammar,consequence,tricks,contacts,close(){grammar.destroy();consequence.destroy();kernel.dispose();owner.dispose();}};
}
