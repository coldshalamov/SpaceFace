...
test('beaming a dependency-blocked component announces the refusal instead of swallowing it', () => {
  // A stabilized hull re-failed by impact retracts its recovery operation and leaves every dependent
  // operation inactive. That path used to return without emitting beam:denied, so the player held the
  // beam on an inert target with no cue and nothing distinguished it from a slow operation.
  const state = authoredState();
