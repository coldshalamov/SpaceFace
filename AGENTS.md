...
| Resume one non-Hitch flyable ship remaster | **`CANONICAL_BUILD_MAP.md`** campaign door → `PQ-050` / [`design/program/roadmap/active/PQ-050.md`](./design/program/roadmap/active/PQ-050.md). One leaf owns one ship; use contribution-driven LOD and the review-first set. Long campaigns follow the completion front instead of exhausting the fleet first. |
