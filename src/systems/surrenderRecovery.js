// Nonlethal surrender recovery.
//
// Surrender is not a decorative terminal state: the player can massline the yielded ship, reel it
// close, and physically tow it into lawful station protection for custody. This system owns only
// its compact recovery records and entity-facing custody annotations; credits and reputation remain
// with their canonical economy/faction owners through events.
...
export const surrenderRecovery = {
  ...
  init(ctx) {
    ...
    this._onSurrendered = (payload) => this._register(payload || {});
  ...
  update(_dt, state) {
    ...
    // Re-adopt a saved yielded entity even though this deliberately transient coordinator is not
    ...
    for (const entity of state.entityList || []) {
      ...
      if (entity && entity.alive !== false && ai.fsm === 'surrender' && !recordForEntity(own, entity.id)) {
        this._register({ entityId: entity.id, reason: 'saved_surrender', factionId: entity.factionId, type: entity.type });
      }
    }
  ...
  _register(payload) {
    ...
    if (!eligibleSurrender(entity, state)) return null;
    const own = ensureState(state);
    ...
    const record = {
      ...
      reason: payload.reason || 'surrendered',
      phase: 'awaiting_tether',
      startedAt: now,
      escapeAt: now + SURRENDER_ESCAPE_S,
      securedAt: null,
      resolvedAt: null,
      resolved: false,
    ...
    annotate(entity, record, 'Latch with massline. Reel inside 60. Tow to station custody.');
  ...
  _reel(payload) {
    ...
    if (!entity || !eligibleSurrender(entity, state)) return false;
  ...
  _release(payload) {
    ...
    record.escapeAt = (Number(this.state.simTime) || 0) + SURRENDER_ESCAPE_S;
    annotate(entity, record, 'Custody lock lost. Relatch before this ship escapes.');
  ...
  _custody(record, entity, jurisdiction, now) {
    ...
    const receipt = {
      ...
      t: now,
      text: 'CUSTODY TRANSFERRED - surrendered hull secured without a kill.',
    };
  ...
  _say(record, entity) {
    ...
    record.spoken = true;
    const text = 'MASSLINE: Reel the yielded ship inside 60, then tow to station custody.';
  ...
  destroy() {
    ...
    this._onSurrendered = this._onLatched = this._onReel = this._onReleased = null;
  },
};
...
function publicRecord(record, entity) {
  return {
    ...
    label: entity && entity.data && (entity.data.name || entity.data.shipName || entity.data.callsign) || 'Surrendered ship',
    phase: record.phase,
