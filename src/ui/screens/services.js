...
function isServiceOffered(type, stationServices) {
  ...
  if (!row) return false;
  if (!Array.isArray(stationServices)) return true; // permissive fallback matches legacy row behavior.
...
function recommendationCandidate(service, quote, stationServices, opts) {
  ...
  const blockedReason = offered
    ...
    : 'This station does not offer ' + (serviceRow(service) && serviceRow(service).label || service) + '.';
  return {
    service,
...
export function serviceReadinessRecommendation(state, entity, stationServices = null) {
  ...
  if (fuelMissing(state) > 0 && fuelFrac < FUEL_WARN_FRAC) {
    const fuelQuote = serviceQuote('refuel', state, entity);
    candidates.push(recommendationCandidate('refuel', fuelQuote, stationServices, {
  ...
  if (missing.total > 0.5 && protFrac < PROTECTION_WARN_FRAC) {
    const repairQuote = serviceQuote('repair', state, entity);
    candidates.push(recommendationCandidate('repair', repairQuote, stationServices, {
