...
function destName(m) { return m.destinationName || m.destName || (m.local ? 'Local sector' : (m.destSectorId || 'Destination')); }
...
export function createContractsScreen(ctx) {
  ...
  function renderDossier(state) {
    ...
    const cargo = m.cargo || (m.cargoCommodityId ? { commodityId: m.cargoCommodityId, qty: m.cargoQty } : null);
    ...
    const standingReq = m.standingRequirement || m.standingReq || (m.minRep ? ('Requires standing ' + m.minRep) : null);
    ...
    dossierEl.innerHTML =
      `<div class="sx-dossier">` +
        ...
        (standingReq ? `<p class="sx-dossier__gate">${icon('factions', 14)}<span>${escapeHtml(String(standingReq))}</span></p>` : '') +
        ...
        `<div class="sx-dossier__foot">` +
          `<button type="button" class="sx-btn-primary" data-accept="${escapeHtml(String(mid(m)))}">Accept Contract</button>` +
          `<button type="button" class="sx-btn-ghost" data-track="${escapeHtml(String(mid(m)))}">Accept &amp; Track</button>` +
        `</div>` +
      `</div>`;
  }
  ...
  el.addEventListener('click', (ev) => {
    ...
    if (acc) { if (ctx.bus) { ctx.bus.emit('ui:acceptMission', { missionId: acc.getAttribute('data-accept') }); ctx.bus.emit('audio:cue', { id: 'ui_accept' }); } setTimeout(() => renderAll(ctx.state || {}), 60); return; }
    const trk = ev.target.closest('[data-track]');
    if (trk) {
      const id = trk.getAttribute('data-track');
      if (ctx.bus) { ctx.bus.emit('ui:acceptMission', { missionId: id }); ctx.bus.emit('ui:trackMission', { missionId: id }); ctx.bus.emit('audio:cue', { id: 'ui_accept' }); }
