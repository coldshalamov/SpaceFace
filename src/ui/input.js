...
export function createUiInput(ctx, screenManager) {
  ...
  function stationTabButtons() {
    ...
    return Array.from(root.querySelectorAll('.st-rail [data-tab]')).filter((el) => {
  ...
  function jumpStationTab(index) {
    const tabs = stationTabButtons();
    if (!tabs.length || index < 0 || index >= tabs.length) return false;
    return clickStationTab(tabs[index]);
  }

  function handleStationTabKey(ev) {
    const key = ev.key;
    if (key === 'ArrowLeft' || key === '[' || key === 'PageUp') return cycleStationTab(-1);
    if (key === 'ArrowRight' || key === ']' || key === 'PageDown') return cycleStationTab(1);
    if (/^[1-8]$/.test(key)) return jumpStationTab(Number(key) - 1);
    return false;
  }
  ...
  function onKeyDown(ev) {
    ...
    if (modalOpen) {
      ...
      if (def && def.id === 'station' && handleStationTabKey(ev)) {
        ev.preventDefault();
        return;
      }
