...
.orr-crest.is-chosen::before { display:block; }
...
.orr-crestorbit__armlayer { position:absolute; left:0; top:0; width:100%; height:100%; overflow:visible; pointer-events:none; }
...
export function createCrestOrbit(host, { crestSize = 50, centreSize = 150 } = {}) {
  ...
  function paintHand(deg) {
    ...
    const r0 = armFromRim && geo.centreNow ? geo.centreNow / 2 + 6 : 0;
  ...
  function layout() {
    ...
    if (rebuilt) {
      ...
      const gapDeg = Math.asin(Math.min(0.9, (cs / 2 + 5) / R)) * 180 / Math.PI;
      const ringD = n > 0
...
export function standingScaleSvg({ rep = 0, tiers = [], aggro = -150, width = 520, rungs = [], brackets = [], compact = false, rungWords = true } = {}) {
  ...
  let fine = '';
  for (let i = 0; i <= n * 4; i += 1) { const x = xAt(i / 4); fine += `M ${f(x)} ${y} L ${f(x)} ${y + 3} `; }
