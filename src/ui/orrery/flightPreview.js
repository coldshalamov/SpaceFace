...
import { createOrdnanceArc } from './ordnanceArc.js';
import {
...
.orr-flightpreview__cluster { position:absolute; left:34px; bottom:22px; }
/* the arc is centred on the free floor between the Cluster and the radar, not on the screen */
.orr-flightpreview__ordnance { position:absolute; left:calc(50% + 150px); bottom:6px; transform:translateX(-50%); }
...
export const ORDNANCE_LAYOUT = Object.freeze([
  { name: 'Ordnance', slots: [{ key: 'Y', name: 'Charge', icon: 'munitions' }, { key: 'R', name: 'Blast', icon: 'fire' }, { key: 'SPC', name: 'Line', icon: 'line' }] },
  { name: 'Fieldwork', slots: [{ key: '4', name: 'Seed', icon: 'seed' }, { key: '5', name: 'Well', icon: 'well' }, { key: '6', name: 'Repel', icon: 'repel' }] },
  { name: 'Rig', slots: [{ key: '7', name: 'Cone', icon: 'cone' }, { key: '8', name: 'Skim', icon: 'skim' }] },
  { name: 'Bay', slots: [{ key: '9', name: 'Frag', icon: 'munitions' }] },
]);
...
export const orreryFlightScreen = {
  mount(root) {
    ...
    const cluster = createFlightCluster({ shipId: 'ship_kestrel', name: 'Hitch', classLine: 'Kestrel class · starter' });
    const ordnance = createOrdnanceArc({ groups: ORDNANCE_LAYOUT, width: 740 });
    ...
    wrap('orr-flightpreview__ordnance', ordnance.el);
    ...
    cluster.update({
      ...
      target: { name: 'Hauler 12', detail: 'civil' }, tether: { state: 'Payload', detail: '960 t' },
      threat: { level: 'contact', text: '2 raiders · closing' },
    });
    ordnance.update({
    ...
    lock.set({ x: 991, y: 407, name: 'Hauler 12', detail: 'civil · towing', distance: 1240, hull: 0.82 });
    ...
    cluster.arrive();
    ordnance.arrive();
    this._parts = { cluster, ordnance, radar, lock };
  },
  onShow() {},
