...
.orr-svg .orr-ltape__tick.is-chosen .orr-ltape__stem { stroke:var(--dp-hand-hot, #ffd98c); stroke-width:2.6; }
.orr-svg .orr-ltape__tick.is-chosen .orr-ltape__stem-bloom { stroke:var(--dp-hand, #f2b950); opacity:.42; }
...
.orr-svg .orr-ltape__tick:is(:hover, :focus-visible) .orr-ltape__stem { stroke:rgb(255 250 240); stroke-width:2.6; }
...
.orr-svg .orr-ltape__arc-bloom { stroke:rgb(${BONE}); opacity:.2; fill:none; }
...
export function createLedgerTape(host, { onPick = null, purseHost = null } = {}) {
  ...
  function layout() {
    ...
    const rule = svg('g', {});
    rule.append(
      svg('path', { d: `M ${x0} ${y} L ${f(nowX)} ${y}`, class: 'orr-band orr-ltape__rule' }),
      svg('path', { d: `M ${x0} ${y} L ${f(nowX)} ${y}`, class: 'orr-edge orr-ltape__rule' }),
    );
    let minor = '';
    const step = 8;
    for (let x = x0; x <= nowX; x += step) minor += `M ${f(x)} ${y + 2} L ${f(x)} ${y + (Math.round((x - x0) / step) % 5 === 0 ? 7 : 4)} `;
    rule.appendChild(svg('path', { d: minor, class: 'orr-core orr-ltape__minor', 'stroke-width': 1.5 }));
    // the origin cap: the tape begins here in both states
    rule.appendChild(svg('path', { d: `M ${f(x0)} ${y - 6} L ${f(x0)} ${y + 12}`, class: 'orr-core orr-ltape__major', 'stroke-width': 2 }));
    ...
    sorted.forEach((e, k) => {
      ...
      if (c && c !== lastCycle) {
        ...
        if (k > 0) layer.appendChild(rise(svg('path', { d: `M ${f(mx)} ${y - 6} L ${f(mx)} ${y + 12}`, class: 'orr-core orr-ltape__major', 'stroke-width': 2 }), 60));
    ...
    // the now cursor: the same bright mark at the tape's right end in both states, named with the current cycle
    {
      ...
      ng.append(
        svg('path', { d: `M ${f(nowX)} ${y - 12} L ${f(nowX)} ${y + 12}`, class: 'orr-bloom orr-ltape__cursor-bloom', 'stroke-width': 10 }),
        svg('path', { d: `M ${f(nowX)} ${y - 12} L ${f(nowX)} ${y + 12}`, class: 'orr-core orr-ltape__cursor', 'stroke-width': 2.4 }),
      );
    ...
    if (sorted.some((e) => Number.isFinite(e.amount) && e.amount !== 0)) {
      ...
      rg.appendChild(svg('path', { d: `M ${f(gx)} ${y} L ${f(gx)} ${f(y - gLen(ghostCeil / 2))}`, class: 'orr-core orr-ltape__ghost-stem', 'stroke-width': 2, 'stroke-dasharray': '3 3' }));
    ...
    sorted.forEach((e, k) => {
      ...
      g.appendChild(svg('path', { d: `M ${f(x)} ${y} L ${f(x)} ${f(tipY)}`, class: `orr-bloom orr-ltape__stem-bloom${draw}`, 'stroke-width': 7, ...drawIn }));
      g.appendChild(svg('path', { d: `M ${f(x)} ${y} L ${f(x)} ${f(tipY)}`, class: `orr-core orr-ltape__stem${amt == null ? ' orr-ltape__stem--fact' : ''}${draw}`, 'stroke-width': 2, ...drawIn }));
      g.appendChild(svg('circle', { cx: f(x), cy: f(tipY), r: 7, class: `orr-ltape__tip-bloom${threat ? ' orr-ltape__tip-bloom--threat' : ''}${arriveNow ? ' orr-ltape__settle' : ''}`, ...settle }));
      g.appendChild(svg('circle', { cx: f(x), cy: f(tipY), r: amt == null ? 2.6 : 3.2, class: `orr-ltape__tip${threat ? ' orr-ltape__tip--threat' : ''}${arriveNow ? ' orr-ltape__settle' : ''}`, ...settle }));
    ...
    if (chosen) {
      ...
      const yRun = Math.round(H) + 34;
      const xEnd = x0 + Math.round(Number.isFinite(data.leaderEndX) ? data.leaderEndX : 148);
      const xl = Math.round(chosen.x);
      const ex = xl - 24;
      ...
      hg.appendChild(svg('path', { d: `M ${f(chosen.x)} ${f(foot)} L ${f(chosen.x)} ${f(foot + 12)}`, class: 'orr-core orr-ltape__hand-drop', 'stroke-width': 2 }));
      // a sold entry's stem crosses the tape through a break in its core
      if (chosen.up) hg.appendChild(svg('path', { d: `M ${f(chosen.x)} ${y - 4} L ${f(chosen.x)} ${y + 5}`, class: 'orr-ltape__break', 'stroke-width': 6 }));
      hg.append(
        svg('circle', { cx: f(chosen.x), cy: f(chosen.tipY), r: 13, class: 'orr-ltape__hand-glow' }),
        svg('circle', { cx: f(chosen.x), cy: f(chosen.tipY), r: 7.5, class: 'orr-core orr-ltape__hand-ring', 'stroke-width': 2 }),
        svg('circle', { cx: f(chosen.x), cy: f(chosen.tipY), r: 3.2, class: 'orr-ltape__hand-bead' }),
        svg('path', { d: leaderD, class: 'orr-core orr-ltape__leader', 'stroke-width': 2, fill: 'none', 'shape-rendering': 'crispEdges' }),
        svg('path', { d: elbowD, class: 'orr-core orr-ltape__leader', 'stroke-width': 2, fill: 'none' }),
      );
    ...
    const drawPurse = (target, acx, acy, r) => {
      ...
      ag.appendChild(svg('path', { d: arcD(acx, acy, r, -150, 150), class: 'orr-band orr-ltape__arc-track' }));
      ag.appendChild(svg('path', { d: arcD(acx, acy, r, -150, 150), class: 'orr-edge orr-ltape__arc-track' }));
      const [zx0, zy0] = polar(acx, acy, r - 5, 0); const [zx1, zy1] = polar(acx, acy, r + 5, 0);
      ag.appendChild(svg('path', { d: `M ${f(zx0)} ${f(zy0)} L ${f(zx1)} ${f(zy1)}`, class: 'orr-core orr-ltape__arc-zero', 'stroke-width': 2 }));
      const mx = Math.max(1, sold, bought);
      const soldDeg = (sold / mx) * 135;
      const boughtDeg = (bought / mx) * 135;
      if (soldDeg > 0.5) {
        const d = arcD(acx, acy, r, 0, soldDeg);
        ag.appendChild(svg('path', { d, class: 'orr-bloom orr-ltape__arc-bloom', 'stroke-width': 11 }));
        ag.appendChild(svg('path', { d, class: 'orr-core orr-ltape__arc', 'stroke-width': 3.5 }));
        const [bx, by] = polar(acx, acy, r, soldDeg);
        ag.appendChild(svg('circle', { cx: f(bx), cy: f(by), r: 8.5, class: 'orr-ltape__arc-headbloom' }));
        ag.appendChild(svg('circle', { cx: f(bx), cy: f(by), r: 4, class: 'orr-ltape__arc-head' }));
      ...
      if (boughtDeg > 0.5) {
        ...
        ag.appendChild(svg('path', { d, class: 'orr-bloom orr-ltape__arc-bloom orr-ltape__arc-bloom--out', 'stroke-width': 11 }));
        ag.appendChild(svg('path', { d, class: 'orr-core orr-ltape__arc orr-ltape__arc--out', 'stroke-width': 3.5 }));
        const [bx, by] = polar(acx, acy, r, -boughtDeg);
        ag.appendChild(svg('circle', { cx: f(bx), cy: f(by), r: 8.5, class: 'orr-ltape__arc-headbloom orr-ltape__arc-headbloom--out' }));
        ag.appendChild(svg('circle', { cx: f(bx), cy: f(by), r: 4, class: 'orr-ltape__arc-head orr-ltape__arc-head--out' }));
