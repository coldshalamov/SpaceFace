...
import { ensurePerfRuntime } from './perfRuntime.js';
  ...
  function frame(now) {
    ...
    try {
      ...
      if (state.timeScale > 0) {
        ...
        if (steps >= MAX_STEPS) { state.accumulator = 0; shedBacklog = true; } // shed backlog, stay responsive
      }
      perf.recordLoop(steps, shedBacklog, state.accumulator);
