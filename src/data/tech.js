...
// PQ-155.00 — leftover nodes that raise a cap or open a charter and do not unlock a verb.
export const TECH_STAT_ONLY_IDS = Object.freeze([
  'tech_drone_swarm',
  'tech_autonomous_fleets',
  'tech_outpost_charter',
]);

export const TECH_STAT_ONLY_JUSTIFICATION = Object.freeze({
  tech_drone_swarm: 'Raises droneTierCap and extraDronePerBay. Same bay verb, more bodies.',
  tech_autonomous_fleets: 'Raises the drone cap and lets you hire NPC traders. Not a new player verb.',
  tech_outpost_charter: 'Authorizes outpost construction and the last drone cap. A charter, not a toy.',
});

export function techUnlocksVerb(node) {
  const unlocks = node && node.unlocks || {};
  if (Array.isArray(unlocks.modules) && unlocks.modules.length > 0) return true;
  if (Array.isArray(unlocks.ships) && unlocks.ships.length > 0) return true;
  if (Array.isArray(unlocks.flags) && unlocks.flags.length > 0) return true;
  return false;
}

export function techNodeKind(node) {
  if (!node) return 'unknown';
  if (TECH_STAT_ONLY_IDS.includes(node.id) || !techUnlocksVerb(node)) return 'stat';
  return 'verb';
}

function nodeById(id) {
  return TECH_NODES.find((entry) => entry.id === id);
}

function ladderRow(hour, verb, nodeId, gate, reachable) {
  const node = nodeById(nodeId);
  return Object.freeze({
    hour,
    verb,
    nodeId,
    cost: Object.freeze({ ...(node && node.cost || {}) }),
    gate,
    reachable,
  });
}

// Hour → verb → cost → gate. Hours are honest play time, not a retune.
// Hour 0.25 is the 15-minute first-upgrade row: not reachable on a fresh save
// (5,000 cr / 0 RP vs Combat Basics 6,000 cr / 10 RP). PQ-155.01 owns the curve sim.
export const VERB_LADDER = Object.freeze([
  ladderRow(0.25, null, 'tech_combat_basics',
    'start 5000 cr / 0 RP; Combat Basics wants 6000 cr / 10 RP — first upgrade not reachable in 15 min',
    false),
  ladderRow(1.5, 'fit small guns and fly the Wasp', 'tech_combat_basics',
    'first recon_scan RP plus a short haul', true),
  ladderRow(2, 'tractor, whip, and coupler heads', 'tech_tractor_systems',
    'logistics branch; after the first RP and a haul', true),
  ladderRow(3, 'afterburner, cloak, and jump', 'tech_drive_tuning',
    'drives branch open', true),
  ladderRow(4, 'haul with a spool', 'tech_bulk_logistics',
    'industry branch; Atlas + spool M', true),
  ladderRow(5, 'mine with a beam', 'tech_focused_extraction',
    'after Industrial Mining (Ironback)', true),
  ladderRow(6, 'rail, concussion, and cannon', 'tech_kinetic_drivers',
    'combat branch after Combat Basics', true),
  ladderRow(7, 'gravity mark, sink, and shunt', 'tech_graviton_drives',
    'after Drive Tuning', true),
  ladderRow(8, 'sweep, snare, and bridle', 'tech_fire_control',
    'after Strike Craft (Hornet); Massline ceiling lives here, not on Flagship Command', true),
  ladderRow(9, 'charge-vector rack', 'tech_impulse_ballistics',
    'after Drive Tuning', true),
  ladderRow(10, 'plasma and disruptors', 'tech_plasma_dynamics',
    'after Kinetic Drivers and Beam Focusing', true),
]);
