// Browser-safe pure oracle kernel shared by Node and Chromium.
// This module intentionally contains no authority executor or node:crypto import.

import { evaluateMetrics, compareThreshold } from './metricRegistry.js';
// Side-effect: register massline + flight metrics for both Node and Chromium hosts.
import '../metrics/masslineMetrics.js';

/**
 * Evaluate the pure oracle families. Fixed-parent equivalence authority stays in oracleEngine.js.
 * @param {object} options
 * @returns {{ ok: boolean, results: object[], failed: object[], firstBadTick: number|null, metrics: object[] }}
 */
export function evaluateOracleKernel({
  trace = [],
  metrics = [],
  assertions = [],
  ctx = {},
  ...
  const temporalResults = evaluateTemporal(assertions.filter((a) => isTemporal(a && a.kind)), trace, ctx);
  const quantitativeFromAssertions = evaluateQuantitativeAssertions(
    assertions.filter((a) => a && (a.kind === 'metric' || a.kind === 'quantitative')),
    metricResults,
    trace,
    ctx,
  );
  ...
  return { ok: failed.length === 0, results: all, failed, firstBadTick, metrics: metricResults };
}
