// scripts/capture-maps-flight.mjs — temporary headless browser script to capture in-flight map screenshots.
// Based on scripts/capture-maps.mjs with a longer flight-load timeout so authored assets can finish.
...
const OUT_DIR = '.devshots/map-rework';
const GALAXY_SHOT = join(OUT_DIR, 'galaxy-map-flight.jpg');
const SYSTEM_SHOT = join(OUT_DIR, 'system-map-flight.jpg');
const LOCAL_SHOT = join(OUT_DIR, 'local-map-flight.jpg');
...
try {
  ...
  const snapExpr = `JSON.stringify((() => {
    ...
    return { sfReady: !!state, mainMenuVisible: !!document.querySelector('[data-screen="mainMenu"]'),
      newGameVisible: !!document.querySelector('[data-screen="newGame"]'),
  ...
  await wait((s) => s.sfReady, 20000, 'boot');
  ...
  if (snap.mainMenuVisible) {
    ...
    // Wait for either flight to start or the New Game screen to appear.
    await wait((s) => s.flightPlayable || s.newGameVisible, 120000, 'new-game or flight');
    snap = await evalJson(snapExpr);
    if (!snap.flightPlayable) {
      console.log('Clicking Launch...');
      await click('Launch');
      await wait((s) => s.flightPlayable, 120000, 'flight after launch');
    }
  }

  console.log('Flight playable, letting world populate...');
  await sleep(3000);
  ...
  // Open map via authority so it receives correct state.
  console.log('Opening galaxyMap...');
  await evalJson(`(() => {
    const sf = window.SF;
  ...
  async function setMapScale(label, zoom) {
    const result = await evalJson(`JSON.stringify((() => {
      ...
      if (!screen || screen.id !== 'galaxyMap') return { ok: false, reason: 'galaxyMap is not active' };
      screen._zoom = ${zoom};
