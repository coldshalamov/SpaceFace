...
{
  // The live station mission board is src/ui/station/screens/contracts.js.
  const contractsSource = readFileSync(new URL('../src/ui/station/screens/contracts.js', import.meta.url), 'utf8');
  assert.equal(contractsSource.includes('Track Nav'), false, 'station mission board must not render dead Track Nav copy for offers');
  assert.equal(contractsSource.includes('data-act="track"'), false, 'station mission board must not render dead Track Nav action for offers');
  assert(contractsSource.includes('function missionDossierSummary('), 'station mission board must render authored contract briefs');
  assert(contractsSource.includes('sx-dossier__summary'), 'station mission board must style contract briefs');
  assert(contractsSource.includes('STATION_DEF'), 'station mission board must resolve station ids into player-facing names');
  assert.equal(contractsSource.includes("m.destName || m.destStationId || m.dest"), false,
