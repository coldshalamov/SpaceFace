    ...
    if (tab === 'shipworks') {
      for (let i = 0; i < 8; i++) {
        ...
        const dump = await page.evaluate(() => {
          ...
          const acquire = document.querySelector('.sx-sw__acquiring');
          const state = window.SF && window.SF.state;
          return {
            canvas: c ? { ...c.dataset, w: c.width, h: c.height, rect: (() => { const r = c.getBoundingClientRect(); return { w: r.width, h: r.height }; })() } : null,
            stageClasses: stage ? stage.className : null,
            acquireText: acquire ? (acquire.textContent || '').replace(/\s+/g, ' ').trim().slice(0, 300) : null,
            acquireHtmlLen: acquire ? acquire.innerHTML.length : 0,
            stageRequest: state && state.ui ? JSON.stringify(state.ui.stageRequest || null).slice(0, 300) : null,
            secondaryBlocked: !!(window.SF && window.SF.ctx && window.SF.ctx.flags && window.SF.ctx.flags.secondaryPreviewWebGlBlocked),
            gpuLost: !!(window.SF && window.SF.ctx && window.SF.ctx.flags && window.SF.ctx.flags.gpuContextLost),
          };
        });
        console.log('SHIPWORKS DUMP', i, JSON.stringify(dump, null, 2));
      }
    }
