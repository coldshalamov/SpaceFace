export const meta = {
  ...
  description: 'Legacy polish pass: balance tuning, fallback visuals, VFX, and deepening shallow systems. Current authored-asset work should use FULL_GRAPHICS_REVAMP_GOAL.md.',
...
READ FIRST: AGENTS.md, ARCHITECTURE.md (§0 constants, §3 schema, §4.4 events), your design spec under design/specs/, and the files you touch. This is an old fallback-code workflow; do not use it as authority for authored GLB production. Run \`node --check\` on every file you write.
RULES: only edit the file(s) listed for you — do NOT edit main.js/registry.js/loop.js/eventBus or other tracks' files. Keep all export names + the system interface stable. Sim logic uses state.rng()/seeded streams, never Math.random() (cosmetic VFX may). Health is 4-layer (hull/armorHp/shield/cap). Entities live on the XZ plane; a ship mesh's +X axis is its nose and renderer sets mesh.rotation.y = -entity.rot, so keep that convention. No silent runtime deps or missing addons; build-time tooling is allowed when documented, and runtime deps/addons require lead sign-off with license, bundle/perf, determinism/save, and maintenance notes.
...
const tasks = [
  ...
  { key: 'art-meshes', file: 'src/render/visualFactory.js + src/render/canvasTextures.js', spec: '10-art-vfx-direction-three-js-primitives-only.md', prompt:
    `Improve the procedural fallback visuals without pretending they replace the authored-asset pipeline. For professional ship/station assets, use FULL_GRAPHICS_REVAMP_GOAL.md and assets/AGENTS.md instead. Rewrite src/render/visualFactory.js only if this lane is intentionally about fallback/generated code meshes (keep \`export function createVisualFactory()\` returning { build(entity) -> THREE.Object3D }) and enrich src/render/canvasTextures.js (export texture builders: noise/greeble/gradient/hull-panel/star). Consume src/data/palettes.js (FACTION_PALETTES, SECTOR_PALETTES, SHIP_RECIPES) and ships.js for class/role.
  ...
  { key: 'vfx-bloom', file: 'src/render/vfx.js + src/render/bloom.js (new)', spec: '10-art-vfx-direction-three-js-primitives-only.md', prompt:
    ...
    (2) Improve the existing bloom/post path instead of creating a parallel renderer. If src/render/bloom.js does not exist, create \`export function createBloom(renderer, width, height)\` returning { render(scene, camera), setSize(w,h), dispose() } with bounded WebGLRenderTarget use, inline GLSL only where it is the best local option, and no silent missing-addon imports. Prefer maintained repo code and dependency-review notes over one-off reinvention. Keep it cheap (half-res/downsampled blur targets) and preserve default visual quality. node --check clean.`,
  },
