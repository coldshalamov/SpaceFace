// scripts/lib/bench/playthroughStrangerPilot.mjs — the stranger archetype, in its own module.
//
// Extracted from playthroughPilots.mjs so it can evolve without racing the instrument lane's
// active writes to that shared file; the playthrough harness (or a later integration sweep)
// re-exports createStrangerPilot from here. The policy knows the CONTROL GRAMMAR (thrust, aim,
// latch, reel, cut, fire, boost, scan, well, dock — the verbs the input contract exposes) and
// reads only the public guidance a player sees: state.nav.waypoint and state.onboarding
// substates. It never scripts the route, so ledgers measure the onboarding rail itself.
//
// KNOWN LIMITATION (2026-09-18, honest by construction): the headless swing completion is
// defeated by emergent physics — a crushing whip recoil can displace the hull thousands of wu,
// and the out-of-residency body then stops answering scripted thrust (body sleep). The beat
// FSM, the raid and claimed beats, and the wanted path are covered by focused tests and the
// checks; the full-hours stranger metrics need this pilot's recovery finished.
      ...
      if (r) {
        if (r.current === 'swing') {
          ...
          if (rock && derelict) {
            ...
            if (!lineAttached && dist(p.pos, rock.pos) > 1200) {
              // The released rock is running: hold station — the rail restages it nearby.
              input.brake = true;
              return;
            }
            if (!lineAttached || latchedId !== rock.id) {
