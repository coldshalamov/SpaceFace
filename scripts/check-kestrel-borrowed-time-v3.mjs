...
assert.deepEqual(lod, { lod0: 18726, lod1: 7792, lod2: 1166 });
assert.ok(lod.lod0 > lod.lod1 && lod.lod1 > lod.lod2 && lod.lod2 > 0, 'monotonic LODs');
assert.ok(lod.lod0 >= 18000, 'near tier is a substantive authored hero, not a proxy');
...
assert.equal(taste.reviewCount, 2);
assert.equal(taste.repairCount, 1);
assert.equal(taste.livePromotion, false);
const production = json(resolve(FAMILY, 'evidence/production_metrics.json'));
assert.equal(production.gateOk, true);
assert.deepEqual(production.gateErrors, []);
assert.deepEqual(
  ['lod0', 'lod1', 'lod2'].map((key) => production.lodBreakdown[key].drawEstimate),
  [14, 11, 7],
  'draw-call structure'
);
...
console.log(`check-kestrel-borrowed-time-v3: PASS (${lod.lod0}/${lod.lod1}/${lod.lod2}, ${(statSync(CANDIDATE).size / 1048576).toFixed(2)} MiB)`);
