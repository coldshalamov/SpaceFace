...
try {
  ...
  const report = await page.evaluate(async ({ warmupMs, durationMs }) => {
    ...
    const info = () => {
      ...
      return {
        ...
        pooledSlots: report && report.scenePools && report.scenePools.submittedInstanceSlots || 0,
        pools: report && report.scenePools && report.scenePools.pools || 0,
        opaqueBatches: report && report.scenePools && report.scenePools.opaqueBatches || 0,
        opaqueBatchInstances: report && report.scenePools && report.scenePools.opaqueBatchInstances || 0,
        shadowCastingInstanceChunks: report && report.scenePools && report.scenePools.shadowCastingInstanceChunks || 0,
  ...
  const out = {
    ...
    pooledSlots: report.last.pooledSlots || 0,
    pools: report.last.pools || 0,
