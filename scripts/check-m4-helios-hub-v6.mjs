#!/usr/bin/env node
/** Candidate-only structural gate for the isolated Helios Hub V6 family. */
...
const ASSETS = [
  { id: 'helios_hub_station', lod0Max: 35000, materials: 4 },
  { id: 'helios_gate', lod0Max: 30000, materials: 4 },
  { id: 'helios_rock_a', lod0Max: 3500, materials: 2 },
  { id: 'helios_rock_b', lod0Max: 3500, materials: 2 },
  { id: 'helios_rock_c', lod0Max: 3500, materials: 2 },
  { id: 'helios_support_gantry', lod0Max: 9000, materials: 3 },
  { id: 'helios_support_dock_arm', lod0Max: 9000, materials: 3 },
  { id: 'helios_nav_spire', lod0Max: 6000, materials: 3 },
];
...
const donor = resolve(FAMILY, 'source/reference/blenderkit_scifi_station/blenderkit_scifi_station_cc0.blend');
const donorProv = resolve(FAMILY, 'source/reference/blenderkit_scifi_station/PROVENANCE.json');
const adaptation = resolve(FAMILY, 'evidence/SOURCE_ADAPTATION.json');
...
if (need(adaptation, 'source adaptation record')) {
  ...
  receipt.packet === PACKET ? ok('adaptation packet exact') : fail(`adaptation packet mismatch: ${receipt.packet}`);
...
console.error(`\ncheck-m4-helios-hub-v6: ${errors ? 'FAIL' : 'PASS'} (${errors} errors)`);
