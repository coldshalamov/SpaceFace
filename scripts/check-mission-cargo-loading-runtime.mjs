...
//
// Complements check-mission-accept-handoff-runtime.mjs. That probe proves Accept + Track surfaces
// the Market/Mission Log/HUD handoff; this one buys the tracked commodity through the real Market
// button and verifies the contract callout advances from "load cargo" to "cargo aboard" without
// losing mission tracking or nav. Probe-only offer/credits are injected after live boot.
...
try {
  ...
  const seeded = await page.evaluate(() => {
    ...
    const cmdtyId = Object.keys(market).find((id) => market[id] && market[id].role !== 'none' &&
      Number.isFinite(market[id].lastBuy) && market[id].lastBuy > 0 && market[id].stock >= 2);
    ...
    board.slots.unshift({
      id: 'cargo_loading_probe_bulk_trade',
      type: 'bulk_trade',
      stationId,
      factionId: 'faction_scn',
      reward_cr: Math.max(300, unit * 4),
      time_limit_s: 900,
      collateral_cr: 0,
      riskTier: 0,
      ...
      title: 'Probe contract cargo loading',
    ...
    function firstOtherStation(s, currentStationId) {
      ...
      for (const sector of Object.values(s.world && s.world.sectors || {})) {
        for (const station of sector.stations || []) {
          if (station && station.id && station.id !== currentStationId) {
            return { stationId: station.id, sectorId: sector.id, name: station.name || station.id };
          }
        }
      }
      return null;
  ...
  assert.equal(accepted, true, 'seeded contract should expose an enabled Accept + Track button');
  ...
  let marketReport = await readTrackedMarketReport(page);
  assert.equal(marketReport.calloutVisible, true, 'Market should show a tracked-contract callout before cargo loading');
  ...
  assert.match(marketReport.calloutText + ' ' + marketReport.rowMissionLine, /load\s+1u more|load\s+1u|before undocking/i,
    'Market should tell the player to load the missing contract cargo: ' + JSON.stringify(marketReport));
  ...
  await page.waitForFunction(({ cmdtyId, beforeOwned }) => {
    const cargo = window.SF.state.player.cargo;
    return !!(cargo && cargo.items && (cargo.items[cmdtyId] || 0) >= beforeOwned + 1);
  }, { cmdtyId: buyReport.cmdtyId, beforeOwned: buyReport.beforeOwned }, { timeout: 10000 });

  marketReport = await readTrackedMarketReport(page);
  ...
  assert.equal(hudReport.waypointMissionId, hudReport.missionId, 'flight nav should still target the loaded contract mission');
  assert(hudReport.trackerText.includes('Probe contract cargo loading'), 'flight HUD should still name the loaded contract mission: ' + JSON.stringify(hudReport));
  assert(hudReport.cargoQty >= 1, 'contract cargo should remain aboard after undock: ' + JSON.stringify(hudReport));
...
async function readTrackedMarketReport(page) {
...
async function openStationTab(page, tabId) {
  ...
  await waitForVisible(page, '[data-screen="station"] #st-panel-' + tabId, 10000, 'station ' + tabId + ' panel');
}
