  ...
  const dbg = await page.evaluate(() => {
    const pick = (el) => {
      ...
      return {
        ...
        hidden: !!el.hidden, rect: { x: r.x, y: r.y, w: r.width, h: r.height },
        gridArea: cs.gridArea, position: cs.position, overflow: cs.overflow,
      };
    };
    const screen = document.querySelector('[data-screen="station"]');
    const panel = document.querySelector('[data-screen="station"] #sx-panel');
    const app = document.querySelector('[data-screen="station"] .sx-app');
    const rootEl = panel ? panel.closest('.k-screen') : null;
    return {
      url: location.href,
      viewport: { w: innerWidth, h: innerHeight },
      screen: pick(screen), rootEl: pick(rootEl), app: pick(app), panel: pick(panel),
      appChildren: app ? [...app.children].map((c) => pick(c)) : null,
      screenParent: screen ? pick(screen.parentElement) : null,
      cs: app ? { display: getComputedStyle(app).display, gridTemplateRows: getComputedStyle(app).gridTemplateRows, height: getComputedStyle(app).height } : null,
      sheets: ['sx-fh-tokens', 'sx-fh-css', 'sx-station-css', 'sx-station-orbital-css'].map((id) => {
        const l = document.getElementById(id);
        let rules = -1;
        try { rules = l && l.sheet ? l.sheet.cssRules.length : -1; } catch (e) { rules = 'err'; }
        return { id, present: !!l, ready: !!(l && l.sheet), rules };
      }),
      vitals: [...document.querySelectorAll('[data-screen="station"] .sxb-vital')].map((v) => {
        const head = v.querySelector('.sxb-vital__head');
        const track = v.querySelector('.sxb-vital__track');
        const value = v.querySelector('.sxb-vital__value');
        return {
          kind: v.className,
          vitalW: v.getBoundingClientRect().width,
          headW: head ? head.getBoundingClientRect().width : 0,
          headMin: head ? getComputedStyle(head).minWidth : '',
          trackW: track ? track.getBoundingClientRect().width : 0,
          valueW: value ? value.getBoundingClientRect().width : 0,
        };
      }),
    };
  });
