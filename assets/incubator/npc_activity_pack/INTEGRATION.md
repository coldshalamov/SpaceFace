...
> **2026-08-10 status update — four families promoted through PQ-045.npc-identity.**
> `ore_barge`, `repair_tender`, `salvage_cutter` and `survey_pin` were **re-authored**
> (not copied) under the review boundary below and are now live whole-ships:
> family `assets/ships/npc_work_fleet/` (builder
> `tools/blender/build_npc_work_fleet.py`, deterministic, Blender 5.1.2-pinned),
> canonical sources `assets/ships/parts/wholeships/{ore_barge,repair_tender,salvage_cutter,survey_pin}.glb`,
> release + render-package rows generated by the normal pipeline, and runtime bindings
> `ore_carrier` (NEW role), `tender`, `salvor`, `surveyor` in
> `src/render/partsLibrary.js` / `src/systems/traffic.js`. Whole-asset G1/G2/G4 and
> independent G7 review remain OPEN for the promoted family (see
> `design/program/roadmap/receipts/PQ-045-npc-identity-REPORT.md`). The remaining
> eleven donor families stay source-only design candidates under the verdict below.
