"""Author, export, and render the isolated SpaceFace Mule production candidate.

Manufactured T1 hauler. Uses in-repo Wasp/Hitch PBR sheets as first-party
material treatment only — no Hitch or Wasp geometry is copied.

Run:
  blender --background --python build_mule_v1.py
"""
...
REQUIRED_SOCKETS = {
    "SOCKET_Weapon_Front": (-8.35, 0.0, 0.85),
    "SOCKET_Mining_Front": (6.4, 0.0, -0.20),
    "SOCKET_Engine_Main": (-7.6, 0.0, 0.10),
    "SOCKET_Trail_Main": (-8.05, 0.0, 0.10),
    "SOCKET_Trail_Port": (-7.85, -1.45, 0.12),
    "SOCKET_Trail_Starboard": (-7.85, 1.45, 0.12),
    "SOCKET_Utility_Dorsal": (4.8, 0.0, 3.05),
    "SOCKET_Cargo_Ventral": (-0.6, 0.0, -1.15),
    "SOCKET_Camera_Focus": (1.6, 0.0, 0.55),
    "SOCKET_RCS_Port": (-1.4, -2.85, 0.20),
    "SOCKET_RCS_Starboard": (-1.4, 2.85, 0.20),
}
TEXTURE_SETS = {
    "Material_Hull": "hull",
...
def create_materials(texture_dir: Path) -> dict[str, bpy.types.Material]:
    mats = {
        "Material_Hull": make_pbr_material(
            "Material_Hull", "hull", texture_dir, (0.22, 0.24, 0.18, 1.0), 0.16, 0.52),
        "Material_Armor": make_pbr_material(
            "Material_Armor", "armor_dark", texture_dir, (0.16, 0.17, 0.15, 1.0), 0.30, 0.46),
...
def add_container(name, loc, mats, collection, lod):
    armor = mats["Material_Armor"]
    mech = mats["Material_Mechanical"]
    warning = mats["Material_Warning"]
    add_box(f"{name}_Box", loc, (1.15, 0.72, 0.62), armor, collection, 0.03)
    add_box(f"{name}_Rim", (loc[0], loc[1], loc[2] + 0.58), (1.12, 0.70, 0.04), mech, collection, 0.01)
    add_box(f"{name}_Door", (loc[0] + 1.12, loc[1], loc[2]), (0.04, 0.58, 0.48), mech, collection, 0.01)
    if lod == 0:
        add_box(f"{name}_Lock", (loc[0] + 1.16, loc[1], loc[2] + 0.08), (0.05, 0.10, 0.10), mech, collection, 0.005)
        add_box(f"{name}_Chevron", (loc[0], loc[1] + 0.72, loc[2]), (0.18, 0.025, 0.16), warning, collection, 0.005)
        add_box(f"{name}_Casting_Fore", (loc[0] + 1.10, loc[1] + 0.66, loc[2] + 0.56), (0.08, 0.08, 0.08), mech, collection, 0.006)
        add_box(f"{name}_Casting_Aft", (loc[0] - 1.10, loc[1] + 0.66, loc[2] + 0.56), (0.08, 0.08, 0.08), mech, collection, 0.006)
        for rib in range(5):
            add_box(
                f"{name}_Rib_{rib}",
                (loc[0] - 0.80 + rib * 0.40, loc[1] + 0.70, loc[2]),
                (0.03, 0.03, 0.55),
                mech,
                collection,
                0.004,
...
def build_ship(lod: int, mats: dict[str, bpy.types.Material]):
    collection = bpy.data.collections.new(f"MULE_PRODUCTION_V1_LOD{lod}")
    ...
    root = add_empty(f"MULE_PRODUCTION_V1_LOD{lod}_ROOT", (0, 0, 0), collection)
    root["spacefaceAsset"] = {
        "assetId": ASSET_ID, "partId": "mule_production_v1", "packet": PACKET,
    ...
    hull_obj = add_chamfer_loft("Pressure_Hull", [
        ...
        (-0.40, 0, 0.08, 1.95, 1.12, 0.22),
        (-3.20, 0, 0.08, 1.90, 1.08, 0.20),
        (-5.80, 0, 0.10, 1.55, 0.92, 0.16),
        (-7.60, 0, 0.12, 1.05, 0.70, 0.12),
    ], hull, collection, 0.05)
    for index, x in enumerate((4.6, 2.2, -0.4, -3.0, -5.4)):
        add_box(f"Hull_Course_{index}", (x, 0.0, 0.10), (0.04, 1.70, 0.95), hull, collection, 0.01)
    for sign, side in ((-1, "Port"), (1, "Starboard")):
        add_box(f"Walk_Rail_{side}", (0.0, 1.95 * sign, 1.18), (5.4, 0.03, 0.03), mech, collection, 0.008)
        add_box(f"Walk_Deck_{side}", (0.0, 1.82 * sign, 0.98), (5.4, 0.10, 0.03), hull, collection, 0.01)
        if lod == 0:
            for post, x in enumerate((-4.2, -2.0, 0.2, 2.4, 4.4)):
                add_box(f"Walk_Stanchion_{side}_{post}", (x, 1.95 * sign, 1.08), (0.03, 0.03, 0.12), mech, collection, 0.004)
    add_box("Ventral_Keel", (0.0, 0.0, -1.15), (6.4, 0.55, 0.08), mech, collection, 0.03)

    add_box("Bridge_Pedestal", (5.15, 0.0, 1.35), (0.85, 0.70, 0.55), armor, collection, 0.03)
    add_chamfer_loft("Bridge_Glass", [
        (5.70, 0, 2.05, 0.42, 0.18, 0.04),
        (5.15, 0, 2.35, 0.62, 0.32, 0.06),
        (4.55, 0, 2.20, 0.48, 0.22, 0.04),
    ], canopy, collection, 0.015)
    add_box("Bridge_Brow", (5.35, 0.0, 2.58), (0.55, 0.48, 0.06), armor, collection, 0.012)
    add_box("Bridge_Frame", (5.15, 0.0, 2.05), (0.06, 0.66, 0.38), armor, collection, 0.01)

    add_drive("Port", -1.45, lod, mats, collection)
    add_drive("Starboard", 1.45, lod, mats, collection)
    add_box("Aft_Bulkhead", (-7.15, 0.0, 0.15), (0.18, 1.85, 0.95), armor, collection, 0.02)
    add_box("Rear_Gun_Blister", (-8.15, 0.0, 0.85), (0.35, 0.22, 0.18), mech, collection, 0.02)
    add_cylinder("Rear_Gun_Barrel", (-8.55, 0.0, 0.85), 0.06, 0.55, armor, collection, vertices=10, bevel=0.008)

    for sign, side in ((-1, "Port"), (1, "Starboard")):
        add_container(f"Cargo_{side}_Low_Fore", (1.6, 2.55 * sign, 0.55), mats, collection, lod)
        add_container(f"Cargo_{side}_Low_Aft", (-1.2, 2.55 * sign, 0.55), mats, collection, lod)
        if lod <= 1:
            add_container(f"Cargo_{side}_High_Fore", (1.6, 2.55 * sign, 1.85), mats, collection, lod)
            add_container(f"Cargo_{side}_High_Aft", (-1.2, 2.55 * sign, 1.85), mats, collection, lod)
        add_box(f"Lock_Frame_{side}", (0.2, 2.05 * sign, 1.15), (2.8, 0.08, 0.08), mech, collection, 0.015)

    add_box("Handling_Chevron", (3.4, -1.85, 0.55), (0.35, 0.03, 0.18), warning, collection, 0.008)
    ...
    if lod == 0:
        for index, x in enumerate((3.8, 1.2, -1.4, -4.0)):
            add_box(f"Hull_Seam_{index}", (x, 0, 1.05), (0.03, 1.45, 0.03), mech, collection, 0.006)
        for sign, side in ((-1, "Port"), (1, "Starboard")):
            add_cylinder(f"RCS_{side}", (-1.4, 2.85 * sign, 0.20), 0.10, 0.16, mech, collection, vertices=10, bevel=0.008, rot=(math.pi / 2, 0, 0))
    ...
    bm = bmesh.new()
    for point in [
        (6.4, 0, 0.2), (4.0, -2.4, 1.2), (4.0, 2.4, 1.2),
        (1.0, -3.2, 1.8), (1.0, 3.2, 1.8), (-4.5, -3.2, 1.8),
        (-4.5, 3.2, 1.8), (-7.8, -1.6, 0.3), (-7.8, 1.6, 0.3),
        (3.5, -1.4, -1.2), (3.5, 1.4, -1.2), (-5.0, -1.2, -1.2), (-5.0, 1.2, -1.2),
    ]:
        bm.verts.new(point)
...
def export_lod(collection: bpy.types.Collection, lod: int) -> Path:
    out = FAMILY / "source" / "wholeships" / f"mule_production_v1_lod{lod}.glb"
...
def render_evidence(collection):
    ...
    views = [
        ("mule_three_quarter", (20, -22, 11), (0, 0, 0.4), 50),
        ("mule_starboard", (16, 22, 10), (0, 0.4, 0.3), 50),
        ("mule_front", (24, 0, 5), (0, 0, 0.3), 52),
        ("mule_rear", (-22, -8, 6), (-1, 0, 0.2), 52),
        ("mule_top", (0, 0, 32), (0, 0, 0), 58),
        ("mule_grazing", (6, -18, 1.4), (0, 0, 0.3), 48),
        ("mule_drive_close", (-13, -4, 1.6), (-6.4, 1.45, 0.1), 70),
        ("mule_bridge_close", (14, -6, 4.2), (5.2, 0, 2.2), 70),
    ]
    receipts = []
    scene = bpy.context.scene
...
def save_blend():
    out = FAMILY / "blender" / "mule_production_v1.blend"
    out.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> int:
    ...
    report = {
        "schema": "spaceface.muleProductionV1.build.v1",
        "packet": PACKET,
        "assetId": ASSET_ID,
