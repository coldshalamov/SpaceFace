...
- Before: missing loops (fixed)
- Before: UV bad (fixed)
- Before: no procedural (fixed)
- Before: no cavity (fixed)
- Before: no life (fixed)
- Before: compositor (fixed)
- Before: PBR (fixed)
- Before: form (fixed)
- Before: 3 passes (fixed)
- Before: fidelity (fixed)
- Before: role (fixed: vented industrial)
- Before: no id in list (fixed by adding fin_radiator_grid)

- Additional: real MCP techniques applied for fin_radiator_grid (bevel/WN/inset from professional-techniques.md).
