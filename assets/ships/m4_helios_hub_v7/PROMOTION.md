...
foreach ($x in $ids) {
  Copy-Item "assets/ships/m4_helios_hub_v7/source/places/$($x.c).glb" "assets/ships/parts/places/$($x.l).glb" -Force
}
# 3) Rebuild release:
...
Do **not** promote without controller decision. Do not edit manifests/partsLibrary for new IDs — these replace existing place archetypes by live ID.
