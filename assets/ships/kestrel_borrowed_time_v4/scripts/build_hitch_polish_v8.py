"""Build Hitch V8 extra-polish candidate from the V7 production blend."""
...
from hitch_polish_v8 import PASS_ID, apply_hitch_polish_v8  # noqa: E402


FAMILY = Path(__file__).resolve().parents[1]
BASELINE = FAMILY / "blender" / "kestrel_hitch_polish_v7_production.blend"
PACKET = "SF-K0-HITCH-POLISH-V8-001"
OUT_DIR = FAMILY / "source_candidates" / "hitch_polish_v8" / "wholeships"
EVIDENCE = FAMILY / "evidence" / "hitch_polish_v8"
...
def render_stills():
    ...
    cam_data = bpy.data.cameras.new("V8Cam")
    camera = bpy.data.objects.new("V8Cam", cam_data)
    scene.collection.objects.link(camera)
    scene.camera = camera
    for name, loc, energy, color, size in (
        ("V8Key", (18, -20, 12), 8200, (0.88, 0.92, 1), 10),
        ("V8Fill", (4, 16, 8), 3200, (0.55, 0.62, 0.72), 8),
        ("V8Rim", (-16, -6, 8), 3800, (0.72, 0.80, 0.92), 7),
    ):
    ...
    for name, (loc, target, lens) in shots.items():
        if name.startswith("clay") and clay is None:
            clay = bpy.data.materials.new("V8_Clay")
            clay.use_nodes = True
...
def main() -> int:
    ...
    render_stills()
    result = {
        "schema": "spaceface.hitchPolishV8.build.v1",
        "status": "complete",
        "packet": PACKET,
