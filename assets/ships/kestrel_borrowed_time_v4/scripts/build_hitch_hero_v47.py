"""Build Hitch hero V47 (cycle 38): lids and straps, not hat frames."""
...
from hitch_hero_v47 import PASS_ID, apply_hitch_hero_v47  # noqa: E402


FAMILY = Path(__file__).resolve().parents[1]
BASELINE = FAMILY / "blender" / "kestrel_hitch_polish_v7_production.blend"
PACKET = "SF-K0-HITCH-HERO-V47-038"
OUT_DIR = FAMILY / "source_candidates" / "hitch_hero_v47" / "wholeships"
EVIDENCE = FAMILY / "evidence" / "hitch" / "cycles" / "cycle_38"
...
def render_stills(bounds):
    ...
    cam_data = bpy.data.cameras.new("V47Cam")
    camera = bpy.data.objects.new("V47Cam", cam_data)
    scene.collection.objects.link(camera)
    scene.camera = camera
    center = Vector(bounds["center"])
    span = max(bounds["dimensions"])
    for name, loc, energy, color, size in (
        ("V47Key", tuple(center + Vector((span * 0.9, -span * 1.1, span * 0.7))), 8200, (0.88, 0.92, 1), 10),
        ("V47Fill", tuple(center + Vector((span * 0.2, span * 0.9, span * 0.45))), 3200, (0.55, 0.62, 0.72), 8),
        ("V47Rim", tuple(center + Vector((-span * 0.9, -span * 0.3, span * 0.5))), 3800, (0.72, 0.80, 0.92), 7),
        ("V47Down", tuple(center + Vector((0.0, 0.0, span * 1.6))), 4200, (0.80, 0.84, 0.90), 14),
    ):
    ...
    for name, (loc, target, lens, angle) in shots.items():
        if name.startswith("clay") and clay is None:
            clay = bpy.data.materials.new("V47_Clay")
            clay.use_nodes = True
...
def main() -> int:
    ...
    result = {
        "schema": "spaceface.hitchHeroV47.build.v1",
        "status": "complete",
        "packet": PACKET,
