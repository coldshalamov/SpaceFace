"""Hitch hero V91 / cycle 82: stepped shingles.

Cycle 81 lapped the tiles flush and they blended back into one skin.
Hide those plates. Rebuild with every other tile 18 mm thicker so the
overlap is a real step the 3Q can count. Keep well holes. No hull
...
from hitch_hero_v90 import apply_hitch_hero_v90, _lapped
...
PASS_ID = "kestrel-hitch-hero-v91"
COLLECTION_NAME = "KESTREL_V91_STEPPED_SHINGLE"
PREFIX = "V91_"
HIDE_PREFIXES = (
    "V90_Plate_",
    "V90_Bolt_",
    ...
    for row_name, t0, t1, thickness, plates in ROWS:
        ...
        for index, (plate_name, x0, x1) in enumerate(lapped):
            ...
            objects.append(_rolled_band(
                ...
                f"{side.lower()} {row_name.lower()} {plate_name.lower()} stepped cone shingle",
            ))
...
def apply_hitch_hero_v91() -> dict:
    prior = apply_hitch_hero_v90()
    ...
    report = {
        "schema": "spaceface.hitchHero.v91",
        "passId": PASS_ID,
        "method": "every other shingled tile is 18 mm thicker so the lap is a step",
        "priorPass": "v90",
        "hiddenDonors": hidden,
