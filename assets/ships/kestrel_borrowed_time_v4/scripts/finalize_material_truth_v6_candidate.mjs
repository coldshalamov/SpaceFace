#!/usr/bin/env node
/**
 * Headlessly finalize the isolated Kestrel V6 material-truth candidates.
 *
...
const PACKET = 'SF-K0-BORROWED-TIME-V6-MATERIAL-TRUTH-001';
...
const SOURCE = [0, 1, 2].map((lod) => resolve(
  FAMILY,
  `source_candidates/material_truth_v6/wholeships/kestrel_borrowed_time_v4_lod${lod}.glb`,
));
...
const RELEASE = [0, 1, 2].map((lod) => resolve(
  FAMILY,
  `release_candidates/material_truth_v6/wholeships/kestrel_borrowed_time_v4_lod${lod}.glb`,
));
...
function stamp(document, lod, build) {
  ...
  asset.generator = `${asset.generator || ''}; SpaceFace finalize_material_truth_v6_candidate.mjs`;
  asset.extras = {
    ...(asset.extras || {}),
    spacefaceAsset: {
      ...sourceMetadata,
...
function validateRelease(path, sourceMetrics, build, lod) {
  ...
  const ktx2MaterialRoles = validateKtx2MaterialRolePayloads(
    parsed.doc,
    parsed.bin,
    `Kestrel V6 staged release LOD${lod}`,
  );
...
async function main() {
  if (!existsSync(BUILD_REPORT)) throw new Error('V6 build report missing');
  ...
  if (build.status !== 'complete' || build.candidateOnly !== true || build.livePromotion !== false) {
    throw new Error(`V6 build is not a complete isolated candidate: ${build.status}`);
  }
  if (sha256(resolve(FAMILY, build.productionBlend)) !== build.productionBlendSha256) {
    throw new Error('V6 production blend hash does not match build receipt');
  }
  ...
  writeFileSync(FINALIZE_REPORT, `${JSON.stringify({
    schema: 'spaceface.kestrelMaterialTruthV6.finalize.v1',
    status: 'building',
    generationFingerprint: build.generationFingerprint,
  ...
  for (let lod = 0; lod < 3; lod += 1) {
    if (!existsSync(SOURCE[lod])) throw new Error(`missing V6 source LOD${lod}`);
    ...
    validateSockets(parsed.doc);
    const usedMaterials = validateMaterialCoverage(parsed.doc);
    validateSourceTextureRoleCoverage(parsed.doc, `Kestrel V6 LOD${lod}`);
    ...
    const canonical = raw(CANONICAL_SOURCE[lod]);
    const collision = collisionFingerprint(parsed);
    const canonicalCollision = collisionFingerprint(canonical);
    if (collision !== canonicalCollision) throw new Error(`collision geometry drift LOD${lod}`);
    const triangles = visibleTriangles(parsed.doc);
    ...
    sourceReports.push({
      ...
      canonicalCollisionFingerprint: canonicalCollision,
      identityMaterial: usedMaterials.includes('Material_V6_MarkingIvory'),
      textureAudit: {
  ...
  const staging = mkdtempSync(join(tmpdir(), 'spaceface-kestrel-v6-finalize-'));
  try {
    const staged = [];
    for (let lod = 0; lod < 3; lod += 1) {
    ...
    const stagedReport = resolve(staging, 'material_truth_v6_finalize_report.json');
    writeFileSync(stagedReport, `${JSON.stringify(result, null, 2)}\n`);
    renameSync(stagedReport, FINALIZE_REPORT);
    process.stdout.write(`V6_MATERIAL_TRUTH_FINALIZE=${JSON.stringify(result)}\n`);
  } finally {
