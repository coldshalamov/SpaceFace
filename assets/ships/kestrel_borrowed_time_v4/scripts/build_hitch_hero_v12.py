"""Build Hitch hero V12 (cycle 03) subtract pass."""
...
from hitch_hero_v12 import PASS_ID, apply_hitch_hero_v12  # noqa: E402


FAMILY = Path(__file__).resolve().parents[1]
BASELINE = FAMILY / "blender" / "kestrel_hitch_polish_v7_production.blend"
PACKET = "SF-K0-HITCH-HERO-V12-003"
OUT_DIR = FAMILY / "source_candidates" / "hitch_hero_v12" / "wholeships"
EVIDENCE = FAMILY / "evidence" / "hitch" / "cycles" / "cycle_03"
...
def render_stills(bounds):
    ...
    cam_data = bpy.data.cameras.new("V12Cam")
    camera = bpy.data.objects.new("V12Cam", cam_data)
    scene.collection.objects.link(camera)
    scene.camera = camera
    center = Vector(bounds["center"])
    span = max(bounds["dimensions"])
    for name, loc, energy, color, size in (
        ("V12Key", tuple(center + Vector((span * 0.9, -span * 1.1, span * 0.7))), 8200, (0.88, 0.92, 1), 10),
        ("V12Fill", tuple(center + Vector((span * 0.2, span * 0.9, span * 0.45))), 3200, (0.55, 0.62, 0.72), 8),
        ("V12Rim", tuple(center + Vector((-span * 0.9, -span * 0.3, span * 0.5))), 3800, (0.72, 0.80, 0.92), 7),
    ):
    ...
    for name, (loc, target, lens) in shots.items():
        if name.startswith("clay") and clay is None:
            clay = bpy.data.materials.new("V12_Clay")
            clay.use_nodes = True
...
def main() -> int:
    ...
    result = {
        "schema": "spaceface.hitchHeroV12.build.v1",
        "status": "complete",
        "packet": PACKET,
