#!/usr/bin/env node
/**
 * Copy hash-bound Hitch V8 source LODs onto the live starter wholeship paths.
 * Does not rewrite gameplay maps. Sockets stay the V4/V7 contract.
 */
...
const BUILD = JSON.parse(readFileSync(resolve(FAMILY, 'evidence/hitch_polish_v8/build_report.json'), 'utf8'));
if (BUILD.status !== 'complete') throw new Error('V8 build is not complete');
...
for (const member of members) {
  ...
  if (!existsSync(src)) throw new Error(`missing V8 LOD${member.lod}`);
  const hash = sha256(src);
  if (hash !== BUILD.lods[member.lod].sha256) throw new Error(`V8 LOD${member.lod} hash drift`);
  const liveSource = resolve(ROOT, 'assets/ships/parts/wholeships', member.live);
  const liveRelease = resolve(ROOT, 'assets/ships/release/parts/wholeships', member.live);
  ...
  mkdirSync(dirname(liveRelease), { recursive: true });
  mkdirSync(dirname(familySource), { recursive: true });
  copyFileSync(src, liveSource);
  copyFileSync(src, liveRelease);
...
const report = {
  schema: 'spaceface.hitchPolishV8.livePromotion.v1',
  status: 'complete',
  packet: BUILD.packet,
  generationFingerprint: BUILD.generationFingerprint,
  members: receipt,
};
writeFileSync(resolve(FAMILY, 'evidence/hitch_polish_v8/promote_report.json'), `${JSON.stringify(report, null, 2)}\n`);
