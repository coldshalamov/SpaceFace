...
```
- `state.timeScale` gates whether `stepSim` runs (0 = paused: sim frozen, render+camera+UI live) and
  scales accumulator time for slow-motion in `[0,1]`.
- `src/core/timeEffects.js` is the **sole runtime writer**. Callers acquire the per-state singleton
  with `createTimeEffects(state)` and use `set(source,{scale})` / `clear(source)`. The effective scale
  is the minimum active request, so hit-stop expiry cannot restore over pause or Flyby Focus.
- Time-effect requests are transient and never serialized. `reset()` clears them on new game and at
  the validated destructive boundary of load; visible UI/runtime owners then reconcile their named
  requests.
...
renderFrame(alpha, frameDt):
  ...
  spaceBackground.update(camera)    // parallax wrap + per-sector nebula (the legacy starfield module is retired)
...
GameState = {
  ...
  timeScale,                      // ⊘ derived 0..1 scalar (core/timeEffects sole writer)
  // --- core sim ---
...
settings = {
  ...
  audio:{ master:0.8, sfx:0.9, music:0.6, muted:false, defaultMuteVersion:2 },
...
- Load validation failures do not touch the running world's time-effect requests. After validation,
  destructive restore emits `save:restoring`, resets transient requests, and acquires a unique
  `save:restore:<sequence>` freeze before mutating state.
- Load sequence: validate fmt+version → migrate (ordered `MIGRATIONS`) → `save:restoring` + unique
  freeze → clear transient runtime (despawn entities, dispose meshes) → `deserialize` in order →
  re-emit `entity:spawned` for each rebuilt entity → `save:loaded` → authored-visual finalizer →
  release only that restore source. Destructive restore is a synchronous critical section: nested
  load/New Game requests replace one transient pending callback (latest wins), and the current load
  starts its visual finalizer before releasing restore ownership and draining that callback. Only
  when the drained route actually starts does `src/core/runTransitionGuard.js` reserve its immutable
  privately branded generation token (clones/forgeries are never current). The queue prevents
  synchronous state interleaving; the token prevents stale async readiness success/failure from
  changing mode, closing UI, or surfacing an error. If the superseded outer restore fails after a
  newer route is queued, its destructive error is suppressed; only the drained winner may publish
  its own load/deferred error channel.
...
### 5.4 Pause & back-nav
- ESC in flight (empty stack) → `pushScreen('pause')`. ScreenManager reconciles the aggregate
  `ui:pausing-screen` request and emits one `sim:pause`/`sim:resume` pair across nested pausing
  screens and docked state. Individual screens never write `timeScale`.
...
| `src/core/timeEffects.js` | per-state transient pause/slow-time arbiter; minimum named request; sole `state.timeScale` writer | `createTimeEffects` | — |
| `src/core/runTransitionGuard.js` | monotonic async New Game/load ownership; private issued-token identity rejects clones/forgeries; stale readiness completions become no-ops | `createRunTransitionGuard` | — |
...
| `src/render/starfield.js` | **RETIRED** - an earlier 4-layer parallax Points + procedural-nebula + distant-planet implementation, superseded by `spaceBackground.js`. The module was deleted on the 2026-09-18 VFX pass; git history retains the technique quarry and `06_RETAINED_FUTURE_BACKLOG.md` records the harvest note. | - | - |
...
| `src/ui/screenManager.js` | screen stack, modal/HUD visibility, aggregate `ui:pausing-screen` request and pause/resume pair | `ScreenManager` | timeEffects |
